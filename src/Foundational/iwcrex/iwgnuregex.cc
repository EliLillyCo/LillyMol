...
IW_gnu_regex::IW_gnu_regex ()
{
  _default_values ();
}

IW_gnu_regex::IW_gnu_regex (const char * s)
{
  _default_values ();
...
IW_gnu_regex::IW_gnu_regex (const const_IWSubstring & s)
{
  _default_values ();
...
IW_gnu_regex::IW_gnu_regex (const IWString & s)
{
  _default_values ();
...
int
IW_gnu_regex::ok () const
{
  if (0 == _compiled_regular_expression.re_nsub && NULL != _regmatch)
  {
    cerr << "IW_gnu_regex::ok: re_nsub/_regmatch mismatch\n";
    return 0;
  }

  if (_compiled_regular_expression.re_nsub && NULL == _regmatch)
  {
    cerr << "IW_gnu_regex::ok: _regmatch/re_nsub mismatch\n";
    return 0;
  }
...
void
IW_gnu_regex::set_use_extended (int s)
{
  if (s)
    _regcomp_flags |= REG_EXTENDED;
  else
...
int
IW_gnu_regex::set_pattern (const char * s)
{
  if (_compiled)
  {
    regfree (&_compiled_regular_expression);
  ...
  if (0 != rc)
  {
    cerr << "IW_gnu_regex::set_pattern: cannot compile '" << s << "'\n";
    return 0;
  }
...
int
IW_gnu_regex::set_pattern (const IWString & s)
{
  return set_pattern (s.rawchars (), s.length ());
}

int
IW_gnu_regex::set_pattern (const const_IWSubstring & s)
{
  return set_pattern (s.rawchars (), s.length ());
}

int
IW_gnu_regex::set_pattern (const char * s, int len)
{
  IWString tmp;
...
int
IW_gnu_regex::matches (const IWString & s)
{
  return matches (s.rawchars (), s.length ());
}

int
IW_gnu_regex::matches (const const_IWSubstring & s)
{
  return matches (s.rawchars (), s.length ());
}

int
IW_gnu_regex::matches (const char * s, int len)
{
  if (! _compiled)
    return 0;
...
int
IW_gnu_regex::matches (const char * s)
{
  assert (ok ());
...
int
IW_gnu_regex::matches_save_subexpressions (const char * s, int len)
{
  IWString tmp;
  tmp.resize (len + 1);
...
int
IW_gnu_regex::matches_save_subexpressions (const const_IWSubstring & s)
{
  return matches_save_subexpressions (s.rawchars (), s.nchars ());
}

int
IW_gnu_regex::matches_save_subexpressions (const IWString & s)
{
  return matches_save_subexpressions (s.rawchars (), s.nchars ());
}

int
IW_gnu_regex::matches_save_subexpressions (const char * s)
{
  return _matches_save_subexpressions (s, -1);    // length unknown right now
}

int
IW_gnu_regex::_matches_save_subexpressions (const char * s, int len)
{
  assert (ok ());
...
int
IW_gnu_regex::dollar (int whichdollar, const_IWSubstring & zresult) const
{
...
IW_gnu_regex::dollar (int whichdollar) const
{
  assert (whichdollar >= 0 && whichdollar <= int (_compiled_regular_expression.re_nsub));
  assert (NULL != _regmatch);
  assert ("" != _string_which_was_matched);
...
int
IW_gnu_regex::dollar (int whichdollar, int & zresult)
{
...
int
IW_gnu_regex::dollar (int whichdollar, float & zresult)
{
  const_IWSubstring tmp;
...
int
IW_gnu_regex::dollar (int whichdollar, double & zresult)
{
  const_IWSubstring tmp;
...
int
IW_gnu_regex::number_subexpressions () const
{
  assert (ok ());
...
int
IW_gnu_regex::s (const IWString & s, const const_IWSubstring & zto,
          IWString & zresult)
{
...
int
IW_gnu_regex::s (IWString & s, const const_IWSubstring & zto)
{
  IWString tmp;

  int rc = IW_gnu_regex::s (s, zto, tmp);
