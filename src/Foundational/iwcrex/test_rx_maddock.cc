...
typedef IW_Regular_Expression_Template<IW_lrx_Maddock> IW_Regular_Expression_Test;
...
//if (! foo.set_pattern ("\\([[:alnum:]]\\+\\) \\([0-9]\\{6\\}\\)"))
  ...
  if ("123456" != dollar)
  {
    cerr << "Yipes, complex pattern $2 invalid '" << dollar << "', should be '123456'\n";
    foo.debug_print (cerr);
    failures++;
  }
  ...
  {
    cerr << "Yipes, iwcrex_s failed, result '" << tochange << "' should be 'wow wow barf'\n";
    failures++;
  }
