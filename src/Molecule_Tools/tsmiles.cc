// Tester for smiles functionality.
// Pass any large collection of molecules to this tool.
...
static int unique_smiles_after_aromatic_errors = 0;
...
// clang-format off
  ...
  // clang-format on
  // clang-format off
  cerr << R"(Tester for smiles functionality.
Tests for consistency of unique smiles generation, parsing aromatic forms and other functionality.
 -p <number>    do <number> permutations per molecule
 -a             test the assignment operator
 -b             test subset unique smiles
 -c <0,1>       include chiral info in smiles (default 1)
 -t <0,1>       include cis trans info in smiles (default 1)
 -h             test set_include_chiral_info_in_smiles
 -w <number>    do tests involving swapping atoms
 -m <number>    test building from aromatic form
 -x             terminate all processing on any error
 -j             stop processing any molecule once it has an error
 -u             remove marked chiral centres that aren't chiral
 -y             remove any un-necessary square brackets (will get radicals wrong)
 -q             test in_same_ring and in_same_ring_system functions
 -R             write SSSR upon failure
 -f             upon failure, destroy aromaticity and see if a non aromatic form would work
 -r <number>    report progress every <n> molecules
 -W             remove all chirality as the molecule is read
 -s <number>    random number seed
 -F <fname>     write smiles for failing molecules to <fname>
 -i <type>      specify input type
 -E <symbol>    create an element with symbol <symbol>
 -k <nrings>    discard molecules with <nrings> or more (def 99)
)";
...
do_destroy_aromaticity(Molecule& m) {
  ...
  for (auto i = 0; i < n; i += 2) {
    m.set_bond_type_between_atoms(double_bonds_to_be_removed[i],
                                  double_bonds_to_be_removed[i + 1], SINGLE_BOND);
  }
...
static int
either_side(const Atom** atom, atom_number_t a1, int n1, int* wside) {
  ...
  for (int i = 0; i < acon; i++) {
    ...
    if (n1 ==
        wside[j]) {  // closing a ring or turning back on ourselves within our subset
      continue;
    }
...
static int
either_side(const Atom** atom, atom_number_t a1, int n1, atom_number_t a2, int n2,
            int* wside) {
  wside[a1] = n1;
...
static int
do_test_subset_unique_smiles(Molecule& m, const Atom** atom, int* subset) {
  ...
  for (int i = 0; i < nb; i++) {
    ...
    //  cerr << "Created " << components.number_elements() << " components with " <<
    //  components[0]->natoms() << " and " << components[1]->natoms() << " atoms\n";

    IWString usmi21 = components[0]->unique_smiles();
    //  cerr << "Computing unique smiles for component with " << components[1]->natoms()
    //  << " atoms\n";
...
do_test_subset_unique_smiles(Molecule& m) {
  const Atom** atoms = new const Atom*[m.natoms()];
...
do_test_assignment(Molecule& m, const IWString& usmi) {
  ...
  if (mamw > 0.0) {
    ...
    if (fabs(tmpamw - mamw) > 0.1) {
      cerr << "Molecular weight mismatch on constructor assignment, " << mamw << " vs "
           << tmpamw << '\n';
      return 0;
    }
  }
...
static int
report_canonical_ranking(std::ostream& os, Molecule& m) {
...
static int
report_aromatic_atom_count_if_different(Molecule& m1, Molecule& m2, std::ostream& os) {
  int matoms = m1.natoms();

  if (matoms != m2.natoms()) {
    cerr << "report_aromatic_atom_count_if_different:atom count mismatch " << matoms
         << " vs " << m2.natoms() << '\n';
    return 0;
  }
  ...
  os << "Aromatic atom count mismatch " << m1.aromatic_atom_count() << " vs "
     << m2.aromatic_atom_count() << '\n';

  return os.good();
}

static int
test_same_ring_system(Molecule& m, const Ring& r1, const Ring& r2) {
  ...
  for (int i = 0; i < r1size; i++) {
    ...
    for (int j = 0; j < r2size; j++) {
      ...
      if (!m.in_same_ring_system(ai, aj)) {
        cerr << "In '" << m.name() << "' not in same ring system, atoms " << ai << " and "
             << r2[j] << '\n';
        cerr << r1 << '\n';
        cerr << r2 << '\n';
        return 0;
...
static int
test_same_ring(Molecule& m, const Ring& r) {
  ...
  for (int i = 0; i < ring_size; i++) {
    ...
    for (int j = i + 1; j < ring_size; j++) {
      if (!m.in_same_ring(ai, r[j])) {
        cerr << "In same ring failure '" << m.name() << "', atoms " << ai << " and "
             << r[j] << '\n';
        return 0;
      }
    }
...
static int
choose_two_atom_numbers(int matoms, atom_number_t& a1, atom_number_t& a2) {
  assert(matoms > 1);
...
report_pi_electrons(Molecule& m) {
  ...
  for (int i = 0; i < matoms; i++) {
    int pi;
    m.pi_electrons(i, pi);
    cerr << "Atom " << i << " " << m.smarts_equivalent_for_atom(i) << " has " << pi
         << " pi electrons\n";
  }
}
#endif
...
class Counters_This_Molecle {
 ...
 public:
  ...
  void
  smiles_interpretation_error() {
    _smiles_interpretation_errors++;
  }

  void
  aromatic_smiles_interpretation_error() {
    _aromatic_smiles_interpretation_errors++;
  }
};

Counters_This_Molecle::Counters_This_Molecle(const IWString& s,
                                             const molecular_weight_t& mw)
    : _usmi(s), _amw(mw) {
...
static int
tsmiles(Molecule& m, Counters_This_Molecle& c, const IWString& rsmi) {
  Molecule x;
...
static int
tsmiles(Molecule& m, const molecular_weight_t amw) {
...
static int
tsmiles(Molecule& m, const IWString& usmiles, const molecular_weight_t amw) {
  ...
  for (int i = 0; i < permutations_per_molecule; i++) {
    if (errors_this_molecule &&
        (abort_on_error || stop_processing_molecule_on_first_error)) {
      break;
    }
  ...
  if (test_build_from_aromatic_form && m.contains_aromatic_atoms()) {
    for (int i = 0; i < test_build_from_aromatic_form; i++) {
      ...
      if (usmiles != xusmi) {
        ...
        unique_smiles_after_aromatic_errors++;
        errors_this_molecule++;
...
tsmiles(Molecule& m) {
  ...
  if (m.nrings() > max_rings) {
    cerr << "Too many rings " << m.nrings() << ", many potential problems, skipping '"
         << m.name() << "'\n";
...
tsmiles(int argc, char** argv) {
  ...
  if (cl.option_present('f')) {
    ...
    if (verbose) {
      cerr << "Upon failure, will destroy aromaticity and see if molecule works without "
              "aromatic bonds\n";
    }
  }
  ...
  if (cl.option_present('k')) {
    if (!cl.value('k', max_rings) || max_rings < 0) {
      cerr << "The maximum number of rings to process option (-k) must be a whole +ve "
              "number\n";
      usage(2);
    }
  ...
  if (cl.option_present('m')) {
    if (!cl.value('m', test_build_from_aromatic_form) ||
        test_build_from_aromatic_form < 1) {
      cerr << "The number of steps for testing build from aromatic form (-m) must be "
              "+ve\n";
      usage(11);
    }
    ...
    if (verbose) {
      cerr << "Will do " << test_build_from_aromatic_form
           << " tests of building from aromatic form\n";
    }
  }
  ...
  if (verbose || failures) {
    ...
    if (molecules_with_errors) {
      cerr << total_error_count << " errors in " << molecules_with_errors
           << " molecules\n";
      cerr << smiles_interpretation_errors << " smiles interpretation errors\n";
      cerr << aromatic_smiles_interpretation_errors
           << " aromatic smiles interpretation errors\n";
      cerr << amw_errors << " molecular weight errors\n";
      cerr << unique_smiles_errors << " unique smiles errors\n";
      cerr << unique_smiles_after_aromatic_errors << " unique smiles errors after aromatisation\n";
      cerr << atom_swap_errors << " atom swap errors\n";

      if (destroy_aromaticity) {
        cerr << molecules_ok_without_aromaticity
             << " molecules that failed were OK once aromaticity was removed\n";
      }
    } else {
