...
static extending_resizable_array<int> invalid_chiral_centres_found;
...
static void
do_append_chiral_count_to_name(Molecule& m, int total_chiral_centres,
...
static int
add_missing_chiral_centres(Molecule& m, const int* is_chiral) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (!c->complete() || !m.valid_chiral_centre(c)) {
      cerr << "id_chirality:yipes, incomplete or invalid chiral centre at atom " << i
           << " in " << m.name() << "\n";
      c->debug_print(cerr);
      return 0;
    }
...
static int
write_chiral_variant_unique_name_(Molecule& m, int& count,
...
static int
write_chiral_variant_unique_name(Molecule& m, int& count,
                                 Molecule_Output_Object& output) {
...
static int
write_chiral_variant(Molecule& m, int& count, IW_STL_Hash_Set& already_created,
                     Molecule_Output_Object& output) {
...
static int
create_permutations(Molecule& m, atom_number_t a, const int* is_chiral,
                    int chiral_centres_this_molecule, int depth, int& count,
                    IW_STL_Hash_Set& already_created, Molecule_Output_Object& output) {
  if (depth == chiral_centres_this_molecule) {
    ...
    for (int i = a + 1; i < matoms; i++) {
      ...
      if (is_chiral[i]) {
        create_permutations(m, i, is_chiral, chiral_centres_this_molecule, depth + 1,
                            count, already_created, output);
      }
    }
  }
  ...
  if (depth == chiral_centres_this_molecule) {
    ...
    for (int i = a + 1; i < matoms; i++) {
      ...
      create_permutations(m, i, is_chiral, chiral_centres_this_molecule, depth + 1, count,
                          already_created, output);
    }
  }
...
static int
create_permutations(Molecule& m, const int* is_chiral, int chiral_centres_this_molecule,
                    int unlabelled_chiral_centres_found, Molecule_Output_Object& output) {
  int matoms = m.natoms();
  ...
  if (only_permute_unlabelled_centres) {
    chiral_centres_this_molecule =
        count_occurrences_of_item_in_array(UNMARKED_CHIRAL_ATOM, matoms, is_chiral);
  }
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    //  cerr << "Creating permuations for atom " << i <<'\n';

    rc += create_permutations(m, i, is_chiral, chiral_centres_this_molecule, 1, count,
                              already_created, output);
  }
...
static int
process_possibly_chiral_atom(Molecule& m, atom_number_t a, int* is_chiral) {
...
possibly_chiral(Molecule& m, atom_number_t i) {
  ...
  // cerr << "Atom " << i << " type " << z << " icon = " << icon << " ih = " << ih <<
  // '\n';
...
static int
identify_existing_chiral_centres(Molecule& m, int* is_chiral) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (check_that_marked_chiral_centres_are_actually_chiral &&
        !is_actually_chiral(m, i)) {
      ++invalid;

      cerr << "Atom " << i << " (" << m.atomic_symbol(i) << ' ' << m.ncon(i)
           << " connections) is not actually chiral\n";
  ...
  ++invalid_chiral_centres_found[invalid];
...
static int
do_hard_coded_queries(Molecule& m, int* is_chiral) {
  int matoms = m.natoms();
...
static int
assign_atoms_to_ignore(Molecule& m, int* is_chiral) {
...
static int
examine_query_hits(Molecule& m, int* is_chiral) {
...
static int
id_chirality_by_substructure_searches(Molecule& m, int* is_chiral) {
...
static int
id_chirality_by_rules(Molecule& m, int* is_chiral) {
...
static int
identify_matched_atoms_with_chiral_centres(
    const Molecule& m, const Set_of_Atoms& e,
...
static int
do_remove_chiral_centres_on_matched_atoms(
  ...
  for (int i = 0; i < nq; i++) {
    ...
    for (int j = 0; j < nhits; j++) {
      ...
      identify_matched_atoms_with_chiral_centres(m, *e,
                                                 atoms_with_chiral_centres_to_be_removed);
    }
  }
...
static void
identify_ring_atoms(Molecule& m, int* is_chiral) {
...
static int
id_chirality(Molecule& m, int* is_chiral, Molecule_Output_Object& output) {
  ...
  int total_chiral_centres =
      chiral_centres_already_present + unlabelled_chiral_centres_found;
  ...
  if (max_allowed_unmarked_centres >= 0 &&
      unlabelled_chiral_centres_found > max_allowed_unmarked_centres) {
  ...
  if (max_allowed_chiral_centres >= 0 &&
      total_chiral_centres > max_allowed_chiral_centres) {
    if (stream_for_rejected_molecules.active()) {
      return stream_for_rejected_molecules.write(m);
    }
  ...
  if (tag.length()) {
    do_append_chiral_count_to_name(m, total_chiral_centres,
                                   unlabelled_chiral_centres_found);
  }
  ...
  if (verbose > 2) {
    for (int i = 0; i < m.natoms(); i++) {
      cerr << "atom " << i << " chirality " << is_chiral[i] << '\n';
    }
  }
  ...
  if (do_permutations) {
    ...
    if (verbose > 2) {
      cerr << permutations_available_this_molecule << " permutations available in '"
           << m.name() << "'\n";
    }

    if (permutations_available_this_molecule <= do_permutations) {
      return create_permutations(m, is_chiral, total_chiral_centres,
                                 unlabelled_chiral_centres_found, output);
    }
  }
...
static int
id_chirality(Molecule& m, Molecule_Output_Object& output) {
  int* tmp = new_int(m.natoms());
...
static int
id_chirality(data_source_and_type<Molecule>& input, Molecule_Output_Object& output) {
  Molecule* m;
...
static int
id_chirality(const char* fname, FileType input_type, Molecule_Output_Object& output) {
...
id_chirality(int argc, char** argv) {
  ...
  if (cl.option_present('p')) {
    if (!cl.value('p', isotope_for_unmarked_centres) ||
        isotope_for_unmarked_centres < 1) {
      cerr << "The isotopic label for unlabelled centres must be > 0\n";
      usage(6);
    }

    if (verbose) {
      cerr << "Unlabelled chiral centres isotopically labelled as "
           << isotope_for_unmarked_centres << '\n';
    }
  }
  ...
  if (cl.option_present('P')) {
    ...
    if (verbose) {
      cerr << "Will generate permutations of up to " << do_permutations
           << " unmarked chiral centres\n";
    }
  ...
  if (verbose && queries.number_elements()) {
    cerr << "defined " << queries.number_elements()
         << " queries to define chiral centres\n";
  }
  ...
  if (cl.option_present('R')) {
    ...
    if (verbose) {
      cerr << "Created " << remove_chiral_centres_on.number_elements()
           << " queries for removing chiral centres\n";
    }
  }
  ...
  if (cl.option_present('u')) {
    if (!cl.option_present('P')) {
      cerr << "For the -u option to make sense you need to generate permutations (-P "
              "option)\n";
      usage(17);
    }
  ...
  if (cl.option_present('m')) {
    if (!cl.value('m', max_allowed_unmarked_centres) ||
        max_allowed_unmarked_centres < 0) {
      cerr << "The maximum allowed number of unmarked chiral centres (-m option) must be "
              "a whole +ve number\n";
      usage(4);
    }

    if (verbose) {
      cerr << "Will discard molecules having more than " << max_allowed_unmarked_centres
           << " unmarked chiral centres\n";
    }
  }
  ...
  if (cl.option_present('n')) {
    if (!cl.value('n', max_allowed_chiral_centres) || max_allowed_chiral_centres < 0) {
      cerr << "The maximum allowed number of chiral centres (-n option) must be a whole "
              "+ve number\n";
      usage(4);
    }

    if (verbose) {
      cerr << "Will discard molecules having more than " << max_allowed_chiral_centres
           << " chiral centres\n";
    }
  }
  ...
  if (cl.option_present('D')) {
    ...
    const char sep = ' ';
    stream_for_chirality_count << "ID" << sep << "poss_chiral" << sep << "unmarked" << sep
                               << "marked\n";
  ...
  if (verbose) {
    ...
    if (check_that_marked_chiral_centres_are_actually_chiral) {
      int tot = 0;
      for (int i = 0; i < invalid_chiral_centres_found.number_elements(); ++i) {
        if (invalid_chiral_centres_found[i] > 0) {
          cerr << invalid_chiral_centres_found[i] << " molecules had " << i << " invalid chiral centres\n";
          tot += invalid_chiral_centres_found[i];
        }
      }
      cerr << tot << " invalid chiral centres\n";
    }
    ...
    for (int i = 0; i < marked_chiral_centres.number_elements(); i++) {
      if (marked_chiral_centres[i]) {
        cerr << marked_chiral_centres[i] << " molecules had " << i
             << " explicit chiral centres\n";
      }
    }

    for (int i = 0; i < unmarked_chiral_centres.number_elements(); i++) {
      if (unmarked_chiral_centres[i]) {
        cerr << unmarked_chiral_centres[i] << " molecules had " << i
             << " unmarked chiral centres\n";
      }
    }
    ...
    for (int i = 0; i < total_chiral_count.number_elements(); i++) {
      if (0 == i || total_chiral_count[i] > 0) {
        float f = static_cast<float>(total_chiral_count[i]) /
    ...
    if (remove_chiral_centres_on.number_elements()) {
      cerr << molecules_with_chiral_centres_removed_by_rmchiral
           << " molecules with chiral centres removed by -R queries\n";
    }
  }
