...
// By default, if a molecule fails, we stop the computation.
static int ignore_failures = 0;
...
// clang-format off
  ...
  cerr << "  -f             fault tolerant mode, ignore failed molecules rather than "
          "stopping\n";
...
get_property_for_each_atom(Molecule& m, double** property_array) {
  ...
  // set up array to put the ghose crippen index value and hydrogen bond donor acceptor
  // array
...
static int
get_property_for_each_atom(const Molecule& m, double** property_array) {
  int matoms = m.natoms();
...
static int
get_atomic_properties(Molecule& m, double* p) {
...
static int
jwmorse(Molecule& m, const double* p, IWString_and_File_Descriptor& output) {
  const int matoms = m.natoms();

  double* morse = new double[NUMBER_OF_PROPERTY * MAX_MORSE_DESCRIPTORS];
  std::unique_ptr<double[]> free_morse(morse);
  std::fill_n(morse, NUMBER_OF_PROPERTY * MAX_MORSE_DESCRIPTORS, 0.0);

  double* sd = new double[MAX_MORSE_DESCRIPTORS];
  std::unique_ptr<double[]> free_sd(sd);
  ...
  for (int i = 0; i < matoms; ++i) {
    ...
    for (int j = i + 1; j < matoms; ++j) {
      ...
      sd[0] = 1.0;
      for (int s = 1; s < MAX_MORSE_DESCRIPTORS;
           ++s)  // very important to get a vectorised sin
      {
        sd[s] = sin(s * d) / (s * d);
      }
...
jwmorse(Molecule& m, const double* p, IWString_and_File_Descriptor& output) {
  ...
  double* morse = new double[NUMBER_OF_PROPERTY * MAX_MORSE_DESCRIPTORS];
  unique_ptr<double[]> free_morse(morse);
  std::fill_n(morse, NUMBER_OF_PROPERTY * MAX_MORSE_DESCRIPTORS, 0.0);
...
static int
jwmorse(Molecule& m, IWString_and_File_Descriptor& output) {
  const int matoms = m.natoms();

  double* p = new double[matoms * NUMBER_OF_PROPERTY];
  std::unique_ptr<double[]> free_p(p);
...
static int
jwmorse(Molecule& m, IWString_and_File_Descriptor& output, double** distance_matrix,
        double** intermediate_matrix, double** property_array, double atomic_charge[],
  ...
  for (int s = 0; s < MAX_MORSE_DESCRIPTORS; s++) {
    ...
    for (int k = 0; k < NUMBER_OF_PROPERTY; k++) {
      ...
      for (int i = 0; i < n_atoms; i++) {
        for (int j = i + 1; j < n_atoms; j++) {
          morse_descriptors[k] +=
              intermediate_matrix[i][j] * property_array[i][k] * property_array[j][k];
        }
      }
    }
...
static int
jwmorse(Molecule& m, IWString_and_File_Descriptor& output, std::ofstream& logfile) {
  int rc = 0;
  ...
  double* morse_descriptors = new double[NUMBER_OF_PROPERTY];
  std::unique_ptr<double[]> free_morse_descriptors(morse_descriptors);

  double* atomic_charge = new double[n_atoms];
  std::unique_ptr<double[]> free_atomic_charge(atomic_charge);
  ...
  //      if (verbose) cerr<<m.name()<<"\tERROR in calculation of Gasteiger partial
  //      charge\n";
  //    }
  ...
  if (0 == rc) {
    ...
    //      for (int i=0; i<n_atoms; i++) cerr<<"atom
    //      i="<<i<<"\tcharge="<<atomic_charge[i]<<'\n';

    rc = jwmorse(m, output, distance_matrix, intermediate_matrix, property_array,
                 atomic_charge, morse_descriptors);
...
static int
jwmorse(data_source_and_type<Molecule>& input, IWString_and_File_Descriptor& output,
...
static int
jwmorse(const char* fname, FileType input_type, IWString_and_File_Descriptor& output,
        std::ofstream& logfile) {
...
jwmorse(int argc, char** argv) {
  Command_Line cl(argc, argv, "vA:i:p:ng:f");
  ...
  if (cl.option_present('f')) {
    ignore_failures = 1;
    if (verbose) {
      cerr << "Fault tolerant mode, will ignore failing molecules\n";
    }
  }
...
#ifdef GIT_HASH
    cerr << __FILE__ << " compiled " << __DATE__ << " " << __TIME__ << " git hash "
         << GIT_HASH << '\n';
#else
    cerr << __FILE__ << " compiled " << __DATE__ << " " << __TIME__ << '\n';
#endif
  ...
  if (verbose) {
    ...
    logfile << "Error Rate: " << (float)number_of_error / (float)molecules_read << '\n';
    if (verbose) {
  ...
  if (failed_calculations) {
    cerr << failed_calculations << " failed calculations ignored\n";
  }
  return rc;
}
