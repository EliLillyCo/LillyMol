...
// We can also write the fingerprint, offset the bits, write it again.
// the number of times we replicte the bits.
static int bit_replicates = 0;
// The bit offset between versions.
static int bit_replicte_offset = 741;  // An arbitrary number.
...
usage(int rc) {
  ...
  cerr << "  -q <qry>       query file specifying the atoms from which shells are grown - by default all atoms\n";
...
static int
check_against_list(Molecule& m, const IWString& smarts,
                   const atom_number_t centre_of_shell, unsigned int sum_so_far,
                   const int radius) {
  std::unordered_map<unsigned int, unsigned int>::iterator f =
      bits_to_investigate.find(sum_so_far);
  ...
  stream_for_bit_meanings << m.smiles() << ' ' << m.name() << " bit " << sum_so_far
                          << " atom " << centre_of_shell << " " << smarts << " radius "
                          << radius << '\n';
...
void
WriteLabelledSmiles(const Molecule& m, int centre_of_shell, int radius,
...
#ifdef USE_IWDIGITS
static void
write_bit(const int centre_of_shell, const IWString& smarts_for_centre_of_shell,
...
write_bit(const int centre_of_shell, const IWString& smarts_for_centre_of_shell,
  ...
  output << centre_of_shell << sep << radius << sep << b << sep
         << smarts_for_centre_of_shell << '\n';
...
static int
write_global_fingerprint(const Sparse_Fingerprint_Creator::FPHash& bits_found, int radius,
                         int molecules_read, IWString_and_File_Descriptor& output) {
  IWString string_radius;

  string_radius << radius << ' ';

  for (Sparse_Fingerprint_Creator::FPHash::const_iterator i = bits_found.begin();
       i != bits_found.end(); ++i) {
    unsigned int b = (*i).first;
    int c = (*i).second;

    float ave =
        static_cast<float>(static_cast<double>(c) / static_cast<double>(molecules_read));
...
static void
increment(unsigned int& sum_so_far, const int bc, const atype_t atom_constant) {
...
static int
generate_shells(const int matoms, int radius, const int max_radius, const Atom* const* a,
                const atype_t* atom_constant, int* processing_status,
                unsigned int sum_so_far, Molecule& m, Sparse_Fingerprint_Creator* sfc) {
  radius++;
...
// #define DEBUG_ECFP_BITS 1
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    for (int j = 0; j < acon; j++) {
      ...
      if (PROCESSING_FINISHED == processing_status[k])  // we are extending the shell
      {
        ...
        //      expect " << (sum_so_far + bc*atom_constant[i]) << '\n';
        increment(sum_so_far, bc, atom_constant[i]);
        //      cerr << "at level " << radius << " added atom " << i << " atype " <<
        //      atom_constant[i] << " BC " << bc << " sum " << sum_so_far << '\n';
        if (add_tails_here) {
  ...
  if (radius > min_shell_radius) {
    ...
    if (looking_for_bit_meanings) {
      check_against_list(*current_molecule, smarts_for_centre_of_shell, centre_of_shell,
                         sum_so_far, radius);  // horrible hack with file scope variable
    } else if (stream_for_all_bits.is_open()) {
      write_bit(centre_of_shell, smarts_for_centre_of_shell, radius, sum_so_far, m,
                stream_for_all_bits);
    }
  }
  ...
  if (each_shell_gets_own_fingerprint) {
    return generate_shells(matoms, radius, max_radius, a, atom_constant,
...
static int
WriteFixedWidthFingerprint(Molecule& m, uint32_t nbits,
...
static int
write_array_of_fingerprints_inner(Sparse_Fingerprint_Creator* sfc,
                                  IWString_and_File_Descriptor& output) {
  IWString tmp;

  // Lazy, this is only applied to the most common case with just one fingerprint.
  if (bit_count_multiplier > 0) {
    sfc[0].Scale(bit_count_multiplier);
  }
...
write_array_of_fingerprints(Sparse_Fingerprint_Creator* sfc,
                            IWString_and_File_Descriptor& output) {
  if (bit_replicates == 0) {
    write_array_of_fingerprints_inner(sfc, output);
    return 1;
  }
  ...
  // Lazy, only handle the case of one fingerprint. The most common case.
  Sparse_Fingerprint_Creator tmp;
  for (auto& [b, c] : sfc[0].bits_found()) {
    tmp.hit_bit(b, c);
  }

  for (auto [b, c] : sfc[0].bits_found()) {
    for (int i = 1; i <= bit_replicates; ++i) {
      uint32_t newbit = b + i * bit_replicte_offset;
      tmp.hit_bit(newbit, c);
    }
  }

  write_array_of_fingerprints_inner(&tmp, output);  // not really an array
...
static int
do_output(Molecule& m, Sparse_Fingerprint_Creator* sfc,
          IWString_and_File_Descriptor& output) {
  if (verbose) {
...
static void
form_bit(Molecule& m, const atype_t* atom_constant, const Atom* const* atoms,
  ...
  if (each_shell_gets_own_fingerprint) {
    generate_shells(matoms, 0, max_shell_radius, atoms, atom_constant, processing_status,
                    e, m, sfc + 1);
  } else {
    generate_shells(matoms, 0, max_shell_radius, atoms, atom_constant, processing_status,
                    e, m, sfc);
...
static void
identify_atoms_within_range(Molecule& m, Set_of_Atoms* atoms_within_range) {
  const int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; ++i) {
    for (int r = 0; r <= max_shell_radius; ++r) {
      cerr << " atom " << i << " rad " << r << " atms "
           << atoms_within_range[i * (max_shell_radius + 1) + r] << '\n';
    }
  }
#endif
}
...
class Atom_Radius_Variance {
 ...
 public:
  Atom_Radius_Variance() {
    _var = 0;
  }

  void
  set_atom(const atom_number_t s) {
    _a = s;
  }

  void
  set_radius(const int s) {
    _radius = s;
  }

  void
  set_var(const double s) {
    _var = s;
  }
...
/*
  We do not really compute the variance, since that would be a floating point number. We
  want an int. So, what we compute is
...
static int
our_own_custom_variance(const int matoms, const int sumv, const int sumv2) {
...
static int
compute_variance(const Set_of_Atoms& s, const int* visited, const int matoms, int sumv,
                 int sumv2) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    sumv += 1;
    sumv2 = sumv2 - (v * v) +
            (v + 1) * (v + 1);  // take away the previous value, and add the value for
                                // visited one extra time - could simplify.....
  }
...
static void
recompute_sums(const int* visited, const int matoms, int& sumv, int& sumv2) {
  sumv = 0;
  sumv2 = 0;
...
static int
do_equalise_atom_coverage(Molecule& m, const Atom* const* atoms,
                          const atype_t* atom_constant, int* processing_status,
                          Sparse_Fingerprint_Creator* sfc) {
  const int matoms = m.natoms();

  Set_of_Atoms* atoms_within_range = new Set_of_Atoms[matoms * (max_shell_radius + 1)];
  ...
  std::unique_ptr<int[]> free_visited(
      visited);  // note initialised with 1 (zero radius shell)
  ...
  if ((static_cast<float>(max_visited - min_visited) / static_cast<float>(max_visited)) >
      0.8f) {  // close enough
    return 1;
  }
  ...
  if (label_by_visited && verbose > 1) {
    ...
    cerr << m.smiles() << ' ' << m.name() << " before equalisation, var "
         << our_own_custom_variance(matoms, sumv, sumv2) << '\n';
  }
  ...
  for (int i = 0; i < equalise_atom_coverage; ++i) {
    for (int j = 0; j < matoms; ++j) {
      for (int r = 0; r <= max_shell_radius; ++r) {
        ...
        var[j * (max_shell_radius + 1) + r] =
            compute_variance(s, visited, matoms, sumv, sumv2);
      }
    }
    ...
    //  cerr << "Iteration " << i << " nmin " << mins.number_elements() << " var " <<
    //  min_variance << '\n';
    ...
    for (int j = 0; j < mins.number_elements(); ++j) {
      ...
      form_bit(m, atom_constant, atoms, s[0], s.number_elements() - 1, processing_status,
               sfc);
    }
...
// For every atoms matched by any of the queries in start_atom_query,
// set the corresponding entry in `processing_status` to `matched_flag`.
static int
IdentifyStartAtoms(Molecule& m, int* processing_status, int matched_flag) {
  Molecule_to_Match target(&m);

  int rc = 0;
  for (Substructure_Query* q : start_atom_query) {
...
// We hare start atom queries and a molecule did not match any of those queries.
// Write an empty fingerprint.
static int
HandleNoStartAtoms(Molecule& m, IWString_and_File_Descriptor& output) {
  if (!function_as_filter) {
    output << smiles_tag << m.smiles() << ">\n";
    output << identifier_tag << m.name() << ">\n";
  }
...
static int
iwecfp(Molecule& m, IWString_and_File_Descriptor& output)

{
  ...
  atype_t* atom_constant = new atype_t[matoms];
  std::unique_ptr<atype_t[]> free_atom_constant(atom_constant);
...
// #define CHECK_ATOM_TYPES_STUFF
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (each_shell_gets_own_fingerprint) {
      generate_shells(matoms, 0, max_shell_radius, atoms, atom_constant,
...
static int
iwecfp(data_source_and_type<Molecule>& input, IWString_and_File_Descriptor& output) {
  Molecule* m;
...
static int
iwecfp_filter(const const_IWSubstring& buffer, IWString_and_File_Descriptor& output) {
...
static int
iwecfp_filter(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
...
static int
iwecfp_filter(const char* fname, IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
  if (!input.good()) {
...
static int
iwecfp(const char* fname, FileType input_type, IWString_and_File_Descriptor& output) {
  if (function_as_filter) {
...
// This is the replicates= directive.
// It can be of the form
//    replicates=nn
//    replicates=nn,oo
// In the second form we also parse the offset.
static int
ParseReplicatesDirective(const const_IWSubstring& y) {
  if (y.empty()) {
    return 0;
  }
  ...
  y.nextword(r, i, ',');
  y.nextword(o, i, ',');  // May be empty;

  if (r.empty()) {
    return 0;
  }

  if (!r.numeric_value(bit_replicates) || bit_replicates < 1) {
  ...
  if (!o.numeric_value(bit_replicte_offset) || bit_replicte_offset == 0) {
...
iwecfp(int argc, char** argv) {
  Command_Line cl(argc, argv, "vE:A:g:J:i:fr:R:P:mt:lxsG:M:bB:Q:Y:q:");
  ...
  if (cl.option_present('r')) {
    ...
    if (verbose) {
      cerr << "Will only fingerprint paths larger than " << min_shell_radius
           << " bonds\n";
    }
  }
  ...
  if (cl.option_present('R')) {
    ...
    if (max_shell_radius < min_shell_radius) {
      cerr << "Inconsistent min " << min_shell_radius << " and max " << max_shell_radius
           << " shell radius\n";
      usage(6);
    }
    ...
    for (int i = 0; cl.value('Y', y, i); ++i) {
      if (y == "flush") {
        ...
        if (!y.numeric_value(descriptor_file_output) || descriptor_file_output < 2) {
          cerr << "The number of columns for descriptor output must be a whole +ve "
                  "number\n";
          return 1;
        }
        if (verbose) {
          cerr << "Will write a descriptor file with " << descriptor_file_output
               << " columns\n";
        }
      } else if (y.starts_with("fixed=")) {
        y.remove_leading_chars(6);
        if (!y.numeric_value(fixed_width_fingerprint) || fixed_width_fingerprint < 2) {
          cerr << "The number of bits for a fixed width fingerprint must be a whole +ve "
                  "number\n";
          return 1;
        }
        if (verbose) {
          cerr << "Will write a fixed width fingerprint with " << fixed_width_fingerprint
               << " bits\n";
        }
      } else if (y.starts_with("scale=")) {
        y.remove_leading_chars(6);
        if (!y.numeric_value(bit_count_multiplier) || bit_count_multiplier < 2) {
          cerr << "The bit count scale= directive must be a whole +ve number > 1, '" << y
               << "' invalid\n";
          return 1;
        }
        if (verbose) {
          cerr << "Bit counts multipled by " << bit_count_multiplier << '\n';
        }
  ...
  if (cl.option_present('q')) {
    if (!process_queries(cl, start_atom_query, verbose, 'q')) {
  ...
  } else  // -P present, and maybe also -J
  {
    ...
    if (verbose) {
      cerr << "Extended connectivity index written as non-colliding sparse fingerprints, "
              "tag '"
           << tag << "'\n";
    }
  }
  ...
  if (cl.option_present('M')) {
    ...
    if (!read_bits_to_investigate(read_fname.null_terminated_chars(),
                                  bits_to_investigate)) {
    ...
    if (verbose) {
      cerr << "Read " << bits_to_investigate.size() << " bits to investigate from '"
           << read_fname << "'\n";
    }
  ...
  if (cl.option_present('Q')) {
    ...
    for (int i = 0; cl.value('Q', q, i); ++i) {
      if ("label" == q) {
        ...
        cerr << "The number of visited equalisation steps (-Q) must be a whole +ve "
                "number\n";
        usage(1);
      }
    }
  ...
  if (verbose) {
    ...
    if (looking_for_bit_meanings) {
      ...
      for (std::unordered_map<unsigned int, unsigned int>::const_iterator i =
               bits_to_investigate.begin();
           i != bits_to_investigate.end(); ++i) {
  ...
  if (nullptr != global_sparse_fingerprint) {
    ...
    for (int i = 0; i < istop; i++) {
      const Sparse_Fingerprint_Creator::FPHash& bits_found =
          global_sparse_fingerprint[i].bits_found();
      write_global_fingerprint(bits_found, i, molecules_read,
                               stream_for_global_fingerprint);
    }
