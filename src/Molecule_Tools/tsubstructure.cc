...
//cerr << "  -K             aromatic bonds lose their Kekule forms - no longer match single or double\n";
  ...
  cerr << "  -R <fname>     create report file, how many times each query matches. Enter '-R help' for info\n";
...
//cerr << "  -M aeuao       Atom Environments match Unmatched Atoms Only - unmatched during matching\n";
  ...
  cerr << "  -M onlysubiso  isotopic atoms in query molecules indicate substitution points. onlysubisoall means all are substituted\n";
...
// The default report file is the number of hits followed by the query name.
//    123 nitro
// If this is set (-R idfirst), the output looks like
// nitro 123
static int report_file_id_first = 0;
...
write_mdl_v30_bond_list(Molecule& m, const const_IWSubstring& query_name,
                        const Set_of_Atoms& e, int* btmp, std::ostream& output) {
  int nb = m.nedges();
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int k = i + 1; k < n; k++) {
      ...
      cerr << "Atoms " << j << " to " << l << " bond number " << ndx << '\n';
...
write_matched_atoms_as_mdl_v30_lists(
    Molecule& m, resizable_array_p<Substructure_Hit_Statistics>& queries,
    Substructure_Results* sresults, int* btmp, std::ostream& output) {
...
write_matched_atoms_as_mdl_v30_lists(
    Molecule& m, resizable_array_p<Substructure_Hit_Statistics>& queries,
    Substructure_Results* sresults, std::ostream& output) {
  int* btmp;
...
do_hits_tag_output(Molecule& m,
                   ...
                   const int* hits, std::ostream& output) {
  if (!work_as_filter) {
...
static int
do_nhits_tag_output(Molecule& m, int number_queries, const int* hits,
                    std::ostream& output) {
  if (!work_as_filter) {
    write_smiles_and_pcn(m, output);
  }
...
static int
do_array_output(const IWString& mname, int nhits, int number_queries, const int* hits,
                std::ostream& output) {
...
static int
do_proto_output(Molecule& m, const int rc,
                const resizable_array_p<Substructure_Hit_Statistics>& queries,
                const int* hits, std::ostream& output) {
...
static int
write_bob_coner_special(const Substructure_Results* sresults, int number_queries,
...
static int
write_directcolorfile(const Molecule& m, const Substructure_Results& sresults,
...
static int
WriteAsProto(Molecule& m, const IWString& mname,
             const resizable_array_p<Substructure_Hit_Statistics>& queries,
             const int* nhits, IWString_and_File_Descriptor& output) {
...
do_single_query(Molecule_to_Match& target, int query_number,
  ...
  if (0 == multiple_matches_each_query_counts_one &&
      nmatches >= report_multiple_matches && stream_for_multiple_matches.good()) {
...
do_all_queries(Molecule& m, resizable_array_p<Substructure_Hit_Statistics>& queries,
  ...
  for (int i = 0; i < number_queries; i++) {
    if (perform_search_even_if_names_the_same)
      ...
      cerr << "Molecule with no name detected, cannot determine self match, "
              "searching...\n";
...
static int
tsubstructure(Molecule& m, resizable_array_p<Substructure_Hit_Statistics>& queries,
              Substructure_Results* sresults, int* queries_matched) {
  if (report_progress()) {
  ...
  if (rx_match_only && !iwre2::RE2PartialMatch(m.name(), *rx_match_only)) {
    ...
    nmatched =
        do_all_queries(m, queries, sresults, tmp, new_elements, atom_isotopic_label);
  }
  ...
  if (nmatched) {
    ...
    if (write_matched_atoms_as_mdl_v30_atom_lists ||
        write_matched_atoms_as_mdl_v30_bond_lists) {
      write_matched_atoms_as_mdl_v30_lists(
          m, queries, sresults,
    ...
    if (report_non_matches) {
      cerr << molecules_read << ": " << nmatched << " matches: " << m.name() << '\n';
    }
...
tsubstructure(data_source_and_type<Molecule>& input,
  ...
  while (nullptr != (m = input.next_molecule())) {
    ...
    molecules_read++;
    if (verbose > 1) {
      cerr << "Processing " << molecules_read << " " << m->name() << '\n';
    }
...
tsubstructure_filter(iwstring_data_source& input,
  ...
  while (input.next_record(buffer)) {
    ...
    if (!tsubstructure_filter(buffer, queries, sresults, queries_matched, output)) {
      cerr << "Fatal error at line " << input.lines_read() << '\n';
      return 0;
    }
  }
...
static int
tsubstructure(const char* input_fname, const FileType input_type,
              resizable_array_p<Substructure_Hit_Statistics>& queries,
              Substructure_Results* sresults, int* queries_matched) {
  assert(nullptr != input_fname);

  if (work_as_filter) {
    return tsubstructure_filter(input_fname, queries, sresults, queries_matched,
                                std::cout);
  }
...
static int
open_match_or_non_match_stream(Command_Line& cl, const char* match_or_non_match,
                               IWString& fname, Molecule_Output_Object& stream_for) {
  ...
  if (!cl.option_present('o')) {
    ...
    cerr << "Cannot determine output types for " << match_or_non_match << '\n';
    return 0;
  }
  ...
  if (verbose) {
    cerr << "Setting stem for " << match_or_non_match << " to " << fname << '\n';
  }
...
static int
handle_file_opening(Command_Line& cl, IWString& fname,
                    resizable_array_p<Substructure_Hit_Statistics>& queries,
                    Molecule_Output_Object& stream_for, const char* match_or_non_match,
  ...
  if (strstr(match_or_non_match, "non")) {
    for (int i = 0; i < queries.number_elements(); i++) {
      ...
      if (!q->set_stream_for_non_matches(output_type, tmp.chars())) {
        cerr << "Cannot set output stream for non matches for query " << i << '\n';
        return 0;
      }

      if (verbose) {
        cerr << "Output stream for " << match_or_non_match << " to query " << i << " is '"
             << tmp << "'\n";
      }
    }
  } else  // matched structures
  {
    for (int i = 0; i < queries.number_elements(); i++) {
      ...
      cerr << "Output stream for matches to query " << i << " '" << q->comment()
           << "' is '" << tmp << "'\n";
    }
  }
...
static int
OpenProtoFile(const IWString& fname, IWString_and_File_Descriptor& output) {
  IWString with_stem;
...
static int
parse_set_of_options(Command_Line& cl, char flag,
                     resizable_array_p<Substructure_Hit_Statistics>& queries,
                     const char* match_or_non_match, int& report_matches,
                     Number_Assigner& number_assigner, int& append_query_details,
                     IWString& fname, Molecule_Output_Object& output_stream) {
  ...
  while (cl.value(flag, tmp, i)) {
    ...
    if (tmp.starts_with("REPO")) {
      ...
      need_to_open_file = 1;
      if (verbose) {
        cerr << "Details of " << match_or_non_match
             << " will be appended to the molecule name\n";
      }
    } else if ("QDTVB" == tmp) {
      append_query_details = 1;
      need_to_open_file = 1;
      ...
      if (verbose) {
        cerr << "Molecules which don't match the query will be written to the match "
                "stream\n";
      }
    } else if ("NONMX" == tmp) {
      if ('m' != flag) {
  ...
  return handle_file_opening(cl, fname, queries, output_stream, match_or_non_match,
...
tsubstructure(int argc, char** argv) {
  Command_Line cl(argc, argv,
                  "w:W:y:R:X:hj:J:E:rcls:S:t:A:K:bBfm:n:uvo:i:q:ag:pkG:Y:N:H:M:x:T:P:");
  ...
  if (cl.option_present('w')) {
    ...
    if (verbose) {
      cerr << "Fewer than " << min_hits_needed
           << " matches will be reported as non matches\n";
    }
  }

  if (cl.option_present('W')) {
    if (!cl.value('W', max_hits_needed) || max_hits_needed < min_hits_needed) {
      cerr << "The -W option requires a positive whole number at least "
           << min_hits_needed << '\n';
      usage(17);
    }

    if (verbose) {
      cerr << "More than " << max_hits_needed
           << " matches will be reported as non matches\n";
    }
  }
  ...
  // Nov 2022. Pass our own Molecule_to_Query to substructure reading.
  ...
  if (cl.option_present('M')) {
    ...
    while (cl.value('M', m, i++)) {
      if ("impring" == m) {
        ...
        if (verbose) {
          cerr << "Each query will find a maximum of " << max_matches_to_find
               << " matches\n";
        }
      } else if ("nosave" == m) {
        zoption++;
        ...
        if (verbose) {
          cerr << "Aromatic bonds in alternating Kekule rings will no longer match "
                  "Kekule forms\n";
        }
      } else if ("kekule" == m) {
        ...
        if (verbose) {
          cerr << "Molecules containing non organics in the same fragment as the matched "
                  "atoms don't match\n";
        }
      } else if ("tdt" == m || "TDT" == m) {
        work_as_filter = 1;
      ...
      } else if (m == "onlysubisoall") {
        mqs.set_substituents_only_at_isotopic_atoms(1);
        mqs.set_must_have_substituent_at_every_isotopic_atom(1);
        if (verbose) {
          cerr << "When building queries from molecules, isotopic atoms indicate "
                  "substitutions. All must be substitued\n";
        }
      } else if ("edno" == m) {
        ...
        if (verbose) {
          cerr << "Molecule to query transformations such that only aromatic atoms match "
                  "aromatic atoms\n";
        }
        ...
        unsigned int r;
        if (!m.numeric_value(r) || r < 1) {
          cerr << "The report multiple matches value (rmm=) must be a whole +ve number '"
               << m << "' not recognised\n";
          return 2;
        }
        ...
        tsubfp.set_each_query_writes_bit(0);
      } else if ("proto" == m) {
        ...
        append_to_matches << ' ' << m;
        if (verbose) {
          cerr << "Will append '" << append_to_matches
               << "' to the names of matched molecules\n";
        }
        NONM_append_non_match_query_details = 0;
        ...
        if (verbose) {
          cerr << "Will only attempt matching on molecules matching '"
               << rx_match_only->pattern() << "'\n";
        }
      } else if ("help" == m) {
        display_dash_M_options();
        return 0;
    ...
    if (0 == max_atom_count)
      ...
      cerr << "Will only match atoms with between " << min_atom_count << " and "
           << max_atom_count << " atoms\n";
    }
  }
  ...
  if (work_as_filter)  // no need to check input specifications
    ...
    if (1 == cl.number_elements() &&
        0 == strcmp("-", cl[0])) {  // reading a pipe, assume smiles
  ...
  queries.reserve(256);  // hopefully large enough to avoid extra mallocs
  ...
  if (cl.option_present('t')) {
    ...
    if (verbose) {
      cerr << "Queries hitting more than " << report_multiple_matches
           << " will be reported\n";
    }
  }
  ...
  if (cl.option_present('o')) {
    if (!cl.option_present('m') && !cl.option_present('n') && !cl.option_present('T')) {
      cerr
          << "Output type specified (-o option) but no output requested (-T, -m or -n)\n";
      usage(63);
    }
  }
  ...
  if (!parse_set_of_options(cl, 'm', queries, "matches", report_matches,
                            matched_structures_number_assigner,
                            append_match_details_to_molecule_name, fname_for_m,
  ...
  if (!parse_set_of_options(cl, 'n', queries, "non-matches", report_non_matches,
                            non_matched_structures_number_assigner,
                            append_non_match_details_to_molecule_name, fname_for_n,
  ...
  for (int i = 0; i < queries.number_elements(); i++) {
    ...
    //  cerr << "Process " << qi->number_elements() << " components of query " << i << '\n';
    ...
    qi->set_append_non_match_details_to_molecule_name(
  ...
  if (cl.option_present('T')) {
    ...
    while (cl.value('T', fname, i++)) {
      if ("QDT" == fname) {
        ...
        if (verbose) {
          cerr << "When looking for multiple matches each query counts once regardless "
                  "of embeddings\n";
        }
      } else {
  ...
  if (cl.option_present('J')) {
    ...
    tsubfp.set_default_fingerprint_nbits(default_fingerprint_nbits);
             ...
             array_output_is_number_of_matches)  //  Will we write Daylight fingerprints
                                                 //  or an array of hits
  {
    write_as_array = 1;
    if (verbose) {
  ...
  if (molecules_read > 0) {
    float fraction =
        static_cast<int>(molecules_which_match) / static_cast<float>(molecules_read);

    cerr << molecules_read << " molecules read, " << molecules_which_match
         << " molecules match fraction " << fraction << '\n';
  }
  ...
  for (int i = 0; i < nqueries; i++) {
    ...
    if (stream_for_report.rdbuf()->is_open()) {
      if (report_file_id_first) {
        stream_for_report << q->comment() << ' ' << q->molecules_which_match() << '\n';
      } else {
        stream_for_report << q->molecules_which_match() << ' ' << q->comment() << '\n';
      }
    }
  }
