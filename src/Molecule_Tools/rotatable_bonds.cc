...
in_ring_of_size(Molecule& m, const Bond* b, int single_bonds_in_rings_are_flexible)
{
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (ri->number_elements() <
        single_bonds_in_rings_are_flexible) {  // we are looking for a ring with at least
                                               // SINGLE_BONDS_IN_RINGS_ARE_FLEXIBLE atoms
      continue;
    }
...
static int
HandlePassingMolecule(Molecule& m) {
...
static int
HandleFailingMolecule(Molecule& m) {
  if (stream_for_molecules_failing.active()) {
...
rotatable_bonds(Molecule& m)
{
  ...
  if (simple_rotbond.active()) {
    ...
    if (append_results_to_name) {
      AppendRotbondToName(m, rotbond);
    }
    if (OkRotbondCount(m.natoms(), rotbond)) {
      return HandlePassingMolecule(m);
    } else {
      return HandleFailingMolecule(m);
    }
  }
  ...
  if (maximum_consecutive_flexible_bonds > 1 &&
      m.natoms() > maximum_consecutive_flexible_bonds) {
  ...
  if (append_longest_inter_ring_bond_distance_to_name ||
      maximum_inter_ring_bond_distance > 0) {
  ...
  if (maximum_inter_ring_bond_distance > 0 &&
      longest_inter_ring_distance > maximum_inter_ring_bond_distance) {
  ...
  if (matoms <= always_write_threshold) {
    ;
  } else if (pass && maximum_consecutive_flexible_bonds > 0 &&
             longest_flexible_chain >= maximum_consecutive_flexible_bonds) {
    pass = 0;
  }

  if (pass) {
    HandlePassingMolecule(m);
  } else {
    HandleFailingMolecule(m);
  }
...
rotatable_bonds(int argc, char** argv)
{
  ...
  if (cl.option_present('r')) {
    if (!cl.value('r', mark_rotatable_bonds) || mark_rotatable_bonds < 1) {
      cerr << "The mark rotatable bonds (-r) option must be followed by a whole positive "
              "number\n";
      usage(5);
    }
  ...
  if (cl.option_present('R')) {
    if (!cl.value('R', single_bonds_in_rings_are_flexible) ||
        single_bonds_in_rings_are_flexible < 3) {
      cerr << "The smallest ring size for flexible single bonds must be > 2\n";
      usage(5);
    }

    if (verbose) {
      cerr << "Single bonds in rings of size " << single_bonds_in_rings_are_flexible
           << " can be flexible\n";
    }
  }
  ...
  if (cl.option_present('m')) {
    if (!cl.value('m', min_rotatable_bonds) || min_rotatable_bonds < 0) {
      cerr << "The minimum number of rotatable bonds must be a non-negative whole "
              "number\n";
      usage(4);
    }

    if (verbose) {
      cerr << "Will discard molecules having " << min_rotatable_bonds
           << " or fewer rotatable bonds\n";
    }
  }

  if (cl.option_present('M')) {
    if (!cl.value('M', max_rotatable_bonds) || max_rotatable_bonds < 1 ||
        max_rotatable_bonds < min_rotatable_bonds) {
      cerr << "The minimum number of rotatable bonds must be a non-negative whole "
              "number\n";
      usage(4);
    }
    ...
    if (verbose) {
      cerr << "Will discard molecules having " << max_rotatable_bonds
  ...
  if (cl.option_present('f')) {
    ...
    while (cl.value('f', f, i++)) {
      if (f.starts_with("maxc=")) {
        ...
        if (!f.numeric_value(max_ncon_along_flexible_chain) ||
            max_ncon_along_flexible_chain < 2) {
          cerr << "The maximum connectivity of atoms along a flexible chain must be a "
                  "whole positive number > 1\n";
          usage(18);
        }
      ...
      } else if (!f.numeric_value(maximum_consecutive_flexible_bonds) ||
                 maximum_consecutive_flexible_bonds < 1 ||
                 maximum_consecutive_flexible_bonds < min_rotatable_bonds) {
        cerr << "The maximum consecutive flexible bonds (-f option) must be a positive "
                "whole number\n";
        usage(4);
      }
    }
    ...
    if (verbose) {
      cerr << "Will discard molecules having " << maximum_consecutive_flexible_bonds
           << " or more consecutive rotatable bonds\n";
    }

    if (verbose && 99 != max_ncon_along_flexible_chain) {
      cerr << "Long flexible chains stop at atoms with " << max_ncon_along_flexible_chain
           << " or more connections\n";
    }
  }
  ...
  if (cl.option_present('C')) {
    if (!cl.value('C', maximum_inter_ring_bond_distance) ||
        maximum_inter_ring_bond_distance < 1) {
      cerr << "The maximum permitted inter ring bond distance (-C) must be a whole +ve "
              "number\n";
      usage(3);
    }

    if (verbose) {
      cerr << "Will discard molecules having inter ring bond distances > "
           << maximum_inter_ring_bond_distance << " bonds\n";
    }
  }
  ...
  if (reduce_to_largest_fragment && use_largest_fragment) {
    cerr << "The -l (chop to largest fragment) and -e (use largest fragment) options are "
            "mutually exclusive\n";
    usage(3);
  }
  ...
  if (cl.option_present('w')) {
    if (!cl.value('w', always_write_threshold) || always_write_threshold < 1) {
      cerr << "The always write atom count threshold (-w) option must be a valid whole "
              "number\n";
      usage(2);
    }

    if (verbose) {
      cerr << "Will always write molecules with " << always_write_threshold
           << " atoms or fewer\n";
    }
  }
  ...
  if (verbose) {
    cerr << "Read " << molecules_read << " molecules, " << molecules_passing_filters
    ...
    for (int i = 0; i < lird.number_elements(); i++) {
      if (lird[i]) {
        cerr << lird[i] << " molecules had longest inter ring distances of " << i
             << " bonds\n";
      }
    }

    if (always_write_threshold > 0) {
      cerr << molecules_passed_by_atom_count << " molecules passed by having "
           << always_write_threshold << " or fewer atoms\n";
    }
  }
