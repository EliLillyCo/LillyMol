...
Group_of_Molecules::write_structure_group(IWString_and_File_Descriptor &output) {
  ...
  if (only_display_common_group) {
    ...
    output << " N = " << _activity.n() << " min " << _activity.minval() << " max "
...
Group_of_Molecules::write_max_activity(
  ...
  return _write_max_activity(_sida[item_with_max_activity], stream_for_smiles,
                             stream_for_activity);
}
...
Group_of_Molecules::write_item_closest_to_average(
  ...
  int rc = _write_max_activity(_sida[item_closest_to_average], stream_for_smiles,
                               stream_for_activity);
...
Group_of_Molecules::write_tabular_output(IWString_and_File_Descriptor &output) const {
  ...
  output << n << kSep << _activity.minval() << kSep << _activity.maxval() << kSep
         << (_activity.maxval() - _activity.minval()) << kSep
         << static_cast<float>(_activity.average());
  for (int i = 0; i < n; i++) {
    output << kSep << _sida[i]->id() << kSep << _sida[i]->activity();
  }
  output << '\n';
...
do_merge_output(const IW_STL_Hash_Map<IWString, Group_of_Molecules *> &structure_group,
  ...
  if (verbose) {
    cerr << "Wrote " << molecules_written << " from "
         << stream_for_inconsistent_groups.size() << " structure groups\n";
    cerr << groups_within_tolerance << " groups within tolerance, "
         << groups_outside_tolerance << " outside tolerance\n";
    if (acc.n()) {
      cerr << "Within group activity differences between " << acc.minval() << " and "
           << acc.maxval() << " ave "
           << static_cast<float>(acc.average_if_available_minval_if_not()) << '\n';
    }
  }
...
write_reconciled_data(
  ...
  for (auto i : structure_group) {
    ...
    if (1 == g->n()) {
      ...
      continue;
    } else if (max_difference_for_merging >= 0.0 &&
               !check_for_within_tolerance(*g, max_difference_for_merging)) {
      if (stream_for_inconsistent_groups.is_open()) {
        g->write_structure_group(stream_for_inconsistent_groups);
      }
      ...
      g->write_random_value_from_range_of_each_group(stream_for_smiles,
                                                     stream_for_activity);
    } else {
...
write_reconciled_data(
  ...
  if (activity_name.empty()) {
    ...
    stream_for_activity << activity_name << '\n';
  }
...
handle_multi_valued_activities(ID_to_Activity &id_to_activity) {
  ...
  cerr << acc_diff.n() << " measurements (same ID, different value), diffs btw "
       << acc_diff.minval() << " and " << acc_diff.maxval() << " ave "
       << static_cast<float>(acc_diff.average());
  if (acc_diff.n() > 2) {
  ...
  if (zero_diff > 0) {
    ...
    cerr << zero_diff << " measurements (same ID, same value), ave diff including these "
         << static_cast<float>(acc_diff.average()) << "\n";
  }
...
activity_consistency(int argc, char **argv) {
  ...
  if (cl.option_present('T')) {
    ...
    if (verbose) {
      cerr << "Initialised " << element_transformations.number_elements()
  ...
  if (cl.option_present('V')) {
    ...
    for (int i = 0; cl.value('V', v, i); ++i) {
      ...
      if ("max" == lcv) {
        ...
        if (verbose) {
          cerr << "Will make a random selection from within the range if any ID has "
                  "multiple values\n";
        }
        specifications_given++;
  ...
  if (cl.option_present('X')) {
    ...
    if (1 == class_name_to_number.size()) {
      cerr << "Got just one class label, something is wrong with the activity file, look "
    ...
    if (multi_valued_data_present) {
      cerr << "Multi valued data present for " << multi_valued_data_present
           << " molecules\n";
  ...
  if (cl.option_present('t')) {
    ...
    if (verbose) {
      cerr << "Differences less than " << max_difference_for_merging
           << " will be merged\n";
    }
  ...
  if (cl.option_present('O')) {
    if (!cl.option_present('t')) {
      cerr << "When using a inconsistent stream (-O) must also specify max tolerance "
              "(-T)\n";
      usage(4);
    }
  ...
  if (verbose) {
    cerr << "Read " << molecules_read << " molecules, " << structure_group.size()
         << " unique_structures\n";
  }
  ...
  if (max_difference_for_merging > static_cast<float>(0.0) && !cl.option_present('M')) {
    ...
    int rc = do_merge_output(structure_group, output);
    FreeMemory(id_to_activity, structure_group);
    return rc;
  }
  ...
  if (cl.option_present('M')) {
    ...
    while (cl.value('M', m, i++)) {
      if ("ave" == m || "AVE" == m) {
        ...
        if (verbose) {
          cerr << "In a classification scenario, will discard structures with "
    ...
    if (write_specifications > 1) {
      cerr
          << "Sorry, must specify just one writing specification for multi-valued data\n";
      usage(5);
    }
  ...
  if (verbose) {
    if (discard_inconsistent_class_assignments) {
      cerr << discarded_for_inconsistent_class_assignments
           << " molecules discarded for inconsistent class assignments\n";
    }
    if (molecules_discarded_for_non_allowed_elements) {
      cerr << molecules_discarded_for_non_allowed_elements
           << " molecules_discarded_for_non_allowed_elements\n";
    }
...
#define CARE_ABOUT_FREEING_MEMORY
