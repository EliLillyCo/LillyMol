...
class DicerFragmentOutput {
 private:
  ...
  int WriteText(const dicer_data::DicedMolecule& proto,
 ...
 public:
  ...
  void
  set_output_is_proto(int s) {
    _output_is_proto = s;
  }

  int
  output_is_proto() const {
    return _output_is_proto;
  }

  void
  set_buffered_output(int s) {
    _buffered_output = s;
  }

  int
  buffered_output() const {
  ...
  int
  write_if_buffer_holds_more_than(uint32_t s) {
...
class AllFragmentsStream {
 ...
 public:
  ...
  // to true.
  template <typename T>
  int MaybeWrite(const char* feature_name, const T& feature_value, bool& need_separator);
...
template <typename T>
int
AllFragmentsStream::MaybeWrite(const char* feature_name, const T& feature_value,
                               bool& need_separator) {
...
class Fragment_Statistics_Report_Support_Specification {
 ...
 public:
  ...
  int
  fractional_support_level_specified() const {
  ...
  int
  meets_support_requirement(int c) const {
    return c >= _int;
  }
};
...
Fragment_Statistics_Report_Support_Specification::build(const const_IWSubstring& s) {
  ...
  if (s.numeric_value(_int)) {
    if (_int <= 0) {
      cerr << "Fragment_Statistics_Report_Support_Specification::build:the support level "
  ...
  if (s.numeric_value(_fraction)) {
    if (_fraction < 0.0F || _fraction > 1.0F) {
      cerr << "Fragment_Statistics_Report_Support_Specification::build:the fractional "
              "support level must be a valid fraction\n";
      return 0;
    }
  ...
  cerr << "Fragment_Statistics_Report_Support_Specification::build:the support level "
          "specification must be a fraction or a +ve integer\n";
...
static int
parse_bond_specification_token(const const_IWSubstring& token, int matoms,
                               atom_number_t& a) {
  int j;
...
class Dicer_Arguments {
 ...
 public:
  ...
  void
  set_current_molecule(Molecule* m) {
    _current_molecule = m;
  }

  int
  atoms_in_molecule() const {
  ...
  int
  is_aromatic_in_parent(atom_number_t a) const {
    return _aromatic_in_parent[a];
  }

  int
  is_aromatic_in_parent(atom_number_t a1, atom_number_t a2, atom_number_t a3,
  ...
  void
  set_write_diced_molecules(int s) {
    _write_diced_molecules = s;
  }

  int
  max_recursion_depth_this_molecule() const {
    return _max_recursion_depth_this_molecule;
  }

  //  void set_max_recursion_depth_this_molecule(int s) {
  //  _max_recursion_depth_this_molecule = s;}
  ...
  const int*
  parent_atom_number_in_initial_molecule() const {
    return _atom_number_in_initial_molecule + (_recursion_depth - 1) * _atoms_in_molecule;
  }

  int*
  atom_number_in_initial_molecule() {
    return _atom_number_in_initial_molecule + _recursion_depth * _atoms_in_molecule;
  }

  int*
  xref_array_this_level() {
    return _xref + _recursion_depth * _atoms_in_molecule;
  }

  int update_numbering_for_subset(const int* xref, const Set_of_Atoms& a0,
                                  const Set_of_Atoms& a1, Set_of_Atoms& b0,
                                  Set_of_Atoms& b1);
  ...
  int write_fragments_found_this_molecule(Molecule& m, int breakable_bonds,
  ...
  int WriteAllFragments(Molecule& parent, Molecule& fragment,
...
static int
convert_to_atom_numbers_in_child(const Set_of_Atoms& rbb, const int* xref,
                                 Set_of_Atoms& s) {
...
class Dicer_Transformation {
 ...
 public:
  ...
  int
  process(Molecule& m, Dicer_Arguments& dicer_args, Breakages& b,
          Breakages_Iterator& bi) const {
    return _process(m, dicer_args, b, bi);
  }

  int
  break_bonds(Molecule& m) const {
    return _break_bonds(m);
  }

  virtual int _debug_print(std::ostream&) const = 0;
  virtual int _process(Molecule&, Dicer_Arguments&, Breakages&,
                       Breakages_Iterator& bi) const = 0;
  virtual int _break_bonds(Molecule&) const = 0;

  int
  write_bonds_to_be_broken(std::ostream& os) const {
...
class Bond_Breakage : public Dicer_Transformation {
 ...
 public:
  ...
  int
  symmetry_equivalent() const {
    return _symmetry_equivalent;
  }

  void
  set_symmetry_equivalent(int s) {
...
class Chain_Bond_Breakage : public Bond_Breakage {
 private:
  ...
  int _process(Molecule& m0, resizable_array<Molecule*>& components,
               const IWString& mname, Dicer_Arguments& dicer_args, Breakages& breakages,
               Breakages_Iterator& bi) const;
...
Chain_Bond_Breakage::Chain_Bond_Breakage(atom_number_t j1, atom_number_t j2)
    : Bond_Breakage(j1, j2) {
...
class Fused_Ring_Breakage : public Bond_Breakage {
 ...
 public:
  Fused_Ring_Breakage(atom_number_t, atom_number_t, atom_number_t, atom_number_t,
                      atom_number_t, atom_number_t);
  ...
  int
  aromatic_in_parent() const {
    return _aromatic_in_parent;
  }

  void
  set_aromatic_in_parent(int s) {
...
Fused_Ring_Breakage::Fused_Ring_Breakage(atom_number_t z11, atom_number_t z1,
                                         atom_number_t z12, atom_number_t z21,
                                         atom_number_t z2, atom_number_t z22)
...
PairTransformation::PairTransformation(Dicer_Transformation* t1,
                                       Dicer_Transformation* t2) {
  _t1 = t1;
  _t2 = t2;
}
...
class Spiro_Ring_Breakage : public Dicer_Transformation {
 ...
 public:
  Spiro_Ring_Breakage(atom_number_t, atom_number_t, atom_number_t, atom_number_t,
                      atom_number_t);
...
class Dearomatise : public Dicer_Transformation {
 ...
 public:
  ...
  void
  set_ring_atoms(const Set_of_Atoms& r) {
    _ring = r;
  }
};
...
class Ring_Bond_Breakage : public Dicer_Transformation, public Set_of_Atoms {
 ...
 public:
  ...
  void
  set_atom_that_must_be_a_ring_atom(atom_number_t a) {
    _atom_that_must_be_a_ring_atom = a;
  }
};

Ring_Bond_Breakage::Ring_Bond_Breakage(const Set_of_Atoms& s, bool is_arom)
    : ::Set_of_Atoms(s) {
  _is_arom = is_arom;
...
class Breakages {
 ...
 public:
  ...
  int discard_breakages_that_result_in_fragments_too_small(
      Molecule& m, const Dicer_Arguments& dicer_args);
...
Dicer_Arguments::adjust_max_recursion_depth(int bonds_to_break_this_molecule,
  ...
  auto fragments_that_will_be_produced = fard.fragments_at_recursion_depth(
      max_recursion_depth, bonds_to_break_this_molecule);

  // #define DEBUG_ADJUST_MAX_RECURSION_DEPTH
#ifdef DEBUG_ADJUST_MAX_RECURSION_DEPTH
  cerr << bonds_to_break_this_molecule << " bonds, max_recursion_depth "
       << max_recursion_depth << " yields " << fragments_that_will_be_produced
       << " fragments\n";
#endif
...
#ifdef DEBUG_ADJUST_MAX_RECURSION_DEPTH
    cerr << "at " << _max_recursion_depth_this_molecule << ", "
         << bonds_to_break_this_molecule << " will produce "
         << fragments_that_will_be_produced << '\n';
#endif
  }
...
Dicer_Arguments::_add_new_smiles_to_global_hashes(const IWString& smiles) {
  ...
  uint32_t s = smiles_to_id.size();

  assert(s == id_to_smiles.size());
...
contains_user_specified_queries(Molecule& m) {
  ...
  for (int i = 0; i < nq; i++) {
    if (user_specified_queries_that_fragments_must_contain[i]->substructure_search(
            target)) {
      return 1;
    }
  }
...
Dicer_Arguments::MaybeAddToHash(const IWString& smiles) {
  ...
  if (f1 == smiles_to_id.end()) {  // fragment never been seen before
    ...
    //  << accumulate_starting_parent_information <<'\n';
...
Dicer_Arguments::is_unique(Molecule& m) {
  ...
  // assert (b->nset() == matoms);    if we have added environment atoms, this will not be
  // true
  ...
  for (int i = 0; i < n; i++) {
    ...
    if ((*b) == *(_stored_bit_vectors[i])) {
      if (record_presence_and_absence_only) {  // sept 2011, I don't think is useful, make
...
int
Dicer_Arguments::write_fragments_found_this_molecule(Molecule& m, int breakable_bonds,
                                                     DicerFragmentOutput& output) const {
  ...
  for (ff_map::const_iterator f = _fragments_found_this_molecule.begin();
    ...
    if (vf_structures_per_page) {
      if (smiles_written > 0 && 0 == smiles_written % vf_structures_per_page) {
        output << _current_molecule->smiles() << ' ' << _current_molecule->name()
               << " PARENT\n";
      }
      smiles_written++;
    }
    ...
    if (append_atom_count) {
      const int n = lillymol::count_atoms_in_smiles(*s);
      output << " NATOMS " << n;
...
int
Dicer_Arguments::ToProto(Molecule& m, int breakable_bonds,
                         dicer_data::DicedMolecule& proto) const {
...
int
Dicer_Arguments::WriteAsProto(Molecule& m, int breakable_bonds,
                              DicerFragmentOutput& output) const {
...
Dicer_Arguments::store_atom_types(Molecule& m, Atom_Typing_Specification& ats) {
  if (nullptr == _atom_type) {
    ...
    _atom_type = new uint32_t[matoms];
  }
...
Breakages::_destroys_too_much_aromaticity(Molecule& m, const int aromatic_atom_count,
                                          const Fused_Ring_Breakage& f) const {
  if (_destroys_too_much_aromaticity(m, aromatic_atom_count, f.a11(), f.a1(), f.a2(),
                                     f.a21())) {
    return 1;
  }

  if (_destroys_too_much_aromaticity(m, aromatic_atom_count, f.a12(), f.a1(), f.a2(),
                                     f.a22())) {
    return 1;
  }
...
create_fused_ring_breakage(Molecule& m, const atom_number_t a11, const atom_number_t a1,
                           const atom_number_t a12, const atom_number_t a21,
                           const atom_number_t a2, const atom_number_t a22) {
...
Breakages::identify_spiro_ring_bonds_to_be_broken(Molecule& m) {
  ...
  for (int i = 0; i < na; i++) {
    ...
    Spiro_Ring_Breakage* s1 = new Spiro_Ring_Breakage(i, neighbors[0], neighbors[1],
                                                      neighbors[2], neighbors[3]);
    Spiro_Ring_Breakage* s2 = new Spiro_Ring_Breakage(i, neighbors[2], neighbors[3],
...
create_ring_bond_breakage(Molecule& m, const Ring* r) {
  ...
  // close the ring, we will be processing bonds
  not_part_of_adjacent_ring.add(not_part_of_adjacent_ring[0]);

  Ring_Bond_Breakage* tmp =
      new Ring_Bond_Breakage(not_part_of_adjacent_ring, r->is_aromatic());
...
Breakages::identify_ring_bonds_to_be_broken(Molecule& m) {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    //  m.isotopically_labelled_smiles() << " ex ring " << atom_in_adjacent_ring << '\n';
...
class Fragment_Info {
 ...
 public:
  ...
  int
  is_subset_of_another() const {
    return _is_subset_of_another;
  }

  void
  set_is_subset_of_another(int s) {
...
Fragment_Info::is_subset_of(const Fragment_Info& rhs) const {
  if (_times_hit !=
      rhs._times_hit) {  // must be present in exactly the same number of molecules
    return 0;
  }
...
do_eliminate_fragment_subset_relations() {
  ...
  for (int i = 0; i < ndx; i++) {
    ...
    cerr << smiles << " ndx = " << fi.ndx() << " times_hit " << fi.times_hit()
         << " natoms " << fi.natoms() << '\n';
  }
#endif
...
static int
do_apply_atomic_number_isotopic_labels(Molecule& m, const atom_number_t j1,
...
SinglyBondedTo(const Molecule& m, atom_number_t zatom, atomic_number_t target) {
...
// atom with atomic number `target`.
std::optional<atom_number_t>
DoublyBondedTo(const Molecule& m, atom_number_t zatom, atomic_number_t target) {
  for (const Bond* b : m.atom(zatom)) {
    if (!b->is_double_bond()) {
      continue;
    }
...
int
IsAmideCarbon(const Molecule& m, atom_number_t zatom) {
  const Atom* a = m.atomi(zatom);
  if (a->ncon() != 3) {
    return 0;
  }
...
int
IsAmideNitrogen(const Molecule& m, atom_number_t zatom) {
...
int
IsAmideOxygen(const Molecule& m, atom_number_t zatom) {
...
int
IsAmide(const Molecule& m, atom_number_t zatom) {
  const Atom* a = m.atomi(zatom);
...
int
IsSulfonamideNitrogen(const Molecule& m, atom_number_t zatom) {
...
int
IsSulfonamideSulphur(const Molecule& m, atom_number_t zatom) {
  int doubly_bonded_oxygens = 0;
  int nitrogens = 0;
  for (const Bond* b : m.atom(zatom)) {
    atom_number_t j = b->other(zatom);
...
int
IsSulfonamideOxygen(const Molecule& m, atom_number_t zatom) {
...
int
IsSulfonamide(const Molecule& m, atom_number_t zatom) {
...
int
IsNitroNitrogen(const Molecule& m, atom_number_t zatom) {
...
int
IsNitroOxygen(const Molecule& m, atom_number_t zatom) {
  const Atom& a = m.atom(zatom);
  if (a.ncon() != 1) {
    return 0;
  }
...
int
IsNitro(const Molecule& m, atom_number_t zatom) {
...
int
IsAliphaticRingAtom(Molecule& m, const atom_number_t zatom, atomic_number_t target) {
...
static Env
IdentifyEnvironment(Molecule& m, const atom_number_t zatom) {
...
static int
do_apply_environment_isotopic_labels(Molecule& m, const atom_number_t j1,
                                     const atom_number_t j2) {
...
static void
ApplyAtomTypeIsotopicLabel(Molecule& m, const atom_number_t j1, const atom_number_t j2,
...
dump_iw_atom_type(Molecule& frag, const Molecule& comp, const int parent_atoms,
  ...
  for (int i = 0; i < parent_atoms; ++i) {
    ...
    //  cerr << " i = " << i << " xref " << xref[i] << " iso " << comp.isotope(xref[i]) <<
    //  " atype " << atypes[i] << '\n';
...
int
Dicer_Arguments::_write_fragment_and_hydrogen(
    Molecule& m, int* subset_specification, IWString_and_File_Descriptor& output) const {
  const int matoms = m.natoms();
...
Dicer_Arguments::AppendFragmentAndComplement(Molecule& m, const IW_Bits_Base& b,
  ...
  // We do not know the ID of this fragment, and we don't care about the number of
  // instances in the molecule.
...
class Atom_Rank_Atom {
 ...
 public:
  ...
  void
  set(atom_number_t af, int c, atom_number_t ac) {
    _in_frag = af;
    _canon = c;
    _in_comp = ac;
  }
...
reset_isotope_multiple_connections(Molecule& m, const atom_number_t zatom,
  ...
  // attached.iwqsort_lambda([] (int i1, int i2) { if (i1 < i2) return -1; if (i1 > i2)
  // return 1; return 0;});
...
class Atom_Rank_Atom_Comparator {
 ...
 public:
  int
  operator()(const Atom_Rank_Atom& ara1, const Atom_Rank_Atom& ara2) const {
...
template <typename T>
void
AppendMaybeWithSeparator(bool& need_separator, char sep, const T& s,
                         IWString_and_File_Descriptor& output) {
  if (need_separator) {
    output << sep;
  } else {
...
int
Dicer_Arguments::WriteAllFragments(Molecule& parent, Molecule& fragment,
                                   AllFragmentsStream& all_fragments_stream) const {
  ...
  all_fragments_stream.MaybeWrite("smiles", _current_molecule->unique_smiles(),
                                  need_separator);
...
int
WriteAllFragments(Molecule& parent, Molecule& fragment,
                  IWString_and_File_Descriptor& stream_for_all_fragments) {
...
write_hash_data(IWString_and_File_Descriptor& output,
  ...
  for (IW_STL_Hash_Map_uint::const_iterator i = smiles_to_bit.begin();
       i != smiles_to_bit.end(); ++i) {
...
void
ToProto(const IWString& smiles, uint32_t count,
        const IW_STL_Hash_Map_String& starting_parent, dicer_data::DicerFragment& proto) {
  proto.set_smi(smiles.AsString());
  proto.set_n(count);
  proto.set_nat(lillymol::count_atoms_in_smiles(smiles));
  if (auto f = starting_parent.find(smiles); f != starting_parent.end()) {
...
static int
WriteFragmentStatisticsAsProto(const IWString& smiles, uint32_t count,
                               IWString_and_File_Descriptor& output) {
...
ReportSuppressed(const IW_STL_Hash_Map_uint& molecules_containing,
                 uint32_t suppressed_by_support_requirement, std::ostream& output) {
  float fraction =
      iwmisc::Fraction<float>(suppressed_by_support_requirement,
...
static int
WriteFragmentStatisticsAsBinaryProto(
    const IW_STL_Hash_Map_uint& molecules_containing,
...
write_fragment_statistics(const IW_STL_Hash_Map_uint& molecules_containing,
  ...
  for (const auto& [s, count] : molecules_containing) {
    ...
    if (append_atom_count) {
      output << " natoms " << lillymol::count_atoms_in_smiles(s);
    }
...
WriteFragmentStatisticsAsBinaryProto(
    const IW_STL_Hash_Map_uint& molecules_containing,
    Fragment_Statistics_Report_Support_Specification& fsrss, IWString& fname) {
  iw_tf_data_record::TFDataWriter output;
...
static int
do_write_to_stream_for_post_breakage(
    Molecule& m, IWString_and_File_Descriptor& stream_for_post_breakage) {
...
static int dicer(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
...
call_dicer(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
  ...
  cerr << "Calling dicer, depth " << dicer_args.recursion_depth() << ", molecule is '"
       << m.smiles() << "', can_go_deeper " << dicer_args.can_go_deeper() << '\n';
  cerr << "max_recursion_depth " << max_recursion_depth << " max recursion_depth this m "
       << dicer_args.max_recursion_depth_this_molecule() << '\n';
#endif
...
int
Chain_Bond_Breakage::_process(Molecule& m0, resizable_array<Molecule*>& components,
                              const IWString& mname, Dicer_Arguments& dicer_args,
                              Breakages& breakages, Breakages_Iterator& bi) const {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (c.natoms() <
        dicer_args.lower_atom_count_cutoff()) {  // cannot be subdivided any more
      continue;
    }
...
Ring_Bond_Breakage::_do_ring_breaking(Molecule& m, const Set_of_Atoms& rbb,
  ...
  if (6 == m.atomic_number(r[n - 2]) &&
      6 == m.atomic_number(
               r[n - 1])) {  // don't break CC ring bonds, just to keep numbers down
    return 0;
  }
  ...
  for (int i = n - 2; i >= 1; i--) {
    ...
    cerr << "i = " << i << " Processing atom " << r[i] << " in '"
         << mcopy.isotopically_labelled_smiles() << '\n';
#endif
    ...
    r.adjust_for_loss_of_atom(r[i],
                              1);  // 1 means keep item in list, do not bother removing it
    ...
    if (i > 1) {
      ...
      (void)convert_to_atom_numbers_in_child(rbb, xref,
                                             r);  // expected that not all items present
    }
  }
...
static void
identify_extra_ring_attachment(const Molecule& m, const Set_of_Atoms& r,
                               atom_number_t zatom, Set_of_Atoms& exr) {
  const Atom* a = m.atomi(zatom);
...
int
Ring_Bond_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args,
                             Breakages& breakages, Breakages_Iterator& bi) const {
  ...
  // cerr << "RBB:Processing atoms " << r << " in " << m.isotopically_labelled_smiles() <<
  // " other ring " << xref[_atom_that_must_be_a_ring_atom] << '\n';
  ...
  // I don't think we want to break aromatic bonds, not sure this ever happens, besides,
  // the code is wrong, prev does not get updated
  if (this->_is_arom) {
    prev = r[0];
  ...
  identify_extra_ring_attachment(mcopy, r, r[r.size() - 2], exr2);
  ...
  for (int i = 0; i < exr2.number_elements(); i++) {
    if (mcopy.are_bonded(r[r.size() - 2], exr2[i])) {
      mcopy.remove_bond_between_atoms(r[r.size() - 2], exr2[i]);
    }
  }
...
int
Fused_Ring_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args,
                              Breakages& breakages, Breakages_Iterator& bi) const {
  ...
  if (dicer_args.is_aromatic_in_parent(a12_in_parent, a1_in_parent, a2_in_parent,
                                       a22_in_parent)) {
  ...
  if (1 == mcopy.number_fragments()) {
    cerr << "Fused_Ring_Breakage::molecule has just one fragment "
         << mcopy.isotopically_labelled_smiles() << " " << mcopy.name() << '\n';
    cerr << "Removed bonds between atoms " << a11 << " and " << a1 << " AND " << a21
         << " and " << a2 << '\n';
...
int
Spiro_Ring_Breakage::_process(Molecule& m, Dicer_Arguments& dicer_args,
                              Breakages& breakages, Breakages_Iterator& bi) const {
  ...
  if (!m.are_bonded(a11, aCenter) || !m.are_bonded(a12, aCenter) ||
      !m.are_bonded(a21, aCenter) || !m.are_bonded(a22, aCenter)) {
    return 0;
  }
...
// #define DEBUG_DO_RING_SPLITTING
  ...
  if (1 == mcopy.number_fragments()) {
    cerr << "Spiro_Ring_Breakage::molecule has just one fragment "
         << mcopy.isotopically_labelled_smiles() << " " << mcopy.name() << '\n';
    cerr << "Removed bonds between atoms " << a11 << " and " << aCenter << " AND " << a12
         << " and " << aCenter << '\n';
    cerr << "Starting molecule " << m.isotopically_labelled_smiles() << '\n';
  }
...
static int
dicer(Molecule& m, Dicer_Arguments& dicer_args, Breakages& breakages,
      Breakages_Iterator bi) {
// #define DEBUG_DICER
#ifdef DEBUG_DICER
  cerr << "Entering dicer " << dicer_args.recursion_depth() << ' ' << m.smiles()
       << " iterator " << bi.state() << '\n';
#endif
  ...
  while (nullptr != dt) {
#ifdef DEBUG_DICER
    std::cerr << "####" << dicer_args.recursion_depth() << "  --  " << bi.state()
              << std::'\n';
#endif
    dicer_args.increment_recursion_depth();
...
Breakages::lower_numbers_if_needed(Molecule& m, int& bonds_to_break_this_molecule,
  ...
  // cerr << "At recursion depth " << max_recursion_depth_this_molecule << " will create "
  // << f << " fragments, cf max " << max_fragments_per_molecule << '\n';
  ...
  // Wow, what to do at this stage!! We have removed low priority breakages, lowering the
  // recursion level will not solve our problem.

  max_recursion_depth_this_molecule--;
  cerr << "Lowered bonds to be broken to " << bonds_to_break_this_molecule
       << " lowering recursion level to " << max_recursion_depth_this_molecule
       << " frags "
...
static int
count_side_of_bond(const Molecule& m, atom_number_t astart, int& encountered_ring,
                   int* tmp) {
  int rc = 1;
...
atoms_on_side_of_bond_satisfy_min_frag_size(const Molecule& m,
                                            const Dicer_Arguments& dicer_args,
                                            atom_number_t b1, atom_number_t b2,
                                            int* tmp) {
  if (1 == m.ncon(b1)) {
  ...
  // cerr << "From " << b2 << " to " << b1 << " count " << c << ", ring " <<
  // encountered_ring << '\n';
...
int
Breakages::discard_breakages_that_result_in_fragments_too_small(
    Molecule& m, const Dicer_Arguments& dicer_args) {
  int matoms = m.natoms();
...
Breakages::identify_bonds_to_break(Molecule& m) {
  ...
  for (int i = 0; i < nq; i++) {
    ...
    for (int j = 0; j < nhits; j++) {
      ...
      if (e->size() < 2) {
        cerr << "dicer:gack, query " << i << " only has " << e->size()
             << " matched atoms\n";
        return 0;
      }
  ...
  if (verbose > 1) {
    cerr << queries_hitting << " of " << nq << " queries produced " << rc
         << " matches to '" << m.name() << "'\n";
  }
...
dicer(Molecule& m, DicerFragmentOutput& output) {
  ...
  if (queries_for_bonds_to_not_break.size()) {
  ...
  if (max_fragments_per_molecule > 0) {
    dicer_args.adjust_max_recursion_depth(bonds_to_break_this_molecule,
                                          max_fragments_per_molecule);
  }
  ...
  USPVPTR* uspvptr = new USPVPTR[m.natoms()];
  std::unique_ptr<USPVPTR[]> free_uspvptr(uspvptr);
  ...
  if (write_smiles) {
    dicer_args.write_fragments_found_this_molecule(m, bonds_to_break_this_molecule,
                                                   output);
  }
...
static int
dicer(data_source_and_type<Molecule>& input, DicerFragmentOutput& output) {
  Molecule* m;
...
static int
dicer_tdt_filter(iwstring_data_source& input, DicerFragmentOutput& output) {
  const_IWSubstring buffer;
...
static int
dicer_tdt_filter(const char* fname, DicerFragmentOutput& output) {
  iwstring_data_source input(fname);
...
display_misc_B_options(std::ostream& os) {
  ...
  os << " -B FMC:<qry>      queries that fragments reported must contain, for smarts: -B FMC:SMARTS:smt \n";
  os << " -B                use zero occurrences to enforce a negative selection:  -B FMC:SMARTS:0[B,P,Br,I]\n";
...
static int
dicer(const char* fname, FileType input_type, DicerFragmentOutput& output) {
...
dicer(int argc, char** argv) {
  ...
  if (cl.option_present('X')) {
    ...
    if (verbose) {
      cerr << "Will produce a max of " << max_fragments_per_molecule
  ...
  if (cl.option_present('J')) {
    ...
    if (!fingerprint_tag.starts_with("NC")) {
      cerr << "Dicer produces sparse fingerprints only, tag must start with 'NC', '"
           << fingerprint_tag << " invalid\n";
      return 1;
    }
  ...
  if (cl.option_present('m')) {
    if (!cl.value('m', lower_atom_count_cutoff) || lower_atom_count_cutoff < 0) {
      cerr << "The minimum number of atoms in a fragment (-m) option must be a whole +ve "
              "number\n";
      usage(4);
    }

    if (verbose) {
      cerr << "Will discard fragments with fewer than " << lower_atom_count_cutoff
           << " atoms\n";
    }
  ...
  if (cl.option_present('M')) {
    ...
    for (int i = 0; cl.value('M', m, i); ++i) {
      if (m.starts_with("maxnr=")) {
        ...
        cerr << "The maximum number of atoms in a fragment (-M) option must be a whole "
                "+ve number\n";
        return 1;
      } else if (verbose) {
        if (verbose) {
          cerr << "Will discard fragments with more than " << upper_atom_count_cutoff
               << " atoms\n";
        }
      }
  ...
  // Too hard to check support levels since some may be numeric, some may be
  // percentages....
  ...
  if (cl.option_present('B')) {
    ...
    while (cl.value('B', b, i++)) {
      if ("term" == b) {
        ...
        if (verbose) {
          cerr << "Will suppress output of fragments that are exact subsets of other "
                  "fragments\n";
        }
        ...
        if (verbose) {
          cerr << "Fragment statistics will be written to '"
               << name_for_fragment_statistics_file << "'\n";
        }
        ...
        if (verbose) {
          cerr << "User specified bond breaking queries will be run in addition to the "
                  "default set\n";
        }
        ...
        if (0 == b.length()) {
          ...
          if (':' != b[0] || 1 == b.length()) {
            cerr << "To specify the number of heteroatoms, must be of the form "
                    "'FMC:heteroatoms:n' '"
                 << b << "' invalid\n";
            usage(1);
          }
          ...
          if (!b.numeric_value(fragments_must_contain_heteroatom) ||
            ...
            cerr << "The FMC:heteroatom: directive must be followed by a whole +ve "
                    "number\n";
            usage(1);
          }
        }
        if (verbose) {
          cerr << "Fragments reported must contain at least "
               << fragments_must_contain_heteroatom << " heteroatoms\n";
        }
      } else if (b.starts_with("FMC:")) {
        b.remove_leading_chars(4);
        if (!process_cmdline_token(
                ' ', b, user_specified_queries_that_fragments_must_contain, verbose)) {
        ...
        if (!b.numeric_value(vf_structures_per_page) || vf_structures_per_page < 1) {
          cerr << "The insert parent every n written value must be a whole +ve integer, '"
               << b << "' is invalid\n";
          return 3;
        }

        if (verbose) {
          cerr << "Will insert parent structure into output every "
               << vf_structures_per_page << " fragments written\n";
        }
      } else if ("flush" == b) {
      ...
      if (user_specified_queries_that_fragments_must_contain.size()) {
        cerr << "Defined " << user_specified_queries_that_fragments_must_contain.size()
    ...
    if (bond_break_file.length()) {
      ...
      if (verbose) {
        cerr << "Bonds to be broken written in .sdf form to '" << bond_break_file
             << "', tag '" << bbrk_tag << "'\n";
      }
    } else {
  ...
  if (nq) {
    ...
    if (add_user_specified_queries_to_default_rules) {
      cerr << "Have been asked to run queries in addition to default rules, but no extra "
              "queries (-q)\n";
      return 3;
    }
  ...
  if (cl.option_present('K')) {
    ...
    if (read_fname.length()) {
      ...
      if (verbose) {
        cerr << "Read " << smiles_to_id.size()
             << " smiles to bit number relationships from '" << read_fname << "'\n";
      }
    }
  ...
  if (collect_time_statistics) {
    cerr << "Processing took between " << time_acc.minval() << " and "
         << time_acc.maxval() << ", ave " << time_acc.average() << '\n';
  }
  ...
  if (0 == molecules_containing.size())
    ...
    WriteFragmentStatisticsAsBinaryProto(molecules_containing, fsrss,
                                         name_for_fragment_statistics_file);
  } else if (name_for_fragment_statistics_file.length()) {
    write_fragment_statistics(molecules_containing, fsrss,
                              name_for_fragment_statistics_file);
  }
