...
static int
complex_chirality(Molecule& m, IWString_and_File_Descriptor& output) {
  output << m.name() << ' ';
  ...
  int* is_amino_acid = new_int(matoms);
  std::unique_ptr<int[]> free_is_amino_acid(is_amino_acid);
  ...
  output << nc << ' ' << amino_acid_chiral_count_this_molecule << ' '
         << adjacent_chiral_centres_this_molecule << '\n';
...
static int
get_smarts(Command_Line& cl, const char flag,
  ...
  if (verbose) {
    ...
    for (int i = 0; i < amino_acid_chiral_count.number_elements(); i++) {
      if (amino_acid_chiral_count[i]) {
        cerr << amino_acid_chiral_count[i] << " molecules had " << i
             << " amino acid chiral centres\n";
      }
    }
    for (int i = 0; i < adjacent_chiral_centres.number_elements(); i++) {
      if (adjacent_chiral_centres[i]) {
        cerr << adjacent_chiral_centres[i] << " molecules had " << i
