...
Known_Fragment_Data::debug_print(std::ostream& os) const {
  os << "Fragment data contains " << _known_salt_mf.size() << " known salts MF and "
     << _known_parent_mf.size() << " known parent MF fragments\n";
...
int
Known_Fragment_Data::_common_print_set(const _formula_usmi& h, std::ostream& os) const {
...
int
Known_Fragment_Data::_common_read(const const_IWSubstring& fname, _formula_usmi& mf,
                                  _formula_usmi& usmi) {
...
Known_Fragment_Data::_common_read(data_source_and_type<Molecule>& input,
                                  _formula_usmi& mf, _formula_usmi& usmi) {
  ...
  while (nullptr != (m = input.next_molecule())) {
    ...
    if (!m->valence_ok()) {
      // This warning is not useful. Lots of things with invalid valences show up
      // as counterions.
      // cerr << m->smiles() << " warning, invalid valence '" << m->name() << "'\n";
    }
...
int
Known_Fragment_Data::_add_to_hash(Molecule& m, _formula_usmi& mf, _formula_usmi& usmi) {
  _natoms[m.natoms()]++;
...
static int
just_acid_and_carbon(Molecule& m, const int* fragment_membership, int f) {
  int matoms = m.natoms();
...
static int
just_nitrogen_and_carbon(Molecule& m, const int* fragment_membership, int f) {
  ...
  if (0 == number_4_connected_nitrogen && 0 == number_3_connected_nitrogen &&
      1 == number_2_connected_nitrogen) {
    return 1;
  }
...
Known_Fragment_Data::process(Molecule& m) {
  ...
  _scan_known_fragments(m, mf, fragments, _known_salt_mf, _known_salt_usmi,
                        fragments_to_be_removed);

  if (fragments_to_be_removed.number_elements() + 1 ==
      fragments.number_elements()) {  // only one fragment NOT being removed
    return m.delete_fragments(fragments_to_be_removed);
  }

  if (fragments_to_be_removed.number_elements() ==
      m.number_fragments())  // everything being removed
  {
  ...
  // arbitrary decision. If we will be left with two or fewer atoms, that cannot
  // be right.
  if ((m.natoms() - atoms_being_removed) <= 2) {
    return 0;
  }

  resizable_array<int> fragments_to_keep;

  _scan_known_fragments(m, mf, fragments, _known_parent_mf, _known_parent_usmi,
                        fragments_to_keep);
...
// Scan the tokens in `molecular_formula` and determine if each fragment
// is part of the known fragments.
// If so, add the fragment numbers to `fragments_identified`.
int
Known_Fragment_Data::_scan_known_fragments(
    Molecule& m, const IWString& molecular_formula,
    resizable_array_p<Molecule>& fragments, const _formula_usmi& mf,
    const _formula_usmi& usmi, resizable_array<int>& fragments_identified) const {
  IWString token;
  // cerr << "Starting molecular formula is " << molecular_formula << "\n";
  // Scan tokens in the molecular formula, `frag` is the index
  for (int i = 0, frag = 0; molecular_formula.nextword(token, i, '.'); ++frag) {
    ...
    if (0 ==
        _natoms[fi->natoms()]) {  // no molecules with this number of atoms in the set
      continue;
    }
...
/*
  There is some ambiguity here if we have been instructed to NOT
  _remove_everything_if_all_fragments_match What should we do in that case? Right now, we
  leave the molecule alone
*/

int
Known_Fragment_Data::_delete_set_of_fragments(
    Molecule& m, const resizable_array<int>& fragments_to_be_removed) const {
