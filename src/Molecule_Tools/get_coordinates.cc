...
static int
do_john_lampe_output(const Molecule& m, const iwaray<IWString>& atom_label,
                     std::ostream& output) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    output << a->x() << ' ' << a->y() << ' ' << a->z() << ' ' << a->atomic_symbol() << ' '
           << aname << '\n';
    atoms_written++;
  }

  if (0 == atoms_written && verbose) {
    cerr << "Huh, no atoms hit in molecule " << m.name() << '\n';
  }
...
static int
get_coordinates(const Molecule& m, const Set_of_Atoms* e, std::ostream& output) {
...
static int
get_coordinates(Molecule& m, int nhits, const Substructure_Results& sresults,
                std::ostream& output) {
  for (int i = 0; i < nhits; i++) {
...
static int
get_coordinates(Molecule& m, std::ostream& output) {
