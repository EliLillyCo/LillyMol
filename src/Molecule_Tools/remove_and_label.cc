/*
 // clang-format off
  ...
  Sept 2022.
  The ability to match any atom type is now available in query
  files, in smarts as #{foobar}. Not clear this is needed,
...
static int extra_bonds_allowed_at_substitution_points =
    0;  // 0 means unspecified, obviously must be at least one
...
class Element_and_Isotope
{
 ...
 public:
  ...
  int
  replace_with_dummy_atom() const
  {
    return _replace_with_dummy_atom;
  }

  int
  match(const Element* e)
  {
    return _ematch.matches(e);
  }
};
...
static int
create_query(MDL_Molecule& m, const resizable_array<int>& attachment_points)
{
  if (verbose) {
    cerr << "Creating query from '" << m.smiles() << "', will write to '"
         << fname_for_query << "'\n";
  }
  ...
  } else  // convey the information about connections via hcount
  {
    for (auto i = 0; i < matoms; ++i) {
      ...
      mad->set_hcount(m.hcount(i) +
                      1);  // ISIS convention for how hydrogens are specified
      mad->set_substitution(0);
...
static int
add_r_group(Molecule& m, int& ndx, atom_number_t a1,
...
remove_and_label(MDL_Molecule& m)
{
  ...
  for (int i = 0; i < nelei; i++) {
    ...
    for (int j = 0; j < matoms; j++) {
      ...
      if (jcon > 2) {
        cerr << "Ignoring non-singly connected '" << e->symbol() << "', atom " << j
             << " in '" << m.name() << "'\n";
        continue;
      }
      ...
      if (!rx_for_allowable_non_organics) {  // no checking to be done
        ;
      } else if (iwre2::RE2PartialMatch(
                     e->symbol(), *rx_for_allowable_non_organics)) {  // great, OK match.
        ;
      } else {
        cerr << "Non-allowed atomic symbol in match atom '" << e->symbol()
             << "', must match '" << rx_for_allowable_non_organics->pattern() << "'\n";
        return 0;
      }
      ...
      if (1 == jcon) {
        ...
        if (0 ==
            extra_bonds_allowed_at_substitution_points) {  // no need to check anything
          ;
        } else if (b->is_single_bond()) {
          ;
        } else if (1 == extra_bonds_allowed_at_substitution_points &&
                   b->is_double_bond()) {
          cerr << "Removing doubly bonded atom, but only 1 extra bond allowed, "
                  "impossible\n";
          return 0;
        } else if (b->is_triple_bond() &&
                   extra_bonds_allowed_at_substitution_points < 3) {
          cerr << "Removing triply bonded atom, but only "
               << extra_bonds_allowed_at_substitution_points
               << " extra bonds allowed, impossible\n";
          return 0;
        }
        ...
        if (verbose > 2) {
          cerr << "Atom " << j << " (" << m.atomic_symbol(j)
               << ") will be removed, attached to atom " << k << " type "
               << m.atomic_symbol(k) << endl;
        }
        ...
        if (verbose > 2) {
          cerr << "Atom " << j << " (" << m.atomic_symbol(j)
               << ") will be removed, attached to atoms " << k1 << " type "
               << m.atomic_symbol(k1) << " and " << k2 << " type " << m.atomic_symbol(k2)
               << endl;
        }
  ...
  int* atoms_to_be_removed_array = new_int(matoms);
  std::unique_ptr<int[]> free_atoms_to_be_removed_array(atoms_to_be_removed_array);
  ...
  cerr << "attachment points " << attachment_points << " Removing atoms "
       << atoms_to_be_removed << endl;
  ...
  for (int i = 0; i < na; i++) {
    ...
    cerr << "Symmetry warning, atom " << j << " in '" << m.name() << "' "
         << number_with_same_symmetry_class << " symmetry equivalent atoms\n";
  }
...
static int
remove_and_label(const char* fname, FileType input_type, Molecule_Output_Object& output)
{
...
remove_and_label(int argc, char** argv)
{
  ...
  if (cl.option_present('H')) {
    ...
    if (verbose) {
      cerr << "Only non organic elements matching '"
           << rx_for_allowable_non_organics->pattern() << "' will be allowed\n";
    }
  }
  ...
  if (cl.option_present('h')) {
    ...
    if (verbose) {
      cerr
          << "Query will only allow substitution at R group sites (directed by Hcount)\n";
    }
  }

  if (cl.option_present('e')) {
    if (!cl.value('e', extra_substitutions_allowed_at_substitution_points) ||
        extra_substitutions_allowed_at_substitution_points < 1) {
      cerr << "The extra connections at substitution points option (-e) must be a whole "
              "+ve number\n";
      usage(4);
    }

    if (verbose) {
      cerr << "Must have " << extra_substitutions_allowed_at_substitution_points
           << " extra substitutions at attachment points\n";
    }
  ...
  if (cl.option_present('b')) {
    if (!cl.value('b', extra_bonds_allowed_at_substitution_points) ||
        extra_bonds_allowed_at_substitution_points < 1) {
      cerr << "The extra bonds allowed at substitution points (-b) option must be a "
              "whole number >= 1\n";
      usage(4);
    }

    if (verbose) {
      cerr << "A maximum of " << extra_bonds_allowed_at_substitution_points
           << " extra bonds allowed at substitution points\n";
    }
  }
