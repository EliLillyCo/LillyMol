...
// Apr 2025.
// This does not work very well as a TDT filter.
// The problem is that if a calculation fails, we end up with an
// invalid TDT as output. I made some intial attempts to mitigate
// the problem, but it seems more work needs to be done.
...
// by default, of a partial charge calculation fails and we are working as
// a TDT filter, we end up with invalid output. We can run in a fault tolerant
// mode where we just assign zero's to the charges instead and continue the
// calculation.
...
usage(int rc) {
  ...
  // clang-format on
  // clang-format off
  cerr << R"(Calculate charge surface area related descriptors for molecules\n";
 -j             compute jurs and extended jurs descriptors.
 -h             compute hydrophobic jurs descriptors.
 -e             compute electronic property related descriptors.
 -s             compute savol like descriptors.
 -a             compute aromaticity related descriptors.
 -b             compute CoMMA related descriptors -- -F option has to be turned on.
                by default, if none of -j -e -s -a -b is specified, all flags are turned on.
 -r <radius>    set solvent probe radius (default = 1.40 for water).
 -J <tag>       tag for fingerprint output.
 -q             when writing fingerprints, ignore failed charge calculations.
                zero charges are applied. The calculation is wrong, but the file is valid.
 -y <scale>     bit count scaling factor (default 10.0).
 -H <..>        donor acceptor assignment, enter '-H help' for info.
 -F file        file in which ghose-crippen parameters are saved.
 -G             use gasteiger partial charge in computation.
 -M min=<m>     specify minimum value for comma descriptors.
 -M max=<m>     specify maximum value for comma descriptors.
 -V ...         VDW radius specificiation, enter '-V help' for info.
 -N ...         Charge assigner options, enter '-N help' for info.
...
static void
hit_bit(Sparse_Fingerprint_Creator& sfc, int b, double v, double minval, double maxval,
        double scale) {
#ifdef CHECK_OUT_OF_RANGE
  if (v < minval || v > maxval) {
    cerr << "Value out of range " << v << " min " << minval << " max " << maxval
         << " feature " << b << endl;
  }
#endif
...
static int
ghose_crippin_procedure(Molecule& m, int* already_hit, double* hydrophobicity_index) {
...
ghose_crippen_procedure(Molecule& m, double hydrophobicity_index[]) {
  ...
  int* already_hit = new_int(n_atoms);
  std::unique_ptr<int[]> free_already_hit(already_hit);
...
static int
compute_polar_surface_area_1(Molecule& m, area_t atom_area[],
                             double& polar_surface_area) {
  int n_atom = m.natoms();
...
static int
compute_polar_surface_area_2(Molecule& m, area_t atom_area[],
                             double& polar_surface_area) {
  int n_atom = m.natoms();
  polar_surface_area = 0.0;
  for (int i = 0; i < n_atom; i++) {
    ...
    } else if ((8 == atom_i->atomic_number()) || (7 == atom_i->atomic_number()) ||
               (16 == atom_i->atomic_number())) {
      polar_surface_area += atom_area[i];
    } else {
      continue;
    }
...
static int
compute_polar_surface_area_3(Molecule& m, area_t atom_area[],
                             double& polar_surface_area) {
  int n_atom = m.natoms();
  polar_surface_area = 0;
  for (int i = 0; i < n_atom; i++) {
    const Atom* atom_i = m.atomi(i);
    if ((atom_i->atomic_number() == 7) || (atom_i->atomic_number() == 8) ||
        ((atom_i->atomic_number() == 1) &&
         ((m.atomi(m.other(i, 0))->atomic_number() == 7) ||
          (m.atomi(m.other(i, 0))->atomic_number() == 8)))) {
      polar_surface_area += atom_area[i];
    }
  }
...
static int
compute_hydrophobic_surface_area(Molecule& m, area_t atom_area[],
  ...
  for (int i = 0; i < n_atom; i++) {
    ...
    //  Note the strangeness of a pyridine here. Only one of the neighbouring carbons has
    //  a multiple bond to a heteroatom. Let's just skip over that complication.
    ...
    if (icon > 1 && (17 == z || 35 == z ||
                     53 == z)) {  // only singly connected halogens are hydrophillic
      continue;
    }
...
static int
molecule_surface_area(Molecule& m, area_t atom_area[], volume_t& total_molecule_volume) {
  ...
  //  cout <<m.name()<<" ";
  Molecule mcopy = m;
  rc = molecule_surface.surface_area(mcopy, atom_area, total_molecule_area,
                                     total_molecule_volume);
  ...
  cerr << "Inside calculate_partial_charge, just exited from surface_area() of "
          "surface_area_molvol.cc"
       << endl;
  for (int i = 0; i < n_atom; i++) {
...
output_result_header(IWString_and_File_Descriptor& output) {
  ...
  if (compute_jurs_descriptor) {
    ...
    output << " jurs_JWJSASA jurs_JWJPPSA1 jurs_JWJPNSA1 jurs_JWJDPSA1 jurs_JWJPPSA2 "
              "jurs_JWJPNSA2 jurs_JWJDPSA2 jurs_JWJPPSA3 jurs_JWJPNSA3 jurs_JWJDPSA3";
    output << " jurs_JWJFPSA1 jurs_JWJFNSA1 jurs_JWJFPSA2 jurs_JWJFNSA2 jurs_JWJFPSA3 "
              "jurs_JWJFNSA3";
    output << " jurs_JWJWPSA1 jurs_JWJWNSA1 jurs_JWJWPSA2 jurs_JWJWNSA2 jurs_JWJWPSA3 "
              "jurs_JWJWNSA3";
    output << " jurs_JWJRPCG jurs_JWJRNCG jurs_JWJRPCS jurs_JWJRNCS jurs_JWJTPSA "
              "jurs_JWJTASA jurs_JWJRPSA jurs_JWJRASA";

    // print out header for hydrogen bond extension of jurs descriptor
    output << " jurs_JWJ2SSAH jurs_JWJ2CHGD jurs_JWJ2ACGD jurs_JWJ2SSAA jurs_JWJ2CNTH "
              "jurs_JWJ2CNTA";
  ...
  if (compute_electronic_descriptor) {
    ...
    output << " jurs_JWELED1 jurs_JWELED2 jurs_JWELED3 jurs_JWELED4 jurs_JWELED5 "
              "jurs_JWELED6 jurs_JWELED7 jurs_JWELED8 jurs_JWELED9";
    output << " jurs_JWELED10 jurs_JWELED11 jurs_JWELED12 jurs_JWELED13 jurs_JWELED14 "
              "jurs_JWELED15 jurs_JWELED16";
  ...
  if (compute_savol_like_descriptor) {
    ...
    output << " jurs_JWSSovEn jurs_JWSVol jurs_JWSPSA12 jurs_JWSPSA22 jurs_JWSPSA32 "
              "jurs_JWSHpbSA";
  }
  ...
  if (compute_hydrophobic_jurs_descriptor) {
    output << " jurs_JWPPHS1 jurs_JWPNHS1 jurs_JWPPHS2 jurs_JWPNHS2 jurs_JWPPHS3 "
              "jurs_JWPNHS3 jurs_JWDHS1 jurs_JWDHS2 jurs_JWDHS3";
    output << " jurs_JWFPHS1 jurs_JWFNHS1 jurs_JWFPHS2 jurs_JWFNHS2 jurs_JWFPHS3 "
              "jurs_JWFNHS3";
    output << " jurs_JWWPHS1 jurs_JWWNHS1 jurs_JWWPHS2 jurs_JWWNHS2 jurs_JWWPHS3 "
              "jurs_JWWNHS3";
    output << " jurs_JWRPH jurs_JWRNH jurs_JWRPHS jurs_JWRNHS jurs_JWSURR1 jurs_JWSURR2 "
              "jurs_JWSURR3";
  }
 ...
 */
static void
compute_electronic_property_comma_descriptor(double dipole_center[],
                                             double dipole_moment[],
                                             double quadrupole_moment[], double center[],
                                             double** reference_axis,
                                             double descriptor[]) {
  double axis[3];
...
static double
compute_quadrupole_moment(Molecule& m, double** quadrupole_moment_tensor,
  ...
  (void)m.find_eigen_value_for_matrix(3, quadrupole_moment_tensor, quadrupole_moment,
                                      quadrupole_moment_axis);
...
static void
compute_Qxx_Qyy_at_dipole_center_using_inertial_axis(int n_atoms,
                                                     double atomic_partial_charge[],
                                                     double** coordinate, double origin[],
                                                     double** axis,
                                                     double quadrupole_moment[]) {
  for (int i = 0; i < 3; i++) {
...
static void
compute_dipole_and_quadrupole_from_origin(int n_atoms, double atomic_partial_charge[],
                                          double dipole_moment[],
...
static void
compute_geometry_and_gravity_center(Molecule& m, double** coordinate,
                                    double geometry_center[], double gravity_center[]) {
  int n_atoms = m.natoms();
  double* geometry_field = new double[n_atoms];
  std::unique_ptr<double[]> free_geometry_field(geometry_field);
  double* gravity_field = new double[n_atoms];
  std::unique_ptr<double[]> free_gravity_field(gravity_field);
 ...
 */
static void
comma_related_descriptor_computation_procedure(Molecule& m, double atom_partial_charge[],
  ...
  // compute moment of inertia for geometry and gravity around their corresponding center
  // Ix, Iy, Iz for geometry and gravity
  compute_moment_of_inertia(m, n_atoms, coordinate, geometry_center, geometry_field,
                            geometry_moment_of_inertia_axis, geometry_moment_of_inertia);

  compute_moment_of_inertia(m, n_atoms, coordinate, gravity_center, gravity_field,
                            gravity_moment_of_inertia_axis, gravity_moment_of_inertia);
  ...
  compute_dipole_and_quadrupole_from_origin(n_atoms, atom_partial_charge, dipole_moment,
                                            quadrupole_moment_tensor, coordinate, origin);
  ...
  if (!compute_center_of_dipole(dipole_moment, quadrupole_moment_tensor, dipole_center)) {
    if (verbose) {
      cerr << "molecule '" << m.name()
           << "' has very small dipole_moment in computation, dipole center procedure "
              "set center to (0,0,0)\n";
    }
  } else {
    // compute the dipole and quadrupole using the center of dipole as the origin
    compute_dipole_and_quadrupole_from_origin(n_atoms, atom_partial_charge, dipole_moment,
                                              quadrupole_moment_tensor, coordinate,
                                              dipole_center);

    // at dipole center, the eigen value for the quadrupole_moment_tensor are p, -p, 0. p
    // is the principal quadrupole moment
    principal_quadrupole_moment =
        compute_quadrupole_moment(m, quadrupole_moment_tensor, quadrupole_moment);

    // at the dipole center, using geometry inertia axis as the reference axis, eigen
    // value Qxx, Qyy, Qzz Qzz = -(Qxx + Qyy)
    compute_Qxx_Qyy_at_dipole_center_using_inertial_axis(
        n_atoms, atom_partial_charge, coordinate, dipole_center,
        geometry_moment_of_inertia_axis, quadrupole_moment);

    // compute the value along geometry moment of inertia axis, of dx, dy, dz
    // (displacement), px, py, pz (dipole), and Qxx, Qyy
    compute_electronic_property_comma_descriptor(
        dipole_center, dipole_moment, quadrupole_moment, geometry_center,
        geometry_moment_of_inertia_axis, geometry_electronic_related);

    // at the dipole center, using gravity inertia axis as the reference axis, eigen value
    // Qxx, Qyy, Qzz Qzz = -(Qxx + Qyy)
    compute_Qxx_Qyy_at_dipole_center_using_inertial_axis(
        n_atoms, atom_partial_charge, coordinate, dipole_center,
        gravity_moment_of_inertia_axis, quadrupole_moment);

    // compute the value along gravity moment of inertia axis, of dx, dy, dz
    // (displacement), px, py, pz (dipole), and Qxx, Qyy
    compute_electronic_property_comma_descriptor(
        dipole_center, dipole_moment, quadrupole_moment, gravity_center,
        gravity_moment_of_inertia_axis, gravity_electronic_related);
...
static int
assign_field_for_at_comma_computation(Molecule& m, double field[], int field_index) {
  int n_atoms = m.natoms();
 ...
 */
static void
atomic_comma2_related_descriptor_computation_procedure(Molecule& m,
                                                       double comma2_descriptor[],
                                                       double** coordinate) {
  int n_atoms = m.natoms();
  ...
  double* hydrophobicity_index = new double[n_atoms];
  std::unique_ptr<double[]> free_hydrophobicity_index(hydrophobicity_index);

  double* field = new double[n_atoms];
  std::unique_ptr<double[]> free_field(field);
  ...
  // compute the comma descriptors (COMMA2 using ghose and crippen hydrophobicity index as
  // field
  ghose_crippen_procedure(m, hydrophobicity_index);

  comma2_related_descriptor_computation_procedure(
      m, n_atoms, hydrophobicity_index, field_comma2_descriptor, coordinate,
      geometry_center, geometry_moment_of_inertia_axis);

  for (int i = 0; i < 7; i++) {
    comma2_descriptor[i] = field_comma2_descriptor[i];
  }

  // for three other field (vdw volume, atomic electronegativity and atomic
  // polarizability) compute COMMA2 descriptors
  for (int i = 0; i < 3; i++) {
    assign_field_for_at_comma_computation(m, field, i);
    comma2_related_descriptor_computation_procedure(
        m, n_atoms, field, field_comma2_descriptor, coordinate, geometry_center,
        geometry_moment_of_inertia_axis);
...
comma_related_descriptor_computation_procedure(Molecule& m, double atom_partial_charge[],
  ...
  comma_related_descriptor_computation_procedure(m, atom_partial_charge, comma_descriptor,
                                                 coordinate_array, ratio);

  // compute 28 atomic comma2 descriptors (geometry center and
  // geometry_meoment_of_inertia_axis as the reference point and axis frame, respectively)
  atomic_comma2_related_descriptor_computation_procedure(m, comma2_descriptor,
                                                         coordinate_array);
...
static int
compute_hydrophobic_jurs_procedure(Molecule& m, area_t atom_area[],
                                   double hydrophobic_jurs[]) {
  int natoms = m.natoms();
  double* hydrophobicity_index = new double[natoms];
  std::unique_ptr<double[]> free_hydrophobicity_index(hydrophobicity_index);
  ghose_crippen_procedure(m, hydrophobicity_index);

  //  cerr<<m.smiles()<<endl;
  //  for (int i=0; i<natoms; i++)
  // cerr<<"Atom "<<i<<"   atomic_symbol "<<m.atomi(i)->atomic_symbol()<<"  Surface
  // area="<<atom_area[i]<<"  hydro="<<hydrophobicity_index[i]<<endl;
...
static bool
ComputePartialCharges(Molecule& m, double* charges) {
  int rc;
  ...
  if (rc == 0) {
    std::fill_n(charges, matoms, 0.0);
    fault_tolerant = 1;
  }

  for (int i = 0; i < matoms; ++i) {
    charges[i] = m.charge_on_atom(i);
  }
...
static int
jwsa(Molecule& m, IWString_and_File_Descriptor& output, double atom_partial_charge[],
     double comma_partial_charge[], area_t atom_area[], int da_results[],
     double comma_descriptor[], double hydrophobic_jurs[]) {
  int rc;  // return status

  int n_atoms = m.natoms();

  int partial_charges_computed = 0;

  // compute partial atomic charge -- only needed when jurs descriptor or
  // electronic_descriptor are needed
  if ((compute_jurs_descriptor) || (compute_electronic_descriptor)) {
    if (!ComputePartialCharges(m, atom_partial_charge)) {
      if (verbose) {
        cerr << m.name() << "\tERROR in calculation of partial charge\n";
      }
      return 0;
    }
    partial_charges_computed = 1;
  }

  if (compute_comma_related_descriptor) {
    if (partial_charges_computed) {
    } else if (!ComputePartialCharges(m, comma_partial_charge)) {
      if (verbose) {
        cerr << m.name() << "\tERROR in calculation of partial charge\n";
      }
      return 0;
    ...
    comma_related_descriptor_computation_procedure(m, comma_partial_charge,
                                                   comma_descriptor);
  }
  ...
  if (compute_jurs_descriptor || compute_savol_like_descriptor ||
      compute_aromaticity_related_descriptor || compute_electronic_descriptor ||
      compute_hydrophobic_jurs_descriptor) {
    ...
    if ((total_positive_charge != 0) && (total_negative_charge != 0)) {
      ...
      // dipolar moment for the molecule & square of dipolar moment  -- electronic
      // descriptor --3 -- total 19
      double pn_distance =
          sqrt((cp_x - cn_x) * (cp_x - cn_x) + (cp_y - cn_y) * (cp_y - cn_y) +
               (cp_z - cn_z) * (cp_z - cn_z));
      ...
      for (int j = i + 1; j < n_atoms; j++) {
        if (m.distance_between_atoms(i, j) != 0.0) {
          if (atom_partial_charge[i] > atom_partial_charge[j]) {
            JW_DP_3 += (atom_partial_charge[i] - atom_partial_charge[j]) /
                       (m.distance_between_atoms(i, j) * m.distance_between_atoms(i, j));
          } else {
            JW_DP_3 += (atom_partial_charge[j] - atom_partial_charge[i]) /
                       (m.distance_between_atoms(i, j) * m.distance_between_atoms(i, j));
          }
        }
      }
    ...
    // Most absolue, positive, negative charge & their corresponding average  -- 6 --
    // total 11
    double JWED_6 = 0.0;
    double JWED_7 = 0.0;
    double JWED_8 = 0.0;
    ...
    // squares of absolute, positive and negative charge and their average -- 5 -- total
    // 16
    double JWED_12 = 0.0;
    double JWED_13 = 0.0;
    double JWED_14 = 0.0;
    ...
    // Total absolute , positive, negative charge & their corresponding average  -- 5 --
    // total 5
    ...
    if (most_positive_charge > -most_negative_charge) {
      JWED_10 = most_positive_charge;
    } else {
      JWED_10 = -most_negative_charge;
    }
    if (0 != n_atoms) {
    ...
    if (compute_savol_like_descriptor) {
      ...
      if (0 == rc) {
        if (verbose) {
          cerr << m.name() << "\tERROR in calculation of solvation energy\n";
        }
        return rc;
      }
      ...
      if (0 == rc) {
        if (verbose) {
          cerr << m.name() << "\tERROR in calculation of polar surface area 1\n";
        }
        return rc;
      }
      ...
      if (0 == rc) {
        if (verbose) {
          cerr << m.name() << "\tERROR in calculation of polar surface area 2\n";
        }
        return rc;
      }
      ...
      if (0 == rc) {
        if (verbose) {
          cerr << m.name() << "\tERROR in calculation of polar surface area 3\n";
        }
        return rc;
      }
      ...
      if (0 == rc) {
        if (verbose) {
          cerr << m.name() << "\tERROR in calculation of hydrophobic surface area\n";
        }
        return rc;
      }
    }
    ...
    if (compute_aromaticity_related_descriptor) {
      ...
      for (int i = 0; i < n_atoms; i++) {
        if (1 == m.atomi(i)->atomic_number()) {
          if (m.is_aromatic(m.other(i, 0))) {
            JW_AromSA += atom_area[i];
          }
    ...
    // start computing the H-bond donor acceptor -- jurs extention -- descriptors  Total
    // of 11
    ...
    double CHGD =
        0.0;  // max charge difference between hydrogen and its corresponding heteroatom
    ...
    double RSHM = 0.0;  // fraction of the total molecular surface area associated with
                        // hydrogens which can be donated
    double RSAM = 0.0;  // fraction of the total molecular surface area associated with
                        // H-bond acceptor groups
    ...
    if (compute_jurs_descriptor)  // if jurs descriptor needs to be calculated,
                                  // assign_donor_acceptor
    {
      for (int i = 0; i < n_atoms; i++) {
          ...
          case 2:
            ...
            for (int j = 0; j < neighbors; j++) {
              ...
              if (atomj->atomic_number() == 1) {
                ...
                double difference_in_charge_for_h_donor_pair =
                    atom_partial_charge[jth_atom] - atom_partial_charge[i];
                total_difference_in_charge_for_h_donor_pair +=
                    difference_in_charge_for_h_donor_pair;
      ...
      if (CNTA != 0) {
        ...
        if (0 == CNTH) {
          ...
          RHTA = 2 * CNTA;  // WHEN CNTA=0, CNTH!=0, RHTA is set as 2* CNTA -- do not want
                            // to set as infinite
        }
        RSAA = 0;
      }
    ...
    if (compute_jurs_descriptor) {
      if (tag.length()) {
        ...
        output << ' ' << SSAH << ' ' << CHGD << ' ' << ACGD << ' ' << SSAA << ' ' << CNTH
               << ' ' << CNTA;
    ...
    if (compute_electronic_descriptor) {
      if (tag.length()) {
        ...
        output << ' ' << JWED_1 << ' ' << JWED_2 << ' ' << JWED_3 << ' ' << JWED_4 << ' '
               << JWED_5;
        output << ' ' << JWED_6 << ' ' << JWED_7 << ' ' << JWED_8 << ' ' << JWED_9 << ' '
               << JWED_10 << ' ' << JWED_11;
        output << ' ' << JWED_12 << ' ' << JWED_13 << ' ' << JWED_14 << ' ' << JWED_15
               << ' ' << JWED_16;
      ...
      if (compute_hydrophobic_jurs_descriptor) {
        if (tag.length()) {
          for (int i = 0; i < NUMBER_OF_HYDROPHOBIC_JURS; ++i) {
            hit_bit(sfc, 68 + i, hydrophobic_jurs[i], min_jurs[68 + i], max_jurs[68 + i],
                    bit_count_scaling_factor + i);
          }
        } else {
          if (!name_written) {
            append_first_token_of_name(m.name(), output);
  ...
  if (compute_comma_related_descriptor) {
    if (tag.length()) {
      for (int i = 0; i < NUMBER_OF_COMMA_DESCRIPTOR; ++i) {
        ...
        hit_bit(sfc, 96 + i, c, min_value_comma_descriptor, max_value_comma_descriptor,
                bit_count_scaling_factor + i);
      }
    } else {
      if (!name_written) {
...
static int
jwsa(Molecule& m, IWString_and_File_Descriptor& output) {
  // calculate_partial_charge need to have all hydrogen added
  // but this has to be controlled by the input to the corina.  Other wise, molecule might
  // have hydrogen but no coordinate
  //  m.make_implicit_hydrogens_explicit();

  int n_atoms = m.natoms();

  double* partial_charge = new double[n_atoms];
  std::unique_ptr<double[]> free_partial_charge(partial_charge);
  double* comma_partial_charge = new double[n_atoms];
  std::unique_ptr<double[]> free_comma_partial_charge(comma_partial_charge);
  double comma_descriptor[NUMBER_OF_COMMA_DESCRIPTOR];
  double hydrophobic_jurs[NUMBER_OF_HYDROPHOBIC_JURS];

  area_t* area = new area_t[n_atoms];
  std::unique_ptr<double[]> free_area(area);
  int* results = new_int(n_atoms);
  std::unique_ptr<int[]> free_results(results);

  int rc = jwsa(m, output, partial_charge, comma_partial_charge, area, results,
                comma_descriptor, hydrophobic_jurs);
...
static int
jwsa(data_source_and_type<Molecule>& input, IWString_and_File_Descriptor& output) {
  Molecule* m;
...
static int
jwsa(const char* fname, FileType input_type, IWString_and_File_Descriptor& output) {
...
static int
jwsa_filter(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
...
static int
jwsa_filter(const char* fname, IWString_and_File_Descriptor& output) {
...
jwsa(int argc, char** argv) {
  Command_Line cl(argc, argv, "vA:ei:sH:ar:jV:bGd:hN:F:ng:M:J:y:qf");
  ...
  if ((cl.option_present('b')) || (cl.option_present('h'))) {
    if (!cl.option_present('F')) {
      cerr << "Must specify one or more control files via the -F option when comma or "
              "hydrophobic jurs descriptor need to be computed\n";
      usage(19);
    }
  ...
  if (!(compute_jurs_descriptor || compute_electronic_descriptor ||
        compute_savol_like_descriptor || compute_aromaticity_related_descriptor ||
        compute_comma_related_descriptor || compute_hydrophobic_jurs_descriptor)) {
  ...
  if (cl.option_present('M')) {
    ...
    if (verbose) {
      cerr << "Comma descriptors restricted to the range " << min_value_comma_descriptor
           << " to " << max_value_comma_descriptor << endl;
    }

    if (min_value_comma_descriptor > max_value_comma_descriptor) {
      cerr << "Invalid range for comma descriptors " << min_value_comma_descriptor
           << " to " << max_value_comma_descriptor << endl;
      usage(4);
    }
  }
  ...
  if (cl.option_present('i')) {
    ...
    if (cl.option_present('q')) {
      fault_tolerant = 1;
      if (verbose) {
        cerr << "Operating in a fault tolerant mode\n";
      }
    }
