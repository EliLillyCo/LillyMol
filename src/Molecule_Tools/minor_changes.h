...
class Options {
  private:
    ...
    uint64_t _molecules_generated;

    uint64_t _invalid_valence;
    ...
    // We can optionally designate atoms that will be either changed or not changed.
    resizable_array_p<Substructure_Query> _atoms_to_change;
    resizable_array_p<Substructure_Query> _atoms_not_changing;
  ...
  // private functions.
    ...
    int SetupDoNotProcessQuery(const std::string& qry);
    int SetupAtomsToChangeQuery(const std::string& qry);
    int SetupAtomsToChangeQueries();
    int SetupAtomsNotChangingQueries();
