...
class Molecular_Abstraction_Base_Class {
 protected:
  ...
  // By default, unique smiles are written
  int _write_unique_smiles;
  ...
  int _is_spinach(Molecule_With_Info_About_Parent& m, int* in_scaffold,
                  atom_number_t aprev, atom_number_t zatom) const;
 ...
 public:
  ...
  const IWString&
  fingerprint_tag() const {
    return _fingerprint_tag;
  }
  ...
  void
  set_write_unique_smiles(int s) {
    _write_unique_smiles = s;
  }
...
class Molecular_Abstraction_Largest_Ring_System
    : public Molecular_Abstraction_Base_Class {
 private:
...
class Molecular_Abstraction_Abstract_Ring_Form : public Molecular_Abstraction_Base_Class {
 private:
  ...
  int _place_abstract_rings(Molecule_With_Info_About_Parent& m,
                            const int* ring_membership, Joins_to_Abstract_Ring* jar);
  void _do_apply_isotopic_label(Molecule_With_Info_About_Parent& m, atom_number_t zatom,
...
class Molecular_Abstraction_Fragment : public Molecular_Abstraction_Base_Class {
 private:
  ...
  int _identify_fragments_hit(Molecule_With_Info_About_Parent& m,
                              Single_Substructure_Query& q, int* frag);
...
class Molecular_Abstraction_Compress_Consecutive
 ...
 private:
  ...
  int _compress_group(Molecule_With_Info_About_Parent& m, int* to_remove, int flag,
...
class Molecular_Abstraction_Ring_Systems : public Molecular_Abstraction_Base_Class {
 private:
  ...
  int _replace_ring_system(Molecule_With_Info_About_Parent& m, int matoms, int flag,
...
class Molecular_Abstraction_Remove_Ring_CH2 : public Molecular_Abstraction_Base_Class {
 private:
  ...
  int _identify_ch2_to_remove(Molecule_With_Info_About_Parent& m, atom_number_t zatom,
                              Set_of_Atoms& lhs, Set_of_Atoms& rhs) const;
  int _identify_sequence(Molecule_With_Info_About_Parent& m, atom_number_t avoid,
                         atom_number_t zatom, Set_of_Atoms& s) const;
...
class Set_of_Molecular_Abstractions {
 ...
 public:
  ...
  // Applied to each abstraction we hold.
  void set_write_unique_smiles(int s);
};
