...
/*
  If someone specifies a histogram on the command line, we store the specification here,
  and then use this to initialise all the histograms
*/
...
/*
  But the feature name array is across all features, including
  positive_negative_acceptor_donor
*/
...
/*
  Note that _global_histogram does not get reset during a normal reset. That histogram is
  designed to accumulate data from all molecules
*/
...
Distances<T>::append_results(IWString& buffer) const {
  ...
  // cerr << "distribution has " << _number_elements << " items, n = " << number_samples
...
Distances<T>::append_results_integer(IWString& buffer) const {
  ...
  cerr << "distribution has " << _number_elements << " items, n = " << number_samples
       << " between " << this->minval() << " and " << this->maxval() << '\n';
#endif
...
class Coords_and_Stuff : public Coordinates {
 ...
 public:
  ...
  int
  single_atom_match() const {
    return INVALID_ATOM_NUMBER != _zatom;
  }
  ...
  void
  set_matched_atom(atom_number_t a) {
    _zatom = a;
  }
};
...
static int
add_matched_atom(const Molecule& m, const atom_number_t zatom, const int query_number,
                 resizable_array_p<Coords_and_Stuff>& c) {
  assert(query_number >= 0);
  if (zatom == INVALID_ATOM_NUMBER) {
    return 0;
  }
...
static int
identify_matched_atoms(Molecule& m, int query_number, const Set_of_Atoms& embedding,
                       resizable_array_p<Coords_and_Stuff>& c) {
  assert(query_number >= 0);

  int atoms_in_embedding = embedding.number_elements();

  // If the smarts contained atoms that are to be excluded from the embedding, there will
  // be INVALID_ATOM_NUMBER items in the embedding. Note we just check the 2nd atom, which
  ...
  tmp->setxyz(static_cast<coord_t>(0.0), static_cast<coord_t>(0.0),
...
static int
do_hard_coded_query_centre_of_aromatic_ring(Molecule& m, int ring_size, int query_number,
                                            resizable_array_p<Coords_and_Stuff>& c) {
  int nr = m.nrings();
...
static int
do_hard_coded_features(Molecule& m, resizable_array_p<Coords_and_Stuff>& c,
                       int& matches_this_molecule) {
  m.compute_aromaticity_if_needed();

  if (hard_coded_query_centre_of_aromatic_6_membered_ring >= 0) {
    matches_this_molecule += do_hard_coded_query_centre_of_aromatic_ring(
        m, 6, hard_coded_query_centre_of_aromatic_6_membered_ring, c);
  }

  if (hard_coded_query_centre_of_aromatic_5_membered_ring >= 0) {
    matches_this_molecule += do_hard_coded_query_centre_of_aromatic_ring(
        m, 5, hard_coded_query_centre_of_aromatic_5_membered_ring, c);
  }
...
static int
do_positive_negative_acceptor_donor(const Molecule& m, int* atom_label,
                                    resizable_array_p<Coords_and_Stuff>& c) {
  const int matoms = m.natoms();
...
static int
identify_matched_atoms(Molecule& m, const isotope_t* isotope,
                       resizable_array_p<Coords_and_Stuff>& c) {
  ...
  for (int i = 0; i < nq; i++) {
    ...
    //  cerr << " Query " << i << " (" << queries[i]->comment() << ") is
    //  query_to_feature_number " << query_to_feature_number[i] << " name " <<
    //  feature_name[query_to_feature_number[i]] << '\n';
...
template <typename T>
void
reset_distances(Distances<T>* d, const int nfeatures) {
...
static int
ok_topology(Molecule& m, const Coords_and_Stuff& ci, const Coords_and_Stuff& cj, int& b) {
  b = 0;
  ...
  if (anumi == anumj) {  // not needed, will be caught by the zero spatial distance check.
                         // Just paranoid...
    return 0;
  }
...
static int
do_spatial_and_topological_distances_computation(
    Molecule& m, const int nfeatures, const resizable_array_p<Coords_and_Stuff>& coords) {
  ...
  for (int i = 0; i < nc; i++) {
    ...
    for (int j = i + 1; j < nc; j++) {
      ...
      // cerr << "btw atoms " << ci->matched_atom() << " and " << cj->matched_atom() << " d " << d << '\n';
...
static int
do_topological_distances_computation(Molecule& m, const int nfeatures,
  ...
  for (int i = 0; i < nc; i++) {
    ...
    if (!ci->single_atom_match()) {  // cannot do topological matches on
                                     // multi-atom-matches
      continue;
    }
    ...
    for (int j = i + 1; j < nc; j++) {
      ...
      const int d = m.bonds_between(ai, aj);

      if (1 == d && ! consider_bonded_features) {
        continue;
      }
...
static int
do_spatial_distances_computation(Molecule& m, const int nfeatures,
                                 const resizable_array_p<Coords_and_Stuff>& coords) {
  reset_distances(spatial_distances, nfeatures);
  ...
  for (int i = 0; i < nc; i++) {
    ...
    for (int j = i + 1; j < nc; j++) {
      ...
      const int jqn = cj->query_number();

      const coord_t d = ci->distance(*cj);

      // cerr << "Btw " << ci->matched_atom() << " and " << cj->matched_atom() << " dist " << d << " iqn " << iqn << " jqn " << jqn << '\n';
      if (d < ignore_atoms_closer_than_3d) {
        continue;
      }
...
gather_results(const resizable_array_p<Coords_and_Stuff>& coords,
               Sparse_Fingerprint_Creator& sfc, const int nfeatures, IWString& buffer) {
  int* tmp = new_int(nfeatures);
  ...
  for (int i = 0; i < nc; i++) {
    ...
    //  cerr << "Item " << i << " is a match to query " << ci->query_number() << '\n';
  }
    ...
    if (tag.length()) {
      for (int i = 0; i < nfeatures; i++) {
        for (int j = i; j < nfeatures; j++) {
          ...
          d.set_bits(bit_offset + 3 * ndx, bit_count_scaling_factor * 15.0f,
...
static int
do_compute_the_descriptors(Molecule& m, const int nfeatures,
                           const resizable_array_p<Coords_and_Stuff>& coords) {
...
template <typename T>
int
write_all_distances(Molecule& m, const int nfeatures, Distances<T>* distances,
...
distance_between_features(Molecule& m, const isotope_t* isotopes, const int nfeatures,
  ...
  const int number_descriptors = do_compute_the_descriptors(m, nfeatures, coords);

  if (stream_for_all_distances.is_open()) {
    if (0 == number_descriptors) {  // no distances found
      return 1;
    }

    if (nullptr != spatial_distances) {
      return write_all_distances(m, nfeatures, spatial_distances,
                                 stream_for_all_distances);
    }
...
static int
preprocess(Molecule& m, std::unique_ptr<isotope_t[]>& isotopes) {
  ...
  if (donor_acceptor_assigner.active()) {
    // may contain an isotopic query, so shield the isotopes
...
static int
distance_between_features(data_source_and_type<Molecule>& input, const int nfeatures,
                          IWString_and_File_Descriptor& output) {
  Molecule* m;
...
static int
distance_between_features(const char* fname, FileType input_type, const int nfeatures,
                          IWString_and_File_Descriptor& output) {
...
static int
distance_between_features_filter_process(Molecule& m, const int nfeatures,
                                         IWString_and_File_Descriptor& output) {
  std::unique_ptr<isotope_t[]> isotopes;
...
static int
distance_between_features_filter(iwstring_data_source& input, const int nfeatures,
                                 IWString_and_File_Descriptor& output) {
...
static int
distance_between_features_filter(const char* fname, const int nfeatures,
                                 IWString_and_File_Descriptor& output) {
...
static void
append_keeper_header(IWString& header, const char* stem, int n, char c[2]) {
  header << stem << "min" << c[0] << c[1];
  header << stem << "max" << c[0] << c[1];
  header << stem << "ave" << c[0] << c[1];

  // cerr << "Processing '" << c[0] << c[1] << "' n = " << nkeep << '\n';
...
static int
build_header(IWString& header, const int nfeatures, const char* stem) {
  assert(' ' == stem[0]);
...
static int
build_header(IWString& header, const int nfeatures) {
  for (int i = 0; i < nfeatures; i++) {
    if (i > 0) {
      header << ' ';
    }
...
static void
display_hard_coded_features(char flag, std::ostream& os) {
...
distance_between_features(int argc, char** argv) {
  ...
  if (cl.option_present('e')) {
    if (!charge_assigner.active() && !donor_acceptor_assigner.active()) {
      cerr << "Warning, request for default features, but charge assigner and donor "
              "acceptor assignor both inactive! Repeat -e to allow\n";
  ...
  if (cl.option_present('W')) {
    ...
    while (cl.value('W', w, i++)) {
      if ("arom5" == w) {
        hard_coded_query_centre_of_aromatic_5_membered_ring =
            (nq + 4 * positive_negative_acceptor_donor) + number_hard_coded_queries;
        if (verbose) {
          cerr << "Will perceive aromatic 5 membered rings, feature "
               << number_hard_coded_queries << '\n';
        }
        number_hard_coded_queries++;
        feature_name.add(new IWString("AROM5"));
      } else if ("arom6" == w) {
        hard_coded_query_centre_of_aromatic_6_membered_ring =
            nq + 4 * positive_negative_acceptor_donor + number_hard_coded_queries;
        if (verbose) {
          cerr << "Will perceive aromatic 6 membered rings, feature "
               << number_hard_coded_queries << '\n';
        }
        number_hard_coded_queries++;
        ...
        feature_name.add(new IWString("AROM6"));
        if (verbose) {
          cerr << "Will perceive 5 and 6 sized aromatic rings, features "
               << hard_coded_query_centre_of_aromatic_5_membered_ring << " and "
               << hard_coded_query_centre_of_aromatic_6_membered_ring << '\n';
        }
      } else if ("help" == w) {
  ...
  all_values_missing << ' ' << missing_value << ' ' << missing_value << ' '
                     << missing_value;
  ...
  if (do_spatial_distances && do_topological_distances) {
    spatial_bond_ratio =
        new Distances<float>[nfeatures *
                             nfeatures];  // actually only used if the -r option is set.
  }
  ...
  if (cl.option_present('b')) {
    if (!cl.value('b', ignore_atoms_closer_than_2d) || ignore_atoms_closer_than_2d < 1) {
      cerr << "The ignore atom pairs cutoff value (-b option) must be a whole positive "
              "number\n";
      usage(5);
    }

    if (verbose) {
      cerr << "Will ignore atom pairs " << ignore_atoms_closer_than_2d
           << " or fewer bonds apart\n";
    }
  }
  ...
  if (cl.option_present('C')) {
    if (0 == nkeep) {
      nkeep = 20;
      cerr << "Request for all distances -C, but no -k specified, default value " << nkeep
           << '\n';
    }
  ...
  if (verbose) {
    ...
    for (int i = 0; i < matches_per_query.number_elements(); ++i) {
      ...
      cerr << " query " << i << " " << *(feature_name[i]) << ": "
           << molecules_matching_query[i] << " molecules matched, "
           << matches_per_query[i] << " sites\n";
    }
  }
