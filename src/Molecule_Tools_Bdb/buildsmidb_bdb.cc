...
// Oct 2023
// When storing chiral suppressed forms, we can optionally add a prefix
// to the names. Typically this is the '@' character, so when someone
// retrieves a molecule that is matched only by loss of chirality, the
// identifier in the db will be '@123' indicating that this smiles
// came from '123' after losing one or more chiral centres.
static char prefix_for_chirality_removed = ' ';
...
buildsmidb(Molecule &m, const Storage_Conditions &sc, const Mol2Graph &mol2graph) {
  ...
  if (all_variants.active()) {
    ...
    rc = buildsmidb(m, sc, 1, mol2graph);
  } else if (sc.remove_chirality() && m.chiral_centres() == 0) {
    rc = buildsmidb(m, sc, 1, mol2graph);
  } else {
    rc = buildsmidb(m, sc, 0, mol2graph, prefix_for_chirality_removed);
  }
...
DisplayDashXOptions(std::ostream& output) {
  output << " -X chiral=<char>      prepend <char> to the names of molecules stored after losing chirality\n";
...
buildsmidb(int argc, char **argv) {
  Command_Line cl(argc, argv, "vi:A:g:N:h:T:d:alE:n:D:U:po:Icr:bzuO:tLwC:S:qyH:x:V:X:");
  ...
  if (cl.option_present('X')) {
    ...
    for (int i = 0; cl.value('X', x, i); ++i) {
      if (x.starts_with("chiral=")) {
        x.remove_leading_chars(7);
        char_name_to_char(x);

        if (x.length() != 1) {
          cerr << "The '-X chiral=' directive must specify a single character\n";
          return 1;
        }

        prefix_for_chirality_removed = x[0];

        if (verbose) {
          cerr << "Will prepend '" << prefix_for_chirality_removed <<
                  "' to molecules stored after losing chiralith\n";
        }
      } else if (x == "help") {
