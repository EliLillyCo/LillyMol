...
Evaluate an xgboost descriptor model that has been built with xgbd_make.
 ...
 -Z <fname>                 linear rescaling. <fname> should be generated by 'iwstats -C <fname>'\n";
...
class XGBDModel {
  private:
    ...
    // Generated by iwstats if -rescore is used during model building
    linear_scaling::LinearScaling _linear_scaling_fn;
  ...
  public:
    ...
    // This is set up by another class.
    linear_scaling::LinearScaling& linear_scaling_fn() {
      return _linear_scaling_fn;
    }
...
XGBDModel::ScoreBatch(const float* values, uint32_t items_in_chunk, float* pred) {
  ...
  if (_linear_scaling_fn.active()) {
    for (uint32_t i = 0; i < items_in_chunk; ++i) {
      double x = static_cast<double>(out_result[i]);
      _linear_scaling_fn.MaybeTransform(x);
      pred[i] = static_cast<float>(x);
    }
  } else {
...
Main(int argc, char** argv) {
  Command_Line_v2 cl(argc, argv, "-v-mdir=dir-XJSON=sfile-META=sfile-i=s-o=s-p=ipos-batch=ipos-report_unused-Z=sfile");
  ...
  if (cl.option_present('Z')) {
    ...
    linear_scaling::LinearScaling& ls = model.linear_scaling_fn();
