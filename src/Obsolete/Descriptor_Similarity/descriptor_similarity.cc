...
{
//_distance = static_cast<D>(9.99e+99);
  _distance = std::numeric_limits<D>::max();
...
template <typename D>
int
Neighbour_List<D>::write (std::ostream & os) const
{
...
template <typename D>
int
Neighbour_List<D>::write (IWString_and_File_Descriptor & os) const
{
  for (int i = 0; i < _neighbours.number_elements(); i++)
  {
    const ID_Distance & sidi = *(_neighbours[i]);
...
Descriptors_and_Neighbours::Descriptors_and_Neighbours(const Descriptors_and_Neighbours& rhs) {
  cerr << "Error, Descriptors_and_Neighbours copy constructor not implemented\n";
  ::abort();
}

Descriptors_and_Neighbours
Descriptors_and_Neighbours::operator=(const Descriptors_and_Neighbours& rhs) {
  cerr << "Error, Descriptors_and_Neighbours assignment operator not implemented\n";
  ::abort();
  return *this;
}
...
static Comparison_Window * window = nullptr;
                       ...
                       const int * xref)
{
  ...
  while (input.next_record (buffer))
  {
    ...
    if (report_progress > 0 && 0 == haystack_records_read % report_progress)
      cerr << "Processed " << haystack_records_read << " haystack records\n";
...
{
  Command_Line cl (argc, argv, "p:n:s:vT:n:X:hoEP:D:Z:zr:O:W:ea");
