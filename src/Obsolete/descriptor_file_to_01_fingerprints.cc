/*
  Converts descriptors to fingerprints.
*/
...
static int
fill_array_with_bit_values(const const_IWSubstring& buffer, int& i, int* numeric_value) {
...
static int
get_smiles(const IWString& id, IWString& smiles) {
...
static int
write_smiles(const IWString& id, IWString_and_File_Descriptor& output) {
  IWString smiles;
...
descriptor_file_to_01_fingerprints_record(const const_IWSubstring& buffer,
  ...
  if (columns_in_input != buffer.nwords()) {
    cerr << "Column count mismatch, got" << buffer.nwords() << " expected "
         << columns_in_input << '\n';
    return 0;
  }
...
static int
descriptor_file_to_01_fingerprints_filter_record_sparse(
    const IWString& tag, const int* c, IWString_and_File_Descriptor& output) {
...
static int
descriptor_file_to_01_fingerprints_filter_record_dense(
    const IWString& tag, const int* c, IWString_and_File_Descriptor& output) {
  IW_Bits_Base fp(nbits);
...
descriptor_file_to_01_fingerprints_filter_record(const_IWSubstring& buffer,
  ...
  if (tag.starts_with("FP")) {
    return descriptor_file_to_01_fingerprints_filter_record_dense(tag, (*f).second,
                                                                  output);
  } else {
    return descriptor_file_to_01_fingerprints_filter_record_sparse(tag, (*f).second,
                                                                   output);
  }
}
...
descriptor_file_to_01_fingerprints_filter(iwstring_data_source& input,
  ...
  while (input.next_record(buffer)) {
    ...
    if (!descriptor_file_to_01_fingerprints_filter_record(buffer, id_and_vector,
                                                          output)) {
...
static int
read_descriptors(iwstring_data_source& input, ID_and_Vector& id_and_vector) {
  const_IWSubstring buffer;
...
static int
read_smiles(const const_IWSubstring& fname, IW_STL_Hash_Map_String& id_to_smiles) {
  iwstring_data_source input(fname);
...
static int
read_descriptors(const const_IWSubstring& fname, ID_and_Vector& id_and_vector) {
  iwstring_data_source input(fname);
...
descriptor_file_to_01_fingerprints(int argc, char** argv) {
  ...
  if (cl.option_present('s') && cl.option_present('S')) {
    cerr << "Sorry, the -s (dummy smiles) and -S (file of smiles) options cannot both be "
            "used\n";
    usage(4);
  }
  ...
  if (cl.option_present('f')) {
    ...
    if (!cl.option_present('P')) {
      cerr << "When working as a TDT filter must specify descriptor file via the -P "
              "option\n";
      usage(2);
    }
    if (cl.option_present('s') || cl.option_present('S')) {
      cerr << "The smiles options (-s or -S) options and -f (work as tdt filter) do not "
              "make sense together\n";
      return 2;
    }
