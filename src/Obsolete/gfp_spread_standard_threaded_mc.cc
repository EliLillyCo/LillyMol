/*
  ...
  Operates on a set of presplit .gfp files
*/
...
// Each fingerprint needs a globally unique identifier. Given that we know our thread
// number and the number of fingerprints assigned to each chunk, we can assign a unique
// identifier to each fingerprint
...
// process
  ...
  int pool_size;
  const char * fname;    // when we start up, we read our fingerprints from this file
};
...
static int
build_pool (const struct Thread_Communication & tc,
            GFP_Standard * pool,
            IWString * smiles,
            IWString * pcn)
{
  iwstring_data_source input (tc.fname);
  if (! input.ok())
  {
    cerr << "Cannot open '" << tc.fname << "' for input\n";
    return 0;
  }

  return build_pool (input, pool, tc.pool_size, smiles, pcn);
}
                                 ...
                                 float * sdistance)
{
  ...
  const auto pool_size = tc->pool_size;

  int ichoose = -1;
  float maxdist = -1.0f;

  int sel = tc->sel;
  const GFP_Standard fpsel = tc->fpsel;    // make our own local copy, or would a const reference be better?

// Do we own the selected fingerprint

  const auto istart = tc->istart;

  if (sel >= istart && sel < istart + pool_size)
...
thread_process (void * v)
{
  ...
  GFP_Standard * pool = new GFP_Standard[tc->pool_size];

  // This is broken, fix sometime...
  if (! build_pool(*tc, pool))
  {
    cerr << "Cannot read fingerprints from '" << tc->fname << "'\n";
    pthread_exit(NULL);
    return NULL;   // never does this
  }

  float * d = new_float(tc->pool_size, 2.0f); std::unique_ptr<float[]> free_d(d);

  while (1)
  {
    sem_wait (time_for_child_to_wake + mythread);
    thread_object_has_been_selected(tc, pool, d);
              ...
              IWString_and_File_Descriptor & output)
{
  ...
  if (! initialise_threads(nthreads, first_selected, pool_size))
  {
...
usage (int rc)
{
  ...
  cerr << "Maximum diversity selection. Takes a set of pre-split .gfp files\n";
  cerr << " -s <number>      number of fingerprints in each input file\n";
...
{
  Command_Line cl (argc, argv, "vs:n:be");
  ...
  if (1 == nthreads)
  {
    cerr << "Must specify more than one pre-split files\n";
    usage(2);
  }
  ...
  {
    cerr << "Must specify number of fingerprints in each pre-split file via the -s option\n";
    usage(2);
  }
