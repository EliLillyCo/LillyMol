...
Substructure_Query::_substructure_search(Molecule_to_Match& target,
  ...
  // The final evaluation of _operator may involve a non-match, so
  // if there is a positive result somewhere in the evaluation, it will
  // be returned as the result.
  int last_positive_result = 0;
  ...
  for (int i = 0; i < _number_elements; i++) {
...
#ifdef DEBUG_SUBSTRUCTURE_SEARCH
    cerr << "Have match tmp " << tmp << ", last_positive_result " << last_positive_result
         << '\n';
#endif
    ...
    // We have a match, but tmp may be zero.
    if (last_positive_result > 0) {
      return last_positive_result;
    }
    return 1;
  }
...
Substructure_Query::set_find_one_embedding_per_atom(int s) {
  ...
  // cerr << "Substructure_Query::set_find_one_embedding_per_atom:has " <<
  // _number_elements << " components\n";
...
int
Substructure_Query::add(Single_Substructure_Query* q, int logop) {
...
void
Substructure_Query::assign_unique_id_from_atom_number_if_set(
...
Substructure_Query::print_connectivity_graph(std::ostream& output) const {
  output << "Substructure_Query::print_connectivity_graph:has " << _number_elements
         << " components\n";
...
int
Substructure_Query::ForgetOriginatingSmarts() {
  int rc = 0;
  for (int i = 0; i < _number_elements; ++i) {
