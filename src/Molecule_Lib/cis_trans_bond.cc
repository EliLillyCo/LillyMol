...
Molecule::_atom_being_unbonded_check_directional_bonds(atom_number_t zatom,
  ...
  if (nullptr != part_of_cis_trans_grouping) {  // breaking up the central part of the
                                                // cis-trans grouping
  ...
  for (int i = 0; i < directional_bonds_found.number_elements(); i++) {
    ...
    atom_number_t j =
        b->other(zatom);  // Atom J may or may not be part of a cis-trans bond

    const Atom* aj = _things[j];

    if (!aj->is_centre_of_cis_trans_bond()) {  // directional single bond bonded to double
                                               // bond that isn't part of a cis-trans
                                               // system
      continue;
    }
    ...
    cerr << "Directional bond from " << zatom << " to " << j << " preserve? "
         << preserve_chirality << ", hj " << hj << endl;
#endif
...
Molecule::_invalidate_directional_bonds_at_end_of_double_bond(atom_number_t zatom) {
  ...
  for (int i = 0; i < acon; i++) {
    ...
    if (!b->is_directional()) {
      cerr << "Molecule::_invalidate_directional_bonds_at_end_of_double_bond:strange, "
...
Molecule::__finished_reading_smiles_assign_and_check_directional_bonds() {
  //_unset_directional_bonds_not_adjacent_to_a_double_bond();  seems to be not needed,
  //watch to see if problems
  ...
  if (1 == nd) {
    ...
    if (display_smiles_interpretation_error_messages()) {
      cerr << "Molecule::_finished_reading_smiles_assign_and_check_directional_bonds:"
              "only one directional bond, impossible\n";
    }
#endif
  ...
  for (int i = 0; i < nd; i++) {
    ...
    //  Jul 2025. This is not correct. The double bond can be an exocyclic double bond.
...
// #define DEBUG_CHECK_INCONCISTENT_CIS_TRANS
    ...
    if (display_smiles_interpretation_error_messages()) {
      cerr << "Molecule::_finished_reading_smiles_assign_and_check_directional_bonds:"
              "invalid or incomplete cis trans bond, atoms "
           << a1 << " and " << a2 << endl;
    }
#endif
...
static int
discern_bond_directionality(const Bond* b, atom_number_t a) {
  if (!b->is_directional()) {
    return 0;
  }
...
int
Molecule::_process_directional_system(atom_number_t lhs1, atom_number_t db1,
                                      int& valid_cis_trans_form_found) {
  // cerr << "Examining possible cis-trans system involving atoms " << lhs1 << " and " <<
  // db1 << endl;
  ...
  if (nullptr == lhsb2) {  // no second bond on LHS
    ...
    if (lhsb2_directionality == lhs1_direction) {
      if (display_smiles_interpretation_error_messages()) {
        cerr << "Molecule::_process_directional_system:inconsistent directional bonding "
                "on bond "
             << (*lhsb2) << endl;
      }
      return 0;
...
int
Molecule::_identify_directional_bonds_across_double_bonds(
...
int
Molecule::_set_any_unambiguous_unset_directional_bonds(
    resizable_array<const Bond*>& directional_bonds) {
...
int
Molecule::_fill_in_missing_directional_bond_specification(
  ...
  if (not_directional->is_directional_up()) {
    cerr << "Non directional bond " << not_directional->a1() << ' '
         << not_directional->a2() << " now up\n";
  } else if (not_directional->is_directional_down()) {
    cerr << "Non directional bond " << not_directional->a1() << ' '
         << not_directional->a2() << " now down\n";
  } else {
    cerr << "HUH\n";
  }
#endif
...
/*int
Molecule::_mark_adjacent_double_bond_with_directional_atoms_at_other_end (atom_number_t
a3, int * process_these_atoms)
{
  ...
  {
    cerr << "Molecule::_mark_adjacent_double_bond_with_directional_atoms_at_other_end:no
doubly bonded connection\n"; return 0;
  }

#ifdef DEBUG_MARK_ADJACENT_DOUBLE_BOND_WITH
  cerr << "Doubly bonded bond from " <<a3 << " is " << doubly_bonded->a1() << " to " <<
doubly_bonded->a2() << endl; #endif
...
//cerr << "At atom " << zatom << " found " << up_bond << " up bonds and " << down_bond <<
" down bonds\n";
...
//cerr << "Non directional bond from " << zatom << " is " <<
non_directional_bond_encountered->a1() << " to " << non_directional_bond_encountered->a2()
<< endl;
...
int
Molecule::_identify_linked_cis_trans_bonds(
    resizable_array<const Bond*>& bonds_to_be_flipped, atom_number_t previous_atom,
    atom_number_t zatom, int* bond_already_done) const {
  const Atom* a = _things[zatom];
...
#ifdef DEBUG_IDENTIFY_LINKED_CIS_TRANS_BONDS
  cerr << "Found " << coupled_atoms.number_elements()
       << " possibly coupled atoms, double bond " << atom_at_end_of_double_bond << endl;
#endif
  ...
  return _identify_linked_cis_trans_bonds(bonds_to_be_flipped, zatom,
                                          atom_at_end_of_double_bond, bond_already_done);
}

int
Molecule::_identify_linked_cis_trans_bonds(
    resizable_array<const Bond*>& bonds_to_be_flipped, const Bond* current_bond,
...
Molecule::flip_cis_trans_bond_and_all_related_directional_bonds(const Bond* b) {
  if (!b->is_directional()) {
    cerr << "Molecule::flip_cis_trans_bond_and_all_related_directional_bonds:bond is not "
            "directional\n";
  ...
  int* bond_already_done = new_int(_bond_list.number_elements());
  std::unique_ptr<int[]> free_bond_already_done(bond_already_done);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (b->is_directional_up()) {
      ...
      cerr << "Molecule::flip_cis_trans_bond_and_all_related_directional_bonds:non "
              "directional bond\n";
    }
  }
...
Molecule::_adjust_cis_trans_bonds_to_canonical_form(const int* canonical_rank) {
  ...
  int* bond_already_done = new_int(nb);
  std::unique_ptr<int[]> free_bond_already_done(bond_already_done);
...
static int
compute_bond_canonical_rank(const Bond* b, int matoms, const int* canonical_rank) {
  atom_number_t a1 = b->a1();
  atom_number_t a2 = b->a2();
...
int
Molecule::_canonicalise_linked_group_of_cis_trans_bonds(
    const resizable_array<const Bond*>& bonds_in_grouping, const int* canonical_rank) {
  int n = bonds_in_grouping.number_elements();

  int highest_score =
      compute_bond_canonical_rank(bonds_in_grouping[0], _number_elements, canonical_rank);
  int ndx_of_highest_score = 0;

  for (int i = 1; i < n; i++) {
    int s = compute_bond_canonical_rank(bonds_in_grouping[i], _number_elements,
                                        canonical_rank);
    if (s > highest_score) {
      highest_score = s;
...
int
Molecule::_set_bond_directionality(atom_number_t a1, atom_number_t a2, int dir) {
  _set_modified();
