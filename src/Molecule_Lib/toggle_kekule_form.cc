...
int
Toggle_Kekule_Form::construct_from_command_line(Command_Line& cl, char c, int verbose) {
  ...
  while (cl.value(c, cv, i++)) {
    ...
    if ('-' == cv[nchars]) {
      ...
      cerr << "Toggle_Kekule_Form::construct_from_command_line: bonds must be '-' or "
              "'=', '"
           << cv << "' is invalid\n";
      return 0;
    }
...
Toggle_Kekule_Form::ok_embedding(const Set_of_Atoms& embedding) const {
  ...
  for (int i = 0; i < _bond.number_elements(); i++) {
    ...
    if (b->a1() >= embedding.number_elements()) {
      cerr << "Toggle_Kekule_Form::ok_embedding: bond " << i << " atom " << b->a1()
           << " invalid\n";
      rc = 0;
    }

    if (b->a2() >= embedding.number_elements()) {
      cerr << "Toggle_Kekule_Form::ok_embedding: bond " << i << " atom " << b->a2()
           << " invalid\n";
      rc = 0;
    }
  }
...
int
Toggle_Kekule_Form::_bond_is_correct(const Molecule& m, const Set_of_Atoms& embedding,
                                     const Bond* b) const {
  ...
  for (int i = 0; i < m.natoms(); i++) {
    for (int j = i + 1; j < m.natoms(); j++) {
      ...
      cerr << "Can change bond between atoms " << i << " and " << j << " value "
           << tkfta.can_change_bond()[i * m.natoms() + j] << '\n';
    }
  }
#endif
...
static boolean
multiple_bond_outside_aromatic_ring(Molecule& m, atom_number_t i, const Atom* a) {
  int acon = a->ncon();
  for (int j = 0; j < acon; j++) {
    const Bond* b = a->item(j);
    if (b->is_single_bond()) {
...
Toggle_Kekule_Form::grow_ring_system(Molecule& m, const Ring* r, int id,
  ...
  cerr << "grow_ring_system: rid " << id << ", ring number " << r->ring_number() << ", "
       << frn << " fused neighbours\n";
#endif
...
Toggle_Kekule_Form::_process(Molecule& m, const Set_of_Atoms& embedding,
...
#ifdef DEBUG_GROW_RING_SYSTEM
  cerr << isolated_rings.number_elements() << " isolated and "
       << fused_rings.number_elements() << " fused rings\n";
#endif
  ...
  for (int i = 0; i < isolated_rings.number_elements(); i++) {
    ...
    if (!_process_single_ring(m, embedding, rid, tkfta)) {
      ...
      if (_display_error_messages) {
        cerr << "Toggle_Kekule_Form::_process: cannot change isolated rid " << rid
             << ", molecule '" << m.name() << "'\n";
      }
    }
  }
  ...
  for (int i = 0; i < fused_rings.number_elements(); i++) {
    ...
    if (!_process_ring_system(m, embedding, rid, tkfta)) {
      rc = 0;
      if (_display_error_messages) {
        cerr << "Toggle_Kekule_Form::_process: cannot change fused rid " << rid
             << ", molecule '" << m.name() << "'\n";
        if (2 == _display_error_messages) {
          cerr << "Starting atoms";
          for (int j = 0; j < embedding.number_elements(); ++j) {
            cerr << ' ' << embedding[j] << ' '
                 << m.smarts_equivalent_for_atom(embedding[j]);
          }
          cerr << '\n';
          for (int j = 0; j < _bond.number_elements(); ++j) {
            cerr << " btw " << _bond[j]->a1() << " and " << _bond[j]->a2() << " type "
                 << _bond[j]->btype() << '\n';
          }
        }
      }
...
next_atom_in_ring(const Atom* a, int rid, const int* process_these,
                  const atom_number_t aprev, const atom_number_t astart2) {
  int acon = a->ncon();
...
void
Toggle_Kekule_Form::_set_all_bonds_to_single(
    Molecule& m, int id, Set_of_Atoms& bonds_to_be_restored,
    Toggle_Kekule_Form_Temporary_Arrays& tkfta) const {
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    for (int j = 0; j < acon; j++) {
      ...
      //    cerr << "Can we change bond between " << i << " and " << k << " " <<
      //    can_change_bond[i*matoms+k] << '\n';
...
int
Toggle_Kekule_Form::_restore_previous_bonding(
    Molecule& m, const Set_of_Atoms& bonds_to_be_restored) const {
  const int n = bonds_to_be_restored.number_elements();

  for (int i = 0; i < n; i += 3) {
    if (1 == bonds_to_be_restored[i + 2]) {
      m.set_bond_type_between_atoms(bonds_to_be_restored[i], bonds_to_be_restored[i + 1],
                                    SINGLE_BOND);
    } else {
      m.set_bond_type_between_atoms(bonds_to_be_restored[i], bonds_to_be_restored[i + 1],
                                    DOUBLE_BOND);
...
int
Toggle_Kekule_Form::_set_our_bonds(Molecule& m, const Set_of_Atoms& embedding, int id,
                                   Toggle_Kekule_Form_Temporary_Arrays& tkfta) const {
  const int* process_these = tkfta.process_these();
  int* can_change_bond = tkfta.can_change_bond();
...
int
Toggle_Kekule_Form::_process_single_ring(Molecule& m, const Set_of_Atoms& embedding,
                                         int rid,
                                         Toggle_Kekule_Form_Temporary_Arrays& tkfta) {
  ...
  cerr << "Toggle_Kekule_Form::_process_single_ring:atoms " << astart1 << " and "
       << astart2 << '\n';
  for (int i = 0; i < matoms; ++i) {
    cerr << i << " " << m.smarts_equivalent_for_atom(i) << " process_these "
         << tkfta.process_these()[i] << " can change " << tkfta.atom_can_change(i)
         << '\n';
  }
#endif
...
int
Toggle_Kekule_Form::_process_single_ring2(Molecule& m, const Set_of_Atoms& embedding,
                                          int rid,
                                          Toggle_Kekule_Form_Temporary_Arrays& tkfta) {
...
// #define DEBUG_TOGGLE_KEKULE_PROCESS_SINGLE_RING
  ...
  while (1) {
    atom_number_t anext =
        next_atom_in_ring(atom[astart2], rid, process_these, aprev, astart2);

#ifdef DEBUG_TOGGLE_KEKULE_PROCESS_SINGLE_RING
    cerr << "Prev " << aprev << ", from atom " << astart2 << " advance to " << anext
         << ", previous_bond " << previous_bond << " hcount(next) " << m.hcount(anext)
         << " '" << m.smiles() << "'\n";
#endif
    ...
    } else if (6 == atom[anext]->atomic_number() &&
               0 == m.hcount(anext)) {  // no room for a double bond
      previous_bond = BOND_TYPE_ONLY(m.bond_between_atoms(astart2, anext)->btype());
    } else if (7 == atom[anext]->atomic_number() && 0 == atom[anext]->formal_charge() &&
               0 == m.hcount(anext)) {
      previous_bond = BOND_TYPE_ONLY(m.bond_between_atoms(astart2, anext)->btype());
    } else {
...
#ifdef DEBUG_TOGGLE_KEKULE_PROCESS_SINGLE_RING
    cerr << "Atom pair is " << astart2 << " to " << anext << " set to " << previous_bond
         << " smiles " << m.smiles() << "\n";
#endif
...
int
Toggle_Kekule_Form::_process_ring_system(Molecule& m, const Set_of_Atoms& embedding,
                                         int rid,
                                         Toggle_Kekule_Form_Temporary_Arrays& tkfta) {
  ...
  for (int i = 0; i < matoms; i++)  // turn back to positive numbers
  {
    ...
    //  cerr << " atom " << i << m.smarts_equivalent_for_atom(i) << " process_these " <<
    //  process_these[i] << '\n';
  }
...
int
Toggle_Kekule_Form::_process_ring_system(
    Molecule& m, resizable_array<int>& atoms_to_process, int rid, int zitem,
    const atom_number_t prev, Toggle_Kekule_Form_Temporary_Arrays& tkfta) const {
  ...
  cerr << curr << " atom_can_change " << tkfta.atom_can_change(curr) << " bonds "
       << acurr->nbonds() << " con " << acon << '\n';
#endif
  ...
  // Seems as if all these checks can be replace by the unsaturation test above
  else if (has_double_bond[curr])  // already has a double bond - what about *=N(=O)-*
                                   // groups?
      tochange = 0;
      ...
      tochange = 0;
  else if (7 == z && 3 == acon && 5 == acurr->nbonds()) tochange = 0;
#endif

#ifdef DEBUG_PROCESS_RING_SYSTEM
  cerr << "Atom " << curr << " " << m.smarts_equivalent_for_atom(curr) << " tochange? "
       << tochange << '\n';
#endif
  ...
  for (int i = 0; i < acon; i++) {
    ...
    cerr << "    bond to " << qq << " process_these " << process_these[qq]
         << " can_change_bond " << can_change_bond[curr * matoms + qq]
         << " has_double_bond " << has_double_bond[qq] << '\n';
#endif
  ...
  // If the atom might be a pyrrole nitrogen or furan, even though we did not place a
  // double bond, that mayu be just fine
...
int
Toggle_Kekule_Form::_all_atoms_aromatic(
    Molecule& m, int id, int display_error_message,
    Toggle_Kekule_Form_Temporary_Arrays& tkfta) const {
  const int* process_these = tkfta.process_these();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (display_error_message) {
      cerr << "Toggle_Kekule_Form::_all_atoms_aromatic: atom " << i << " not aromatic ("
...
int
Toggle_Kekule_Form::add_bond(int a1, int a2, bond_type_t bt) {
...
Toggle_Kekule_Form::add_bond_from_msi_attribute(const msi_attribute& msi) {
  if (3 != msi.number_int_values()) {
    cerr << "Toggle_Kekule_Form::add_bond_from_msi_attribute:invalid specification '"
         << msi << "'\n";
    return 0;
  }
  ...
  if (a1 < 0 || a2 < 0 || a1 == a2) {
    cerr << "Toggle_Kekule_Form::add_bond_from_msi_attribute:invalid atom specification '"
         << msi << "' atoms " << a1 << " and " << a2 << '\n';
    return 0;
  }
  ...
  if (1 == b) {
    ...
    cerr << "Toggle_Kekule_Form::add_bond_from_msi_attribute:unrecognised bond type '"
         << msi << "'\n";
    return 0;
  }
...
Toggle_Kekule_Form::ConstructFromProto(const ToggleKekuleForm::ToggleKekuleForm& proto) {
  ...
  for (const auto& bond : proto.bond()) {
    if (!bond.has_a1() || !bond.has_a2()) {
      cerr << "Toggle_Kekule_Form::ConstructFromProto:no a1/a2 "
           << proto.ShortDebugString() << '\n';
      return 0;
    }

    if (bond.a1() == bond.a2()) {
      cerr << "Toggle_Kekule_Form::ConstructFromProto:invalid a1/a2 "
           << proto.ShortDebugString() << '\n';
      return 0;
    }

    if (!bond.has_btype()) {
      cerr << "Toggle_Kekule_Form::ConstructFromProto:no bond type "
           << proto.ShortDebugString() << '\n';
      return 0;
    }
    ...
    switch (bond.btype()) {
      ...
      case SubstructureSearch::SS_DOUBLE_BOND:
        ...
        break;
      default:
        cerr << "Toggle_Kekule_Form::ConstructFromProto:unrecognized bond type "
             << proto.ShortDebugString() << '\n';
        return 0;
    }
...
int
Toggle_Kekule_Form::write_msi(std::ostream& os, const IWString& ind,
                              const char* attribute_name) const {
  for (int i = 0; i < _bond.number_elements(); i++) {
    const Bond* b = _bond[i];

    os << ind << "(A I " << attribute_name << " (" << b->a1() << ' ' << b->a2() << ' '
       << btype_back_to_mdl_form(b->btype()) << "))\n";
  }
...
/*
  this is kind of bizzare. All we do is set up the conditions where the regular process()
  member function can be called...
*/

int
Toggle_Kekule_Form::process(Molecule& m, atom_number_t a1, atom_number_t a2,
                            bond_type_t bt, int& changed) {
  ...
  // cerr << "Toggle_Kekule_Form making change rc " << rc << " changed " << changed <<
  // '\n';
