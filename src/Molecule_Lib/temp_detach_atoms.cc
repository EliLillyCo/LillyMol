...
Temp_Detach_Atoms::reattach_atoms(Molecule& m) {
  ...
  // cerr << "Temp_Detach_Atoms::reattach_atoms:replacing " <<
  // _chiral_centre.number_elements() << " chiral centres\n";

  // Transfer our saved chiral centres back to the molecule.
...
int
Temp_Detach_Atoms::ReattachAtoms(Molecule& m,
                const int* no_reattachments) {
  assert(_matoms == m.natoms());
  ...
  for (int i = 0; i < _matoms; i++) {
    ...
    // Atom i has not been detached, no destination available.
    if (o < 0) {
      continue;
    }

    if (no_reattachments != nullptr && no_reattachments[o]) {
      atoms_to_be_removed.add_if_not_already_present(i);
      continue;
    }
  ...
  // Transfer our saved chiral centres back to the molecule.
  for (const Chiral_Centre* c: _chiral_centre) {
    if (! m.valid_chiral_centre(c)) {
      continue;
    }
    std::unique_ptr<Chiral_Centre> ccopy = std::make_unique<Chiral_Centre>(*c);
    m.add_chiral_centre(ccopy.release(), 1);
  }

  if (_remove_hydrogens_no_longer_needed && atoms_to_be_removed.size() > 0) {
    m.remove_atoms(atoms_to_be_removed);
  }
