...
int
Atomic_Numbers_Encounterd::extra(const Bond* b, const Element* e, int ncon) {
  assert(ncon > 0);
...
int
Path_Scoring::advance(Atom* const* atom, const int* claimed) {
  ...
  Set_of_Atoms
      new_edge_atoms;  // easier than doing an in-place replacement into _edge_atom

  if (new_edge_atoms.elements_allocated() < ne * 2 + 5) {
    new_edge_atoms.resize(ne * 2 + 5);
  }

  _sum *= 7899 * _sum;  // arbitrary number.
...
#endif
      _sum += BondConstant(b) * atomic_number_from_element(atom[i]->element());
...
static int
straight_bond(const Coordinates& ab, const Coordinates& bc, const Coordinates& cd,
...
static int
identify_attached_bonds(const Atom* a, Bond*& b12, Bond*& b13) {
  b12 = nullptr;
...
int
Molecule::identify_ez_atoms(atom_number_t a3, atom_number_t a4, atom_number_t& alhs,
  ...
  if (a3con < 2 || a3con > 3 || a4con < 2 || a4con > 3) {
    //  cerr << "Molecule::score_ez_bond: atom " << a3 << " (" << a3con << " connections)
    //  or atom " << a4 << " (" << a4con << " connections) cannot form a E/Z bond\n";
    return 0;
  }

  bond_type_t bt;
  if (!aa3->is_bonded_to(a4, bt) || !IS_DOUBLE_BOND(bt)) {
    cerr << "Molecule::score_ez_bond: atoms " << a3 << " and " << a4
         << " not bonded via a double bond\n";
    return 0;
  }
  ...
  int* already_done = new_int(_number_elements);
  ...
  if (INVALID_ATOM_NUMBER == alhs)  // 2 substituents, need to resolve
  {
    ...
    cerr << "After scoring atom " << a3 << " (atoms " << a1 << " and " << a2 << ") lhs "
         << lhs << '\n';
#endif
...
#ifdef DEBUG_SCORE_EZ
    cerr << "After scoring atom " << a4 << " (atoms " << a5 << " and " << a6 << ") rhs "
         << rhs << '\n';
#endif
...
int
Molecule::ez_by_geometry(atom_number_t a1, atom_number_t a2, atom_number_t a3,
                         atom_number_t a4, angle_t tolerance) const {
#ifdef DEBUG_EZ_GEOMETRY
  cerr << "Checking geometry for atoms " << a1 << ", " << a2 << ", " << a3 << " and "
       << a4 << '\n';
#endif
  ...
  if (static_cast<coord_t>(0.0) == ab.x() && static_cast<coord_t>(0.0) == ab.y() &&
      static_cast<coord_t>(0.0) == ab.z()) {
  ...
  if (x1.z() < 0.0 && x2.z() < 0.0) {
    ...
    cerr << "Molecule::::_ez_by_geometry: atoms " << a1 << ", " << a2 << ", " << a3
         << ", " << a4 << " possibly collinear\n";
    return 0;
  }
#endif
}
...
int
Molecule::_score_ez_bond(int* already_done, Path_Scoring& ps1, Path_Scoring& ps2) const {
  while (1) {
...
int
resolved(resizable_array_p<Path_Scoring>& ps, int& stopped) {
  stopped = 0;
...
#ifdef DEBUG_RESOLVED
  cerr << "After scoring " << ps.size() << " path_scoring_comparitor objects\n";
  for (const auto* p : ps) {
    cerr << *p << '\n';
  }
#endif
...
Molecule::_discern_cis_trans_bond_from_depiction(Bond* b) {
  ...
  if (!identify_ez_atoms(a3, a4, alhs, arhs)) {  // redundancy between this call and the
                                                 // two calls to identify_attached_atoms
    return 1;
  }
  ...
  if (INVALID_ATOM_NUMBER == a2) {  // just one connection, nothing to check
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent "
            "directionality\n";
    cerr << "Atoms " << a1 << " (" << a1_direction << ") - " << a3 << " - " << a2 << " ("
         << a2_direction << ")\n";
    return 0;
  }
  ...
  if (INVALID_ATOM_NUMBER == a6) {  // just one connection, nothing to check
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent "
            "directionality\n";
    cerr << "Atoms " << a5 << " (" << a5_direction << ") - " << a4 << " - " << a6 << " ("
         << a6_direction << ")\n";
    return 0;
  }
...
#ifdef DEBUG_DISCERN_CIS_TRANS_BOND_FROM_DEPICTION
  cerr << "Directions:\n";
  cerr << "a1 " << a1 << ' ' << a1_direction << " a2 " << a2 << ' ' << a2_direction
       << " a5 " << a5 << ' ' << a5_direction << " a6 " << a6 << ' ' << a6_direction
       << '\n';
#endif
  ...
  if (0 == a1_direction || 0 == a5_direction) {
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent 1,5 "
            "directionality across bond\n";
    cerr << "Atoms " << a1 << " (" << a1_direction << ") - " << a3 << " = " << a4 << " - "
         << a5 << " (" << a5_direction << ") molecule '" << _molecule_name << "'\n";
    return 0;
  }
  ...
  if (INVALID_ATOM_NUMBER == a2 || 0 == a5_direction || 0 == a2_direction) {
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent 2,5 "
            "directionality across bond\n";
    cerr << "Atoms " << a2 << " (" << a2_direction << ") - " << a3 << " = " << a4 << " - "
         << a5 << " (" << a5_direction << ") molecule '" << _molecule_name << "'\n";
    return 0;
  }
  ...
  if (INVALID_ATOM_NUMBER == a2 || INVALID_ATOM_NUMBER == a6 || 0 == a2_direction ||
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent "
            "directionality across bond\n";
    cerr << "Atoms " << a2 << " - " << a3 << " = " << a4 << " - " << a6 << " molecule '"
         << _molecule_name << "'\n";
    return 0;
  }
  ...
  if (INVALID_ATOM_NUMBER == a6 || 0 == a1_direction || 0 == a6_direction) {
    ...
    cerr << "Molecule::_discern_cis_trans_bond_from_depiction:inconsistent "
            "directionality across bond\n";
    cerr << "Atoms " << a1 << " - " << a3 << " = " << a4 << " - " << a6 << " molecule '"
         << _molecule_name << "'\n";
    return 0;
  }
  ...
  if (0 == a1_direction && 0 == a2_direction && 0 == a5_direction &&
      0 == a6_direction)  // hopefully the most common case
  {
...
/*
  Atom ZATOM is attached to (but not part of) a double bond that has cis-trans
  stereochemistry. Is it part of another cis-trans grouping? We only return 0 if there is
  an error
*/
