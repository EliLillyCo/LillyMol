...
void
fetch_closing_paren(const_IWSubstring& buffer, IWString& token) {
  int paren_level = 1;
  ...
  while (smarts.length() > 0) {
    ...
    if (smarts.starts_with('(')) {
      ...
      if (s != nullptr && s->length() > 0) {
        ...
        components.add(s);
        //      cerr << "Smarts now " << smarts << " s '" << *s << "' s->length() " <<
        //      s->length() << '\n';
        smarts += s->length();
      }
    }
...
IWReaction::construct_from_smirks(const const_IWSubstring& reagents,
  ...
  if (0 != reagents.balance('(', ')')) {
    cerr << "IWReaction::construct_from_smirks:unbalanced parentheses in reagent "
            "specification '"
         << reagents << "'\n";
    return 0;
  }
...
// #define DEBUG_SMIRKS_TOKENISING
  ...
  if (product_molecule.any_query_atom([](const Substructure_Atom* q) {
        if (q->element().number_elements() > 1) {
          return 1;
        } else {
          return 0;
        }
  ...
  for (int i = 1; i < ramn.number_elements(); ++i) {
    ...
    cerr << "IWReaction::construct_from_smirks:atom map number " << i << " found "
         << ramn[i] << " occurrences\n";
    return 0;
  }
  ...
  // We need to assign atom map numbers to unmapped atoms in the product. These are, by
  // definition orphan atoms.
  ...
  // Ensure that all atom numbers are present. Identify any orphans - atoms in the
  // products that do not have a corresponding atom in the reagents
  ...
  for (int i = 1; i < istop; i++) {
    ...
    cerr << "IWReaction::construct_from_smirks:atom number mismatch, atom " << i
         << " in reagents " << ramn[i] << " in products " << pamn[i] << '\n';
    return 0;
  }
...
static int
add_bond_to_orphan(Molecule& orphan_molecule, const atom_number_t a1,
                   const atom_number_t a2, const Substructure_Bond* b) {
  ...
  if (IS_SINGLE_BOND(bt)) {
    ...
    cerr << "IWReaction::_create_orphan_molecule:bond type not unitary form " << bt
...
int
IWReaction::_create_orphan_molecule(
    const resizable_array<int>& orphan_atoms,  // atom map numbers
    Molecule& orphan_molecule, const Substructure_Query& product_molecule,
  ...
  int* mol2qry = new int[n];
  std::unique_ptr<int[]> free_mol2qry(mol2qry);
  int* amap2mol = new_int(mx + 1, -1);
  std::unique_ptr<int[]> free_amap2mol(amap2mol);

  const Substructure_Atom** query_atoms = new const Substructure_Atom*[n];
  std::unique_ptr<const Substructure_Atom*[]> free_query_atoms(query_atoms);
  ...
  for (int i = 0; i < orphan_atoms.number_elements(); ++i) {
    ...
    if (e.empty()) {
      cerr << "IWReaction::_create_orphan_molecule:RHS orphan atom has no element "
              "specified, impossible. Atom map "
           << oi << '\n';
      return 0;
    }
  ...
  // Now the atoms have been added do the bonds. We need to handle both bonds within the
  // orphan atoms, and bonds to other parts of the products
  ...
  for (int i = 0; i < orphan_atoms.number_elements(); ++i) {
    ...
    for (int j = 0; j < nc;
         ++j)  // scan all atoms attached to the corresponding query atom
    {
...
/*
  ...
  If there are orphan atoms on the RHS, not present on the LHS, then we must assume this
  is a fixed reagent sidechain.
...
int
IWReaction::_identify_changes_from_smirks(
    const extending_resizable_array<int>& atom_map_numbers_in_reagents, const int istop,
    const Molecule& orphan_molecule, const Substructure_Query& product_molecule) {
#ifdef DEBUG_CONSTRUCT_FROM_SMIRKS
  cerr << "IWReaction::_identify_changes_from_smirks:must examine " << istop
       << " mapped atoms\n";
  ...
  // Any bond changes. this must be done on a per sidechain basis because there may be
  // bonding changes that occur just within a given sidechain e.g.
  // ([CH3:1].[NH2D1:2]).[OH:3]-[C:4]=[O:5]>>[CH2:1]-[NH:2]-[C:4]=[O:5]
  ...
  for (int i = 0; i < istop; i++) {
    ...
    for (int j = i + 1; j < istop; j++) {
      //    cerr << i << ',' << j << " atom_map_numbers_in_reagents " <<
      //    atom_map_numbers_in_reagents[i] << " j " << ' ' <<
      //    atom_map_numbers_in_reagents[j] << '\n';
      if (atom_map_numbers_in_reagents[i] <= 0 &&
          atom_map_numbers_in_reagents[j] <= 0) {  // numbers not used
        continue;
      }
      ...
      const Substructure_Bond* bonded_in_product =
          product_molecule.bond_between_atom_map_numbers(i, j);
      //    cerr << "Between mapped atoms " << i << " and " << j << " bnd " <<
      //    bonded_in_product << " Product\n";
      ...
      if (nullptr == bonded_in_reagent &&
          nullptr == bonded_in_product) {  // no bond btw I and J either side
        continue;
      }

      bond_type_t btr = nullptr == bonded_in_reagent
                            ? INVALID_BOND_TYPE
                            : BOND_TYPE_ONLY(bonded_in_reagent->types_matched());
      bond_type_t btp = nullptr == bonded_in_product
                            ? INVALID_BOND_TYPE
                            : BOND_TYPE_ONLY(bonded_in_product->types_matched());

      if (btr == btp) {  // same bond type, no need to change anything
        continue;
      }

      //    the bonding changes. Is it a removal, an addition or a change? What components
      //    are involved?

      //    cerr << "Between atoms " << i << " and " << j << " reagent? " <<
      //    bonded_in_reagent << " (" << btr << ") products " << bonded_in_product << " ("
      ...
      if (i_site == j_site)  // new/changed or removed bond in the same component
      {
        ...
        //      cerr << "Atom numbers " << ai->initial_atom_number() << " and " <<
        //      aj->initial_atom_number() << " xBTP " << btp << '\n';
        if (INVALID_BOND_TYPE == btp) {  // bond is removed
          i_site->add_bond_to_be_broken(ai->initial_atom_number(),
                                        aj->initial_atom_number());
        } else {  // new or changed
          i_site->add_bond_to_be_made(ai->initial_atom_number(),
                                      aj->initial_atom_number(), btp);
        }
      ...
      if (nullptr != i_site &&
          nullptr != j_site)  // new inter partile bond btw existing reagents
      {
      ...
      //    cerr << "Bond involves different sites. I " << i_site << " J " << j_site << "
      //    type " << btp << '\n';
...
int
IWReaction::_from_smirks_add_inter_particle_bond(int amap1, Reaction_Site* a1site,
                                                 int amap2, Reaction_Site* a2site,
                                                 bond_type_t bt) {
  ...
  if (a1site == this) {
    ...
    _sidechains[r2]->add_inter_particle_bond(-1, a1->initial_atom_number(),
                                             a2->initial_atom_number(), bt);
  } else if (a2site == this) {
    const int r1 = _sidechain_with_mapped_atom(amap1);
    _sidechains[r1]->add_inter_particle_bond(-1, a2->initial_atom_number(),
                                             a1->initial_atom_number(), bt);
  } else  // different sidechains
  {
    const int r1 = _sidechain_with_mapped_atom(amap1);
    const int r2 = _sidechain_with_mapped_atom(amap2);
    _sidechains[r1]->add_inter_particle_bond(r2, a2->initial_atom_number(),
                                             a1->initial_atom_number(), bt);
  }
...
IWReaction::_from_smirks_add_inter_particle_bond_involves_scaffold(int a1, int a2,
  ...
  if (r2 < 0) {
    cerr << "IWReaction::_from_smirks_add_inter_particle_bond_involves_scaffold:no "
            "sidechain for "
         << a2 << '\n';
    return 0;
  }
...
int
IWReaction::_discern_atomic_changes(Reaction_Site& r, const Substructure_Atom& q1,
...
int
IWReaction::_discern_atomic_changes_specifier(Reaction_Site& r, const atom_number_t a,
...
template <typename T>
int
test_reaction_from_smirks_record(const const_IWSubstring& buffer, T& output) {
  IWReaction rxn;
...
template <typename T>
int
test_reaction_from_smirks(iwstring_data_source& input, T& output) {
  const_IWSubstring buffer;
...
template <typename T>
int
test_reaction_from_smirks(const char* fname, T& output) {
...
static int
test_reaction_from_smirks(const int argc, char** argv) {
