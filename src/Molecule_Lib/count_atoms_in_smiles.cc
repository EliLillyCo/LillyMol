...
int
Smiles_Text::build(const char* s, const int nchars, const int is_smiles) {
...
Smiles_Text::_handle_dot(int& cstart) {
  ...
  if (kOpenBrace != _smiles[cstart + 3])  // just three dots
  {
  ...
  const int close_brace_pos = _index_of_closing(cstart + 3, kOpenBrace, kCloseBrace);
  if (close_brace_pos < 0) {
    return _error(cstart, "Mismatched {}\n");
  }

  _token[_ntokens++].set(_smiles.rawdata() + cstart, close_brace_pos - cstart, SSC_DOT);
  cstart = close_brace_pos + 1;
...
Smiles_Text::_consume_smarts_bond(int& i) {
  ...
  _token[_ntokens++].set_bond(_smiles.rawdata() + i - consumed, consumed,
                              SSC_SMARTS_BOND);
...
static int
test_smiles_tokeniser_buffer(const const_IWSubstring& buffer, const int is_smiles,
...
static int
test_smiles_tokeniser(iwstring_data_source& input, const int is_smiles,
                      IWString_and_File_Descriptor& output) {
...
static int
test_smiles_tokeniser(const char* fname, const int is_smiles,
                      IWString_and_File_Descriptor& output) {
