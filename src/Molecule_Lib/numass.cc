...
int
Number_Assigner::initialise(Command_Line& cl, char nflag, int verbose) {
  ...
  for (int i = 0; cl.value(nflag, n, i); ++i) {
    int tmp;
    if (n.numeric_value(tmp)) {
      if (tmp < 0) {
        cerr << "Number_Assigner::initialise: the '" << nflag
             << "' option must be followed by a positive whole number\n";
        return 0;
      }
    ...
    if (n.starts_with("ndigits=")) {
      ...
      if (verbose) {
        cerr << "Numeric component padded to " << _number_digits
             << " digits with leading 0's\n";
      }
  ...
  if (verbose) {
    cerr << "Molecules will be assigned numbers starting with " << _prefix_string;
    if (_include_parentheses) {
      cerr << kOpenParen;
    }
    cerr << _next_number_to_assign;
    if (_include_parentheses) {
      cerr << kCloseParen;
    }
    cerr << '\n';
...
Number_Assigner::initialise(int i) {
  if (i < 0) {
    cerr << "Number_Assigner::initialise: the initial value must be >= 0, " << i
         << " not allowed\n";
    return 0;
  }
...
Number_Assigner::process(IWString& s) {
  ...
  // cerr << "LIne " <<__LINE__ << " buffer '" <<  buffer << "' s '" << s << "' _replace_name " << _replace_name << '\n';
  if (_replace_name) {
    ;
  } else if (s.length()) {
...
display_standard_number_assigner_options(std::ostream& os, char flag) {
  // a common prefix written on each line
  IWString myflag;
  myflag << " -" << flag;

  // clang-format off
  os << myflag << " <number>    assign sequential numbers R(%d) starting with <number>\n";
  os << myflag << " <string>    use <string> rather than 'R'\n";
  os << myflag << " 'replace'   replace the entire name with R(nnn)\n";
  os << myflag << " xrf=fname   create a cross reference file\n";
  os << myflag << " noparen     omit parentheses when forming the names\n";
  os << myflag << " digits=<n>  left pad numbers with leading 0's to yield <n> digits\n";
  os << myflag << " nochange    only apply if the existing name is empty\n";
  os << myflag << " nsep='c'    separator between number and existing name (default ' ')\n";
  os << myflag << " seq         assign unadorned sequential numbers - no prefix or parentheses\n";
  // clang-format on
