...
// Now ask INCHI API to build an INCHI molecule from the INCHI string
  ...
  char *szInChI = new char[tmp.nchars()+1]; std::unique_ptr<char[]> free_szInChI(szInChI);
...
// then get the inchi string from the inchi molecule and put into S
  ...
  inchi_Atom * at =new inchi_Atom [_number_elements]; std::unique_ptr<inchi_Atom[]> free_a(at);
  ...
  for (int i = 0; i < _number_elements; i++)
  {
    ...
    for (int j = 0; j < icon; j++)
    {
      ...
      {
        cerr << "Molecule::InChI:unrecogised bond type " << b->btype() << '\n';
        bond_type = INCHI_BOND_TYPE_SINGLE;
      }
  ...
  inchi_Stereo0D  * st =new inchi_Stereo0D [ncc]; std::unique_ptr<inchi_Stereo0D[]> free_s(st);
