...
Molecule::identify_largest_organic_fragment(Set_of_Atoms& atoms_to_be_removed,
  ...
  const resizable_array<int>& atoms_in_fragment =
      _fragment_information.atoms_in_fragment();

  int* exclude = new_int(_number_elements);
  std::unique_ptr<int[]> free_exclude(exclude);
  ...
  //   cerr << fragments_same_size_as_largest_organic << " fragments size " <<
  //   max_organic_atoms_in_fragment << ' ' << _molecule_name << endl;
...
class Fragment_Data {
 ...
 public:
  ...
  void
  another_organic_atom() {
    _organic_atoms++;
  }

  int
  organic_atoms() const {
    return _organic_atoms;
  }
  ...
  void
  extra_hydrogens(int h) {
    _hydrogens++;
  }

  void
  another_non_organic_atom() {
    _non_organic_atoms++;
  }

  int
  phosphorus_atoms() const {
    return _phosphorus;
  }
  ...
  int
  nitrogen_atoms() const {
    return _non_nitro_nitrogen;
  }
  ...
  int
  hydrogen_atoms() const {
    return _hydrogens;
  }

  int
  non_organic_atoms() const {
    return _non_organic_atoms;
  }
};
...
Fragment_Data::debug_print(std::ostream& os) const {
  os << "Fragment data for fragment " << _frag_id << ", " << _organic_atoms
     << " organic atoms\n";
...
Molecule::reduce_to_largest_fragment_carefully() {
  ...
  identify_largest_organic_fragment(atoms_to_be_removed,
                                    number_instances_of_largest_fragment);
  ...
  Fragment_Data* fd = new Fragment_Data[nf];
  std::unique_ptr<Fragment_Data[]> free_fd(fd);

  int* already_counted = new_int(_number_elements);
  std::unique_ptr<int[]> free_already_counted(already_counted);

#ifdef DEBUG_CAREFUL_FRAG
  cerr << _molecule_name << " has " << number_instances_of_largest_fragment
       << " instances of largest fragment\n";
#endif
...
fragment_data_comparitor(const void* v1, const void* v2) {
  ...
  if (f1->non_organic_atoms() ==
      f2->non_organic_atoms()) {  // most common case, both zero
    ;
...
int
Molecule::_reduce_to_largest_fragment_carefully(Fragment_Data* fd, int* already_counted) {
  int nf = number_fragments();
