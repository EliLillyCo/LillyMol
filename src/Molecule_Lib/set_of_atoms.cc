...
Set_of_Atoms::Set_of_Atoms(int initial_size)
    : resizable_array<atom_number_t>(initial_size) {
}

Set_of_Atoms::Set_of_Atoms(const Set_of_Atoms& rhs)
    : resizable_array<atom_number_t>(rhs) {
...
Set_of_Atoms::offset_atom_numbers(int offset) {
  for (int i = 0; i < _number_elements;
       i++) {  // should we worry about invalid_atom_number
    { _things[i] += offset; }
