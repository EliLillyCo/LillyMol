...
Substructure_Atom_Environment::debug_print(std::ostream& os) const {
  os << "Details on Substructure_Atom_Environment object with " << _number_elements
     << " components\n";
...
int
Substructure_Atom_Environment::involves_aromatic_bond_specifications(
    int& _need_to_compute_ring_membership) const {
  for (int i = 0; i < _number_elements; i++) {
    if (_things[i]->involves_aromatic_bond_specifications(
            _need_to_compute_ring_membership)) {
      return 1;
    }
  }
...
Substructure_Atom_Environment::create_from_smarts(const Atomic_Smarts_Component& env) {
  ...
  cerr << "Substructure_Atom_Environment::create_from_smarts: parsing " << env
       << " from primitive " << env.from_primitive() << '\n';
  _operator.debug_print(cerr);
#endif

  if (env.from_primitive()) {
    std::unique_ptr<Substructure_Atom_Specifier> spec =
    ...
    if (!spec->construct_from_smarts_token(mysmarts)) {
      cerr
          << "Substructure_Atom_Environment::create_from_smarts: cannot parse component '"
          << mysmarts << "'\n";
      return 0;
    }
...
#ifdef DEBUG_SS_ATOM_ENV_CREATE_FROM_SMARTS
  cerr << "ENV _number_elements " << _number_elements << " unary " << env.unary_operator()
       << " op " << env.op() << " results in operator " << _operator.number_results()
       << " from " << env << "\n";
#endif
...
int
Substructure_Atom_Environment::matches(Target_Atom& target_atom, int atoms_in_target,
                                       const int* already_matched) {
#ifdef DEBUG_SS_ATOM_ENV_MATCHES
  cerr << "Substructure_Atom_Environment::matches: " << _number_elements
       << " components, trying to match atom " << target_atom.atom_number() << '\n';
#endif
...
int
Substructure_Atom_Environment::_matches(Target_Atom& target_atom, int atoms_in_target,
...
#ifdef DEBUG_SS_ATOM_ENV_MATCHES
    cerr << "Result for component " << i << " is " << _operator.result(i) << '\n';
    _operator.debug_print(cerr);
#endif
...
Substructure_Atom_Environment::_match_component(Target_Atom& target_atom,
                                                ...
                                                int* already_matched) {
#ifdef DEBUG_SS_ATOM_ENV_MATCHES
  cerr << "Substructure_Atom_Environment::_match_component: component " << which_component
       << " trying to match atom " << target_atom.atom_number() << '\n';
#endif
...
#ifdef DEBUG_SS_ATOM_ENV_MATCHES
      cerr << "Atom " << j << " already_matched " << already_matched[j] << '\n';
#endif
    }
  }
  ...
  while (atom_to_process >= 0) {
    ...
    if (!a->move_to_next_match_from_current_anchor(already_matched, matched_atoms)) {
      ...
      cerr << "Move to next match succeeded " << a->unique_id() << "("
           ...
           << " matched = " << matched_atoms.number_elements() << '\n';
#endif
      int orid = a->or_id();
      if (orid) {
