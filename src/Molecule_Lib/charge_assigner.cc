...
Charge_Assigner::report(std::ostream& os) const {
  ...
  os << "Changed " << _molecules_changed << " of " << _molecules_examined
     << " molecules examined\n";
  if (0 == _molecules_examined) {
    return os.good();
  }

  os << "Assigned " << _negative_charges_assigned << " negative charges to "
     << _molecules_receiving_negative_charges << " molecules\n";
  os << "Assigned " << _positive_charges_assigned << " positive charges to "
     << _molecules_receiving_positive_charges << " molecules\n";
...
void
display_standard_charge_assigner_options(std::ostream& os, char cflag) {
  os << "  -" << cflag << " <...>       Charge assigner specification, enter \"-" << cflag
     << " help\" for usage\n";
...
void
display_all_charge_assigner_options(std::ostream& os, char cflag) {
  // clang-format off
...
int
Charge_Assigner::construct_from_command_line(Command_Line& cl, int verbose, char cflag) {
  _verbose = verbose;
  ...
  while (cl.value(cflag, opt, i++)) {
    ...
    if (opt.starts_with("over")) {
      ...
      if (verbose) {
        cerr << "Charge_Assigner::construct_from_command_line: existing formal charges "
                "can be overwritten\n";
      }
    } else if (opt == "simple") {
      simple_flag = 1;
      ...
      if (nullptr ==
          (_positive_element = get_element_from_symbol(opt, _positive_isotope))) {
        cerr << "Cannot discern positive element '" << opt << "'\n";
        return 0;
      }
      if (verbose) {
        cerr << "Positively charged atoms will be replace by element '"
             << _positive_element->symbol() << "'\n";
      }
    } else if (opt.starts_with("N:")) {
      opt.remove_leading_chars(2);
      if (nullptr ==
          (_negative_element = get_element_from_symbol(opt, _negative_isotope))) {
        cerr << "Cannot discern negative element '" << opt << "'\n";
        return 0;
      }
      if (verbose) {
        cerr << "Negatively charged atoms will be replace by element '"
             << _negative_element->symbol() << "'\n";
      }
    } else if ("isotope" == opt) {
      ...
      opt.remove_leading_chars(2);
      // queries always in same directory as control file
      static constexpr int kInheritDirectoryPath = 1;
      if (!queries_from_file(opt, me, kInheritDirectoryPath, verbose)) {
      ...
      if (verbose) {
        cerr << "Will attach an explicit Hydrogen atom to atoms assigned positive "
                "charges\n";
      }
    } else if (opt.starts_with("minsep=")) {
      opt.remove_leading_chars(7);
      if (!opt.numeric_value(_min_distance_between_charges) ||
          _min_distance_between_charges < 1) {
  ...
  if (nullptr == _negative_element && nullptr == _positive_element &&
      _apply_isotopic_labels) {
    cerr << "You have asked to apply isotopic labels, but have not specified atoms to "
            "change\n";
...
void
Charge_Assigner::_increment_global_counters(
    const Set_of_Atoms& positive_charges_assigned,
...
Charge_Assigner::_all_queries_have_numeric_value() const {
  for (int i = 0; i < _number_elements; i++) {
    // the (possibly) composite substructure query
    const Substructure_Query* cq = _things[i];

    int nq = cq->number_elements();
    for (int j = 0; j < nq; j++) {
      const Single_Substructure_Query* q = cq->item(j);
      if (!_numeric_value_present(q)) {
        cerr << "Charge_Assigner::_all_queries_have_numeric_value: no numeric value "
                "found, query "
             << i << ", component " << j << '\n';
        cerr << cq->comment() << '\n';
        return 0;
...
Charge_Assigner::_add_charge_to_atom(Molecule_to_Match& target,
                                     ...
                                     atom_number_t zatom, formal_charge_t fc) const {
...
atom_with_numeric_value(const Query_Atoms_Matched& qami, const Set_of_Atoms* e) {
...
#ifdef DEBUG_REMOVE_LOWER_PREFERENCE_MATCHES
      cerr << "Preference values " << qami->preference_value() << " and "
           << qamj->preference_value() << '\n';
#endif
...
#ifdef DEBUG_REMOVE_LOWER_PREFERENCE_MATCHES
  cerr << "After removal of lower preference values " << sresults.number_embeddings()
       << " embeddings\n";
#endif
    ...
    for (int j = 0; j < nhits; j++) {
      ...
      if (_which_atom && _which_atom[i] >= 0) {
        ...
        if (_add_charge_to_atom(target, positive_charges_assigned,
                                negative_charges_assigned, charges_assigned, l,
                                _charge_to_assign[l])) {
          rc++;
        }
        continue;
      ...
      for (int k = 0; k < ns; k++) {
        ...
        if (_add_charge_to_atom(target, positive_charges_assigned,
                                negative_charges_assigned, charges_assigned, l, fc)) {
          rc++;
        }
      }
...
void
Charge_Assigner::_remove_positive_charge_hits_on_chiral_atoms(
    Molecule& m, Set_of_Atoms& positive_charges_assigned,
...
static int
advance_shell(const Molecule& m, int* tmp, int flag) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    //  cerr << "Growing frontier from atom " << i << '\n';
...
identify_survivor(Molecule& m, atom_number_t a1, atom_number_t a2, int* tmp) {
  ...
  while (1) {
    //  cerr << "Advancing shell for atom " << a1 << '\n';
    int c1 = advance_shell(m, tmp, 1);
    //  cerr << "Advancing shell for atom " << a2 << '\n';
    int c2 = advance_shell(m, tmp, 8);
    //  cerr << "Shells expanded " << c1 << " and " << c2 << '\n';
...
int
Charge_Assigner::_identify_charged_atoms_too_close(Molecule& m, const Set_of_Atoms& s,
...
#ifdef DEBUG_IDENTIFY_CHARGED_ATOMS_TOO_CLOSE
      cerr << m.bonds_between(ai, aj) << " bonds between " << ai << " and " << aj << '\n';
#endif
...
#ifdef DEBUG_IDENTIFY_CHARGED_ATOMS_TOO_CLOSE
      cerr << "Atoms " << ai << " '" << m.smarts_equivalent_for_atom(ai) << "' and " << aj
           << " '" << m.smarts_equivalent_for_atom(aj) << "' too close "
           << m.bonds_between(ai, aj) << '\n';
#endif
...
Charge_Assigner::_enumerate_possibilities0(
    Molecule& m, const Set_of_Atoms& s,
...
int
Charge_Assigner::_enumerate_possibilities1(
    Molecule& m, const Set_of_Atoms& s, int istart,
    resizable_array_p<Set_of_Atoms>& possibilities) const {
...
int
Charge_Assigner::_process(Molecule& m, formal_charge_t* charges_assigned) {
  Set_of_Atoms positive_charges_assigned;
  Set_of_Atoms negative_charges_assigned;
  ...
  int rc = _process(target, positive_charges_assigned, negative_charges_assigned,
                    charges_assigned);

  if (m.chiral_centres()) {  // one or more chiral centres present
    _remove_positive_charge_hits_on_chiral_atoms(m, positive_charges_assigned,
                                                 charges_assigned);
  }

  // cerr << "_min_distance_between_charges " << _min_distance_between_charges << ", npos
  // " << positive_charges_assigned.number_elements() << '\n';

  if (_min_distance_between_charges > 0) {
    _remove_hits_too_close_isolation_score(m, positive_charges_assigned,
                                           charges_assigned);
    //  _remove_hits_too_close(m, negative_charges_assigned, charges_assigned);  this
    //  messes up putting 2 charges on a phosphoric acid
  }
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (_remove_chiral_centres_from_changed_atoms &&
        (_positive_element || _negative_element) &&
        nullptr != m.chiral_centre_at_atom(i)) {
      m.remove_chiral_centre_at_atom(i);
    }

    if (_verbose > 1) {
      cerr << "Charge_Assigner::_process: atom " << i << " (" << m.atomic_symbol(i)
           << ") assigned " << charges_assigned[i] << '\n';
    }
  }
...
/*
  October 2013. Nasty problems with a 3D molecule that had an explicit hydrogen early in
  the connection table. We use the temp detach hydrogens, process the molecule, which adds
  a hydrogen atom. But when temp detach does the re-attach, if notices that the atom is no
  longer needed and so drops it. That totally messes up the ordering in the
  charges_assigned array. To be safe, we always move explicit hydrogens to the end of the
  connection table
*/

int
Charge_Assigner::process(Molecule& m, formal_charge_t* charges_assigned) {
...
static void
transfer_charges(Molecule& m, const formal_charge_t* charges_assigned) {
  int matoms = m.natoms();
...
int
Charge_Assigner::_process(Molecule& m, resizable_array_p<Molecule>& charged_forms) {
  int matoms = m.natoms();
  ...
  if (0 == _process(target, positive_charges_assigned, negative_charges_assigned,
                    charges_assigned)) {
    return 0;
  }

#ifdef DEBUG_CHARGE_ASSIGNER_PROCESS
  cerr << "To '" << m.name() << " assign " << positive_charges_assigned.number_elements()
...
Charge_Assigner::_do_make_implicit_hydrogens_explicit(Molecule& m,
  ...
  for (int i = 0; i < matoms; i++) {
    //  if (0 != charges_assigned[i])
    //    cerr << "Atom " << m.smarts_equivalent_for_atom(i) << " has charge " <<
    //    charges_assigned[i] << ", ih " << m.implicit_hydrogens(i) << '\n';
