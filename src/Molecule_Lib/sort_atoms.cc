...
//  the property on which we sort
  ...
  public:
    ...
    atom_number_t initial_atom_number() const { return _initial_atom_number;}
    T property() const { return _property;}
};
...
template<typename T>
void
Atom_and_Info<T>::initialise(const Atom * a,
                             atom_number_t b,
                             T prop)
{
  _atom = a;
...
{
  const Atom_and_Info<int> * ai1 = (const Atom_and_Info<int> *) aip1;
  const Atom_and_Info<int> * ai2 = (const Atom_and_Info<int> *) aip2;
...
{
  return - atom_and_info_comparitor_int_larger(aip1, aip2);
}

int
Molecule::sort(const int * criterion, int direction)
{
  assert (ok());
  assert (0 != direction);

  Atom_and_Info<int> * ai = new Atom_and_Info<int>[_number_elements]; std::unique_ptr<Atom_and_Info<int>[]> free_ai(ai);

  for (int i = 0; i < _number_elements; i++)
  {
    Atom_and_Info<int> & aii = ai[i];
    aii.initialise(_things[i], i, criterion[i]);
  }

  if (direction > 0)
    ::qsort(ai, _number_elements, sizeof(Atom_and_Info<int>), atom_and_info_comparitor_int_larger);
  else if (direction < 0)
    ::qsort(ai, _number_elements, sizeof(Atom_and_Info<int>), atom_and_info_comparitor_int_smaller);

// We need to now tell each atom what its new atom number is

  int * new_number = new int[_number_elements]; std::unique_ptr<int[]> free_new_number(new_number);

  for (int i = 0; i < _number_elements; i++)
  {
    const Atom_and_Info<int> & aii = ai[i];
...
Molecule::renumber_atoms(const int * new_number)
{
  ...
  for (int i = 0; i < _number_elements; i++)
  {
    ...
    {
      cerr << "Molecule::renumber_atoms: i = " << i << " new number " << new_number[i] << " is invalid. Natoms = " << _number_elements << endl;
      abort();
      return 0;
    }
  }

  int * complete = new_int(_number_elements); std::unique_ptr<int[]> free_complete(complete);
...
int
Molecule::_renumber_atoms(const int * new_number,
                          int * complete)
{
  for (int i = 0; i < _number_elements; i++)
  {
...
//    cerr << "Atom " << a2 << " now at position " << i << " its number " << new_number[a2] << endl;
    }
  }
