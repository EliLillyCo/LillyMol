...
void
set_coordinate_scaling(float scale) {
...
remove_suffix(IWString& fname) {
  ...
  if (FILE_TYPE_INVALID ==
      moleculeio::string_to_file_type(suffix)) {  // not a known suffix
    return;
  }
...
int
create_file_with_appropriate_name(const const_IWSubstring& old_name, IWString& new_name,
                                  FileType file_type, int keep_existing_suffix) {
  const char* new_suffix = suffix_for_file_type(file_type);
  if (nullptr == new_suffix) {
    cerr << "create_file_with_appropriate_name: unrecognised type " << file_type << '\n';
...
append_appropriate_suffix(IWString& fname, FileType file_type) {
  const char* new_suffix = suffix_for_file_type(file_type);
  if (nullptr == new_suffix) {
    cerr << "append_appropriate_suffix: unrecognised type " << file_type << '\n';
...
discern_file_type_from_name(const IWString& file_name) {
  ...
  if (file_name.ends_with(".textproto") || file_name.ends_with(".txtproto") ||
...
static int
process_option(const Command_Line& cl, FileType& type, const char c,
...
/*
  ...
  The old behaviour can be obtained by setting
  _unconnect_covalently_bonded_non_organics_on_read=2
*/
...
Molecule::write_molecule(std::ostream& os, FileType output_type,
  ...
  if (FILE_TYPE_MDL == output_type || FILE_TYPE_SDF == output_type) {
    ...
    rc =
        write_molecule_mmod(os);  // seems that no function defined with a comments field?
...
process_input_type(const Command_Line& cl, FileType& input_type) {
  ...
  // cerr << "Inside process_input_type, present " << cl.option_present('i') << '\n';
  ...
  while (cl.value('i', optval, i++)) {
    ...
    //  else if (optval.starts_with("mdlquiet="))     seems like a good idea, implement it
    //  sometime
    //  {
    ...
    //  }
      ...
      moleculeio::set_number_connection_table_errors_to_skip(
      ...
      if (!optval.numeric_value(n) || n < 0) {
        cerr << "The skip first molecules directive 'skip=nn' must be followed by a "
      ...
      if (!optval.numeric_value(n) || n < 0) {
        cerr << "The do only molecules directive 'do=nn' must be followed by a whole "
                "positive number\n";
        return 0;
      }
