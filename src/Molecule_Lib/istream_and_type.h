...
class data_source_and_type : public iwstring_data_source {
 private:
  ...
  uint64_t _connection_table_errors_encountered;
  uint64_t _connection_table_errors_allowed;
  int _dos;
  int _verbose;
 ...
 public:
  ...
  connection_table_errors_encountered() const {
    return _connection_table_errors_encountered;
  }

  int
  stopped_because_of_error() const {
    return _connection_table_errors_encountered > _connection_table_errors_allowed;
  }
...
data_source_and_type<T>::_file_opened_ok(const IWString& fname) {
  if (!good()) {
    std::cerr << "data_source_and_type::_file_opened_ok: cannot open '" << fname
              << "' for input\n";
    return 0;
  }
  ...
  if (moleculeio::seek_to_from_command_line() > 0) {
    if (!iwstring_data_source::seekg(moleculeio::seek_to_from_command_line())) {
      std::cerr << "data_source_and_type::_file_opened_ok: cannot seek to "
                << moleculeio::seek_to_from_command_line() << '\n';
    }
  }
...
template <typename T>
data_source_and_type<T>::data_source_and_type(FileType input_type, std::string& fname)
    : iwstring_data_source(fname.c_str()) {
  if (!_default_values(input_type)) {
    return;
  }
...
template <typename T>
data_source_and_type<T>::data_source_and_type(FileType input_type, const IWString& fname)
    : iwstring_data_source(fname) {
  if (!_default_values(input_type)) {
    return;
  }
...
data_source_and_type<T>::data_source_and_type(const IWString& fname)
  ...
  if (FILE_TYPE_INVALID == (tmp = discern_file_type_from_name(fname))) {
    std::cerr << "Data_source_and_type::constructor: cannot determine file type '"
              << fname << "'\n";
    return;
  }
...
data_source_and_type<T>::~data_source_and_type() {
  ...
  if (_connection_table_errors_encountered) {
    std::cerr << "data_source_and_type:: " << _connection_table_errors_encountered
              << " connection table errors encountered\n";
  }
...
template <typename T>
int
data_source_and_type<T>::do_open(const char* fname, FileType input_type) {
  if (valid_file_type(input_type)) {
    _input_type = input_type;
  } else if (FILE_TYPE_INVALID == input_type) {
    input_type = discern_file_type_from_name(fname);
    if (input_type == FILE_TYPE_INVALID) {
      std::cerr << "data_source_and_type::do_open:cannot discern input type from '"
                << fname << "'\n";
      return 0;
    }
    _input_type = input_type;
...
data_source_and_type<T>::next_molecule() {
  ...
  if (_do_only > 0 && _molecules_read >= _do_only) {
    std::cerr << "data_source_and_type::next_molecule: already read " << _molecules_read
              << " molecules\n";
    _valid = 0;
    return nullptr;
  }
  ...
  while (_connection_table_errors_encountered <= _connection_table_errors_allowed) {
    if (!is_pipe())  // tellg does not work on stdin
    {
      ...
      if (_offset_for_most_recent_molecule >=
          moleculeio::max_offset_from_command_line()) {
        return nullptr;
      }
    }
    ...
    std::cerr << "data_source_and_type::next_molecule: Skipping connection table error "
              << _connection_table_errors_encountered << " record " << lines_read()
              << '\n';
    ...
    if (_stream_for_connection_table_errors.is_open()) {
      ...
      //    std::cerr << "From " << here << " go back to " <<
      //    _offset_for_most_recent_molecule << " echo " << bytes_to_echo << " bytes\n";

      if (!seekg(_offset_for_most_recent_molecule)) {
        std::cerr << "data_source_and_type::next_molecule:sorry, cannot seek back to "
                  << _offset_for_most_recent_molecule << '\n';
      } else {
        echo(_stream_for_connection_table_errors, static_cast<off_t>(bytes_to_echo));
        seekg(here);

        std::cerr << "from " << _offset_for_most_recent_molecule << " to " << here
                  << " now " << tellg() << '\n';
      }
    }

    if (_connection_table_errors_encountered > _connection_table_errors_allowed) {
      std::cerr << "data_source_and_type::next_molecule:too many connection table errors "
                << _connection_table_errors_allowed << '\n';
      return nullptr;
    }
  }
...
data_source_and_type<T>::set_connection_table_error_file(const IWString& fname) {
  ...
  if (!_stream_for_connection_table_errors.open(tmp.null_terminated_chars())) {
    std::cerr << "data_source_and_type:set_connection_table_error_file: cannot open '"
              << fname << "'\n";
    return 0;
  }
...
data_source_and_type<T>::_skip_csv_header_record() {
  ...
  if (!lillymol_csv::ok_header_record(buffer)) {
    std::cerr << "data_source_and_type::_skip_csv_header_record:invalid csv header '"
              << buffer << "'\n";
    return 0;
  }
...
data_source_and_type<T>::_do_merck_skip_records() {
  for (uint64_t i = 0; i < _skip_first; i++) {
    if (!_skip_merck_connection_table()) {
      std::cerr
          << "data_source_and_type::_do_merck_skip_records: fatal error trying to skip "
          << _skip_first << " records, got " << i << '\n';
      return 0;
    }
  }
...
data_source_and_type<T>::_skip_merck_connection_table() {
  ...
  if (!iwstring_data_source::next_record(buffer)) {
    std::cerr << "data_source_and_type::_skip_merck_connection_table: cannot read 2nd "
                 "header record\n";
    return 0;
  }

  if (!buffer.starts_with("MOL ")) {
    std::cerr << "data_source_and_type::_skip_merck_connection_table: 2nd record looks "
                 "invalid, line "
              << lines_read() << '\n';
    std::cerr << buffer << '\n';
    return 0;
  ...
  if (!iwstring_data_source::next_record(buffer)) {
    std::cerr << "data_source_and_type::_skip_merck_connection_table: cannot read "
                 "atom/bond count record\n";
    return 0;
  }

  if (buffer.nwords() < 2) {
    std::cerr << "data_source_and_type::_skip_merck_connection_table: invalid na,nb "
                 "record, line "
              << lines_read() << '\n';
    std::cerr << buffer << '\n';
    return 0;
  ...
  if (!token.numeric_value(na) || na < 1) {
    std::cerr << "data_source_and_type::_skip_merck_connection_table: invalid number of "
                 "atoms\n";
    std::cerr << buffer << '\n';
    return 0;
  }
  ...
  // std::cerr << "Skipping " << na << " atoms and " << nb << " bonds or " << (na +
  // bond_records) << " records\n";

  for (int i = 0; i < (na + bond_records); i++) {
    if (!data_source_and_type::next_record(buffer)) {
      std::cerr << "data_source_and_type::_skip_merck_connection_table: eof im middle of "
                   "molecule\n";
      return 0;
    }
  }
...
template <typename T>
size_t
data_source_and_type<T>::_average_size(off_t offset, RE2& rx, int n) {
...
data_source_and_type<T>::estimate_molecules_in_file() {
  ...
  std::cerr << "Sizes: start " << start_of_file << " middle " << middle_of_file << " and "
            << end_of_file << '\n';
