...
#define NAME_OF_ENVIRONMENTS_CAN_SHARE_ATTACHMENT_POINTS \
  "env_matches_share_attachment_points"
...
#define NAME_OF_DO_NOT_PERCEIVE_SYMMETRIC_EQUIVALENTS \
  "do_not_perceive_symmetric_equivalents"
...
#define NAME_OF_RINGS_THAT_MUST_MATCH_RING_SIZE_ATTRIBUTE \
  "rings_that_must_match_ring_size"
...
#define NAME_OF_ENVIRONMENT_MUST_MATCH_UNMATCHED_ATOMS_ATTRIBUTE \
  "environment_must_match_unmatched_atoms"
...
is_rejection(const msi_object &msi) {
  ...
  int rc;
  if (!rj->value(rc)) {
    cerr << "The " << NAME_OF_REJECTION_ATTRIBUTE
         << " attribute must have an int value\n";
    cerr << msi << endl;
    iwabort();
    return 0;
  }
...
static int
get_float_attribute(const msi_object &msi, const char *att_name, float &v, float minval,
                    float maxval) {
...
template <typename M>
int
append_int_values(const msi_object &msi, const IWString &attribute_name, int min_val,
                  int max_val, M &specifier, int &attributes_specified) {
  assert(specifier.ok());
...
set_int_value(const msi_attribute &msi, int &value, int min_val, int max_val) {
  ...
  if (MIN_NOT_SPECIFIED != min_val && tmp < min_val) {
    cerr << "msi attribute value out of range, got " << tmp << " min is " << min_val
         << endl;
    cerr << msi << endl;
    return 0;
  }
  ...
  if (MAX_NOT_SPECIFIED != max_val && tmp > max_val) {
    cerr << "msi attribute value out of range, got " << tmp << " max is " << max_val
         << endl;
    cerr << msi << endl;
    return 0;
  }
...
/*
  ...
  ATTRIBUTES_SPECIFIED by one, since it is really
  Substructure_Atom_Specifier::_attributes_specified M will be either a Min_Max_Specifier
  or Matcher class.
*/

template <typename T, typename M>
int
_really_gruesome(M &specifier, const msi_object &msi, const IWString &attribute_name,
                 int &attributes_specified, T min_value_allowed, T max_value_allowed) {
...
template <typename T, typename M>
int
really_gruesome(M &specifier, const msi_object &msi, const IWString &attribute_name,
                int &attributes_specified, T min_value_allowed, T max_value_allowed) {
  assert(specifier.ok());
...
template int really_gruesome(Min_Max_Specifier<int> &specifier, const msi_object &msi,
                             const IWString &attribute_name, int &attributes_specified,
                             int min_value_allowed, int max_value_allowed);
                 ...
                 int ok_not_present = 1)
{
  ...
  {
    cerr << "The " << attribute_name << " attribute must be between " << minval << " and "
<< maxval << endl; cerr << msi; return 0;
  }
...
Substructure_Atom_Specifier::construct_from_msi_object(const msi_object &msi) {
  ...
  if (att) {
    if (nullptr != msi.attribute(NAME_OF_FORMAL_CHARGE_ATTRIBUTE)) {
      cerr << "Substructure_Atom_Specifier::construct_from_msi_object: warning, the "
           << NAME_OF_CHARGED_ATTRIBUTE << " and " << NAME_OF_FORMAL_CHARGE_ATTRIBUTE
           << " attributes may clash\n";
    }
  ...
  if (!really_gruesome(_ncon, msi, NAME_OF_NCON_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_nbonds, msi, NAME_OF_NBONDS_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_nrings, msi, NAME_OF_NRINGS_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_ring_bond_count, msi, NAME_OF_RING_BOND_COUNT_ATTRIBUTE,
                       _attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_ncon2, msi, "ncon2", _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_formal_charge, msi, NAME_OF_FORMAL_CHARGE_ATTRIBUTE,
                       _attributes_specified, -4, 4)) {
    return 0;
  }

  if (!really_gruesome(_hcount, msi, NAME_OF_HCOUNT_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_ring_size, msi, NAME_OF_RING_SIZE_ATTRIBUTE,
                       _attributes_specified, 3, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_attached_heteroatom_count, msi,
                       NAME_OF_ATTACHED_HETEROATOM_COUNT_SPECIFIER, _attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }
  ...
  if (!really_gruesome(_isotope, msi, NAME_OF_ISOTOPE_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_aryl, msi, NAME_OF_ARYL_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_vinyl, msi, NAME_OF_VINYL_ATTRIBUTE, _attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }
  ...
  if (att) {
    if (!att->value(_chirality)) {
      cerr << "Substructure_Atom_Specifier::construct_from_msi_object: bad chirality "
              "specifier\n";
      cerr << (*att) << endl;
      return 0;
    }
  ...
  if (att) {
    if (!att->value(_userAtomType)) {
      cerr << "Substructure_Atom_Specifier::construct_from_msi_object: bad User Atom "
              "type\n";
      cerr << (*att) << endl;
      return 0;
    }
  ...
  if (att) {
    ...
    if (!att->value(iso) || iso < 0) {
      cerr << "Substructure_Atom_Specifier::construct_from_msi_object: bad isolated ring "
              "specifier\n";
      cerr << (*att) << endl;
      return 0;
    }
  ...
  if (!really_gruesome(_heteroatoms_in_ring, msi, NAME_OF_HETEROATOMS_IN_RING_ATTRIBUTE,
                       _attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_aromatic_ring_size, msi, NAME_OF_AROMATIC_RING_SIZE_ATTRIBUTE,
                       _attributes_specified, 3, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_aliphatic_ring_size, msi, NAME_OF_ALIPHATIC_RING_SIZE_ATTRIBUTE,
                       _attributes_specified, 3, MAX_NOT_SPECIFIED)) {
    return 0;
  }
...
Substructure_Atom_Specifier::_fetch_symmetry_group(const msi_object &msi) {
  ...
  if (1 != att->number_int_values()) {
    cerr << "Substructure_Atom_Specifier::_fetch_symmetry_group:multiple values not "
            "supported, contact LillyMol on github "
            "(https://github.com/EliLillyCo/LillyMol)\n";
    return 0;
  }

  if (!att->value(_symmetry_group) || _symmetry_group <= 0) {
    cerr << "Substructure_Atom_Specifier::_fetch_symmetry_group:invalid symmetry group "
            "specification\n";
    return 0;
  }

  // cerr << "Substructure_Atom_Specifier::_fetch_symmetry_group:set " << _symmetry_group
  // << endl;
...
// Fetch the values for _nrings.
  ...
  if (! really_gruesome (_nrings, msi, NAME_OF_NRINGS_ATTRIBUTE, attributes_specified, 0,
MAX_NOT_SPECIFIED)) return 0;
  ...
  if (att)
  {
    ...
    {
      cerr << "The " << NAME_OF_AROMATICITY_ATTRIBUTE << " specifier must be a non
negative whole number\n"; return 0;
  ...
  if (att)
  {
    ...
    if (! att->value (ar) || ar < 0)
    {
      cerr << "The " << NAME_OF_OR_AROMATICITY_SPECIFIER << " specifier must be a non
negative whole number\n"; return 0;
    }
...
int
Substructure_Bond::write_as_msi_attribute(std::ostream &os, int indentation) const {
  assert(ok());
...
static int
write_ord_bond_types(bond_type_t bt, std::ostream &os) {
  int written = 0;
...
Substructure_Bond::__write_as_smarts(std::ostream &os) const {
  ...
  // cerr << "Substructure_Bond::__write_as_smarts: '" << _b->smarts (cerr) << "'
  // operators = " << _logexp.number_operators() << endl;
...
Substructure_Atom_Specifier::write_msi(std::ostream &os, int indentation,
  ...
  if (IW_LOGEXP_UNDEFINED == op) {  // don't write the operator
    ...
    os << ind << "(A C " << NAME_OF_OPERATOR_ATTRIBUTE << " \""
       << NAME_OF_LOW_PRIORITY_AND_OPERATOR << "\")\n";
  } else {
    abort();
  }
  ...
  _attached_heteroatom_count.write_msi(os, NAME_OF_ATTACHED_HETEROATOM_COUNT_SPECIFIER,
                                       indentation);
...
int
Substructure_Atom::_write_children_msi(std::ostream &os, int &object_id,
                                       int indentation) {
...
Substructure_Atom::write_msi(std::ostream &os, int &object_id,
  ...
  if (_initial_atom_number >= 0) {
    os << ind << "  (A I " << NAME_OF_INITIAL_ATOM_NUMBER_ATTRIBUTE << " "
       << _initial_atom_number << ")\n";
  }
  ...
  if (_text_identifier.length()) {
    os << ind << "  (A C " << NAME_OF_TEXT_IDENTIFIER_ATTRIBUTE << " '"
       << _text_identifier << "')\n";
  }
  ...
  for (int i = 0; i < np; i++) {
    os << ind << "  (" << (object_id++) << ' ' << NAME_OF_QUERY_ATOM_PREFERENCE_OBJECT
       << endl;
...
int
Substructure_Environment::write_msi(std::ostream &os, int &object_id, int indentation) {
  assert(ok());
  assert(os.good());
  ...
  if (_no_other_substituents_allowed) {
    os << ind << "  (A I " << NAME_OF_NO_OTHER_SUBSTITUENTS_ALLOWED_ATTRIBUTE << ' '
       << _no_other_substituents_allowed << ")\n";
  }

  if (_max_environment_matches_per_attachment_point < std::numeric_limits<int>::max()) {
    os << ind << "  (A I " << NAME_OF_MAX_ENVIRONMENT_MATCHES_PER_ATTACHMENT_POINT << ' '
       << _max_environment_matches_per_attachment_point << ")\n";
  }
  ...
  os << ind << "  (A I " << NAME_OF_ENVIRONMENTS_CAN_SHARE_ATTACHMENT_POINTS << ' '
     << _environments_can_share_attachment_points << ")\n";

  os << ind << "  (A I " << NAME_OF_HYDROGEN_OK_AS_ENVIRONMENT_MATCH << ' '
     << _hydrogen_ok_as_environment_match << ")\n";
...
int
Substructure_Atom_Environment::write_msi(std::ostream &os, int &object_id,
                                         int indentation) const {
  IWString ind;
  ind.extend(indentation, ' ');
...
static int
extract_operator(const msi_object &msi, IW_Logical_Expression &logexp,
                 int default_operator, const char *caller) {
  IWString op;
...
Substructure_Atom::_add_component(const msi_object &msi) {
  ...
  if (msi.number_elements()) {
    cerr << "Substructure_Atom::_add_component: components for specifiers cannot have "
            "sub-objects\n";
    cerr << msi;
    return 0;
  }
  ...
  if (_components.empty()) {  // no operator with first object
    ;
  } else if (!extract_operator(msi, _operator, IW_LOGEXP_OR,
                               "Substructure_Atom::_add_component")) {
    return 0;
  }
...
Substructure_Atom::_add_bond(Substructure_Bond *b) {
  ...
  cerr << "Substructure_Atom::_add_bond:to atom " << b->a()->unique_id() << " now have "
       << _bonds.number_elements() << " bonds\n";
#endif
...
static int
parse_smarts_bond_attribute(const msi_attribute *att, Set_of_Atoms &other_end,
  ...
  if (nw < 2) {
    cerr << "parse_smarts_bond_attribute: bond smarts must have at least two tokens '"
         << Satt << "'\n";
    return 0;
  }
...
int
Substructure_Atom::_process_attribute_smarts_bond(
    const msi_attribute *att, extending_resizable_array<Substructure_Atom *> &completed) {
  Set_of_Atoms other_end;
  IWString bond_smarts;

  if (!parse_smarts_bond_attribute(att, other_end, bond_smarts)) {
    cerr << "Substructure_Atom::_process_attribute_smarts_bond:invalid bond smarts "
         << *att << endl;
    return 0;
  }

  if (other_end.number_elements() > 1) {
    cerr << "Substructure_Atom::_process_attribute_smarts_bond:cannot handle "
            "multi-valued attachment points\n";
    return 0;
  }
  ...
  if (nullptr == completed[o]) {
    cerr << "Substructure_Atom::_process_attribute_smarts_bond: atom " << o
  ...
  if (!b->construct_from_smarts(bond_smarts.rawchars(), bond_smarts.length(),
...
int
Substructure_Atom::_process_attribute_bond(
    const msi_attribute *att, bond_type_t bond_type,
    extending_resizable_array<Substructure_Atom *> &completed) {
  ...
  if (nullptr == completed[other_end]) {
    cerr << "Substructure_Atom::_process_attribute_bond: atom " << other_end
         << " has not been defined\n";
    return 0;
  }
  ...
  if (att->number_int_values() > 1) {
    ...
    for (int i = 1; i < att->number_int_values(); i++) {
      ...
      if (1 == j) {
        ...
        cerr << "Substructure_Atom::_add_attribute_bond: what kind of bond is " << j
             << endl;
        return 0;
      }
    }
...
int
Substructure_Environment::_process_attribute_bond(
    const msi_attribute *att, bond_type_t bond_type,
    extending_resizable_array<Substructure_Atom *> &completed) {
  ...
  if (1 == nb)  // NOT multi-valued
  {
    int a;
    if (!att->value(a) || a < 0) {
      cerr << "Substructure_Environment::_process_attribute_bond: invalid atom '"
           << (*att) << endl;
      return 0;
    }

    if (nullptr == completed[a]) {
      cerr << "Substructure_Environment::_process_attribute_bond: atom " << a
           << " not defined\n";
      return 0;
    }
...
static int
attribute_is_bond(const msi_attribute *att, bond_type_t &result) {
...
int
Substructure_Atom::_process_attribute_bonds(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed) {
...
int
Substructure_Environment::_process_attribute_bonds(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed) {
  int rc = 0;
  ...
  int i = 0;
  while (nullptr != (att = msi.attribute(i++))) {
    //  cerr << "Substructure_Environment::_process_attribute_bonds: examining " << *att
    //  << endl;
  ...
  while (nullptr != (att = msi.attribute(i++))) {
    ...
    if (!parse_smarts_bond_attribute(att, other_end, bond_smarts)) {
      cerr << "Substructure_Environment::_process_attribute_bond:invalid bond smarts "
           << *att << endl;
      return 0;
    }

    int characters_processed;
    if (!_bond.construct_from_smarts(bond_smarts.rawchars(), bond_smarts.length(),
                                     characters_processed)) {
      cerr << "Substructure_Environment::_process_attribute_bond:invalid bond smarts "
           << bond_smarts << endl;
      return 0;
    ...
    for (int j = 0; j < other_end.number_elements(); ++j) {
      ...
      if (nullptr == completed[a]) {
        cerr << "Substructure_Environment::_process_attribute_bonds: atom " << a
             << " has not been defined\n";
        return 0;
      }
...
int
Substructure_Atom::_create_environment_from_msi_object(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed) {
  ...
  for (int i = 0; i < nmsi; i++) {
    ...
    if (!a->construct_from_msi_object(*m, completed)) {
      cerr << "_create_environment_from_msi_object: cannot create environment component "
           << i << endl;
      delete a;
      return 0;
    }
...
Substructure_Atom_Environment::create_from_msi_object(msi_object &msi) {
  ...
  if (msi.string_value_for_attribute(NAME_OF_SMARTS_ATTRIBUTE, smarts)) {
    if (msi.number_elements()) {
      cerr << "Substructure_Atom_Environment::create_from_msi_object: smarts and "
              "multiple objects specified, impossible\n";
      return 0;
    }
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (NAME_OF_QUERY_ATOM_OBJECT != oi->name()) {
      cerr << "Substructure_Atom_Environment::create_from_msi_object: unrecognised "
              "object\n";
      cerr << (*oi);
      return 0;
    }
...
int
Substructure_Atom::_construct_from_msi_object(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed) {
  if (NAME_OF_QUERY_ATOM_OBJECT != msi.name()) {
    cerr << "Substructure_Atom::_construct_from_msi_object: name must be '"
         << NAME_OF_QUERY_ATOM_OBJECT << "', '" << msi.name() << "' is invalid\n";
    return 0;
  }
  ...
  if (nullptr != completed[_unique_id]) {
    cerr << "Substructure_Atom::_construct_from_msi_object:Yipes, atom " << _unique_id
         << " already allocated\n";
    cerr << msi;
    return 0;
    iwabort();
  ...
  if (nullptr != att) {
    if (!att->value(_initial_atom_number) || _initial_atom_number < 0) {
      cerr << "Substructure_Atom::_construct_from_msi_object: initial atom number must "
  ...
  if (att) {
    if (!att->value(_or_id) || 0 == _or_id) {
      cerr << "Substructure_Atom::_construct_from_msi_object: or id must be a positive "
              "number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_ring_id) || 0 == _ring_id) {
      cerr << "Substructure_Atom::_construct_from_msi_object: ring id must be a positive "
              "number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_fragment_id) || 0 == _fragment_id) {
      cerr << "Substructure_Atom::_construct_from_msi_object: fragment id must be a "
              "positive number\n";
      return 0;
    }
  }
  ...
  if (att) {
    ...
    if (!att->value(tmp)) {
      cerr << "Substructure_Atom::_construct_from_msi_object: the "
           << NAME_OF_NUMERIC_VALUE_ATTRIBUTE << " attribute must be numeric\n";
      return 0;
    }
  ...
  if (att) {
    ...
    if (!att->value(tmp)) {
      cerr << "Substructure_Atom::_construct_from_msi_object: the "
           << NAME_OF_INCLUDE_IN_EMBEDDING_ATTRIBUTE
  ...
  if ((nullptr != msi.attribute(NAME_OF_ATOM_SMARTS_ATTRIBUTE)) +
          ...
          (nullptr != msi.attribute(NAME_OF_SMILES_ATTRIBUTE)) >
      1) {
    cerr << "The '" << NAME_OF_ATOM_SMARTS_ATTRIBUTE << "', '" << NAME_OF_SMILES_ATTRIBUTE
         << "' and '" << NAME_OF_SMILES_ATTRIBUTE
  ...
  if (att) {
    ...
    if (!(csmarts.starts_with('[') && csmarts.ends_with(']'))) {
      cerr << "Substructure_Atom::_construct_from_msi_object: atomic smarts must be "
              "within [] '"
           << csmarts << "'\n";
      return 0;
    }
    ...
    if (!construct_from_smarts_token(csmarts)) {
      cerr << "Substructure_Atom::_construct_from_msi_object: cannot interpret smarts '"
           << csmarts << "'\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_sum_all_preference_hits)) {
      cerr << "Substructure_Atom::_construct_from_msi_object: the "
           << NAME_OF_SUM_ALL_PREFERENCE_HITS_ATTRIBUTE
  ...
  for (int i = 0; i < nmsi; i++) {
    ...
    } else if (NAME_OF_QUERY_BOND_OBJECT ==
               m.name()) {  // will be processed by calling function
      ;
    } else {
...
int
Substructure_Atom::construct_from_msi_object(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed) {
  if (is_rejection(msi)) {
...
is_fixed_kekule_form(const Molecule &m, const Ring &r) {
  ...
  for (Ring_Bond_Iterator j(r); j != r.zend(); j++) {
    ...
    //  cerr << "From " << a1 << " to " << a2 << " bt " << bt << " prev " <<
    //  previous_bond_was << endl;
...
static int
determine_part_of_non_kekule_aromatic(Molecule &m, const Bond &b) {
  m.compute_aromaticity_if_needed();

  int nr = m.nrings();

  // cerr << "Checking '" << m.name() << "' for fixed kekule aromatic, nr = " << nr <<
  // endl;
  for (int i = 0; i < nr; i++) {
    ...
    if (is_fixed_kekule_form(m, *ri)) {
      cerr << "In '" << m.name() << " bond btw " << b.a1() << " and " << b.a2()
           << " in fixed kekule ring of size " << ring_size << endl;
      return 1;
    }
  }
...
static int
initialise_bond_attributes(Substructure_Bond &sb, const MDL_Bond_Data *mdlbd,
                           const Bond *b, atom_number_t j,
                           extending_resizable_array<Substructure_Atom *> &completed,
...
// #define DEBUG_INITIALISE_BOND_ATTRIBUTES
  ...
  cerr << "atoms " << b->a1() << " to " << b->a2() << " mdlbd->btype() " << mdlbd->btype()
       << " aromatic? " << b->is_aromatic() << endl;
#endif
  ...
  if (0 == mdlbd->bond_topology()) {  // nothing known
    ...
    cerr << "Unrecognised bond topology for atom " << b->other(j) << " "
         << mdlbd->bond_topology() << endl;
  }
...
int
Substructure_Atom::_build_atomic_number_matches_from_atom_list(
    const ISIS_Atom_List &ali) {
  if (ali.normal_list()) {
...
static int
count_heteroatoms_in_aromatic_ring(Molecule &m, atom_number_t zatom) {
  int nr = m.nrings();
...
int
Substructure_Atom::_set_match_any_aromatic(
    int convert_all_aromatic_atoms_to_generic_aromatic, Molecule &m,
...
/*
  ...
  NOte that if we are using mqs.isotope_means_match_any_atom(), isotopic
  labels matching that will be removed from `m`.
*/

int
Substructure_Atom::create_from_molecule(
    Molecule &m, const MDL_File_Data &mdlfd, atom_number_t my_atom_number,
    Substructure_Atom *my_parent, const Molecule_to_Query_Specifications &mqs,
    extending_resizable_array<Substructure_Atom *> &completed,
    const int *include_these_atoms) {
  // cerr << "Substructure_Atom::creating from molecule with " << m.natoms() << " atoms,
  // atom " << my_atom_number << endl;
  ...
  if (mqs.interpret_atom_alias_as_smarts() && mdlad->alias().length()) {
    ...
    if (s.numeric_value(notused) && notused >= 0) {
      ...
      if (!smarts_specified.starts_with('[') && smarts_specified.ends_with(']')) {
        cerr << "Substructure_Atom::create_from_molecule:invalid atomic smarts as alias '"
             << smarts_specified << "'\n";
        return 0;
      }
    }
  } else if (!mqs.interpret_atom_alias_as_smarts() &&
             mdlad->alias().length()) {  // probably from Marvin, AH, AQ...
    ...
    if (1 == e->atomic_number() &&
        (mqs.convert_explicit_hydrogens_to_match_any_atom() ||
         mqs.convert_explicit_hydrogens_to_match_any_atom_including_hydrogen())) {
      ;
    } else {
      set_element(e);
    }
    ...
    _build_atomic_number_matches_from_atom_list(mdlad->atom_list());
  } else if ("L" == initial_atomic_symbol) {  // looks like an ISIS atom list - should be
                                              // handled previously
    ;
...
#ifdef DEBUG_CREATE_FROM_MOLECULE
  cerr << "Atom " << my_atom_number << " type " << m.atomic_number(my_atom_number)
       << " has " << m.hcount(my_atom_number) << " hydrogens\n";
#endif
  ...
  if (smarts_specified.length() > 0) {
    ...
    //  cerr << "Atom " << my_atom_number << " substitution " << mdlad->substitution() <<
    //  endl;
...
// #define DEBUG_HCOUNT_STUFF
  ...
  // cerr << "preserve_saturation " << mqs.preserve_saturation() << ", uns " <<
  // mdlad->unsaturated() << endl;
  ...
  if (smarts_specified.length()) {
    ...
    //  cerr << "Ring stuff for " << _initial_atom_number << " r = " << r << ", nr = " <<
    //  nr << ", rbc " << rbc << endl;
    if (-1 == r) {
  ...
  // cerr << "For atom " << my_atom_number << " nr = " << nr << " arom " <<
  // m.is_aromatic(my_atom_number) << endl;
  ...
  if (smarts_specified.length()) {
    ...
    _set_match_any_aromatic(mqs.convert_all_aromatic_atoms_to_generic_aromatic(), m,
                            my_atom_number);
    ...
    if (known_aromaticity < 0)  // need to compute it
    {
      ...
      if (m.aromaticity(my_atom_number, arom)) {
        ...
        //      if (is_aliphatic_atom(arom))     // only make positive decisions about
        //      aromaticity
        //        update_aromaticity(arom);
      }
  ...
  // We take the easy way out here and just use our component. 
  // Probably should do all of them if present

  // cerr << "Building query " << mqs.use_preference_values_to_distinguish_symmetry() <<
  // endl;
  ...
  if (INVALID_ATOM_NUMBER != parent_atom_number) {
    ...
    //  When dealing with Hydrogens that can be anything, we need to allow the bond type
    //  to be anything. Bit of a kludge here I'm afraid

    if (1 == e->atomic_number() &&
        (mqs.convert_explicit_hydrogens_to_match_any_atom() ||
         mqs.convert_explicit_hydrogens_to_match_any_atom_including_hydrogen())) {
      initialise_bond_attributes(*sb, mdlfd.mdl_bond_data(bnumber), nullptr,
                                 parent_atom_number, completed, mqs, 0);
    } else if (bnumber >=
               mdlfd.number_bonds()) {  // underlying mdl_molecule may have been expanded
      ;
    } else if (aromatic_bonds_lose_kekule_identity()) {
      initialise_bond_attributes(*sb, mdlfd.mdl_bond_data(bnumber), b, parent_atom_number,
                                 completed, mqs,
                                 determine_part_of_non_kekule_aromatic(m, *b));
    } else {
      initialise_bond_attributes(*sb, mdlfd.mdl_bond_data(bnumber), b, parent_atom_number,
                                 completed, mqs, 0);
    }
    ...
    if (smarts_specified.length()) {
      ...
      if (!construct_from_smarts_token(smarts_specified)) {
        cerr << "Substructure_Atom::create_from_molecule:invalid smarts in alias '"
             << smarts_specified << "'\n";
        return 0;
      }
  ...
  for (int i = 0; i < acon; i++) {
    ...
    if (aromatic_bonds_lose_kekule_identity()) {
      initialise_bond_attributes(*sb, mdlfd.mdl_bond_data(bnumber), b, j, completed, mqs,
                                 determine_part_of_non_kekule_aromatic(m, *b));
    } else {
      initialise_bond_attributes(*sb, mdlfd.mdl_bond_data(bnumber), b, j, completed, mqs,
                                 0);
...
#ifdef DEBUG_CREATE_FROM_MOLECULE
    cerr << "From atom " << my_atom_number << " child " << j << " inc "
         << (nullptr == include_these_atoms ? '1' : include_these_atoms[j])
         << " complete " << completed[j] << ' ' << m.smarts_equivalent_for_atom(j)
         << endl;
#endif
...
int
Substructure_Atom::_parse_smiles_specifier(
    Molecule &m, const MDL_File_Data &mdlfd,
    extending_resizable_array<Substructure_Atom *> &completed) {
  ...
  if (_hcount.number_elements()) {
    int hc = _hcount[0];
    if (0 == hc) {
      cerr << "Substructure_Atom::_parse_smiles_specifier: warning, no open valence in '"
           << m.smiles() << "'\n";
      _hcount.resize(0);
    } else {
      _hcount.resize(0);
...
Single_Substructure_Query::_initialise_sub_array (Molecule & m,
                                                  Substructure_Query &
substitutions_only_at, Molecule_to_Query_Specifications & mqs)
{
  ...
  if (0 == nhits)
  {
    cerr << "Single_Substructure_Query::_initialise_sub_array:no hits to
substitutions_only_at query\n"; return 0;
  }
...
Single_Substructure_Query::create_from_molecule(MDL_Molecule &m,
...
#ifdef DEBUG_CREATE_FROM_MOLECULE
  cerr << "Single_Substructure_Query::create_from_molecule line " << __LINE__ << " "
       << m.name() << endl;
  ...
  // cerr << "Single_Substructure_Query::create_from_molecule:BEGIN   " << m.smiles() <<
  // endl;
...
int
Single_Substructure_Query::_build_element_hits_needed(
    const MDL_Molecule &m, const Molecule_to_Query_Specifications &mqs,
    const int *include_these_atoms) {
  int *count = new_int(HIGHEST_ATOMIC_NUMBER + 1);
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (mqs.isotope_means_match_any_atom() &&
        m.isotope(i) == mqs.isotope_means_match_any_atom()) {
      continue;
    }
  ...
  for (int i = 0; i <= HIGHEST_ATOMIC_NUMBER; i++) {
    ...
    Elements_Needed *tmp = new Elements_Needed();
    tmp->add_atomic_number(i);
    tmp->set_min(count[i]);
...
first_isotopically_labelled_atom_in_fragment(Molecule &m, int f) {
  int matoms = m.natoms();
...
static atom_number_t
first_allowed_atom_in_fragment(
    Molecule &m, const int f,
...
Single_Substructure_Query::_create_from_molecule(MDL_Molecule &m,
  ...
  if (mqs.condense_explicit_hydrogens_to_anchor_atoms()) {
    if (nullptr != include_these_atoms) {
      cerr << "Single_Substructure_Query::_create_from_molecule:include_these_atoms not "
              "handled, contact LillyMol on github "
              "(https://github.com/EliLillyCo/LillyMol)\n";
      exit(1);
    }
  ...
  if (_min_fraction_atoms_matched > _max_fraction_atoms_matched) {
    cerr << "Single_Substructure_Query::_create_from_molecule:inconsistent min "
         << _min_fraction_atoms_matched << " and max " << _max_fraction_atoms_matched
         << " fraction matched\n";
    return 0;
  }
...
// #define ECHO_SUBSET
    ...
    for (int i = 0; i < matoms; ++i) {
      cerr << " atom " << i << ' ' << m.smarts_equivalent_for_atom(i) << " inc "
           << include_these_atoms[i] << endl;
    }
#endif
    for (int i = 0; i < nf; ++i) {
      ...
      while (INVALID_ATOM_NUMBER !=
        ...
        if (!r->create_from_molecule(m, m, astart, nullptr, mqs, tmp,
                                     include_these_atoms)) {
          cerr << "Single_Substructure_Query::_create_from_molecule:failure to build "
                  "from atom "
               << astart << " in fragment " << i << endl;
          return 0;
        }
  ...
  // cerr << "Created query with " << _root_atoms.number_elements() << " root atoms, atoms
  // in query " << _max_atoms_in_query << endl;
  ...
  if (mqs.environment_near_substitution_points_specified() ||
      mqs.environment_no_match_near_substitution_points_specified()) {
    if (!_add_environment_according_to_matched_atoms(mqs)) {
      cerr << "Single_Substructure_Query::_create_from_molecule:cannot build "
              "environment\n";
      return 0;
    }
  }
...
#ifdef DEBUG_CREATE_FROM_MOLECULE
  cerr << "Substructure_Query::create_from_molecule:created " << _number_elements
       << " molecules\n";
...
int
Substructure_Query::_enumerate_list_atom_possibilities(
    const MDL_Molecule &m, Molecule_to_Query_Specifications &mqs,
    const resizable_array_p<Link_Atom> &link_atoms, Link_Atom_Current_State *lacc,
    int nlink, int ndx, const int *include_these_atoms) {
  lacc[ndx].reset();
...
int
Single_Substructure_Query::_add_environment_according_to_matched_atoms(
    Molecule_to_Query_Specifications &mqs) {
  ...
  if (0 == n) {
    cerr << "Single_Substructure_Query::_add_environment_according_to_matched_atoms:no "
            "substitution points\n";
    return 0;
  }
...
Single_Substructure_Query::write_msi(std::ostream &os, int &object_id,
  ...
  _heteroatoms_matched.write_msi(os, NAME_OF_HETEROATOMS_MATCHED_ATTRIBUTE,
                                 indentation + 2);
  ...
  if (_all_hits_in_same_fragment) {
    os << ind << "  (A I " << NAME_OF_ALL_HITS_IN_SAME_FRAGMENT_ATTRIBUTE << ' '
       << _all_hits_in_same_fragment << ")\n";
  }

  if (_only_keep_matches_in_largest_fragment) {
    os << ind << "  (A I " << NAME_OF_ONLY_MATCH_LARGEST_FRAGMENT_ATTRIBUTE << ' '
       << _only_keep_matches_in_largest_fragment << ")\n";
  }
  ...
  if (0 == _environment_must_match_unmatched_atoms) {
    os << ind << "  (A I " << NAME_OF_ENVIRONMENT_MUST_MATCH_UNMATCHED_ATOMS_ATTRIBUTE
       << " 0)\n";
  }
  ...
  _strongly_fused_rings.write_msi(os, NAME_OF_STRONGLY_FUSED_RING_COUNT_SPECIFIER,
                                  indentation + 2);
  ...
  _non_aromatic_rings.write_msi(os, NAME_OF_NON_AROMATIC_RING_COUNT_SPECIFIER,
                                indentation + 2);

  _ncon.write_msi(os, NAME_OF_NCON_ATTRIBUTE, indentation + 2);

  _distance_between_hits.write_msi(os, NAME_OF_DISTANCE_BETWEEN_HITS_ATTRIBUTE,
                                   indentation + 2);

  if (_matched_atoms_to_check_for_hits_too_close > 0) {
    os << ind << "  (A I " << NAME_OF_DISTANCE_BETWEEN_HITS_NCHECK_ATTRIBUTE << ' '
       << _matched_atoms_to_check_for_hits_too_close << ")\n";
  }

  if (_fail_if_embeddings_too_close) {
    os << ind << "  (A I " << NAME_OF_FAIL_IF_EMBEDDINGS_TOO_CLOSE_ATTRIBUTE << ' '
       << _fail_if_embeddings_too_close << ")\n";
  }
  ...
  if (static_cast<float>(0.0) != _min_fraction_atoms_matched) {
    os << ind << "  (A D " << NAME_OF_MIN_FRACTION_ATOMS_MATCHED << ' '
       << _min_fraction_atoms_matched << ")\n";
  }

  if (static_cast<float>(0.0) != _max_fraction_atoms_matched) {
    os << ind << "  (A D " << NAME_OF_MAX_FRACTION_ATOMS_MATCHED << ' '
       << _max_fraction_atoms_matched << ")\n";
  }

  _ring_atoms_matched.write_msi(os, NAME_OF_RING_ATOMS_MATCHED_ATTRIBUTE,
                                indentation + 2);
  _number_isotopic_atoms.write_msi(os, NAME_OF_NUMBER_ISOTOPIC_ATOMS_ATTRIBUTE,
                                   indentation + 2);
  ...
  _distance_between_root_atoms.write_msi(
      os, NAME_OF_DISTANCE_BETWEEN_ROOT_ATOMS_SPECIFIER, indentation + 2);

  for (int i = 0; i < _elements_needed.number_elements(); i++) {
    _elements_needed[i]->write_msi(os, NAME_OF_ELEMENTS_NEEDED_OBJECT, object_id,
                                   indentation + 2);
  }

  for (int i = 0; i < _element_hits_needed.number_elements(); i++) {
    _element_hits_needed[i]->write_msi(os, NAME_OF_ELEMENT_HITS_NEEDED_OBJECT, object_id,
                                       indentation + 2);
  }
  ...
  if (_no_matched_atoms_between.number_elements()) {
    for (int i = 0; i < _no_matched_atoms_between.number_elements(); i++) {
      ...
      os << ind << "  (A I " << NAME_OF_NO_MATCHED_ATOMS_BETWEEN_SPECIFIER << " ("
         << b->a1() << ' ' << b->a2() << "))\n";
    }
  }
  ...
  int rc = 1;
  for (int i = 0; i < _root_atoms.number_elements(); i++) {
    if (!_root_atoms[i]->write_msi(os, object_id, NAME_OF_QUERY_ATOM_OBJECT,
                                   indentation + 2)) {
      return 0;
    }
  }
...
Single_Substructure_Query::_parse_ring_specifier_object(const msi_object &msi) {
  ...
  if (_ring_specification.empty()) {  // no operators with first object
    ;
  } else if (!extract_operator(
                 msi, _ring_specification_logexp, IW_LOGEXP_AND,
...
/*
  In constructing from an msi object, we have found a
  Substructure_Ring_System_Specification object. Parse it.
*/
...
Single_Substructure_Query::_parse_ring_system_specifier_object(const msi_object &msi) {
  ...
  Substructure_Ring_System_Specification *r =
      new Substructure_Ring_System_Specification();
  if (!r->construct_from_msi_object(msi)) {
    delete r;
  ...
  if (_ring_system_specification.empty()) {
    ;
                 ...
                 msi, _ring_system_specification_logexp, IW_LOGEXP_AND,
                 "Single_Substructure_Query::_parse_ring_system_specifier_object")) {
    delete r;
    return 0;
  }
...
int
Single_Substructure_Query::_construct_environment_from_msi_object(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed,
...
int
Substructure_Environment::_add_possible_parent(
    atom_number_t possible_parent, bond_type_t possible_parent_bond_type,
    extending_resizable_array<Substructure_Atom *> &completed) {
  if (0 !=
      possible_parent_bond_type) {  // only add if bond_type has defined a particular type
...
int
Substructure_Environment::construct_from_msi_object(
    const msi_object &msi, extending_resizable_array<Substructure_Atom *> &completed,
    atom_number_t possible_parent, bond_type_t possible_parent_bond_type) {
  int nmsi = msi.number_elements();
  ...
  if (0 == np) {
    cerr << "Substructure_Environment::construct_from_msi_object: environment not "
            "connected\n";
    return 0;
  }

  int notused = 0;
  if (!really_gruesome(_hits_needed, msi, NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE, notused, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  const msi_attribute *att =
      msi.attribute(NAME_OF_NO_OTHER_SUBSTITUENTS_ALLOWED_ATTRIBUTE);
  if (att) {
    //  cerr << "Got " << NAME_OF_NO_OTHER_SUBSTITUENTS_ALLOWED_ATTRIBUTE << endl;
    if (!att->value(_no_other_substituents_allowed)) {
      cerr << "Substructure_Environment::construct_from_msi_object: the "
           << NAME_OF_NO_OTHER_SUBSTITUENTS_ALLOWED_ATTRIBUTE
  ...
  if (att) {
    if (!att->value(_hydrogen_ok_as_environment_match) ||
        _hydrogen_ok_as_environment_match < 0) {
      cerr << "Substructure_Environment::construct_from_msi_object: the "
           << NAME_OF_HYDROGEN_OK_AS_ENVIRONMENT_MATCH << " must be a whole number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_max_environment_matches_per_attachment_point) ||
        _max_environment_matches_per_attachment_point <= 0) {
      cerr << "Substructure_Environment::construct_from_msi_object: then "
           << NAME_OF_MAX_ENVIRONMENT_MATCHES_PER_ATTACHMENT_POINT
           << " must be a +ve whole number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_environments_can_share_attachment_points) ||
        _environments_can_share_attachment_points < 0) {
      cerr << "Substructure_Environment::construct_from_msi_object: the "
           << NAME_OF_ENVIRONMENTS_CAN_SHARE_ATTACHMENT_POINTS
           << " attribute must be a whole +ve number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_max_matches_to_find) || _max_matches_to_find < 1) {
      cerr << "Substructure_Environment::construct_from_msi_object:the "
           << NAME_OF_MAX_MATCHES_TO_FIND << " attribute must be a whole +ve number\n";
      return 0;
    }
  }
  ...
  if (att) {
    if (!att->value(_or_id) || _or_id <= 0) {
      cerr << "Substructure_Environment::construct_from_msi_object: the "
           << NAME_OF_OR_OPERATOR << " must be a positive whole number\n";
      return 0;
    }
  }
#endif
  ...
  if (att) {
    if (!att->value(_and_id) || _and_id <= 0) {
      cerr << "Substructure_Environment::construct_from_msi_object: the "
           << NAME_OF_AND_OPERATOR << " must be a positive whole number\n";
      return 0;
    }
  }

  if (_and_id && _or_id) {
    cerr << "Substructure_Environment::construct_from_msi_object:cannot have both OR and "
            "AND specifications\n";
    return 0;
  }
  ...
  while (nullptr != (att = msi.attribute(NAME_OF_SMARTS_ATTRIBUTE, i++))) {
    ...
    if (x.length() && (isdigit(s[0]) || '>' == s[0] || '<' == s[0])) {
      ...
      if (!a->parse_smarts_specifier(x)) {
        cerr << "Substructure_Environment::construct_from_msi_object:invalid smarts '"
             << x << "'\n";
        return 0;
      }
    }
...
static int
next_disconnected(const const_IWSubstring &smarts, int &istart,
...
template <typename T>
void
transfer_to_our_array(resizable_array_p<T> &to, resizable_array<T *> &from) {
...
template void transfer_to_our_array(resizable_array_p<Substructure_Atom> &,
                                    resizable_array<Substructure_Atom *> &);
template void transfer_to_our_array(resizable_array_p<Bond> &, resizable_array<Bond *> &);
template void transfer_to_our_array(resizable_array_p<Link_Atom> &,
                                    resizable_array<Link_Atom *> &);
...
Single_Substructure_Query::_parse_smarts_specifier(const const_IWSubstring &smarts) {
  ...
  if (smarts.starts_with('.')) {
    cerr << "Single_Substructure_Query::_parse_smarts_specifier: smarts cannot start "
            "with '.'\n";
  ...
  while (next_disconnected(smarts, i, disconnected)) {
    ...
    if (disconnected.starts_with('(')) {
      if (component_grouping) {
        cerr << "Single_Substructure_Query::_parse_smarts_specifier: component grouping "
                "cannot be nested\n";
        return 0;
      }
    ...
    if (!a->parse_smarts_specifier(disconnected, pst, query_atoms_created)) {
      cerr << "Single_Substructure_Query::_parse_smarts_specifier: cannot parse '"
           << disconnected << "'\n";
      delete a;
      return 0;
    }
  ...
  if (!_build_no_matched_atoms_between(pst.three_dots())) {
    cerr << "Single_Substructure_Query::_parse_smarts_specifier:invalid 3 dots "
            "specification\n";
    return 0;
  }
  ...
  transfer_to_our_array(_down_the_bond, pst.down_the_bond());
...
#ifdef DEBUG_SSSQ_PARSE_SMARTS_SPECIFIER
  cerr << "After parsing '" << smarts << "' we have " << _root_atoms.number_elements()
       << " root atoms\n";
#endif
  ...
  if (att->valid_as_int()) {
    ...
    while (s.nextword(token, i)) {
      ...
      if (e->atomic_number() < 0) {
        cerr << "fetch_heteroatom_definitions::element " << s
             << " has no atomic number\n";
        return 0;
      }
...
Single_Substructure_Query::_construct_from_msi_object(const msi_object &msi,
  ...
  for (int i = 0; i < natt; i++) {
    ...
    if (NAME_OF_COMMENT_ATTRIBUTE == att->name()) {
      ...
      int v;
      if (!att->value(v) || 2 != v) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: can only read "
                "version 2 queries\n";
        return 0;
      }
      ...
      int oe;
      if (!att->value(oe)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_ONE_EMBEDDING_PER_START_ATOM
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(ue)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_UNIQUE_EMBEDDINGS_ONLY << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(nrc) || nrc < 0) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_NORMALISE_RC_PER_HITS_NEEDED
      ...
      if (!att->value(nsr)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_SUBTRACT_FROM_RC << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_respect_initial_atom_numbering)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_RESPECT_INITIAL_NUMBERING_ATTRIBUTE
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(nisc) || nisc < 0) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_NCON_IGNORE_SINGLY_CONNECTED
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_do_not_perceive_symmetry_equivalent_matches)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_DO_NOT_PERCEIVE_SYMMETRIC_EQUIVALENTS
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_implicit_ring_condition)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_IMPLICIT_RING_CONDITION
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_all_hits_in_same_fragment)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_ALL_HITS_IN_SAME_FRAGMENT_ATTRIBUTE
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_only_keep_matches_in_largest_fragment)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_ONLY_MATCH_LARGEST_FRAGMENT_ATTRIBUTE
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_embeddings_do_not_overlap)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_EMBEDDINGS_DO_NOT_OVERLAP_ATTRIBUTE
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_sort_by_preference_value)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_SORT_BY_PREFERENCE_VALUE_ATTRIBUTE
             << "' attribute requires a whole number\n";
        return 0;
      }
      ...
      if (!create_from_smarts(smarts)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: cannot parse '"
             << NAME_OF_SMARTS_ATTRIBUTE << "'\n";
        return 0;
      }
      attribute_recognised[i] = 1;
      _originating_smarts = smarts;
    } else if (NAME_OF_NUMERIC_VALUE_ATTRIBUTE == att->name()) {
      double d;
      if (!att->value(d)) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the "
             << NAME_OF_NUMERIC_VALUE_ATTRIBUTE << " must be numeric\n";
        cerr << (*att) << endl;
        return 0;
      }
      ...
      if (2 != att->number_int_values()) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the "
             << NAME_OF_NO_MATCHED_ATOMS_BETWEEN_SPECIFIER
             << " attribute must have two int values\n";
        cerr << (*att) << endl;
        return 0;
      }
      ...
      if (!att->value(_matched_atoms_to_check_for_hits_too_close) ||
        ...
        cerr << "Single_Substructure_Query::_construct_from_msi_object:the '"
             << NAME_OF_DISTANCE_BETWEEN_HITS_NCHECK_ATTRIBUTE
      ...
      if (!att->value(_fail_if_embeddings_too_close) ||
          _fail_if_embeddings_too_close < 0) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object:the '"
             << NAME_OF_FAIL_IF_EMBEDDINGS_TOO_CLOSE_ATTRIBUTE
             << " must be a whole positive number\n";
        cerr << (*att) << endl;
        return 0;
      }
      ...
      if (!att->value(_environment_must_match_unmatched_atoms) ||
          _environment_must_match_unmatched_atoms < 0) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object:the '"
             << NAME_OF_ENVIRONMENT_MUST_MATCH_UNMATCHED_ATOMS_ATTRIBUTE
      ...
      if (!att->value(environments_can_share_attachment_points) ||
          environments_can_share_attachment_points < 0) {
        cerr << "Single_Substructure_Query::_construct_from_msi_object: the '"
             << NAME_OF_ENVIRONMENTS_CAN_SHARE_ATTACHMENT_POINTS
             << " attribute must be a whole non negative number\n";
        return 0;
      }
      ...
      while (s.nextword(token, j)) {
        ...
        if ("ncon" == token) {
          ...
          cerr << "Single_Substructure_Query:construct_from_msi_attribute:unrecognised "
                  "sort attribute '"
               << s << "'\n";
          return 0;
        }
  ...
  if (!_hits_needed.ok()) {
    cerr << "Single_Substructure_Query::_construct_from_msi_object:inconsistent hits "
            "needed values\n";
    return 0;
  }
  ...
  if (_only_keep_matches_in_largest_fragment && _all_hits_in_same_fragment) {
    cerr << "Single_Substructure_Query::_construct_from_msi_object:the "
         << NAME_OF_ONLY_MATCH_LARGEST_FRAGMENT_ATTRIBUTE << " and "
         << NAME_OF_ALL_HITS_IN_SAME_FRAGMENT_ATTRIBUTE
         << " attributes are mutually inconsistent\n";
    return 0;
  }
  ...
  int nat = 0;
  if (!really_gruesome(_attached_heteroatom_count, msi,
                       NAME_OF_ATTACHED_HETEROATOM_COUNT_SPECIFIER, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }
  ...
  if (_hits_needed.is_set()) {  // from a numeric qualifier on a smarts
    ;
  } else if (!really_gruesome(_hits_needed, msi, NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE, nat,
                              0, MAX_NOT_SPECIFIED)) {
    return 0;
  }
  ...
  if (!really_gruesome(_heteroatoms_in_molecule, msi, NAME_OF_HETEROATOMS_ATTRIBUTE, nat,
                       0, MAX_NOT_SPECIFIED)) {
    cerr << "Single_Substructure_Query::_construct_from_msi_object: Cannot parse '"
         << NAME_OF_HETEROATOMS_MATCHED_ATTRIBUTE << "' attribute(s)\n";
    return 0;
  }

  if (_heteroatoms.number_elements() &&
      (!_attached_heteroatom_count.is_set() && !_heteroatoms_in_molecule.is_set())) {
    cerr << "Single_Substructure_Query::_construct_from_msi_object: "
         << _heteroatoms.number_elements()
         << " heteroatoms defined, but no attached_heteroatom_count\n";
    return 0;
  }

  if (!really_gruesome(_natoms, msi, NAME_OF_NATOMS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_nrings, msi, NAME_OF_NRINGS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }
  ...
  if (!really_gruesome(_fused_rings, msi, NAME_OF_FUSED_RING_COUNT_SPECIFIER, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_strongly_fused_rings, msi,
                       NAME_OF_STRONGLY_FUSED_RING_COUNT_SPECIFIER, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_isolated_rings, msi, NAME_OF_ISOLATED_RING_COUNT_SPECIFIER, nat,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_isolated_ring_objects, msi, NAME_OF_ISOLATED_RING_OBJECTS, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_aromatic_rings, msi, NAME_OF_AROMATIC_RING_COUNT_SPECIFIER, nat,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_non_aromatic_rings, msi,
                       NAME_OF_NON_AROMATIC_RING_COUNT_SPECIFIER, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_distance_between_hits, msi,
                       NAME_OF_DISTANCE_BETWEEN_HITS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_number_isotopic_atoms, msi,
                       NAME_OF_NUMBER_ISOTOPIC_ATOMS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_number_fragments, msi, NAME_OF_NUMBER_FRAGMENTS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_distance_between_root_atoms, msi,
                       NAME_OF_DISTANCE_BETWEEN_ROOT_ATOMS_SPECIFIER, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_atoms_in_spinach, msi, NAME_OF_SPINACH_ATOMS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_inter_ring_atoms, msi, NAME_OF_INTER_RING_ATOMS_ATTRIBUTE, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_unmatched_atoms, msi, NAME_OF_UNMATCHED_ATOMS, nat, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_net_formal_charge, msi, NAME_OF_NET_FORMAL_CHARGE_ATTRIBUTE, nat,
                       MIN_NOT_SPECIFIED, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!get_float_attribute(msi, NAME_OF_MIN_FRACTION_ATOMS_MATCHED,
                           _min_fraction_atoms_matched, 0.0, 1.0)) {
    return 0;
  }

  if (!get_float_attribute(msi, NAME_OF_MAX_FRACTION_ATOMS_MATCHED,
                           _max_fraction_atoms_matched, 0.0, 1.0)) {
    return 0;
  }
  ...
  if (_root_atoms.empty()) {
    cerr
        << "Single_Substructure_Query::_construct_from_msi_object: Root atom not found\n";
    ...
    if (_element_hits_needed.number_elements() || _distance_between_root_atoms.is_set() ||
        _no_matched_atoms_between.number_elements() ||
        _attached_heteroatom_count.is_set() || _ncon.is_set() ||
        _heteroatoms_matched.is_set() || _ring_atoms_matched.is_set()) {
      cerr << "Single_Substructure_Query::_construct_from_msi_object: global conditions "
              "incompatible with no root atom\n";
      return 0;
    }
  ...
  if (query_environment_present) {
    for (int i = 0; i < nmsi; i++) {
      ...
      if (NAME_OF_ENVIRONMENT_OBJECT == mi.name()) {
        if (!_construct_environment_from_msi_object(mi, completed, _environment)) {
          cerr << "Single_Substructure_Query::_construct_from_msi_object: environment "
        ...
        if (!_construct_environment_from_msi_object(mi, completed,
                                                    _environment_rejections)) {
          cerr << "Single_Substructure_Query::_construct_from_msi_object: environment "
                  "interpretation failed\n";
          cerr << mi << endl;
          return 0;
    ...
    if (environments_can_share_attachment_points >= 0) {
      for (int i = 0; i < _environment.number_elements(); ++i) {
        _environment[i]->set_environments_can_share_attachment_points(
            environments_can_share_attachment_points);
      }
      for (int i = 0; i < _environment_rejections.number_elements(); ++i) {
        _environment_rejections[i]->set_environments_can_share_attachment_points(
            environments_can_share_attachment_points);
      }
    }
  }
...
Single_Substructure_Query::construct_from_msi_object(const msi_object &msi) {
  ...
  if (NAME_OF_QUERY_OBJECT != msi.name()) {
    cerr << "Single_Substructure_Query::construct_from_msi_object: msi object of "
            "incorrect type\n";
    return 0;
  }
  ...
  if (n) {
    for (int i = 0; i < msi.number_attributes(); i++) {
      ...
      if (!_build_chirality_specification_from_msi_attribute(att->stringval())) {
        cerr << "Single_Substructure_Query::construct_from_msi_object:invalid chirality '"
             << (*att) << "'\n";
        return 0;
      }
    }
  ...
  if (rc && attribute_recognised) {
    for (int i = 0; i < nat; i++) {
      if (0 == attribute_recognised[i]) {
        //      implement this some time. The problem is things like 'min_....' are not
        //      recognised yet const msi_attribute * att = msi.msi_attribute (i); cerr <<
        //      "Unrecognised attribute "; rc = 0;
      }
    }
  }
  ...
  for (int i = 0; i < _no_matched_atoms_between.number_elements(); i++) {
    ...
    if (b->a1() >= _min_atoms_in_query || b->a2() >= _min_atoms_in_query) {
      cerr << "Single_Substructure_Query::construct_from_msi_object: illegal "
           << NAME_OF_NO_MATCHED_ATOMS_BETWEEN_SPECIFIER << " specifier\n";
      cerr << "There are as few as " << _min_atoms_in_query << " query atoms\n";
      cerr << "No matched atoms between " << i << " specifies atoms " << b->a1()
           << " and " << b->a2() << endl;
...
int
Single_Substructure_Query::_parse_and_consume_optional_leading_numeric_qualifiers(
    const_IWSubstring &smarts) {
  int m;  // the numeric precondition
  int nchars;

  // Small limitation, the first number is limited to single digits. Fix if it ever
  // becomes an issue.
  if (smarts.length() > 3 && isdigit(smarts[0]) && smarts[1] == '-' &&
      isdigit(smarts[2])) {
    return ParseRange(smarts);
  }
  ...
  if ('>' == smarts[0]) {
    ...
    if ((0 == (nchars = fetch_numeric(smarts, m))) || (m < 0)) {
      cerr << "Single_Substructure_Query::_parse_and_consume_optional_leading_numeric_"
              "qualifiers: the '>' directive must be followed by a whole number\n";
      return 0;
    }

    if (_hits_needed.is_set()) {
      cerr << "Single_Substructure_Query::_parse_and_consume_optional_leading_numeric_"
              "qualifiers:_hits_needed already set\n";
      return 0;
    }
    ...
    if (m < 0) {
      cerr << "Single_Substructure_Query::_parse_and_consume_optional_leading_numeric_"
              "qualifiers: the count directive must be a whole positive number\n";
      return 0;
    }
...
// Parse a range directive at the start of a smiles.
// Looks like ^(\d+)-(\d+)
// A lot of the checks done here are not necessary because they
// were done by the caller. But play it safe.
int
Single_Substructure_Query::ParseRange(const_IWSubstring &smarts) {
  int minval;
  int nchars_first = fetch_numeric(smarts, minval);
  if (nchars_first == 0) {
    cerr << "Single_Substructure_Query::ParseRange:invalid start of range\n";
    return 0;
  }
  smarts += nchars_first;

  if (smarts[0] != '-') {
    cerr << "Single_Substructure_Query::ParseRange:not a range\n";
    return 0;
  }
  ...
  int maxval;
  int nchars_second = fetch_numeric(smarts, maxval);
  if (nchars_second == 0) {
    cerr << "Single_Substructure_Query::ParseRange:invalid end of range\n";
    return 0;
  }
  smarts += nchars_second;

  if (minval > maxval) {
    cerr << "Single_Substructure_Query::ParseRange:invalid range " << minval << " to "
         << maxval << '\n';
    return 0;
  }

  _hits_needed.set_min(minval);
  _hits_needed.set_max(maxval);
  // Chars consumed consists of the two numbers and the - between them.
  return nchars_first + 1 + nchars_second;
}
...
Single_Substructure_Query::create_from_smarts(const IWString &smarts) {
  ...
  if (0 == mysmarts.nchars()) {
    cerr << "Single_Substructure_Query::create_from_smarts: no characters to process '"
         << smarts << "'\n";
    return 0;
  }
...
Substructure_Query::write_msi(std::ostream &os, int &object_id, int indentation) {
  ...
  if (_each_component_search) {
    os << ind << "  (A I " << NAME_OF_EACH_COMPONENT_SEARCH_ATTRIBUTE << ' '
       << _each_component_search << ")\n";
  }
...
int
Substructure_Query::_parse_smarts_components(const IWString &smarts, char separator) {
  int rc = 0;
...
static int
fetch_next_smarts_component(const const_IWSubstring &smarts, int smartslen, int &ndx,
                            const_IWSubstring &s) {
...
static int
fetch_next_smarts_operator(const const_IWSubstring &smarts, int &ndx, int &op) {
...
int
Substructure_Query::_add_component_from_smarts(const const_IWSubstring &smarts, int op) {
  Single_Substructure_Query *q = new Single_Substructure_Query;
...
Substructure_Query::_single_query_construct_from_msi_object(const msi_object &msi) {
  ...
  if (NAME_OF_AND_OPERATOR == op) {
    ...
    cerr << "Substructure_Query::_single_query_construct_from_msi_object: which operator "
            "is this '"
         << op << "'\n";
    return 0;
  }
...
Substructure_Query::_composite_query_construct_from_msi_object(const msi_object &msi) {
  ...
  if (0 == nmsi && nullptr == att) {
    cerr << "Substructure_Query::_composite_query_construct_from_msi_object: no objects "
            "or smarts\n";
    return 0;
  }
  ...
  while (nullptr != (att = msi.attribute(NAME_OF_SMARTS_ATTRIBUTE, aptr++))) {
    ...
    if (!create_from_smarts(smarts)) {
      cerr << "Substructure_Query::_composite_query_construct_from_msi_object: cannot "
              "parse smarts\n";
      return 0;
    }
  }
  ...
  for (int i = 0; i < nmsi; i++) {
...
#ifdef DEBUG_READ_COMPOSITE
    cerr << "Components " << _number_elements << " operators "
         << _operator.number_operators() << endl;
#endif
    ...
    if (NAME_OF_AND_OPERATOR == op) {
      ...
      cerr << "Substructure_Query::_composite_query_construct_from_msi_object: what "
              "operator is this '"
           << op << "'\n";
      abort();
    }
  ...
  if (nullptr != att) {
    if (!att->value(_each_component_search)) {
      cerr << "Substructure_Query::_composite_query_construct_from_msi_object:invalid "
              "value for "
           << NAME_OF_EACH_COMPONENT_SEARCH_ATTRIBUTE << " attribute\n";
      return 0;
    }
  }
...
Substructure_Query::construct_from_msi_object(const msi_object &msi) {
  ...
  if (NAME_OF_QUERY_OBJECT == msi.name()) {
    ...
    cerr
        << "Substructure_Query::construct_from_msi_object: unrecognised msi object type '"
        << msi.name() << "'\n";
    cerr << msi;
    return 0;
  }
...
Substructure_Ring_Specification::construct_from_msi_object(const msi_object &msi) {
  ...
  if (!really_gruesome(_ring_size, msi, NAME_OF_RING_SIZE_ATTRIBUTE, attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_fused, msi, NAME_OF_RING_FUSED_ATTRIBUTE, attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_fused_aromatic_neighbours, msi,
                       NAME_OF_FUSED_AROMATIC_NEIGHBOURS_SPECIFIER, attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_fused_non_aromatic_neighbours, msi,
                       NAME_OF_FUSED_NON_AROMATIC_NEIGHBOURS_SPECIFIER,
...
int
Substructure_Ring_Specification::write_msi(std::ostream &os, int &object_id,
                                           int indentation) const {
  assert(ok());
  assert(os.good());
  ...
  _fused_aromatic_neighbours.write_msi(os, NAME_OF_FUSED_AROMATIC_NEIGHBOURS_SPECIFIER,
                                       indentation + 2);
  _fused_non_aromatic_neighbours.write_msi(
      os, NAME_OF_FUSED_NON_AROMATIC_NEIGHBOURS_SPECIFIER, indentation + 2);
  _largest_number_of_bonds_shared_with_another_ring.write_msi(
      os, NAME_OF_LARGEST_NUMBER_SHARED_BONDS_SPECIFIER, indentation + 2);
  _strongly_fused_ring_neighbours.write_msi(
      os, NAME_OF_STRONGLY_FUSED_RING_NEIGHBOUR_SPECIFIER, indentation + 2);
...
Substructure_Ring_Base::construct_from_msi_object(const msi_object &msi,
                                                  int &attributes_specified) {
  if (msi.number_elements()) {
    cerr << "Substructure_Ring_Base::construct_from_msi_object:no objects allowed in "
            "specification\n";
    return 0;
  }
  ...
  for (int i = 0; i < nat; i++) {
    ...
    if (NAME_OF_REJECTION_ATTRIBUTE == att->name()) {
      int tmp;
      if (!att->value(tmp)) {
        cerr << "Substructure_Ring_Base::construct_from_msi_object: the "
             << NAME_OF_REJECTION_ATTRIBUTE << " attribute must be a whole number\n";
        return 0;
      }
      ...
      if (!att->value(_all_hits_in_same_fragment)) {
        cerr << "Substructure_Ring_Specification::construct_from_msi_object: the "
             << NAME_OF_ALL_HITS_IN_SAME_FRAGMENT_ATTRIBUTE
             << " attribute must be a whole number\n";
        return 0;
      }
      ...
      if (_environment_atom.number_elements()) {
        cerr << "Substructure_Ring_Base::construct_from_msi_object:environment already "
                "specified '"
             << (*att) << "'\n";
        return 0;
      }
      ...
      if (!_construct_environment(env)) {
        cerr << "Substructure_Ring_Base::construct_from_msi_object:invalid environment '"
             << (*att) << "'\n";
        return 0;
      }
      ...
      if (!att->value(_environment_can_match_in_ring_atoms)) {
        cerr << "Substructure_Ring_Base::construct_from_msi_object:invalid "
             << NAME_OF_ENVIRONMENT_CAN_MATCH_IN_RING_ATOMS << " specification\n";
        return 0;
      }
      ...
      if (!fetch_heteroatom_definitions(*att, _is_heteroatom)) {
        cerr << "Substructure_Ring_Base::construct_from_msi_object:invalid heteroatom "
                "definitions '"
             << (*att) << "'\n";
        return 0;
      }
  ...
  if (!really_gruesome(_ncon, msi, NAME_OF_NCON_ATTRIBUTE, attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_heteroatom_count, msi, NAME_OF_HETEROATOMS_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_attached_heteroatom_count, msi,
                       NAME_OF_ATTACHED_HETEROATOM_COUNT_SPECIFIER, attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_within_ring_unsaturation, msi, NAME_OF_UNSATURATION_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_atoms_with_pi_electrons, msi, NAME_OF_ATOMS_WITH_PI_ELECTRONS,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_largest_number_of_bonds_shared_with_another_ring, msi,
                       NAME_OF_LARGEST_NUMBER_SHARED_BONDS_SPECIFIER,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_strongly_fused_ring_neighbours, msi,
                       NAME_OF_STRONGLY_FUSED_RING_NEIGHBOUR_SPECIFIER,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }
...
Substructure_Ring_System_Specification::construct_from_msi_object(const msi_object &msi) {
  ...
  if (!really_gruesome(_rings_in_system, msi, NAME_OF_NRINGS_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_ring_sizes, msi, NAME_OF_RING_SIZE_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_aromatic_ring_count, msi, NAME_OF_AROMATIC_RING_COUNT_SPECIFIER,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_non_aromatic_ring_count, msi,
                       NAME_OF_NON_AROMATIC_RING_COUNT_SPECIFIER, attributes_specified, 0,
                       MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_degree_of_fusion, msi, NAME_OF_RING_FUSED_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_atoms_in_system, msi, NAME_OF_NATOMS_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_strongly_fused_ring_count, msi,
                       NAME_OF_STRONGLY_FUSED_RING_COUNT_SPECIFIER, attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_number_spinach_groups, msi, NAME_OF_NUMBER_SPINACH_GROUPS,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_number_non_spinach_groups, msi, NAME_OF_NUMBER_NON_SPINACH_GROUPS,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_atoms_in_spinach_group, msi, NAME_OF_ATOMS_IN_SPINACH_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_length_of_spinach_group, msi, NAME_OF_LENGTH_OF_SPINACH_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_distance_to_another_ring, msi,
                       NAME_OF_DISTANCE_TO_ANOTHER_RING_ATTRIBUTE, attributes_specified,
                       0, MAX_NOT_SPECIFIED)) {
    return 0;
  }

  if (!really_gruesome(_rings_that_must_match_ring_sizes, msi,
                       NAME_OF_RINGS_THAT_MUST_MATCH_RING_SIZE_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    return 0;
  }
...
int
Substructure_Ring_Base::write_msi_attributes(std::ostream &os, int &object_id,
  ...
  _ncon.write_msi(os, NAME_OF_NCON_ATTRIBUTE, indentation + 2);
  _within_ring_unsaturation.write_msi(os, NAME_OF_UNSATURATION_ATTRIBUTE,
                                      indentation + 2);
  _atoms_with_pi_electrons.write_msi(os, NAME_OF_ATOMS_WITH_PI_ELECTRONS,
                                     indentation + 2);
  ...
  if (_environment_atom.number_elements()) {
    ...
    cerr
        << "Substructure_Ring_Base::write_msi_attributes:cannot write ring environment\n";
  }
...
int
Substructure_Ring_System_Specification::write_msi(std::ostream &os, int &object_id,
                                                  int indentation) const {
  assert(ok());
  assert(os.good());
  ...
  _rings_that_must_match_ring_sizes.write_msi(
      os, NAME_OF_RINGS_THAT_MUST_MATCH_RING_SIZE_ATTRIBUTE, indentation + 2);
  _aromatic_ring_count.write_msi(os, NAME_OF_AROMATIC_RING_COUNT_SPECIFIER,
                                 indentation + 2);
  _non_aromatic_ring_count.write_msi(os, NAME_OF_NON_AROMATIC_RING_COUNT_SPECIFIER,
                                     indentation + 2);
  ...
  _strongly_fused_ring_count.write_msi(os, NAME_OF_STRONGLY_FUSED_RING_COUNT_SPECIFIER,
                                       indentation + 2);
  _number_spinach_groups.write_msi(os, NAME_OF_NUMBER_SPINACH_GROUPS, indentation + 2);
  _number_non_spinach_groups.write_msi(os, NAME_OF_NUMBER_NON_SPINACH_GROUPS,
                                       indentation + 2);
  _atoms_in_spinach_group.write_msi(os, NAME_OF_ATOMS_IN_SPINACH_ATTRIBUTE,
                                    indentation + 2);
  _length_of_spinach_group.write_msi(os, NAME_OF_LENGTH_OF_SPINACH_ATTRIBUTE,
                                     indentation + 2);
  _distance_to_another_ring.write_msi(os, NAME_OF_DISTANCE_TO_ANOTHER_RING_ATTRIBUTE,
                                      indentation + 2);
...
int
Elements_Needed::write_msi(std::ostream &os, const char *object_name, int &object_id,
                           int indentation) const {
  if (_elements.empty()) {
    cerr << "Elements_Needed::write_msi: cannot write empty elemets array\n";
    return 0;
  }
  ...
  for (const Element *e : _elements) {
    os << ind << "  (A C " << NAME_OF_ATOMIC_SYMBOL_ATTRIBUTE << " \"" << e->symbol()
       << "\")\n";
  }
  switch (_operator) {
    case Op::NOT_SET:
      break;
    case Op::AND:
      os << ind << "  (A C " << NAME_OF_OPERATOR_ATTRIBUTE << " \"and\")\n";
      break;
    case Op::OR:
      os << ind << "  (A C " << NAME_OF_OPERATOR_ATTRIBUTE << " \"or\")\n";
      break;
    case Op::XOR:
      os << ind << "  (A C " << NAME_OF_OPERATOR_ATTRIBUTE << " \"xor\")\n";
      break;
    default:
      cerr << "Elements_Needed::write_msi:unrecognised operator\n";
      return 0;
  }
  _hits_needed.write_msi(os, NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE, indentation + 2);
...
Elements_Needed::construct_from_msi_object(const msi_object &msi) {
  ...
  if (att != nullptr) {
    if (! att->value(z)) {
      cerr << "Elements_Needed::construct_from_msi_object:invalid atomic number\n";
      return 0;
    }
  } else {
    att = msi.attribute(NAME_OF_ATOMIC_SYMBOL_ATTRIBUTE);
    if (att == nullptr) {
      cerr << "Elements_Needed::construct_from_msi_object:must specify element\n";
      return 0;
    }
    IWString token;
    att->value(token);
    const Element* e = get_element_from_symbol_no_case_conversion(token);
    if (e == nullptr) {
      cerr << "Elements_Needed::construct_from_msi_object:invalid atomc number '" << token << "'\n";
      return 0;
    }
    z = e->atomic_number();
  }
  ...
  if (!REASONABLE_ATOMIC_NUMBER(z)) {
    cerr << "Elements_Needed::construct_from_msi_object: bad element specification\n";
    return 0;
  }

  if (!add_atomic_number(z)) {
    cerr << "Elements_Needed::construct_from_msi_object:invalid atomic number " << z
         << '\n';
    return 0;
  }
  ...
  if (!really_gruesome(_hits_needed, msi, NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE,
                       attributes_specified, 0, MAX_NOT_SPECIFIED)) {
    cerr << "Elements_Needed::construct_from_msi_object: cannot parse "
         << NAME_OF_QUERY_HITS_NEEDED_ATTRIBUTE << " attribute\n";
    return 0;
  }
...
Link_Atom::construct_from_msi_attribute(const msi_attribute *msi) {
  ...
  if (s.nwords() < 3) {
    cerr << "Link_Atom::construct_from_msi_object:invalid specification, must have at "
            "least 3 words '"
         << s << "'\n";
    return 0;
  }
  ...
  if (!token.numeric_value(_a1) || _a1 < 0) {
    cerr << "Link_Atom::construct_from_msi_object:invalid a1 specification, must be a "
            "positive number'"
         << s << "'\n";
    return 0;
  }
  ...
  if (!token.numeric_value(_a2) || _a2 < 0 || _a2 == _a1) {
    cerr << "Link_Atom::construct_from_msi_object:invalid a2 specification, must be a "
            "positive number'"
         << s << "'\n";
    return 0;
  }
...
int
Link_Atom::write_msi(std::ostream &os, const IWString &ind, const char *attname) const {
...
Single_Substructure_Query::add_link_atom(const Link_Atom &ila) {
  ...
  cerr << "Initial atom number " << ila.a1() << " becomes query atom " << a->unique_id()
       << endl;
  ...
  cerr << "Initial atom number " << ila.a2() << " becomes query atom " << a->unique_id()
       << endl;
...
// Being built from a smarts, and one or more ... or ...{..} construct have been
// encountered. Try to fill in the _nmab array with this info.
int
Single_Substructure_Query::_build_no_matched_atoms_between(
    const resizable_array<ThreeDots *> &three_dots) {
  // cerr << "Single_Substructure_Query::_build_no_matched_atoms_between " << three_dots.size() << " dots\n";
  ...
  for (ThreeDots *thrdts : three_dots) {
    ...
    if (!nmab->Initialise(*thrdts)) {
      cerr << "Single_Substructure_Query::_build_no_matched_atoms_between:invalid three "
              "dots "
           << thrdts->qualifier() << endl;
      return 0;
    }
...
int
Substructure_Query::CreateFromSmarts(const std::string& smt) {
  const const_IWSubstring s(smt.data(), smt.size());
