...
Element_to_Remove::Element_to_Remove(const Element* e) : Element_Matcher(e) {
  _default_values();
}

Element_to_Remove::Element_to_Remove(atomic_number_t z) : Element_Matcher(z) {
  _default_values();
}

Element_to_Remove::Element_to_Remove(const char* s) : Element_Matcher(s) {
  _default_values();
}

Element_to_Remove::Element_to_Remove(const IWString& s) : Element_Matcher(s) {
  _default_values();
}
...
int
Element_to_Remove::_process(Molecule& m, const int* process_these, int id) {
  // cerr << "Element_to_Remove::_process:processing molecule with " << m.natoms() << "
  // atoms\n";
  ...
  for (int i = m.natoms() - 1; i >= 0; i--)  // scan backwards to preserve numbering
  {
    ...
    cerr << "Element matcher matches atom " << i << '\n';
    int icon = m.ncon(i);
    if (_maxcon_to_remove < 0 || (_maxcon_to_remove >= 0 && icon <= _maxcon_to_remove)) {
      atom_number_t a1, a2;
      a1 = a2 = INVALID_ATOM_NUMBER;
...
int
Element_to_Remove::process(Molecule& m, const int* process_these, int id) {
  _molecules_examined++;
...
int
Elements_to_Remove::construct_from_command_line(Command_Line& cl, int verbose,
                                                char option) {
  int i = 0;
  IWString ele;
