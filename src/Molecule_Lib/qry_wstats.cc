...
Substructure_Hit_Statistics::report(std::ostream& os, int verbose) const {
  ...
  float fraction = iwmisc::Fraction<float>(
      _molecules_which_match, _molecules_which_match + _molecules_which_do_not_match);
  ...
  for (int i = 0; i < _molecules_which_match_n_times.number_elements(); i++) {
    if (_molecules_which_match_n_times.item(i)) {
      os << " " << _molecules_which_match_n_times[i] << " molecules had " << i
         << " hits\n";
  ...
  if (number_different_contributions) {
    ...
    os << iwmisc::Fraction<float>(sites, _molecules_which_match) << 
          " sites per molecule that matches\n";
  }
...
int
Substructure_Hit_Statistics::set_stream_for_matches(FileType output_type,
                                                    const char* fname) {
  assert(_stream_for_matches.good());

  if (_stream_for_non_matches.valid() && _stream_for_non_matches.fname() == fname) {
    cerr << "Substructure_Hit_Statistics::set_stream_for_matches: name collision '"
         << fname << "'\n";
    return 0;
  }
...
int
Substructure_Hit_Statistics::set_stream_for_non_matches(FileType output_type,
                                                        const char* fname) {
  assert(_stream_for_matches.good());

  if (_stream_for_matches.valid() && _stream_for_matches.fname() == fname) {
    cerr << "Substructure_Hit_Statistics::set_stream_for_non_matches: name collision '"
         << fname << "'\n";
    return 0;
  }

  if (!_set_stream(output_type, fname, _stream_for_non_matches)) {
    cerr << "Substructure_Hit_Statistics::set_stream_for_non_matches: cannot use '"
         << fname << "'\n";
    return 0;
  }
...
Substructure_Hit_Statistics::_update_name_if_needed(int nmatches, Molecule* m) {
  if (nmatches && _append_match_details_to_molecule_name &&
      nmatches >= _append_match_details_to_molecule_name) {
    IWString new_name;
  ...
  if (0 == nmatches && _append_non_match_details_to_molecule_name) {
    ...
    if (_use_vertical_bars_for_query_details) {  // not implemented here here, nobody uses
                                                 // it.
    } else if (_csr_query_details) {
...
int
Substructure_Hit_Statistics::_update_matches(uint32_t nmatches, Molecule* m,
...
int
Substructure_Hit_Statistics::ConstructFromProto(
    const SubstructureSearch::SubstructureQuery& proto) {
