...
int
read_scaling_data(const char* fname, int verbose) {
...
void
Spread_Object::_update_nsn_stuff(const Spread_Object& fpsel, similarity_type_t d) {
...
int
Spread_Object::object_has_been_selected(Spread_Object& fpsel, float blurr_distances) {
  similarity_type_t new_distance = IW_General_Fingerprint::distance(fpsel);
...
int
Spread_Object::object_has_been_selected(Spread_Object& fpsel, const Tversky& tversky) {
  similarity_type_t new_distance = static_cast<similarity_type_t>(1.0) -
                                   IW_General_Fingerprint::tversky(fpsel, tversky);
...
Spread_Object::_determine_scale_from_column(int c) {
  ...
  if (!token.numeric_value(_scale) ||
      _scale < 0.0)  // even though a zero value makes no sense.
  {
...
int
Spread_Object::_determine_scale_from_tag(const IW_TDT& tdt, const IWString& scale_tag) {
  const_IWSubstring s;
...
Spread_Object::construct_from_tdt(IW_TDT& tdt, int& fatal) {
  ...
  if (!tdt.dataitem_value(smiles_tag, _smiles)) {
    cerr << "Spread_Object::construct_from_tdt: no smiles tag '" << smiles_tag
         << "' in TDT\n";
    return 0;
  }
...
similarity_type_t
do_blurring(similarity_type_t d, float blurr_distances) {
...
Spread_Object::set_distance_to_previously_selected_from_column(int col) {
  ...
  if (!_id.word(col, token)) {
    cerr << "Spread_Object::set_distance_to_previously_selected_from_column:cannot "
            "extract column "
         << (col + 1) << endl;
    return 0;
  }
  ...
  if (!token.numeric_value(d) || d < 0.0) {
    cerr << "Spread_Object::set_distance_to_previously_selected_from_column:invalid "
            "distance "
         << token << "'\n";
    return 0;
  }
