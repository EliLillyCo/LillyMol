...
static Accumulator<double> distance_stats;
// The number of nearest neighbour zero distance nbrs;
static uint32_t closest_is_zero = 0;
static uint32_t zero_distance_neighbours = 0;

static Fraction_as_String fraction_as_string;

// statistics of the nearest neighbour of each molecule

static Accumulator<double> nearest_neighbour_distance_stats;
...
//  Deprecated, seldom a reason to do this.
...
// When writing a TFDataRecord, adding the smiles of all the nearest neighbours
// can make the resulting file very large. We can optionally suppress adding the
// smiles of the neighbours.
static int include_neighbour_smiles_in_tfdata = 1;
...
class IW_GFP_D_ID : public IW_GFP_D {
 ...
 public:
  ...
  int index_in_pool() const {
    return _ndx;
  }

  int write_smiles_and_id(IWString&) const;

  void set_smiles(const const_IWSubstring& s) {
    _smiles = s;
  }
...
// The set of fingerprints that will be searched.
...
static int
WriteTfData(const IW_GFP_D_ID& target, IW_GFP_D_ID** neighbours, int number_neighbours,
  ...
  for (int i = 0; i < number_neighbours; ++i) {
    ...
    if (include_neighbour_smiles_in_tfdata) {
...
static int
write_neighbour_list(const IW_GFP_D_ID& target, IW_GFP_D_ID** neighbours,
                     int number_neighbours, IWString& output) {
...
insert_in_neighbour_list(IW_GFP_D_ID** neighbours, int neighbours_to_find,
                         int& neighbours_found, IW_GFP_D_ID& neighbour, int where) {
...
static void
do_neighbour_list_insertion(IW_GFP_D_ID** neighbours,
                            int neighbours_to_find_this_fingerprint,
                            int& neighbours_found, similarity_type_t t,
                            IW_GFP_D_ID& neighbour) {
  // Can we put it at the head of the list

  if (0 == neighbours_found || t <= neighbours[0]->distance()) {
    insert_in_neighbour_list(neighbours, neighbours_to_find_this_fingerprint,
                             neighbours_found, neighbour, 0);
    return;
  }
  ...
  if (t >= neighbours[neighbours_found - 1]->distance()) {
    ...
    insert_in_neighbour_list(neighbours, neighbours_to_find_this_fingerprint,
                             neighbours_found, neighbour, neighbours_found);
    return;
  }
  ...
  insert_in_neighbour_list(neighbours, neighbours_to_find_this_fingerprint,
                           neighbours_found, neighbour, middle + 1);
...
similarity_type_t
compute_the_distance(IW_GFP_D_ID& fp1, IW_GFP_D_ID& fp2, const Tversky& tversky) {
  if (tversky.active()) {
...
static int
do_rescan_for_not_enough_neighbours(IW_GFP_D_ID& fp, IW_GFP_D_ID** neighbours,
                                    int neighbours_to_find_this_fingerprint) {
  molecules_rescanned++;
  ...
  for (int i = 0; i < pool_size; i++) {
    ...
    if (0.0 == t && do_not_compare_molecules_with_themselves &&
        fp.id() ==
            pool[i].id()) {  // delay expensive and rare string comparison till here
      continue;
    }

    pool[i].set_distance(t);

    do_neighbour_list_insertion(neighbours, neighbours_to_find_this_fingerprint,
                                neighbours_found, t, pool[i]);
  }
...
// #define DEBUG_NN
    ...
    if (neighbours_found == neighbours_to_find_this_fingerprint &&
        t >= neighbours[neighbours_found - 1]->distance()) {
      continue;
    }

    // delay expensive and rare string comparison till here
    if (static_cast<similarity_type_t>(0.0) == t &&
        do_not_compare_molecules_with_themselves && fp.id() == pool[i].id()) {
      continue;
    }
  ...
  if (neighbours_found < rescan_if_not_enough_neighbours) {
    return do_rescan_for_not_enough_neighbours(fp, neighbours,
                                               rescan_if_not_enough_neighbours);
  }
...
nearneighbours(std::unique_ptr<iw_tf_data_record::TFDataWriter>& tfdata,
  ...
  IW_GFP_D_ID** neighbours = new IW_GFP_D_ID*[nbrs];
  std::unique_ptr<IW_GFP_D_ID*[]> free_neighbours(neighbours);
...
static int
do_write_histogram(std::ostream& output) {
  if (write_minimal_histogram) {
    return histogram_nearnest_neighbour_distances.write_terse(output);
  } else {
    return histogram_nearnest_neighbour_distances.write(output);
  }
}

static int
nearneighbours(int argc, char** argv) {
  Command_Line cl(argc, argv, "vs:n:I:t:T:P:F:W:Q:X:V:A:zr:hoK:xH:N:bypj:R:S:d");
  ...
  if (cl.option_present('r')) {
    if (!cl.value('r', rescan_if_not_enough_neighbours) ||
        rescan_if_not_enough_neighbours < 1) {
      cerr << "Invalid value for min number of neighbours (-r option)\n";
      usage(13);
    }

    if (verbose) {
      cerr << "Molecules will always have at least " << rescan_if_not_enough_neighbours
           << " neighbours\n";
    }
  }
  ...
  if (cl.option_present('X')) {
    if (!cl.value('X', abandon_distance_threshold) || abandon_distance_threshold < 0.0 ||
        abandon_distance_threshold > 1.0) {
  ...
  if (cl.option_present('S')) {
    ...
    if (verbose) {
      cerr << "Serialized nnbr::Nearneighbours protos written in TFDataRecord format to '"
           << fname << "'\n";
    }

    if (cl.option_present('d')) {
      include_neighbour_smiles_in_tfdata = 0;
      if (verbose) {
        cerr << "Will exclude neighbour smiles from TFDataRecord\n";
      }
    }
  }
  ...
  if (verbose) {
    ...
    cerr << closest_is_zero << " closest neighbour is zero\n";
    cerr << zero_distance_neighbours << " zero distance neighbours\n";

    if (molecules_rescanned) {
      cerr << molecules_rescanned << " molecules rescanned to get at least "
           << rescan_if_not_enough_neighbours << " neighbours\n";
    }
