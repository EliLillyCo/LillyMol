...
fpobj_spread(IWString_and_File_Descriptor &output) {
  ...
  while (number_selected < number_to_select) {
    ...
    if (verbose > 2) {
      cerr << molecules_skipped_for_being_too_close
           << " molecules_skipped_for_being_too_close\n";
    }
    ...
    if (!use_atom_count_window) {
      ...
      ;
    } else if (static_cast<double>(molecules_skipped_for_being_too_close) /
                   static_cast<double>(pool_size - number_selected) >
               0.8) {
      strikes++;
  ...
  if (cl.option_present('M')) {
    ...
    while (cl.value('M', m, i++)) {
      ...
      if ("recomp" == m) {
        ...
        if (verbose) {
          cerr << "Molecules with no initial nearest neighbour will be recomputed "
                  "without window\n";
        }
      } else if ("nscale" == m) {
