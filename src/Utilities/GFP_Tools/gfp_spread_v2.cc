...
//  Run this fingerprint against everything in the pool
  ...
  if (retest_no_neighbours)
    rescan_for_no_neighbours(input, filter);
                             ...
                             IW_TDT_Filter & filter)
{
  ...
  return establish_initial_distances(input, filter);
}
...
//cerr << " -A file,FILTER:  file name with filter to be applied to that file\n";
...
// Now that the pool is built, we can switch identifiers if needed
  ...
  {
    cerr << "The tdt filter option (-O) only works with a previously selected set (-A)\n";
    usage(18);
  }
...
// Each can have its own filter if the name starts with FILTER:
  ...
  if (cl.option_present('A'))
  {
    ...
    IW_TDT_Filter filter;
    if (cl.option_present('O'))     // the global filter
    {
      const_IWSubstring o = cl.string_value('O');
      if (! filter.build_from_string(o))
      {
        cerr << "Cannot initialise TDT filter from '" << o << "'\n";
        usage(19);
      }

      if (verbose)
      {
        cerr << "TDT filter initialised\n";
    ...
    {
      IW_TDT_Filter * filter_to_use = &filter;    // either the global one (-O option) or one specified with this file
      const_IWSubstring fname = a;     // by default

      IW_TDT_Filter myfilter;    // may not get initialised

      if (a.contains(",FILTER:"))
      {
        if (! parse_filter_and_file(a, fname, myfilter))
        {
          cerr << "Cannot parse filter/file name combination '" << fname << "'\n";
          usage(21);
        }

        filter_to_use = &myfilter;
      }

      if (! establish_initial_distances(fname, *filter_to_use))
      {
        cerr << "Cannot establish initial distances from '" << fname << "'\n";
        return 54;
      }

      if (verbose && &myfilter == filter_to_use)
        myfilter.report(cerr);
    }
