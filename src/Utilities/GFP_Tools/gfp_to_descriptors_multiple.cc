...
// Awkward use of global variable. Could be made local to the
// main function, but then some of the other functions end up with
// too many arguments.
std::unique_ptr<int[]> collision_count;
...
static int
discard_because_of_dash_y(const IWString& pcn, const int nbits) {
  // cerr << "cmp " << nbits << " with " << min_non_zero_values_needed_on_each_row <<
  // '\n';
...
static int
write_descriptors(const IWString& pcn, const int* tmp, const int number_descriptors,
...
rejected_by_upper_or_lower_population(unsigned int zbit, unsigned int zcount,
  ...
  if (verbose > 2) {
    cerr << "Fingerprint " << zbit << " in " << zcount << " molecules, ratio " << ratio
         << '\n';
  }
...
establish_header_and_columns_folded(
  ...
  if (verbose) {
    ...
    for (auto i = 0; i < fold_to_constant_width; ++i) {
      ...
      if (verbose > 1) {
        cerr << " " << to_col[i] << " non colliding bits hashed to bit " << i << '\n';
      }
    }

    cerr << zero_bits_map_here << " of " << fold_to_constant_width
         << " fixed width bits never set\n";
    cerr << "Of bits set, set between " << acc.minval() << " and " << acc.maxval()
  ...
  if (verbose) {
    report_bits_above_and_below_threshold(cerr, bits_below_threshold,
                                          bits_above_threshold);
  }
...
do_sort_by_frequency(const IW_Hash_Map<unsigned int, unsigned int>& fpcount,
  ...
  for (auto f = fpcount.cbegin(); f != fpcount.cend(); ++f) {
    if (rejected_by_upper_or_lower_population((*f).first, (*f).second, pool_size,
                                              bits_below_threshold,
                                              bits_above_threshold)) {
      continue;
    }
...
fingerprints_to_descriptors(const IW_General_Fingerprint* pool, const int pool_size,
  ...
  for (int i = 0; i < pool_size; ++i) {
    ...
    for (int j = 0; j < nfp; ++j) {
      // fixed fingerprints will have fingerprint numbers below 1000, sparse above
      if (fpnumber[j] < 1000) {
        nbits += fill_output_array(pool[i][fpnumber[j]], col[j], tmp);
      } else {
        nbits += fill_output_array(pool[i].sparse_fingerprint(fpnumber[j] - 1000), col[j],
                                   tmp);
      }
    }
...
static int
fingerprints_to_descriptors(const IW_General_Fingerprint* pool, const int pool_size,
                            const resizable_array_p<IWString>& fptag, const int* fpnumber,
                            IWString_and_File_Descriptor& output) {
  ...
  for (int i = 0; i < pool_size; ++i) {
    for (int j = 0; j < nfp; ++j) {
      // fixed fingerprints will have fingerprint numbers below 1000, sparse above
      if (fpnumber[j] < 1000) {
        scan_fixed_width_fingerprint(pool[i][fpnumber[j]], fpcount[j], nbits[j],
                                     *fptag[j]);
      } else {
        scan_non_colliding_fingerprint(pool[i].sparse_fingerprint(fpnumber[j] - 1000),
                                       fpcount[j]);
      }
    }
  }
  ...
  for (int i = 0; i < nfp; ++i) {
    if (0 == fpcount[i].size()) {
      cerr << "Very strange, no fingerprints counted for fingerprint " << i << " across "
           << pool_size << " fingerprints\n";
      return 0;
    }

    if (verbose) {
      cerr << "Found " << fpcount[i].size() << " bits set in " << *fptag[i]
           << " fingerprint\n";
    }
  }
  ...
  // cerr << "Building header, lower_support_cutoff_number " <<
  // lower_support_cutoff_number << " upper_support_cutoff_number " <<
  // upper_support_cutoff_number << ", fold_to_constant_width " << fold_to_constant_width
  // << '\n';

  if (sort_by_frequency) {
    for (int i = 0; i < nfp; ++i) {
      do_sort_by_frequency(fpcount[i], col[i], dprefix[i], number_descriptors, header,
                           pool_size);
    }
  } else if (fold_to_constant_width > 0) {
    establish_header_and_columns_folded(fold_to_constant_width, fpcount, nfp, col,
                                        dprefix, number_descriptors, header);
  } else if (bit_count_limits_set) {
    establish_header_and_columns_with_thresholds(fpcount, nfp, col, dprefix,
                                                 number_descriptors, header);
  } else {
    for (int i = 0; i < nfp; ++i) {
  ...
  return fingerprints_to_descriptors(pool, pool_size, nfp, fpnumber, col,
                                     number_descriptors, output);
}

static IW_General_Fingerprint*
read_fingerprints(iwstring_data_source& input, int& pool_size) {
...
static IW_General_Fingerprint*
read_fingerprints(const char* fname, int& pool_size) {
...
fingerprints_to_descriptors(int argc, char** argv) {
  ...
  if (cl.option_present('y')) {
    if (0 == support_level_specified) {
      cerr << "The -y option only makes sense when used with the -x,-n and/or -X,-N "
              "options\n";
      usage(4);
    }
    ...
    if (verbose) {
      cerr << "Will discard records that don't have "
           << min_non_zero_values_needed_on_each_row << " or more non-zero values\n";
    }
  ...
  if (cl.option_present('x')) {
    ...
    if (verbose) {
      cerr << "Will discard bits set less than " << lower_population_cutoff
           << " of the time\n";
    }
  } else if (cl.option_present('n')) {
  ...
  if (cl.option_present('X')) {
    ...
    if (verbose) {
      cerr << "Will discard bits set more than " << upper_population_cutoff
           << " of the time\n";
    }
  } else if (cl.option_present('N')) {
  ...
  std::unique_ptr<IW_General_Fingerprint[]> free_pool(pool);
  ...
  if (fptag.empty()) {  // consume all of them
  ...
  if (fptag.number_elements() > 1 && sort_by_frequency) {
    cerr << "WARNING, sorting by frequency with multiple fingerprints does not sort "
            "across fingeprints!\n";
  }

  // In order to maintain a single cross reference from both fixed and sparse
  // fingerprint numbers in the input to positions in the output, fixed
  // width fingerprints are assigned an index which is the same as their
  // fingerprint number, while sparse fingerprints get 1000 + their
  // fingerprint number.
  int* fpnumber = new_int(fptag.number_elements(), -1);
  std::unique_ptr<int[]> free_fpnumber(fpnumber);
  ...
  if (cl.option_present('d')) {
    ...
    collision_count.reset(new_int(fold_to_constant_width));
  }
  ...
  if (collision_count) {
    if (verbose) {
      ...
      cerr << bits_with_collisions << " of " << fold_to_constant_width
           << " bits with collisions (fraction "
           << (static_cast<float>(bits_with_collisions) /
               static_cast<float>(fold_to_constant_width))
           << ") max collision count " << max_collisions << '\n';
    }
  }
