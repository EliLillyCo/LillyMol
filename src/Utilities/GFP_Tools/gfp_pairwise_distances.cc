...
read_pool(iwstring_data_source& input) {
  ...
  if (static_cast<unsigned int>(pool_size) != id_to_ndx.size()) {
    cerr << "Huh, must be duplicate ID's, " << pool_size << " fingerprints, but "
         << id_to_ndx.size() << " items in ID hash\n";
    return 1;
  }
...
compute_the_distance(int ndx1, int ndx2, Tversky& tversky) {
  if (tversky.active()) {
    ...
    return static_cast<similarity_type_t>(1.0) -
           pool[ndx1].equal_weight_tanimoto(pool[ndx2]);
  } else {
...
static int
gfp_pairwise_distances(const char* fname, IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
gfp_pairwise_distances(int argc, char** argv) {
  ...
  if (cl.option_present('r')) {
    ...
    while (cl.value('K', k, i++)) {
      if (!k.contains(',')) {
        cerr << "Sorry, command line pairs must be comma separated '" << k
             << "' is invalid\n";
        rc = i + 1;
        break;
      }
  ...
  if (verbose) {
    if (duplicate_identifiers_encountered) {
      cerr << "Skipped " << duplicate_identifiers_encountered
           << " duplicate identifiers\n";
    }

    if (distance_acc.n() > 1) {
      cerr << distance_acc.n() << " distances between " << distance_acc.minval()
           << " and " << distance_acc.maxval() << " ave "
           << static_cast<float>(distance_acc.average()) << endl;
    }

    if (cl.option_present('T')) {
      cerr << discarded_by_upper_distance_threshold
           << " comparisons discarded by upper distance threshold "
           << upper_distance_threshold << endl;
    }
