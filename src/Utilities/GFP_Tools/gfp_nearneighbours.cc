...
static int
echo_tdt_items(IW_TDT& tdt, resizable_array_p<IWString>& items_to_echo,
               std::ostream& output) {
...
static int
write_neighbour_list(IW_TDT& target, IW_GFP_D_ID** neighbours, int number_neighbours,
                     std::ostream& output) {
...
// #define DEBUG_NN
    ...
    assert(t >= static_cast<similarity_type_t>(0.0) &&
           t <= static_cast<similarity_type_t>(1.0));
...
nearneighbours(iwstring_data_source& input, std::ostream& output) {
  ...
  while (tdt.next(input) && output.good()) {
    ...
    if (neighbours_to_find_tag.length()) {
      if (tdt.index_of_dataitem(neighbours_to_find_tag) <
          0) {  // no value specified, use the global default
        cerr << "No " << neighbours_to_find_tag << " tag present\n";
      } else if (!tdt.dataitem_value(neighbours_to_find_tag,
                                     neighbours_to_find_this_fingerprint)) {
        cerr << "Invalid neighbours to find value\n";
        cerr << tdt;
        return 0;
      } else if (neighbours_to_find_this_fingerprint > pool_size) {
        cerr << "Cannot find " << neighbours_to_find_this_fingerprint
             << " neighbours, only " << pool_size << " available\n";
        neighbours_to_find_this_fingerprint = pool_size;
      } else if (verbose > 1) {
        cerr << "Will find " << neighbours_to_find_this_fingerprint << " neighbours for '"
             << fp.id() << "'\n";
      }
    }
...
// clang-format off
  ...
  // cerr << " -e <dataitem>    target object dataitems to be echo'd (default $SMI and
  // PCN)\n"; cerr << " -e ALL           echo all dataitems in the input in target\n";
  // cerr << " -E <dataitem>    pool object dataitems to be echo'd (default $SMI and
...
nearneighbours(int argc, char** argv) {
  ...
  if (cl.option_present('N')) {
    ...
    if (verbose) {
      cerr << "The number of neighbours to find will be from the '"
           << neighbours_to_find_tag << "' dataitem\n";
    }
