...
int
Tversky::parse_command_line(Command_Line& cl, char aflag, char bflag, int verbose) {
...
int
Tversky::parse_command_line(Command_Line& cl, char flag, int verbose) {
  ...
  if (_a >= 0.0 && _b >= 0.0)  // great, both specified
  {
    ...
    cerr << "tversky::parse_command_line: incomplete specification a = " << _a
         << " b = " << _b << "\n";
    return 0;
  }
...
similarity_type_t
Tversky::tanimoto(int nset1, int nset2, int bic) const {
  //  cerr << "Tversky::tanimoto " << nset1 << ' ' << nset2 << " bic " << bic << '\n';
  if (0 == nset1 || 0 == nset2) {
    if (0 == nset1 && 0 == nset2) {
    ...
    if (nset1) {
      return iwmisc::Fraction<similarity_type_t>(1, nset1 + 1);
    } else {
      return iwmisc::Fraction<similarity_type_t>(1, nset2 + 1);
    }
  }
