...
static int nearneighbours (const const_IWSubstring & fname, IW_TDT_Filter & filter, IWString_and_File_Descriptor & output);
                              ...
                              IWString_and_File_Descriptor & output)
{
  ...
  while (input.next_record(buffer))
  {
    ...
    if (! nearneighbours(buffer, filter, output))
    {
                ...
                IWString_and_File_Descriptor & output)
{
  ...
  if (fname.starts_with("F:"))
    return nearneighbours_list_of_files(input, filter, output);

  if (verbose)
    cerr << "Processing '" << fname << "'\n";

  return nearneighbours(input, filter, output);
}
...
usage (int rc)
{
  ...
  cerr << "                  the filter is applied to the files on the command line\n";
...
{
  Command_Line cl (argc, argv, "vs:n:p:I:i:e:t:T:r:F:P:W:O:Q:V:hB:K:D:N:m:g:E:d");
...
// that the pool is built first
  ...
  if (cl.option_present('O'))
  {
    ...
    if (verbose)
      cerr << "Will filter input TDT's according to filter\n";
  }
...
// If they didn't ask for anything to be echo'd, just use defaults
  ...
  {
    (void) nearneighbours(cl[i], filter, output);
  }
