...
class Classification_Data {
 ...
 public:
  ...
  int number_classes() const {
    return _class_label_for_class_number.number_elements();
  }

  const int *items_in_class() const {
    return _items_in_class.rawdata();
  }
};

int
Classification_Data::operator()(const const_IWSubstring &token, int &c) {
...
/*
  For each bit, we need information about how many times it was assigned to the various
  classes
*/
...
class Class_Membership {
 ...
 public:
  ...
  void class_membership_ratios(const Classification_Data &classification_data, int n,
                               float *r) const;
};
...
Class_Membership::class_membership_ratios(const Classification_Data &classification_data,
  ...
  for (int i = 0; i < _number_classes; i++) {
    double probability_this_class =
        static_cast<double>(iic[i]) / static_cast<double>(pool_size);

    double pcorr =
        static_cast<double>(_in_class[i] + 1) / (probability_this_class * _n + 1.0);
...
int
Class_Membership::report(int n, IWString_and_File_Descriptor &output) const {
  if (_n == n) {
    output << " constant\n";
    return 1;
  }
  ...
  for (int i = 0; i < _number_classes; i++) {
    ...
    //  pcorr = static_cast<double>(_in_class[i] + 1) / static_cast<double>(_n + k) /
    //  probability_active;
    double pcorr =
        static_cast<double>(_in_class[i] + 1) / (probability_this_class * _n + 1.0);
    ...
    pcorr = log2(pcorr);
    //  pcorr = static_cast<double>(_in_class[i] + probability_active * 10) /
    //  static_cast<double>(_n + 10) / probability_active;
...
static int
write_table_output(IWString_and_File_Descriptor &output, unsigned int b, int times_hit,
                   float ave) {
  output << bit_prefix << b << " hit " << times_hit << " ave " << ave << " diff "
         << (ave - average_activity_of_whole_collection) << '\n';
...
write_pair_of_activities(float act1, float act2, IWString_and_File_Descriptor &output) {
  output << " diff ";
...
static int
build_bit_cross_reference_meeting_support(
    int f, IW_Hash_Map<unsigned int, unsigned int> &bit_to_sequence,
  ...
  for (IW_Hash_Map<unsigned int, unsigned int>::iterator i = molecules_containing.begin();
  ...
  if (verbose) {
    cerr << "Suppressed " << suppressed_by_support_requirement << " of "
         << molecules_containing.size() << " bits due to support requirements\n";
  }
...
// Profile sparse fingerprint number `f`.
static int
build_bit_cross_reference(int f, IW_Hash_Map<unsigned int, unsigned int> &bit_to_sequence,
                          resizable_array<unsigned int> &bit_number) {
  for (int i = 0; i < pool_size; i++) {
    const IW_General_Fingerprint &fpi = pool[i];
...
sort_bits_by_activity(const activity_type_t *activity_with_bit,
                      const resizable_array<int> *items_with_bit, const int nbits,
                      int *bits_ordered_by_activity) {
  typedef std::pair<int, activity_type_t> IA;

  IA *p = new IA[nbits];
  std::unique_ptr<IA[]> free_p(p);
  ...
  std::sort(p, p + nbits,
            [](const IA &lhs, const IA &rhs) { return lhs.second > rhs.second; });
...
class Subset_of_Bits {
 ...
 public:
  ...
  int nbits() const {
    return _bit.number_elements();
  }

  int compare_ad_swap_if_better(const Subset_of_Bits &rhs);

  const resizable_array<int> &bits() const {
    return _bit;
  }
  ...
  activity_type_t average_activity() const {
    return _average_activity;
  }

  int initialise(const int pool_size, const int nsel);
  int initialise(const int *bits_ordered_by_activity, const int pool_size,
                 const int nsel);

  activity_type_t compute_activity(const resizable_array<int> *items_with_bit,
                                   const activity_type_t *activity_with_bit);
  activity_type_t compute_activity_slow(const resizable_array<int> *items_with_bit,
                                        const activity_type_t *activity);

  activity_type_t recompute_average_activity_if_needed(
      const resizable_array<int> *items_with_bit, const activity_type_t *activity);

  void lose_bit();
  void gain_bit(const int nbits);

  void add_bit(const int b) {
    _bit.add(b);
  }
...
Subset_of_Bits::compute_activity_slow(const resizable_array<int> *items_with_bit,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    //  cerr << " bit " << b << " had " << wb.number_elements() << " molecules hitting the
    //  bit, ave act " << (sum_activities(wb, activity)/nb) << endl;
  }
...
static int
check_accsave(const activity_type_t *activity_with_bit, const int nbits) {
  int rc = 1;
  ...
  for (int i = 0; i < nbits; ++i) {
    ...
    cerr << "check_accsave:bit " << i << " changed, was " << accsave[i] << " now "
         << activity_with_bit[i] << endl;
    abort();
    rc = 0;
  }
...
Subset_of_Bits::compute_activity(const resizable_array<int> *items_with_bit,
  ...
  cerr << "Computing activity with subset with " << _bit.number_elements()
       << " bits set\n";
  ...
  for (int i = 0; i < n; ++i) {
    ...
    //  cerr << "Fetched activity for items with bit " << bb[i] << " extra " << xx << "
    //  sum " << tot << " count " << count << endl;
  }
...
int
Subset_of_Bits::initialise(const int pool_size, const int nsel) {
  int rc = 0;
...
int
Subset_of_Bits::initialise(const int *bits_ordered_by_activity, const int nbits,
                           const int nsel) {
  int rc = 0;
  std::array<double, 2> intervals{0.0, nbits - 2.0};
  std::array<double, 2> weights{1.0, 0.0};

  std::piecewise_linear_distribution<double> u(intervals.begin(), intervals.end(),
                                               weights.begin());
...
static void
remove_dups(resizable_array<int> &bits, std::mt19937_64 &rng,
            const int min_bits_remaining_in_a_subset, const int nbits) {
  int *b = bits.rawdata();
...
Subset_of_Bits::gain_bit(const int nbits) {
  ...
  for (int i = 0; i < 100;
       ++i)  // really should be a while(1) loop, but just being careful
  {
    int b = u(_rng);
    if (_bit.contains(b)) {
      continue;
...
activity_type_t
Subset_of_Bits::recompute_average_activity_if_needed(
...
sum_activity_with_bit(const resizable_array<int> &items_with_bit,
  ...
  for (int i = 0; i < n; ++i) {
    //  cerr << "sum_activity_with_bit i = " << i << " bit " << items_with_bit[i] << " act
    //  " << activity[items_with_bit[i]] << endl;
...
template <typename T>
int
identify_molecules_containing_bits_fixed(
    const IW_General_Fingerprint *pool, const int pool_size, const int fpnum,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (f == local_bit_to_global_bit.end()) {
      cerr << "identify_molecules_containing_bits:yipes, did not find bit " << b
           << " in local_bit_to_global_bit hash\n";
      return 0;
    }
...
static int
identify_molecules_containing_bits_sparse(
    const IW_General_Fingerprint *pool, const int pool_size, const int fpnum,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (c.number_elements() > 0) {
      cerr << "Yipes, processing sparse bit " << b << " in fingerprint " << fpnum
           << " alread have " << c.number_elements() << " items counted\n";
      return 0;
    }
    ...
    //  cerr << "In sparse fp " << fpnum << " found " << c.number_elements() << "
    //  molecules with bit " << b << endl;
  }
...
sparse_bits_meeting_support_levels(const IW_General_Fingerprint *pool,
                                   const int pool_size, const int fpnum,
...
static int
fixed_bits_meeting_support_levels(const IW_General_Fingerprint *pool, const int pool_size,
                                  const int fpnum, resizable_array<int> &bits) {
  const int nbits = pool[0][fpnum].nbits();

  int *is_set = new_int(nbits);
  std::unique_ptr<int[]> free_is_set(is_set);
...
setup_index_arrays(const int fpnum,
                   const resizable_array<T>
                       &bits,  // the bits already found to meet the support requirements
                   unsigned int *global_bit_to_local_bit, int *global_bit_to_fpnum,
                   std::unordered_map<T, int> &local_bit_to_global_bit, int &ndx) {
  const int n = bits.number_elements();
...
static int
bits_meeting_support_level(const IW_General_Fingerprint *pool, const int pool_size,
...
static void
recursive_enumeratin(const int b, const int nbits, Subset_of_Bits &sb,
                     const activity_type_t *activity_with_bit,
                     const resizable_array<int> *items_with_bit, const int depth,
                     const int max_depth, Accumulator<activity_type_t> *acc,
                     Subset_of_Bits *best, IWString_and_File_Descriptor &output) {
  for (int i = b; i < nbits; ++i) {
    ...
    if (depth < max_depth) {
      recursive_enumeratin(i + 1, nbits, sb, activity_with_bit, items_with_bit, depth + 1,
                           max_depth, acc, best, output);
    }
    sb.pop_bit();
    if (0 == depth) {
      cerr << "Top level completed bit " << i << " of " << nbits << " bits\n";
      for (int i = 0; i <= max_depth; ++i) {
        cerr << " depth " << i << " max " << acc[i].maxval() << " N = " << acc[i].n()
             << endl;
      }
...
static int
remove_some_dups(Subset_of_Bits **sbp, const int nsubset, const int nbits,
                 std::uniform_int_distribution<int> &initial_bits, std::mt19937_64 &rng) {
  int rc = 0;
...
static int
multi_bit_combinations(const IW_General_Fingerprint *pool, const int pool_size,
                       const activity_type_t *activity,
  ...
  resizable_array<int> *fixed = new resizable_array<int>[nfixed];
  std::unique_ptr<resizable_array<int>[]> free_fixed(fixed);
  resizable_array<unsigned int> *sparse = new resizable_array<unsigned int>[nsparse];
  std::unique_ptr<resizable_array<unsigned int>[]> free_sparse(sparse);
  ...
  if (verbose) {
    cerr << "Across " << nfixed << " fixed and " << nsparse
         << " sparse fingerprints, find " << nbits
         << " bits meeting support requirements\n";
  }
  ...
  // We need data structures that can map a bit in the local fingerprint to where that bit
  // is in the global array

  std::unordered_map<int, int> *local_bit_to_global_bit_fixed =
      new std::unordered_map<int, int>[nfixed];
  std::unique_ptr<std::unordered_map<int, int>[]> free_local_bit_to_global_bit_fixed(
      local_bit_to_global_bit_fixed);
  std::unordered_map<unsigned int, int> *local_bit_to_global_bit_sparse =
      new std::unordered_map<unsigned int, int>[nsparse];
  std::unique_ptr<std::unordered_map<unsigned int, int>[]>
      free_local_bit_to_global_bit_sparse(local_bit_to_global_bit_sparse);

  // for each bit, which molecules have that bit

  resizable_array<int> *items_with_bit = new resizable_array<int>[nbits];
  std::unique_ptr<resizable_array<int>[]> free_items_with_bit(items_with_bit);

  // for each bit, what is the bit number in the initial fingerprint

  unsigned int *global_bit_to_local_bit = new unsigned int[nbits];
  std::unique_ptr<unsigned int[]> free_global_bit_to_local_bit(global_bit_to_local_bit);

  // for each bit, from which fingerprint did it come

  int *global_bit_to_fpnum = new int[nbits];
  std::unique_ptr<int[]> free_global_bit_to_fpnum(global_bit_to_fpnum);

  int ndx = 0;

  for (int i = 0; i < nfixed; ++i) {
    setup_index_arrays(i, fixed[i], global_bit_to_local_bit, global_bit_to_fpnum,
                       local_bit_to_global_bit_fixed[i], ndx);
  }

  for (int i = 0; i < nsparse; ++i) {
    setup_index_arrays(i, sparse[i], global_bit_to_local_bit, global_bit_to_fpnum,
                       local_bit_to_global_bit_sparse[i], ndx);
  }

  for (int i = 0; i < nfixed; ++i) {
    identify_molecules_containing_bits_fixed(
        pool, pool_size, i, fixed[i], local_bit_to_global_bit_fixed[i], items_with_bit);
  }

  for (int i = 0; i < nsparse; ++i) {
    identify_molecules_containing_bits_sparse(
        pool, pool_size, i, sparse[i], local_bit_to_global_bit_sparse[i], items_with_bit);
  }
  ...
  for (int i = 0; i < nbits; ++i) {
    ...
    for (unsigned int j = 0; j < x.size(); ++j) {
      if (x[j] < 0 || x[j] >= pool_size) {
        cerr << "Possibly invalid items_with_bit value, i = " << i << " j = " << j
             << " value " << x[j] << endl;
      }
    }
  }

  activity_type_t *activity_with_bit = new activity_type_t[nbits];
  std::unique_ptr<activity_type_t[]> free_activity_with_bit(activity_with_bit);
  ...
  int *include_bit = new_int(nbits, 1);
  std::unique_ptr<int[]> free_include_bit(include_bit);
  ...
  for (int b = 0; b < nbits; ++b) {
    ...
    if (fabs(normalised - 0.5f) < normalised_activity_cutoff) {
      ...
      bits_included++;
      cerr << "Bit " << b << " ave activity " << activity_with_bit[b] << " norm "
           << normalised << endl;
    }
  }

  int *bits_ordered_by_activity = new int[nbits];
  std::unique_ptr<int[]> free_bits_ordered_by_activity(bits_ordered_by_activity);
  sort_bits_by_activity(activity_with_bit, items_with_bit, nbits,
                        bits_ordered_by_activity);
  ...
  if (verbose) {
    cerr << "Based on minimum normalised activity threshold "
         << normalised_activity_cutoff << " left with " << bits_included << " bits\n";
  }
  ...
  Subset_of_Bits *sb = new Subset_of_Bits[nsubset];
  std::unique_ptr<Subset_of_Bits[]> free_sb(sb);

  Subset_of_Bits **sbp = new Subset_of_Bits *[nsubset];
  std::unique_ptr<Subset_of_Bits *[]> free_sbp(sbp);
  ...
  std::sort(sbp, sbp + nsubset, [](const Subset_of_Bits *sb1, const Subset_of_Bits *sb2) {
    return sb1->average_activity() > sb2->average_activity();
  });
  ...
  if (verbose) {
    ...
    cerr << "Across " << nsubset << " random subsets, activities between " << acc.minval()
         << " and " << acc.maxval() << " ave " << static_cast<float>(acc.average())
         << endl;
  }
  ...
  if (max_recursion_depth > 0) {
    Accumulator<activity_type_t> *acc =
        new Accumulator<activity_type_t>[max_recursion_depth + 1];
    std::unique_ptr<Accumulator<activity_type_t>[]> free_acc(acc);
    Subset_of_Bits *best = new Subset_of_Bits[max_recursion_depth + 1];
    std::unique_ptr<Subset_of_Bits[]> free_best(best);

    Subset_of_Bits s;
    recursive_enumeratin(0, nbits, s, activity_with_bit, items_with_bit, 0,
                         max_recursion_depth, acc, best, output);
    cerr << "Following recursive exhaustive enumeration\n";
    for (int i = 0; i <= max_recursion_depth; ++i) {
      cerr << (i + 1) << " bits btw " << acc[i].minval() << " and " << acc[i].maxval()
           << " ave " << acc[i].average() << endl;
  ...
  for (unsigned int i = 0; i < istop; ++i) {
    ...
    for (int j = 0; j < nsubset; ++j) {
      acc2.extra(
          sb[j].recompute_average_activity_if_needed(items_with_bit, activity_with_bit));
    }

    std::sort(sbp, sbp + nsubset,
              [](const Subset_of_Bits *sb1, const Subset_of_Bits *sb2) {
                return sb1->average_activity() > sb2->average_activity();
              });
    ...
    if (0 == i % 1000) {
      ...
      if (search_duration > 0 && i > 0) {
        ...
        cerr << i << " iterations, now " << tnow << " t0 " << tzero << " diff "
             << (tnow - tzero) << endl;
    ...
    for (int j = 0; j < nbits; ++j) {
      if (accsave[j] != activity_with_bit[j]) {
        cerr << "Cycle " << i << " value " << j << " changed, old " << accsave[j]
             << " new " << activity_with_bit[j] << endl;
      }
    }
#endif
  }

  cerr << "Found " << acc2.n() << " values, btw " << acc2.minval() << " and "
       << acc2.maxval() << " ave " << static_cast<float>(acc2.average()) << endl;
  for (int i = 0; i < nsubset; ++i) {
...
do_write_data_for_gfp_adjust(const Classification_Data &classification_data,
  ...
  float *rtmp = new float[number_classes];
  std::unique_ptr<float[]> free_rtmp(rtmp);

  Ndx_and_Ratio *nr = new Ndx_and_Ratio[nbits];
  std::unique_ptr<Ndx_and_Ratio[]> free_nr(nr);
...
static int
profile_sparse_fingerprint(int f, const int *activity,
                           IWString_and_File_Descriptor &output) {
  ...
  Class_Membership *cm = new Class_Membership[nbits];
  std::unique_ptr<Class_Membership[]> free_cm(cm);
...
// Profile sparse fingerprint number `f`.
// For each member of `pool`, the activity is in `activity[i]`.
// Results written to `output`.
static int
profile_sparse_fingerprint(int f, const activity_type_t *activity,
                           IWString_and_File_Descriptor &output) {
  IW_Hash_Map<unsigned int, unsigned int> bit_to_sequence;
  resizable_array<unsigned int> bit_number;
  ...
  std::unique_ptr<Accumulator<activity_type_t>[]> free_acc_set(acc_set);
  std::unique_ptr<Accumulator<activity_type_t>[]> free_acc_nset(acc_nset);

  int *set_this_molecule = new int[nbits];
  std::unique_ptr<int[]> free_set_this_molecule(set_this_molecule);
  ...
  for (int i = 0; i < nbits; i++) {
    ...
    //  cerr << bit_number[i] << " n = " << acc.n() << " min " << acc.minval() << " max "
    //  << acc.maxval() << endl;
    ...
    if (0 == acc.n()) {  // no range to show
      ...
      output << " Nset " << acc_nset[i].n() << " " << acc_nset[i].minval() << " to "
             << acc_nset[i].maxval() << " ave "
             << static_cast<float>(acc_nset[i].average());
    ...
    if (stream_for_table_output.is_open()) {
      write_table_output(stream_for_table_output, bit_number[i], acc.n(),
                         acc.average_if_available_minval_if_not());
    }
  }

  if (verbose) {
    cerr << "Average activities between " << ave_act_acc.minval() << " and "
         << ave_act_acc.maxval() << "\n";
  }
...
static int
profile_fingerprint(int f, const int *c, IWString_and_File_Descriptor &output) {
...
static int
profile_fingerprint(int f, const activity_type_t *activity,
                    IWString_and_File_Descriptor &output) {
  const IWDYFP &fp = pool[0].item(f);
  int nbits = fp.nbits();
  ...
  for (int i = 0; i < nbits; i++) {
    ...
    double aset, anset;
    aset = 0.0;  // keep the compiler quiet.
    anset = 0.0;   // keep the compiler quiet.
    ...
    if (constant_bit) {
      ...
      if (stream_for_table_output.is_open()) {
        write_table_output(stream_for_table_output, i, s.n(),
                           s.average_if_available_minval_if_not());
      }
    }
...
template <typename T>
int
get_experimental_data_from_identifier(int experimental_column, T *activity) {
  for (int i = 0; i < pool_size; i++) {
...
static int
add_to_hash(IW_STL_Hash_Set &identifiers_in_pool, const IWString &id) {
...
static int
in_hash(const IW_STL_Hash_Set &identifiers_in_pool, const IWString &query) {
...
template <typename T>
int
get_activity(const IW_STL_Hash_Map<IWString, T> &id_2_act, const IWString &id, T &act) {
...
int
read_experimental_data_from_file(iwstring_data_source &input, int experimental_column,
                                 T *activity, C &conv) {
  ...
  if (verbose) {
    cerr << "Read " << identifiers_in_pool.size()
         << " identifiers from input fingerprint file\n";
  }
  ...
  while (input.next_record(buffer)) {
    ...
    if (experimental_column < 0) {
      ...
      if (!buffer.word(experimental_column, token)) {
        cerr << "Cannot extract column " << (experimental_column + 1) << " from '"
             << buffer << "'\n";
        return 0;
      }
    }
  ...
  if (id_2_act.size() < static_cast<unsigned int>(pool_size)) {
    cerr << "Read " << id_2_act.size() << " activity values, but " << pool_size
         << " fingerprints, cannot continue\n";
    return 0;
  }
...
template <typename T, typename C>
int
read_experimental_data_from_file(const const_IWSubstring &fname, int experimental_column,
                                 T *activity, C &conv) {
...
static int
report_class_memberships(const int *c, int pool_size, std::ostream &os) {
  ...
  int *items_in_class = new_int(max_class_number - min_class_number + 1);
  std::unique_ptr<int[]> free_iic(items_in_class);
...
static int
do_classification(const IWString &experimental_file_name, int experimental_column,
                  IWString_and_File_Descriptor &output) {
  int *c = new int[pool_size];
  std::unique_ptr<int[]> free_c(c);

  int rc = 0;
  if (experimental_file_name.length()) {
    rc = read_experimental_data_from_file(experimental_file_name, experimental_column, c,
                                          classification_data);
...
static int
read_smiles_record(const const_IWSubstring &buffer, IW_STL_Hash_Map_String &smiles) {
  if (buffer.nwords() < 2) {
...
static int
read_smiles(iwstring_data_source &input, IW_STL_Hash_Map_String &smiles) {
  const_IWSubstring buffer;
...
static int
read_smiles(const char *fname, IW_STL_Hash_Map_String &smiles) {
  iwstring_data_source input(fname);
...
gfp_profile_activity(int argc, char **argv) {
  ...
  if (cl.option_present('r')) {
    ...
    if (verbose) {
      cerr << "Will report pcorr values outside range " << upper_pcorr_report_threshold
           << endl;
    }
  ...
  if (cl.option_present('u')) {
    ...
    if (verbose) {
      cerr << "Will only output bits with a minimum support level of "
           << min_support_level << " compounds hit\n";
    }
  }
  ...
  if (cl.option_present('n')) {
    ...
    if (verbose) {
      cerr << "Will write the " << write_most_informative_bits
           << " most informative bits only\n";
    }
  }
  ...
  if (cl.option_present('E')) {
    ...
    while (cl.value('E', e, i++)) {
      if (e.starts_with("col=")) {
        ...
        if (verbose) {
          cerr << "Experimental result is column " << experimental_column
               << " of the identifier\n";
        }
  ...
  activity_type_t *activity = new activity_type_t[pool_size];
  std::unique_ptr<activity_type_t[]> free_activity(activity);

  int rc;
  if (experimental_file_name.length()) {
    rc = read_experimental_data_from_file(experimental_file_name, experimental_column,
                                          activity, continuous_response);
  } else if (experimental_column > 0) {
  ...
  if (verbose) {
    cerr << "Activity values between " << acc.minval() << " and " << acc.maxval()
         << " ave " << acc.average() << '\n';
  }
