...
int
initialise_entropy_weighting_for_fingerprints(Command_Line& cl, char flag, int verbose) {
...
/*
  When we read fingerprints in Daylight ascii representation, the counts are truncated at
  255. But if we read the ascii form 'b,c', then counts are not limited This is no longer
  needed, the bic table is not used in Sparse_Fingerprint.
*/
...
static int default_multiconformer_fixed_size_counted_distance_metric =
    DENSE_DISTANCE_METRIC_TANIMOTO;
static int default_multiconformer_sparse_distance_metric =
    SPARSE_DISTANCE_METRIC_TANIMOTO;
...
static int _continuous_property_distance_metric =
    CONTINUOUS_PROPERTY_DISTANCE_METRIC_RATIO;
...
static int default_fixed_size_counted_fingerprint_distance_metric =
    FIXED_SIZE_COUNTED_FINGERPRINT_DISTANCE_METRIC_TANIMOTO;
                  ...
                  int mcsparse)  // multiconformer, sparse
{
  ...
  if (cfp_to_allocate > 0) {
    ...
    if (nullptr == _fixed_size_counted_fingerprint_tag) {
      cerr << "set_nfingerprints: " << cfp_to_allocate
           << " fixed size counted fingerprints, memory failure\n";
      return 0;
    }

    for (int i = 0; i < cfp_to_allocate; i++) {
      _fixed_size_counted_fingerprint_weight[i] =
          1.0 / static_cast<float>(total_fingerprints);
      _fixed_size_counted_fingerprint_distance_metric[i] =
          default_fixed_size_counted_fingerprint_distance_metric;
    }
  ...
  if (mc01 > 0) {
    ...
    if (nullptr == _multiconformer_01_fingerprint_tag) {
      cerr << "set_nfingerprints: " << mc01
           << " multiconformer 0/1 fingerprints, memory failure\n";
      return 0;
    }
  ...
  if (mcfsc > 0) {
    ...
    if (nullptr == _multiconformer_fixed_size_counted_fingerprint_tag) {
      cerr << "set_nfingerprints: " << mcfsc
           << " multiconformer fixed size counted fingerprints, memory failure\n";
      return 0;
    }

    for (int i = 0; i < mcfsc; i++) {
      _multiconformer_fixed_size_counted_weight[i] = 1.0 / float(total_fingerprints);
      _multiconformer_fixed_size_counted_distance_metric[i] =
          default_multiconformer_fixed_size_counted_distance_metric;
    }
  ...
  if (mcsparse > 0) {
    ...
    if (nullptr == _multiconformer_sparse_fingerprint_tag) {
      cerr << "set_nfingerprints: " << mcsparse
           << " multiconformer sparse fingerprints, memory failure\n";
      return 0;
    }

    for (int i = 0; i < mcsparse; i++) {
      _multiconformer_sparse_weight[i] = 1.0 / float(total_fingerprints);
      _multiconformer_sparse_distance_metric[i] =
          default_multiconformer_sparse_distance_metric;
    }
...
static IWString fingerprint_rx_default(
    "^(FP[A-Z,0-9]+|HX[A-Z,0-9]+|NC[A-Z,0-9]+|FC[A-Z,0-9]+|MC[A-Z,0-9]+)<");
...
set_fingerprint_regular_expression(const const_IWSubstring& pattern) {
  if (!pattern.starts_with('^')) {
    cerr << "set_fingerprint_regular_expression: WARNING, rx does not start with '^' :"
         << pattern << '\n';
  }
...
print_tags_and_weights(std::ostream& os) {
  for (int i = 0; i < number_fingerprints_to_use_in_computations; i++) {
    os << "Fingerprint " << i << " '" << _fingerprint_tag[i] << "' weight "
       << _fingerprint_weight[i];
  ...
  if (convert_sparse_fingerprint_to_fixed_width_counted ||
      convert_sparse_fingerprint_to_bits) {
    ;
  } else {
    for (int i = 0; i < _number_sparse_fingerprints; i++) {
      os << "Non colliding fingerprint " << i << " '" << _sparse_fingerprint_tag[i]
         << "' weight " << _sparse_fingerprint_weight[i];
  ...
  for (int i = 0; i < number_fixed_size_counted_fingerprints_to_use_in_computations;
       i++) {
    os << "Fixed counted fingerprint " << i << " '"
       << _fixed_size_counted_fingerprint_tag[i] << "' weight "
       << _fixed_size_counted_fingerprint_weight[i] << '\n';
  }

  for (int i = 0; i < number_multiconformer_01_fingerprints_in_file; i++) {
    os << "Multiconformer 01 " << i << " '" << _multiconformer_01_fingerprint_tag[i]
       << "' weight " << _multiconformer_01_weight[i] << '\n';
  }

  for (int i = 0; i < number_multiconformer_fixed_size_counted_fingerprints_in_file;
       i++) {
    os << "Multiconformer FSC " << i << " '"
       << _multiconformer_fixed_size_counted_fingerprint_tag[i] << "' weight "
       << _multiconformer_fixed_size_counted_weight[i] << '\n';
  }

  for (int i = 0; i < number_multiconformer_sparse_fingerprints_in_file; i++) {
    os << "Multiconformer SPARSE " << i << " '"
       << _multiconformer_sparse_fingerprint_tag[i] << "' weight "
       << _multiconformer_sparse_weight[i] << '\n';
  }

  if (molecular_property_integer_tag.length()) {
    os << "Property weight " << _property_weight_integer << '\n';
  }

  if (molecular_property_continuous_tag.length()) {
    os << "Continuous property weight " << _property_weight_continuous << '\n';
  }
...
Molecular_Properties_Continuous::Molecular_Properties_Continuous() {
  if (number_continuous_molecular_properties <= 0) {
    ...
    if (nullptr == _property) {
      cerr << "Molecular_Properties_Continuous::cannot allocate space for "
           << _nproperties << " properties\n";
    }
  }
...
int
Molecular_Properties_Continuous::construct_from_descriptor_record(
  ...
  if (ALL_CONTINUOUS_MOLECULAR_PROPERTIES == number_continuous_molecular_properties) {
    ...
    cerr << "Molecular_Properties::construct_from_descriptor_record: "
         << number_continuous_molecular_properties << " descriptors in input\n";
  }
  ...
  for (int i = 0; i < number_continuous_molecular_properties; i++) {
    ...
    if (!tmp.nextword(token, j)) {
      cerr << "Molecular_Properties::construct_from_descriptor_record: insufficient "
              "tokens, i = "
           << i << "\n";
      cerr << tmp << '\n';
      return 0;
    ...
    if (!token.numeric_value(_property[i])) {
      cerr << "Molecular_Properties::construct_from_descriptor_record: invalid token '"
           << token << "'\n";
      return 0;
    }
...
int
Molecular_Properties_Integer::construct_from_tdt_fp_record(
    const const_IWSubstring& buffer) {
  IWDYFP fp;

  if (!fp.construct_from_tdt_record(buffer)) {
    cerr << "Molecular_Properties::construct_from_tdt_record: cannot parse '" << buffer
         << "' as FP\n";
    return 0;
  }
...
Molecular_Properties_Integer::dice_coefficient(
    const Molecular_Properties_Integer& rhs) const {
  ...
  return static_cast<float>(bits_in_common) /
         static_cast<float>(na + nb - bits_in_common);
}

int
Molecular_Properties_Integer::bits_in_common(
    const Molecular_Properties_Integer& rhs) const {
  int bic = 0;
...
similarity_type_t
Molecular_Properties_Continuous::similarity(
    const Molecular_Properties_Continuous& rhs) const {
  assert(nullptr != _property && nullptr != rhs._property);

#ifdef ECHO_PROPERTIES
  for (int i = 0; i < _nproperties; i++) {
    cerr << " i = " << i << ' ' << _property[i] << " " << rhs._property[i] << '\n';
  }
#endif
  ...
  for (int i = 1; i < _nproperties; i++) {
    if (_property[i] == rhs._property[i]) {
      rc += static_cast<similarity_type_t>(1.0);
    } else if (static_cast<float>(0.0) == _property[i] ||
               static_cast<float>(0.0) == rhs._property[i]) {
  ...
  if (rc < static_cast<similarity_type_t>(0.0) ||
      rc > static_cast<similarity_type_t>(1.0)) {
    cerr << "Molecular_Properties::distance: properties sum out of range " << rc << '\n';
    for (int i = 0; i < _nproperties; i++) {
...
similarity_type_t
Molecular_Properties_Continuous::cartesian_distance(
    const Molecular_Properties_Continuous& rhs) const {
  similarity_type_t rc = static_cast<similarity_type_t>(0.0);

  for (int i = 0; i < _nproperties; i++) {
    rc += (_property[i] - rhs._property[i]) * (_property[i] - rhs._property[i]);
    //  cerr << _property[i] << " and " << rhs._property[i] << " rc = " << rc << '\n';
  }

  // cerr << "Sum over " << _nproperties << " properties, rc = " << rc << '\n';

  return sqrt(rc);
}

similarity_type_t
Molecular_Properties_Continuous::exp1_distance(
    const Molecular_Properties_Continuous& rhs) const {
  double rc = 0.0;

  for (int i = 0; i < _nproperties; i++) {
    rc += exp(continuous_property_distance_metric_exp1_value *
              fabs(_property[i] - rhs._property[i]));
  }
...
similarity_type_t
Molecular_Properties_Continuous::exp2_distance(
    const Molecular_Properties_Continuous& rhs) const {
  double rc = 0.0;

  for (int i = 0; i < _nproperties; i++) {
    rc +=
        exp(continuous_property_distance_metric_exp2_value *
            fabs((_property[i] - rhs._property[i]) * (_property[i] - rhs._property[i])));
  }
...
similarity_type_t
Molecular_Properties_Continuous::dice_coefficient(
    const Molecular_Properties_Continuous& rhs) const {
...
IW_General_Fingerprint::tanimoto(IW_General_Fingerprint& rhs) {
  ...
  for (int i = 0; i < number_fingerprints_to_use_in_computations; i++) {
...
#ifdef DEBUG_TANIMOTO
    cerr << "Metric " << _dense_fingerprint_distance_metric[i] << " weight "
         << _fingerprint_weight[i] << '\n';
#endif
    ...
    cerr << " FP " << i << " similarity " << _fingerprint[i].tanimoto(fp2) << " weight "
         ...
         << _fingerprint[i].bits_in_common(rhs._fingerprint[i]) << '\n';
#endif

#ifdef DEBUG_TANIMOTO
    cerr << "Fingerprint component " << i << " "
         << (_fingerprint[i].tanimoto(fp2) * _fingerprint_weight[i]) << ", result so far "
         << result << '\n';
#endif
  }
  ...
  for (int i = 0; i < _number_sparse_fingerprints_to_use_in_computations; i++) {
    ...
    if (SPARSE_DISTANCE_METRIC_TANIMOTO == _sparse_fingerprint_distance_metric[i]) {
      if (sparse_fingerprint_counts_limited) {
        tmp = _sparse_fingerprint_weight[i] *
              _sparse_fingerprint[i].tanimoto(rhs._sparse_fingerprint[i]);
      } else {
        tmp = _sparse_fingerprint_weight[i] *
              _sparse_fingerprint[i].tanimoto_with_unlimited_counts(
                  rhs._sparse_fingerprint[i]);
      }
    } else if (SPARSE_DISTANCE_METRIC_BINARY == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].tanimoto_binary(rhs._sparse_fingerprint[i]);
    } else if (FVB_MODIFIED_TANIMOTO == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].fvb_modified_tanimoto(rhs._sparse_fingerprint[i]);
    } else if (MANHATTAN_DISTANCE_METRIC == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].manhattan_distance(rhs._sparse_fingerprint[i]);
    } else if (SOERGEL_DISTANCE_METRIC == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].soergel_similarity(rhs._sparse_fingerprint[i]);
    } else if (SOERGEL_VARIANT_DISTANCE_METRIC ==
               _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].soergel_variant_similarity(rhs._sparse_fingerprint[i]);
    } else if (CTAN_DISTANCE_METRIC == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].continuous_tanimoto(rhs._sparse_fingerprint[i]);
    } else if (DICE_DISTANCE_METRIC == _sparse_fingerprint_distance_metric[i]) {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].cosine_coefficient(rhs._sparse_fingerprint[i]);
    } else {
      tmp = _sparse_fingerprint_weight[i] *
            _sparse_fingerprint[i].cosine_measure(rhs._sparse_fingerprint[i]);
    }
...
#ifdef DEBUG_TANIMOTO
    cerr << "sparse fingerprint component " << i << ", weight "
         << _sparse_fingerprint_weight[i] << ", value " << tmp << " total " << result
         << '\n';
#endif
  }
  ...
  for (int i = 0; i < number_fixed_size_counted_fingerprints_to_use_in_computations;
       i++) {
    if (SPARSE_DISTANCE_METRIC_TANIMOTO ==
        _fixed_size_counted_fingerprint_distance_metric[i]) {
      result += _fixed_size_counted_fingerprint_weight[i] *
                _fixed_size_counted_fingerprint[i].tanimoto(
                    rhs._fixed_size_counted_fingerprint[i]);
    } else if (FVB_MODIFIED_TANIMOTO ==
               _fixed_size_counted_fingerprint_distance_metric[i]) {
      result += _fixed_size_counted_fingerprint_weight[i] *
                _fixed_size_counted_fingerprint[i].fvb_modified_tanimoto(
                    rhs._fixed_size_counted_fingerprint[i]);
    } else {
      cerr << "IW_General_Fingerprint::unrecognised fixed counted distance metric "
           << _fixed_size_counted_fingerprint_distance_metric[i] << '\n';
      abort();
    }
#ifdef DEBUG_TANIMOTO
    cerr << "After fixed size counted fingerprint " << i << " result " << result << '\n';
#endif
  }

#ifdef DEBUG_TANIMOTO
  cerr << "Any multiconformer_fingerprints_present "
       << multiconformer_fingerprints_present << ", result so far " << result << '\n';
#endif

  if (multiconformer_fingerprints_present) {
    for (int i = 0; i < number_multiconformer_01_fingerprints_in_file; i++) {
      if (multiconformer_use_average_similarity) {
        result += _multiconformer_01[i].average_tanimoto(rhs._multiconformer_01[i]) *
                  _multiconformer_01_weight[i];
      } else {
        result += _multiconformer_01[i].tanimoto(rhs._multiconformer_01[i]) *
                  _multiconformer_01_weight[i];
      }
#ifdef DEBUG_TANIMOTO
      cerr << "After multiconformer 01 " << result << '\n';
#endif
    }

    for (int i = 0; i < number_multiconformer_fixed_size_counted_fingerprints_in_file;
         i++) {
      result += _multiconformer_fixed_size_counted[i].tanimoto(
                    rhs._multiconformer_fixed_size_counted[i]) *
                _multiconformer_fixed_size_counted_weight[i];
    }

    for (int i = 0; i < number_multiconformer_sparse_fingerprints_in_file; i++) {
      result += _multiconformer_sparse[i].tanimoto(rhs._multiconformer_sparse[i]) *
                _multiconformer_sparse_weight[i];
#ifdef DEBUG_TANIMOTO
      cerr << "After multiconformer sparse " << i << " result " << result
           << ", w = " << _multiconformer_sparse_weight[i] << '\n';
#endif
    }
  }
  ...
  if (_molecular_properties_integer.active()) {
    result = result + _property_weight_integer * _molecular_properties_integer.similarity(
                                                     rhs._molecular_properties_integer);
  }

#ifdef DEBUG_TANIMOTO
  if (_molecular_properties_integer.active()) {
    cerr << "Molecular properties integer "
         << (_property_weight_integer *
             _molecular_properties_integer.similarity(rhs._molecular_properties_integer))
  ...
  if (_molecular_properties_continuous.active()) {
    result = result +
             _property_weight_continuous * _molecular_properties_continuous.similarity(
                                               rhs._molecular_properties_continuous);
  }

#ifdef DEBUG_TANIMOTO
  if (_molecular_properties_continuous.active()) {
    cerr << "Continuous molecular properties "
         << (_property_weight_continuous * _molecular_properties_continuous.similarity(
                                               rhs._molecular_properties_continuous))
         << ", result so far " << result << '\n';
  }
  ...
  if (result > static_cast<similarity_type_t>(1.0)) {
    if (result < static_cast<similarity_type_t>(1.01)) {
      //    cerr << "IW_General_Fingerprint::tanimoto: ignoring numeric roundoff " <<
      //    (result - 1.0) << '\n';   happens often, don't bother printing
      return static_cast<similarity_type_t>(1.0);
    }

    cerr << "IW_General_Fingerprint::tanimoto: invalid similarity " << result
         << " between '" << _id << "' and '" << rhs._id << "'\n";
    cerr << _property_weight_integer << " property weight\n";
    for (int i = 0; i < number_fingerprints_to_use_in_computations; i++) {
      cerr << " i = " << i << " fixed weight " << _fingerprint_weight[i] << '\n';
    }
    for (int i = 0; i < _number_sparse_fingerprints_to_use_in_computations; i++) {
      cerr << " i = " << i << " sparse weight " << _sparse_fingerprint_weight[i] << '\n';
    }
    return static_cast<similarity_type_t>(1.0);
  }
...
similarity_type_t
IW_General_Fingerprint::tversky(IW_General_Fingerprint& rhs, const Tversky& t) {
...
#ifdef DEBUG_TVERSKY
    cerr << "Tversky to component " << i << " value "
         << (_fingerprint[i].tversky(fp2, t) * _fingerprint_weight[i]) << '\n';
#endif
  }

  for (int i = 0; i < number_fixed_size_counted_fingerprints_to_use_in_computations;
       i++) {
    result += _fixed_size_counted_fingerprint[i].tversky(
                  rhs._fixed_size_counted_fingerprint[i], t) *
              _fixed_size_counted_fingerprint_weight[i];
  }

  for (int i = 0; i < _number_sparse_fingerprints_to_use_in_computations; i++) {
    result += _sparse_fingerprint[i].tversky(rhs._sparse_fingerprint[i], t) *
              _sparse_fingerprint_weight[i];
  }

  if (_molecular_properties_integer.active()) {
    result += _property_weight_integer *
              _molecular_properties_integer.similarity(rhs._molecular_properties_integer);
  }

  if (_molecular_properties_continuous.active()) {
    result += _property_weight_continuous * _molecular_properties_continuous.similarity(
                                                rhs._molecular_properties_continuous);
  }

#ifdef DEBUG_TVERSKY
  cerr << "Tversky between " << _id << " and " << rhs.id() << " tversky is " << result
       << '\n';
#endif
...
IW_General_Fingerprint::optimistic_distance(IW_General_Fingerprint& rhs,
  ...
  for (int i = 0; i < number_fingerprints_to_use_in_computations; i++) {
    ...
    cerr << "To fingerprint component " << i << " value "
         << (_fingerprint[i].optimistic_distance(fp2, tv) * _fingerprint_weight[i])
         << ", result so far " << result << '\n';
#endif
  }

  for (int i = 0; i < _number_sparse_fingerprints_to_use_in_computations; i++) {
    result += _sparse_fingerprint[i].optimistic_distance(rhs._sparse_fingerprint[i], tv) *
              _sparse_fingerprint_weight[i];

#ifdef DEBUG_OPTIMISTIC_DISTANCE
    cerr << "Sparse component " << i << " dist "
         << (_sparse_fingerprint[i].optimistic_distance(rhs._sparse_fingerprint[i], tv) *
             _sparse_fingerprint_weight[i])
         << ", result so far " << result << '\n';
#endif
  }
  ...
  if (_molecular_properties_integer.active()) {
    result +=
        _property_weight_integer *
        (static_cast<similarity_type_t>(1.0) -
         _molecular_properties_integer.similarity(rhs._molecular_properties_integer));
  }
  ...
  if (_molecular_properties_integer.active()) {
    cerr << "Molecular properties integer "
         << (_property_weight_integer * (static_cast<similarity_type_t>(1.0) -
                                         _molecular_properties_integer.similarity(
                                             rhs._molecular_properties_integer)))
         << ", result so far " << result << '\n';
  }
#endif
  ...
  if (_molecular_properties_continuous.active()) {
    result += _property_weight_continuous * (static_cast<similarity_type_t>(1.0) -
                                             _molecular_properties_continuous.similarity(
                                                 rhs._molecular_properties_continuous));
  }
  ...
  if (_molecular_properties_continuous.active()) {
    cerr << "Continuous molecular properties "
         << (_property_weight_continuous * (static_cast<similarity_type_t>(1.0) -
                                            _molecular_properties_continuous.similarity(
                                                rhs._molecular_properties_continuous)))
         << ", result so far " << result << '\n';
  }
...
#ifdef DEBUG_OPTIMISTIC_DISTANCE
  cerr << "Between " << _id << " and " << rhs.id() << " optimistic dist is " << result
       << '\n';
#endif
...
IW_General_Fingerprint::_allocate_fingerprint_array() {
  ...
  if (number_fixed_size_counted_fingerprints_to_allocate > 0) {
    _fixed_size_counted_fingerprint = new Fixed_Size_Counted_Fingerprint
        [number_fixed_size_counted_fingerprints_to_allocate];
  ...
  if (number_multiconformer_01_fingerprints_in_file > 0) {
    _multiconformer_01 =
        new Multiconformer_01[number_multiconformer_01_fingerprints_in_file];
    assert(nullptr != _multiconformer_01);
  }

  if (number_multiconformer_fixed_size_counted_fingerprints_in_file > 0) {
    _multiconformer_fixed_size_counted = new Multiconformer_Fixed_Counted
        [number_multiconformer_fixed_size_counted_fingerprints_in_file];
    assert(nullptr != _multiconformer_fixed_size_counted);
  }

  if (number_multiconformer_sparse_fingerprints_in_file > 0) {
    _multiconformer_sparse =
        new Multiconformer_Sparse[number_multiconformer_sparse_fingerprints_in_file];
...
IW_General_Fingerprint::_default_values() {
  ...
  if (bit_fingerprints_in_file > 0 || _number_sparse_fingerprints ||
      multiconformer_fingerprints_present) {
    _allocate_fingerprint_array();
  }
...
IW_General_Fingerprint::initialise(const IW_TDT& tdt) {
  ...
  if (nfp <= 0 && sfp <= 0 && cfp <= 0 && mc01 <= 0 && mcfsc <= 0 && mcsparse <= 0) {
    cerr << "IW_General_Fingerprint::_initalize_auto: no fingerprints '"
         << fingerprint_rx->pattern() << "' in tdt\n";
  ...
  if (report_fingerprint_status) {
    ...
    if (number_fixed_size_counted_fingerprints_in_file) {
      cerr << ' ' << number_fixed_size_counted_fingerprints_in_file
    ...
    if (number_multiconformer_fixed_size_counted_fingerprints_in_file) {
      cerr << ' ' << number_multiconformer_fixed_size_counted_fingerprints_in_file
           << " mcfsc\n";
    }
  ...
  if (molecular_property_integer_tag.length())  // look for molecular properties
  {
    ...
    {
      cerr << "IW_General_Fingerprint::_initalize_auto: no '"
           << molecular_property_integer_tag << "' present\n";
      molecular_property_integer_tag = "";
    }
  }
  ...
  if (molecular_property_continuous_tag.length())  // look for molecular properties
  {
    ...
    if (!tdt.dataitem_value(molecular_property_continuous_tag, notused)) {
      cerr << "IW_General_Fingerprint::_initalize_auto: no '"
           << molecular_property_continuous_tag << "' present\n";
  ...
  if (report_fingerprint_status && molecular_property_integer_tag.length()) {
    cerr << "Molecular property tag '" << molecular_property_integer_tag << "', weight "
         << _property_weight_integer << ", nfp = " << nfp << '\n';
  }
  ...
  // The default weight that will be assigned to everything that doesn't have a weight
  // already

  if (weights_specified >= static_cast<float>(1.0)) {
    cerr << "IW_General_Fingerprint::initialise: huh, there are "
         << items_needing_default_weight
         << " items without weights, but total specified = " << weights_specified << '\n';
    return 0;
  }
  ...
  {
    cerr << "IW_General_Fingerprint::initialise: bad news default weight out of range "
         << w << '\n';
    cerr << "Total weight specified " << weights_specified << ", "
         << items_needing_default_weight << " items needing default weight\n";
    return 0;
  }
  ...
  if (report_fingerprint_status) {
    ...
    if (_number_sparse_fingerprints_to_use_in_computations) {
      cerr << _number_sparse_fingerprints_to_use_in_computations
           << " sparse fingerprint ";
    }
    if (number_fixed_size_counted_fingerprints_to_use_in_computations) {
      cerr << number_fixed_size_counted_fingerprints_to_use_in_computations
           << " fixed counted ";
    }
    cerr << "weights to " << w << '\n';
  }
...
IW_General_Fingerprint::_read_molecular_properties_integer(IW_TDT& tdt,
  ...
  if (!tdt.dataitem(tag, mpr)) {
    cerr
        << "IW_General_Fingerprint::_read_molecular_properties_integer: no property tag '"
        << tag << "'\n";
    return 0;
  }
...
int
Molecular_Properties_Continuous::construct_from_tdt_record(
    const const_IWSubstring& buffer) {
...
IW_General_Fingerprint::_read_molecular_properties_continuous(IW_TDT& tdt,
  ...
  const_IWSubstring mpr;
  if (!tdt.dataitem(tag, mpr)) {
    cerr << "IW_General_Fingerprint::_read_molecular_properties_continuous: no property "
            "tag '"
         << tag << "'\n";
    return 0;
  }
...
int
IW_General_Fingerprint::_construct_from_tdt(IW_TDT& tdt, int& fatal) {
  if (!tdt.dataitem_value(identifier_tag, _id)) {
    cerr << "Ignoring TDT with no identifier '" << identifier_tag << "' in tdt\n";
    return 0;
  }

  if (0 == bit_fingerprints_in_file && 0 == _number_sparse_fingerprints &&
...
// #define ECHO_FINGERPRINT_DATA
  ...
  cerr << "_nfingerprints = " << bit_fingerprints_in_file << '\n';
  for (int i = 0; i < bit_fingerprints_in_file; i++) {
    cerr << _bits_in_fingerprint[i] << " bits in fingerprint " << i << '\n';
  }
#endif

  assert(bit_fingerprints_in_file > 0 || _number_sparse_fingerprints > 0 ||
         number_fixed_size_counted_fingerprints_in_file ||
         multiconformer_fingerprints_present);

  if (nullptr == _fingerprint && nullptr == _sparse_fingerprint &&
      nullptr == _fixed_size_counted_fingerprint) {
    _allocate_fingerprint_array();
  }
  ...
  for (int i = 0; i < bit_fingerprints_in_file; i++) {
    ...
    if (!tdt.dataitem(_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find fixed width "
              "fingerprint '"
           << _fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    ...
    if (LOWERCASE_HEX_FORM == _fingerprint_type[i]) {
      ...
      if (!_fingerprint[i].construct_from_ascii_01_representation(zascii.rawchars(),
                                                                  zascii.length())) {
        cerr << "Cannot parse ascii form '" << zascii << "'\n";
        fatal = 1;
        return 0;
      }
      //    std::cout << "Fingerprint has " << _fingerprint[i].nbits() << " bits, nset "
      ...
      //    cout << "Fingerprint has " << _fingerprint[i].nbits() << " bits, nset " <<
      //    _fingerprint[i].nset() << '\n'; _fingerprint[i].PrintOn(cout, '1', '0'); cout
      //    << '\n';
    } else if (SPARSE_FORM == _fingerprint_type[i]) {
      const_IWSubstring zascii(fp);
      remove_tdt_stuff(zascii);
      ...
      if (!_fingerprint[i].construct_from_sparse_representation(zascii)) {
        cerr << "IW_General_Fingerprint::construct_from_tdt: cannot parse sparse bits "
                "representation\n";
        cerr << "'" << zascii << "'\n";
        return 0;
      }
      ...
      if (!_fingerprint[i].construct_from_descriptor_record(fp)) {
        cerr
            << "IW_General_Fingerprint::_construct_from_tdt: invalid descriptor record\n";
        cerr << fp << '\n';
        fatal = 1;
        return 0;
      }
    } else if (!_fingerprint[i].construct_from_tdt_record(fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot parse fp "
           << _fingerprint_tag[i] << "\n";
      return 0;
    }
    ...
    if (_bits_in_fingerprint[i] <= 0) {  // must be first time through
      ...
      cerr << "Bit count mismatch, stored = " << _bits_in_fingerprint[i] << ", input has "
           << _fingerprint[i].nbits() << '\n';
      fatal = 1;
      return 0;
    }
  }
  ...
  for (int i = 0; i < _number_sparse_fingerprints; i++) {
    ...
    if (!tdt.dataitem(_sparse_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find sparse '"
           << _sparse_fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    }
    ...
    if (DAYLIGHT_COMPRESSED_FORM == _sparse_fingerprint_type[i]) {
      ...
      cerr << "IW_General_Fingerprint:_construct_from_tdt:what to do with sparsefptype "
           << _sparse_fingerprint_type[i] << '\n';
      return 0;
    }
  ...
  for (int i = 0; i < number_fixed_size_counted_fingerprints_in_file; i++) {
    ...
    if (!tdt.dataitem(_fixed_size_counted_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find fixed counted '"
           << _fixed_size_counted_fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    }

    if (!_fixed_size_counted_fingerprint[i].construct_from_tdt_record(fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: invalid fixed counted "
              "fingerprint\n";
      cerr << fp << '\n';
      fatal = 1;
      return 0;
  ...
  for (int i = 0; i < number_multiconformer_01_fingerprints_in_file; i++) {
    ...
    if (!tdt.dataitem(_multiconformer_01_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find multiconformer "
              "01 '"
           << _multiconformer_01_fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    }
    ...
    if (!_multiconformer_01[i].construct_from_tdt_record(fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: invalid multiconformer 01 "
              "fingerprint\n";
      cerr << fp << '\n';
      fatal = 1;
      return 0;
  ...
  for (int i = 0; i < number_multiconformer_fixed_size_counted_fingerprints_in_file;
    ...
    if (!tdt.dataitem(_multiconformer_fixed_size_counted_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find multiconformer "
              "fsc '"
           << _multiconformer_fixed_size_counted_fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    }
    ...
    if (!_multiconformer_fixed_size_counted[i].construct_from_tdt_record(fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: invalid multiconformer fsc "
              "fingerprint\n";
      cerr << fp << '\n';
      fatal = 1;
      return 0;
  ...
  for (int i = 0; i < number_multiconformer_sparse_fingerprints_in_file; i++) {
    ...
    if (!tdt.dataitem(_multiconformer_sparse_fingerprint_tag[i], fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: cannot find multiconformer "
              "sparse '"
           << _multiconformer_sparse_fingerprint_tag[i] << "'\n";
      fatal = 1;
      return 0;
    }
    ...
    if (!_multiconformer_sparse[i].construct_from_tdt_record(fp)) {
      cerr << "IW_General_Fingerprint::_construct_from_tdt: invalid multiconformer "
              "sparse fingerprint\n";
      cerr << fp << '\n';
      fatal = 1;
      return 0;
...
int
IW_General_Fingerprint::construct_from_tdt(IW_TDT& tdt, int& fatal) {
...
IW_General_Fingerprint::iwor(const IW_General_Fingerprint& rhs) {
  if (_molecular_properties_continuous.active() ||
      _molecular_properties_integer.active()) {
...
IW_General_Fingerprint::tanimoto(IW_General_Fingerprint& rhs,
  ...
  if (_molecular_properties_integer.active()) {
    const similarity_type_t mpr_integer =
        _molecular_properties_integer.similarity(rhs._molecular_properties_integer);

    if (mpr_integer < must_be_larger_than) {  // sometimes including properties is a good
                                              // idea, sometimes not
      return 0;
    }
  ...
  if (_molecular_properties_continuous.active()) {
    const similarity_type_t mpr_continuous =
        _molecular_properties_continuous.similarity(rhs._molecular_properties_continuous);
  ...
  for (int i = 0; i < number_fixed_size_counted_fingerprints_to_use_in_computations;
       i++) {
    similarity_type_t tmp = _fixed_size_counted_fingerprint[i].tanimoto(
        rhs._fixed_size_counted_fingerprint[i]);
    if (tmp < must_be_larger_than) {
      return 0;
    }
  ...
  if (multiconformer_fingerprints_present) {
    ...
    for (int i = 0; i < number_multiconformer_fixed_size_counted_fingerprints_in_file;
         i++) {
      similarity_type_t tmp = _multiconformer_fixed_size_counted[i].tanimoto(
          rhs._multiconformer_fixed_size_counted[i]);
      if (tmp < must_be_larger_than) {
        return 0;
      }
    ...
    for (int i = 0; i < number_multiconformer_sparse_fingerprints_in_file; i++) {
      similarity_type_t tmp =
          _multiconformer_sparse[i].tanimoto(rhs._multiconformer_sparse[i]);
      if (tmp < must_be_larger_than) {
        return 0;
      }
...
set_atom_count_window(int w, int verbose) {
  ...
  for (int i = 1; i < MAX_NATOMS; i++) {
    ...
    if (verbose) {
      cerr << "Molecules with " << i << " atoms compared with "
           << lower_atom_count_cutoff[i] << " to " << upper_atom_count_cutoff[i]
           << " atoms\n";
    }
  }
...
set_ring_count_window(int w, int verbose) {
  ...
  for (int i = 1; i < MAX_NRINGS; i++) {
    ...
    if (verbose) {
      cerr << "Molecules with " << i << " rings compared with "
           << lower_ring_count_cutoff[i] << " to " << upper_ring_count_cutoff[i]
           << " rings\n";
    }
  }
...
int
can_be_compared(const IW_General_Fingerprint& fp1, const IW_General_Fingerprint& fp2) {
  ...
  if (atom_count_window_present) {
    ...
    cerr << "Can we compare '" << fp1.id() << " " << na1 << " ("
         << lower_atom_count_cutoff[na1] << ',' << upper_atom_count_cutoff[na1]
         << ") and '" << fp2.id() << " " << na2
         << '\n';  //" (" << lower_atom_count_cutoff[na2] << ',' <<
                   //upper_atom_count_cutoff[na2] << ")\n";
#endif
    ...
    //  Jul 2016. In general however, we do need to compare both ways. Have not bothered
    //  changing the others, but should do it...
...
#ifdef DEBUG_CAN_BE_COMPARED
    cerr << "Rings?? " << nr1 << " (" << lower_ring_count_cutoff[nr1] << ','
         << upper_ring_count_cutoff[nr1] << ") and " << nr2 << " ("
         << lower_ring_count_cutoff[nr2] << ',' << upper_ring_count_cutoff[nr2] << ")\n";
#endif
...
display_standard_gfp_options(std::ostream& os) {
  ...
  os << " -F <TAG,sparse>  fingerprint 'TAG', sparse bit representation <1,6-10,22;32> "
        "DNU\n";
  ...
  os << " -P <TAG>         integer molecular properties in 'TAG' (default '"
     << molecular_property_integer_tag << "')\n";
  ...
  os << " -P scale=x.xx    scale factor applied to each continuous property as read in "
        "DNU\n";
  ...
  os << " -Q dsdm=xxx      default sparse distance metric (tan fvb manh soergel soergelv "
        "ctan dice binary binary) DNU\n";
...
parse_guard_fingerprint_specification(const const_IWSubstring& g) {
  ...
  if (!tmp.numeric_value(guard_fingerprint_distance) ||
      guard_fingerprint_distance < 0.0) {
...
class Parse_Tag_Weight_Args {
 ...
 public:
  ...
  int
  distance_metric() const {
    return _distance_metric;
  }

  void
  set_distance_metric(int s) {
    _distance_metric = s;
  }
};
...
static int
parse_tag_weight(const const_IWSubstring& f, const_IWSubstring& tag,
  ...
  if (SPARSE_DISTANCE_METRIC_COSINE == ptwa.distance_metric() &&
      NON_COLLIDING_FORM != ptwa.ftype()) {
...
static int
parse_tag_weight(const const_IWSubstring& f, const_IWSubstring& tag, float& weight,
                 int& nfold, int& ztype, double& ncscale, int& distance_metric) {
  tag = f;
...
static int
parse_desc_specification(const const_IWSubstring& p, int verbose) {
  ...
  while (p.nextword(token, i, ',')) {
    ...
    if (token.starts_with("desc=")) {
      ...
      if (0 == token.length()) {
        ...
        if (!token.numeric_value(number_continuous_molecular_properties) ||
            number_continuous_molecular_properties <= 0) {
      ...
      token.remove_leading_chars(2);
      if (!token.numeric_value(_property_weight_continuous) ||
          _property_weight_continuous < 0.0) {
      ...
      if (continuous_property_distance_metric_exp1_value > 0.0) {
        continuous_property_distance_metric_exp1_value =
            -continuous_property_distance_metric_exp1_value;
      }
      ...
      if (continuous_property_distance_metric_exp2_value > 0.0) {
        continuous_property_distance_metric_exp2_value =
            -continuous_property_distance_metric_exp2_value;
      }
    } else if (token.starts_with("scale=")) {
      token.remove_leading_chars(6);
      ...
      if (verbose) {
        cerr << "Each individual continuous property multipled by '"
             << individual_property_scaling_factor << " on reading\n";
      }
    } else if ("dice" == token) {
  ...
  if (CONTINUOUS_PROPERTY_DISTANCE_METRIC_RATIO == _continuous_property_distance_metric) {
    ;
  } else if (CONTINUOUS_PROPERTY_DISTANCE_METRIC_CART ==
             _continuous_property_distance_metric) {
    cerr << ", cartesian distances\n";
  } else if (CONTINUOUS_PROPERTY_DISTANCE_METRIC_EXP1 ==
             _continuous_property_distance_metric) {
    cerr << ", exp1 distances\n";
  } else if (CONTINUOUS_PROPERTY_DISTANCE_METRIC_EXP2 ==
             _continuous_property_distance_metric) {
    cerr << ", exp2 distances\n";
  }
  cerr << '\n';
...
initialise_fingerprints(Command_Line& cl, int verbose) {
    ...
    if (q.starts_with("dddm=")) {
      ...
      if (q.starts_with("tan")) {
        ...
        if (!q.numeric_value(x) || 0.0 == x) {
          cerr << "The continuous tanimoto exponent (ctan=" << q
      ...
      q.remove_leading_chars(5);
      if (q.starts_with("tan")) {
        _fixed_size_counted_fingerprint_distance_metric =
            FIXED_SIZE_COUNTED_FINGERPRINT_DISTANCE_METRIC_TANIMOTO;
  ...
  if (0 == number_dash_F) {
    set_fingerprint_regular_expression(fingerprint_rx_default);
    // cerr << "All fingerprints matching '" << fingerprint_rx->pattern() << "' will be
    // processed\n";
  } else {
    ...
    if (!set_nfingerprints(nfp, sfp, cfp, mc01, mcfsc, mcsparse)) {
      cerr << "initialise_fingerprints: yipes, cannot initialise for " << nfp
           << " fingerprints\n";
      return 0;
    }
  }
  ...
  if (nfp > 0 || sfp > 0 || cfp > 0 || mc01 > 0 || mcfsc > 0 || mcsparse > 0) {
    ...
    for (int i = 0; i < number_dash_F; i++) {
      ...
      if (NON_COLLIDING_FORM == ftype && 0 == ptwa.ascii()) {
        ...
        //      cerr << "Sparse fingerprint " << sfp << " distance metric " <<
        //      distance_metric << '\n';
        sfp++;
        ...
        cerr << "Multiconformer 01 " << mc01 << " distance metric " << distance_metric
             << '\n';
        mc01++;
        ...
        cerr << "Multiconformer FSC " << mcfsc << " distance metric " << distance_metric
             << '\n';
        mcfsc++;
        ...
        cerr << "Multiconformer sparse " << mcsparse << " distance metric "
             << distance_metric << '\n';
        mcsparse++;
      } else {
        if (ptwa.ascii()) {
  ...
  if (cl.option_present('P')) {
    ...
    for (int i = 0; cl.value('P', p, i); ++i) {
      if ("none" == p) {
        ...
        if (verbose) {
          cerr << "Molecular properties in '" << molecular_property_integer_tag
               << "' field, weight " << _property_weight_integer << '\n';
        }
      }
    }
  ...
  if (nfp || sfp || cfp || mc01 || mcfsc || mcsparse)  // they entered something
  {
    ...
    for (int i = 0; i < bit_fingerprints_in_file; i++) {
      if (_fingerprint_weight[i] >= 0.0)  // was set by the user
      {
        ...
        cerr << "Fingerprint " << i << " weight " << _fingerprint_weight[i] << '\n';
      }
    }
    ...
    for (int i = 0; i < _number_sparse_fingerprints; i++) {
      if (_sparse_fingerprint_weight[i] >= 0.0) {
        ...
        cerr << "Non colliding fingerprint " << i << " weight "
             << _sparse_fingerprint_weight[i] << '\n';
      }
    }
    ...
    for (int i = 0; i < number_fixed_size_counted_fingerprints_in_file; i++) {
      if (_fixed_size_counted_fingerprint_weight[i] >= 0.0) {
        ...
        cerr << "Fixed counted fingerprint " << i << " weight "
             << _fixed_size_counted_fingerprint_weight[i] << '\n';
      }
    }
    ...
    {
      if (wset > 1.0) {
        cerr << "Warning, weights sum to " << wset << '\n';
      }
    } else if (wset > 1.0) {
      cerr << "Sorry, the weights entered are > 1.0, sum = " << wset << '\n';
      return 0;
  ...
  if (cl.option_present('W')) {
    ...
    while (cl.value('W', w, i++)) {
      if (w.starts_with("R:")) {
        ...
        if (verbose) {
          cerr << "Will only compare molecules with at least " << minxtra
               << " extra atoms\n";
        }
        windows_present++;
        ...
        if (verbose) {
          cerr << "Will only compare molecules with at most " << maxxtra
               << " extra atoms\n";
        }
        windows_present++;
        ...
        if (verbose) {
          cerr << "Aromatic atom count window set to " << window << '\n';
        }
    ...
    if (maxxtra >= 0 || minxtra >= 0) {
      if (atom_count_window_present) {
        cerr << "Sorry, cannot have a percentage based atom count window as well as "
                "minextra/maxextra\n";
        return 0;
      }
      if (maxxtra >= 0 && minxtra >= 0 && minxtra > maxxtra) {
        cerr << "Inconsistent values for max extra atoms " << maxxtra
             << " and min extr atoms " << minxtra << '\n';
        return 0;
      }
  ...
  if (cl.option_present('G')) {
    ...
    if (!parse_guard_fingerprint_specification(g)) {
      cerr << "IW_General_Fingerprint::initialise: invalid guard fingerprint "
              "specification '"
           << g << "'\n";
      return 0;
    }
  ...
  if (number_multiconformer_01_fingerprints_in_file ||
      number_multiconformer_fixed_size_counted_fingerprints_in_file ||
      number_multiconformer_sparse_fingerprints_in_file) {
    multiconformer_fingerprints_present = 1;
  }
...
static int
initialise_fingerprints_2(iwstring_data_source& input, int verbose) {
  IW_TDT tdt;
...
static int
initialise_fingerprints(iwstring_data_source& input, int verbose) {
#ifdef GFP_PARALLEL
...
IW_General_Fingerprint::print_all_bits(std::ostream& output) const {
  ...
  for (int i = 0; i < bit_fingerprints_in_file; i++) {
    output << i << " " << _fingerprint_tag[i] << ' ' << _fingerprint[i].nbits()
           << " bits " << _fingerprint[i].nset() << " bits set\n";
...
int
IW_General_Fingerprint::convert_to_non_sparse_forms(
    const Set_of_Sparse_Fingerprint_Collection_Profile& sfpcp) {
  if (convert_sparse_fingerprint_to_bits) {
...
int
IW_General_Fingerprint::_convert_sparse_fingerprints_to_bits(
    const Set_of_Sparse_Fingerprint_Collection_Profile& sfpcp) {
  for (int i = 0; i < _number_sparse_fingerprints; i++) {
...
int
IW_General_Fingerprint::_convert_sparse_fingerprints_to_fixed_width_counted(
    const Set_of_Sparse_Fingerprint_Collection_Profile& sfpcp) {
  for (int i = 0; i < _number_sparse_fingerprints; i++) {
    ...
    sfpcp.convert_to_fixed_width(i, _sparse_fingerprint[i],
                                 _fixed_size_counted_fingerprint[j], extra_bits,
                                 extra_count);

    //  if (extra_count)
    //    _fixed_size_counted_fingerprint[j].set_nset
    //    (_fixed_size_counted_fingerprint[j].nset() + extra_count);
  }
...
IW_General_Fingerprint::equal_weight_tanimoto(IW_General_Fingerprint& rhs) {
  ...
  if (_molecular_properties_integer.active()) {
    ...
    bic +=
        _molecular_properties_integer.bits_in_common(rhs._molecular_properties_integer);
  }
  ...
  if (0 == bic) {
    if (0 == na && 0 == nb) {  // otherwise we have identical molecules that will have
                               // non-zero distances
      return static_cast<similarity_type_t>(1.0);
    }
...
IW_General_Fingerprint::equal_weight_dot_product(IW_General_Fingerprint& rhs) {
  ...
  if (_molecular_properties_integer.active()) {
    //  rc +=
    //  _molecular_properties_integer.bits_in_common(rhs._molecular_properties_integer);
    //  wrong, but this will never be used, implement if ever needed
  }
...
IW_General_Fingerprint::debug_print(std::ostream& output) const {
  output << "IW_General_Fingerprint::debug_print: " << _id << '\n';
