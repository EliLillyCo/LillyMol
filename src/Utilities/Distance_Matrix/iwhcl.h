       ...
       T * diss)
{
  ...
  T dmin;
  im = 0;   // keep the compiler quiet
  jj = 0;   // keep the compiler quiet
  jm = 0;   // keep the compiler quiet
...
// We add 1 to the indices to preserve compatability with fortran version
...
// average link (or group average) method - iopt=4.
     ...
     diss[ind1]=static_cast<T>((membr[i2]*diss[ind1]+membr[j2]*diss[ind2])/
                       (membr[i2]+membr[j2]));

// mcquitty's method - iopt=5.

    else if (iopt == 5)
      diss[ind1] = static_cast<T>((diss[ind1]+diss[ind2])*0.5);

// median (gower's) method - iopt=6.

    else if (iopt == 6)
      diss[ind1]=static_cast<T>(0.5*diss[ind1]+0.5*diss[ind2]-0.25*xx);
