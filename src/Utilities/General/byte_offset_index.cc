...
static int
byte_offset_index(const resizable_array<off_t>& offset, const off_t file_size,
                  std::ostream& output) {
...
static int
byte_offset_index(iwstring_data_source& input, std::ostream& output) {
  ...
  if (0 == items_per_chunk) {
    if (number_chunks > nr) {
      cerr << "Sorry, input file contains " << nr << " records, so cannot partition into "
    ...
    if (verbose) {
      cerr << "With " << nr << " records and " << number_chunks << " chunks, there are "
           << items_per_chunk << " items in each chunk\n";
    }
  }
...
byte_offset_index(int argc, char** argv) {
  ...
  if (cl.option_present('s')) {
    ...
    if (verbose) {
      cerr << "initial file record count guess " << initial_file_size_guess << '\n';
    }
  }
