...
class Descriptor {
 ...
 public:
  ...
  int
  range_specified() const {
    return _range_specified;
  }
...
Descriptor::allocate_bucket_counter() {
  ...
  if (nb <= 0) {
    cerr << "Descriptor::allocate_bucket_counter:invalid bucket count, min " << _min
         << " max " << _max << " dx " << _dx << " nb " << nb << '\n';
    return 0;
  }
...
Descriptor::build(const const_IWSubstring& s) {
  if (1 == s.ccount(':') && s.ccount(',') >= 2)
    ...
    cerr << "Descriptor::build:descriptor bit specifications must contain ':' and ',' "
            "separators\n";
    return 0;
  }
  ...
  for (int ndx = 0; s.nextword(token, i, ','); ndx++) {
    if (0 == ndx) {
      ...
      if (_min + _dx > _max) {
        cerr << "Descriptor::build:inconsistent dx, min = " << _min << ", _max = " << _max
             << " dx = " << _dx << ", impossible\n";
        return 0;
      }
    } else if (3 == ndx) {
...
int
Descriptor::set_bits(float v, Sparse_Fingerprint_Creator& sfc) {
  int b;
  if (v <= _min) {
    b = 0;
  } else {
...
int
Descriptor::set_bits(const const_IWSubstring& buffer, Sparse_Fingerprint_Creator& sfc) {
  const_IWSubstring token;
...
class Set_of_Descriptors {
 ...
 public:
  ...
  int
  number_descriptors() const {
    return _nd;
  }

  int identify_descriptors_being_processed(const const_IWSubstring& buffer, IWString& id,
                                           float* dvalues) const;
  int initialise_any_descriptors_not_already_specified(
...
int
Set_of_Descriptors::initialise(Command_Line& cl, char flag, int verbose) {
...
void
Set_of_Descriptors::set_bits(const float* v, Sparse_Fingerprint_Creator& sfc) {
  for (int i = 0; i < _nd; i++) {
...
static int
descriptors_to_fingerprint_record(const_IWSubstring& buffer, Set_of_Descriptors& sod,
                                  IWString_and_File_Descriptor& output) {
  Sparse_Fingerprint_Creator sfc;
...
Set_of_Descriptors::_initialise_already_specified(const const_IWSubstring& buffer) {
  ...
  for (int col = 0; nfound < _nd; col++) {
    if (!buffer.nextword(token, i)) {
      cerr << "Did not find one or more descriptors, col = " << col << " nfound "
           << nfound << "\n";
  ...
  if (nfound != _nd) {
    cerr << "Set_of_Descriptors::initialise::problems initialising columns, have " << _nd
         << " descriptors, matched " << nfound << '\n';
    return 0;
  }
...
static int
descriptors_to_fingerprint(iwstring_data_source& input, Set_of_Descriptors& sod,
                           IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
...
Set_of_Descriptors::identify_descriptors_being_processed(const const_IWSubstring& buffer,
  ...
  cerr << "Yipes, only identified " << nfound << " of " << _nd
...
static int
read_descriptors(iwstring_data_source& input, Descriptor_Values& descriptor_values,
                 Set_of_Descriptors& sod) {
  const_IWSubstring buffer;
...
static int
read_descriptors(const char* fname, Descriptor_Values& descriptor_values,
                 Set_of_Descriptors& sod) {
...
static int
descriptors_to_fingerprint(const char* fname, Set_of_Descriptors& sod,
                           IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
int
Set_of_Descriptors::initialise_any_descriptors_not_already_specified(
    const Descriptor_Values& descriptor_values) {
  ...
  for (Descriptor_Values::const_iterator i = descriptor_values.begin();
       i != descriptor_values.end(); ++i)
...
descriptors_to_fingerprint(int argc, char** argv) {
  ...
  // TODO. If all information about the descriptors has been specified, then we do not
  // really need the -P file(s).
  ...
  if (cl.option_present('f')) {
    ...
    if (0 == np) {
      cerr << "When working as a TDT filter, must specify precomputed descriptor file "
              "via the -P option\n";
      usage(2);
    }
    ...
    if (verbose) {
      cerr << "Read " << descriptor_values.size()
           << " precomputed descriptor values from '" << p << "'\n";
    }
    ...
    if (np > 1) {
      ...
      if (np > 1) {
        cerr << "Read " << descriptor_values.size()
             << " precomputed descriptor values from " << np << " files\n";
      }
    }
  }
