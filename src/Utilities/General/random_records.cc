...
static int
random_records_sequentual_scan(IWString_Data_Source_MMAP& input, const double p,
  ...
  if (records_selected < records_to_select) {
    cerr << "random_records_sequentual_scan:requested " << records_to_select
         << " but only wrote " << records_selected << endl;
  }
...
static int
choose_records_sequtentially(IWString_Data_Source_MMAP& input, off_t start_position,
                             IWString_and_File_Descriptor& output) {
  ...
  if (chunk_size < 2) {
    cerr << "Cannot sequentially select " << records_to_select << " from file of size "
         << fsize << endl;
    return 0;
  }

  if (verbose) {
    cerr << "For selecting " << records_to_select << " from file of size " << fsize
         << " initial chunk size " << chunk_size << endl;
  }
  ...
  while (1) {
    ...
    int ave_record_length =
        static_cast<int>(record_length.average_if_available_minval_if_not()) + 1;

    chunk_size = (fsize - input.tellg()) / (records_to_select - nsel) - ave_record_length;

    //  cerr << "Offset now at " << input.tellg() << ", chunk size " << chunk_size <<
    //  endl;
  }
...
static int
can_be_output(IW_Hash_Map<off_t, int, streampos_hash_fn>& selected, const off_t& p,
...
static int
fetch_next_record(IWString_Data_Source_MMAP& input, int& rx_matched) {
  if (rx) {
    rx_matched = 0;
  } else {
    rx_matched = 1;
...
static int
choose_a_record(IWString_Data_Source_MMAP& input, off_t start_position, off_t& p,
                int& rx_matched, int& fatal) {
  ...
  while (1) {
    ...
    if (fetch_next_record(input,
                          rx_matched))  // will fail if we seek'd into the last record
    {
      p = o;
      return 1;
    }

    //  Must have seek'd into last record. Since we have very little change of getting the
    //  first record otherwise, do it here
...
static int
get_extra_records(IWString_Data_Source_MMAP& input, int& ndx, off_t* ov) {
  off_t o = input.tellg();
  ...
  for (int i = 0; i < extra_records_to_select; i++) {
    int notused;
    if (!fetch_next_record(input, notused)) {
      //    cerr << "Only got " << i << " of " << extra_records_to_select << " extra
      //    records\n";
      break;
    }
...
do_write_records_as_they_are_selected(IWString_Data_Source_MMAP& input,
  ...
  std::mt19937_64 rng(rd());
  std::uniform_int_distribution<off_t> u(start_position,
                                         file_size - average_record_length);
  ...
  for (auto i = 0; i < buffer_size; ++i) {
    ...
    if (o > average_record_length) {
      ...
      if (!input.seekg(p[i])) {
        cerr << "Huh, within file if size " << file_size << " cannot seek to " << p[i]
             << endl;
        return 0;
      }

      if (!input.next_record(buffer)) {
        cerr << "Huh, could not get next record, position " << p[i] << " file size "
             << file_size << endl;
        return 0;
      }
    } else {
    ...
    if (!input.next_record(buffer)) {
      cerr << "Huh, cannot fetch record near " << p[i] << " file size " << file_size
           << endl;
      return 0;
    }
    ...
    for (auto j = 0; j < extra_records_to_select; ++j) {
      if (!input.next_record(buffer)) {
        cerr << "Cannot fetch extra record '" << j << " from near " << p[i]
  ...
  if (rc < records_to_select) {
    cerr << "Warning, only retrieved " << rc << " of " << records_to_select
         << " requested\n";
  }
...
static int
choose_like_grep(IWString_Data_Source_MMAP& input, off_t& start_position, off_t* ov) {
  int records_found = 0;
...
static int
choose_specific_records(IWString_Data_Source_MMAP& input, off_t& start_position,
                        off_t* ov) {
  int records_read = 0;
  ...
  while (fetch_next_record(input, rx_matched)) {
    ...
    if (!rx_matched) {
      cerr << "Warning, record " << records_read
...
static int
choose_the_records_uniformly(IWString_Data_Source_MMAP& input, off_t& start_position,
                             off_t* ov) {
  ...
  if (static_cast<unsigned int>(o.number_elements()) <= records_to_select &&
      0 == with_replacement)  // should be more careful about is_descriptor_file, where
                              // one less record is to be selected
  {
    cerr << "Only " << o.number_elements() << " records in the file, requested "
         << records_to_select << ", fetching all records\n";
  ...
  if (verbose) {
    cerr << "Selected " << selected.size() << " of " << o.number_elements()
         << " records for output\n";
  }
...
static int
choose_the_records(IWString_Data_Source_MMAP& input, off_t start_position, off_t* ov) {
  // cerr << "Choosing between " << start_position << " and " << input.file_size() <<
  // endl;
...
static int
echo_the_records(IWString_Data_Source_MMAP& input, const off_t* p, int records_selected,
                 IWString_and_File_Descriptor& output) {
...
static int
echo_file(IWString_Data_Source_MMAP& input, IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
...
static int
_at_least_X_records_remaining(IWString_Data_Source_MMAP& input, int records_to_select,
...
static int
at_least_X_records_remaining(IWString_Data_Source_MMAP& input, int records_to_select,
                             int& average_record_length) {
  const auto sbegin = input.tellg();

  const auto rc =
      _at_least_X_records_remaining(input, records_to_select, average_record_length);
...
static int
random_records(const char* fname, const double prob,
               IWString_and_File_Descriptor& output) {
  ...
  // If the file is a descriptor file, we need to make sure we never select the first
  // record again
  ...
  off_t* p = new off_t[records_to_select + records_to_select * extra_records_to_select];
  std::unique_ptr<off_t[]> free_p(p);
...
random_records(int argc, char** argv) {
  ...
  if (cl.option_present('n')) {
    if (!cl.value('n', records_to_select) || records_to_select < 1) {
      cerr << "The records to select option (-n) must be followed by a whole positive "
              "number\n";
      usage(3);
    }
  ...
  if (cl.option_present('e')) {
    if (!cl.value('e', extra_records_to_select) || extra_records_to_select < 0) {
      cerr << "The extra records to choose option (-e) must be followed by a whole "
              "number\n";
      usage(6);
    }

    if (verbose) {
      cerr << "Will select " << extra_records_to_select
           << " extra records along with each randomly chosen record\n";
    }
  }
  ...
  if (cl.option_present('a')) {
    int a;
    if (!cl.value('a', a) || a < 1) {
      cerr << "The maximum iterations per record flag (-a) must be followed by a whole "
              "positive number\n";
      usage(6);
    }
    ...
    if (verbose) {
      cerr << "A maximum of " << a
           << " attempts to identify each record will be performed\n";
    }
  } else if (p > 0.0) {
    ;
  } else {
