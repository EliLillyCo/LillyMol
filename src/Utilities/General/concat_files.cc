...
static IW_Regular_Expression suffix_exclusion_list;
...
concat_files (int argc, char ** argv)
{
  ...
  if (cl.option_present('X'))
  {
    ...
    if ("." == x)
    {
      suffix_exclusion_list.set_pattern("\\.(smi|log)$");
    }
    else
    {
      if (! suffix_exclusion_list.set_pattern(x))
      {
    ...
    if (verbose)
      cerr << "Files matching '" << suffix_exclusion_list.source() << "' will be skipped\n";

    int valid_files_on_command_line = 0;
    int files_excluded = 0;
    for (int i = 0; i < cl.number_elements(); i++)
    {
      if (suffix_exclusion_list.matches(cl[i]))
        files_excluded++;
      else
...
// We will echo the first file and append the others. Establish offsets in the others
  ...
  for (int i = 1; i < nfiles; i++)
  {
    if (suffix_exclusion_list.active() && suffix_exclusion_list.matches(cl[i]))
      continue;
