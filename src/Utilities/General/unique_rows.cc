// Filters an input file to the first occurrence of a token in a column
...
fill_my_columns_to_check(const const_IWSubstring& buffer,
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (c + ncol < 0) {
      cerr << "Record has only " << ncol << " columns, cannot fetch column " << c << '\n';
      return -1;
    }
...
static int
nextword_quoted_tokens(const const_IWSubstring& buffer, const_IWSubstring& token, int& i,
                       const char token_separator) {
#ifdef DEBUG_NEXTWORD_QUOTED_TOKENS
  cerr << "Ftch from '" << buffer << "', i = " << i << '\n';
#endif
  ...
  for (; i < n; ++i) {
    //  cerr << "Checking '" << buffer[i] << "' quote " << in_quote << '\n';
    if (dquote == buffer[i]) {
      in_quote = !in_quote;
    } else if (in_quote) {
      ;
...
static int
unique_rows(const const_IWSubstring& buffer, const int record_number,
  ...
  if (negative_column_numbers_present) {
    highest_column_number_to_check =
        fill_my_columns_to_check(buffer, list_of_column_numbers, my_columns_to_process);
...
// #define DEBUG_UNIQUE_ROWS
  ...
  cerr << "Looking at " << columns_to_process.number_elements()
       << " columns to process\n";
  for (int i = 0; i < columns_to_process.number_elements(); i++) {
    cerr << " process column " << i << " " << columns_to_process[i] << '\n';
  }
  for (int i = 0; i < columns_to_ignore.number_elements(); i++) {
    cerr << " ignore column " << i << " " << columns_to_ignore[i] << '\n';
  }
#endif
  ...
  if (previously_seen.contains(comparison_text)) {
    cerr << "Seen " << times_seen[comparison_text] << '\n';
  }
#endif
    ...
    if (1 == record_number && is_descriptor_file) {
      ...
      if (number_instances_to_allow > 1) {  // we need to count
        times_seen[comparison_text] = 1;
             ...
             (times_seen[comparison_text] + show_counts) <
                 number_instances_to_allow)  // not considered a duplicate, strangeness
                                             // with show_counts because incremented below
  {
    if (!show_counts) {  // was incremented above if show_counts
      times_seen[comparison_text]++;
    }
  } else if (probability_reject_duplicate < 1.0 &&
             distribution(generator) < probability_reject_duplicate) {
    if (!show_counts) {
      times_seen[comparison_text]++;
    }
  ...
  // cerr << "After insertion hash contains " << previously_seen.size() << '\n';
...
determine_columns_to_process(const const_IWSubstring& buffer,
  ...
  int* dfound = new_int(nd);
  std::unique_ptr<int[]> free_dfound(dfound);
  ...
  for (int col = 0; get_next_token(buffer, token, i, token_separator); ++col) {
    for (int j = 0; j < nd; ++j) {
      if (token == *descriptors_to_process[j]) {
        ...
        if (verbose) {
          cerr << "Descriptor '" << *descriptors_to_process[j] << "' found in column "
               << (col + 1) << '\n';
        }
        break;
      }
...
static int
unique_rows(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;

  if (header_records_to_skip > 0) {
    for (int i = 0; i < header_records_to_skip; ++i) {
      if (!input.next_record(buffer)) {
        cerr << "Cannot read header record " << i << '\n';
        return 0;
      }
    }
  }
  ...
  while (input.next_record(buffer)) {
    if (0 == lines_read && descriptors_to_process.number_elements() > 0) {
      if (!determine_columns_to_process(buffer, descriptors_to_process,
                                        columns_to_process)) {
...
static int
unique_rows(const char* fname, IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
static int
extract_column_range(const const_IWSubstring& cs, resizable_array<int>& col) {
...
static int
extract_comma_separated_columns(const const_IWSubstring& cs, resizable_array<int>& col) {
  int i = 0;
  const_IWSubstring token;
...
static int
fetch_columns(Command_Line_v2& cl, char flag, resizable_array<int>& col) {
  int i = 0;
...
struct StringCountComparator {
 public:
  int
  operator()(const std::pair<IWString, int>* sc1, const std::pair<IWString, int>* sc2) {
    ...
    if (sc1->first.length() > sc2->first.length()) {
      return 1;
    }
    if (sc1->first.length() < sc2->first.length()) {
      return -1;
    }
    return sc1->first.strcmp(sc2->first);
  };
};

template <typename O>
int
write_count_data_sorted(const IW_STL_Hash_Map_int& times_seen, O& os) {
  const auto n = times_seen.size();

  resizable_array_p<std::pair<IWString, int>> string_and_count(n);

  Accumulator_Int<int> acc;

  for (const auto& iter : times_seen) {
    string_and_count << new std::pair<IWString, int>(iter.first, iter.second);

    if (verbose) {
      acc.extra(iter.second);
    }
  }

  StringCountComparator strc;

  string_and_count.iwqsort(strc);
  ...
  if (verbose) {
    cerr << "Items seen between " << acc.minval() << " and " << acc.maxval()
         << " times, ave " << static_cast<float>(acc.average_if_available_minval_if_not())
         << '\n';
  }
...
template <typename O>
int
write_count_data(const IW_STL_Hash_Map_int& times_seen, O& os) {
  ...
  for (IW_STL_Hash_Map_int::const_iterator i = times_seen.begin(); i != times_seen.end();
       ++i) {
...
unique_rows(int argc, char** argv) {
  ...
  Command_Line_v2 cl(argc, argv,
                     "-v-c=s-x=ipos-D=s-R=s-z-nc-o-trunc=s-n-r=ipos-whash-P=sfile-O=s-"
                     "tab-csv-vbar-comma-s-all-p=f-q-nwz-d=s-h=ipos-j=ipos-i=s-subset=s");
  ...
  int specifications = (0 != cl.option_present('c')) + (0 != cl.option_present('x')) +
                       (0 != cl.option_present('R')) + (cl.option_count('d') > 0);
  ...
  if (cl.option_present("trunc")) {
    ...
    if (t.length() > 1) {
      cerr << "Sorry, the truncate value can be just a single character '" << t
           << "' not valid\n";
      usage(4);
    }
  ...
  if (cl.option_present("h")) {
    if (!cl.value("h", header_records_to_skip) || header_records_to_skip < 0) {
      cerr << "The number of header records to skip (-h) option must be a whole +ve "
              "number\n";
      usage(1);
    }
  ...
  if (cl.option_present('r')) {
    ...
    if (verbose) {
      cerr << "Will allow at most " << number_instances_to_allow
           << " instances of each unique item\n";
    }
  }

  if (cl.option_present('p')) {
    if (!cl.value('p', probability_reject_duplicate) ||
        probability_reject_duplicate <= 0.0 || probability_reject_duplicate > 1.0) {
      cerr << "The probability of rejecting a duplicate record (-p) must be a valid "
              "fraction\n";
      usage(1);
    }

    if (verbose) {
      cerr << "Duplicate records rejected with probability "
           << probability_reject_duplicate << '\n';
    }
  }
  ...
  if (cl.option_present('P')) {
    ...
    if (verbose) {
      cerr << "Read " << previously_seen.size() << " previously seen identifiers from '"
           << p << "'\n";
    }
  }
  ...
  if (cl.option_present("subset")) {
    ...
    if (verbose) {
      cerr << "Only the parts of a column that match " << text_subset->pattern()
           << " will be considered for uniqueness\n";
    }
  }
  ...
  if (verbose) {
    cerr << "Read " << lines_read << ", " << duplicate_lines_found
         << " duplicates, wrote " << lines_written << '\n';
  ...
  if (!show_counts) {
    ...
    IWString_and_File_Descriptor ofile;
    if (!ofile.open(count_fname.null_terminated_chars())) {
...
#ifdef SHOW_SET
  for (IWString_STL_Hash_Set::const_iterator f = previously_seen.begin();
       f != previously_seen.end(); ++f) {
    cerr << "Saw '" << (*f) << "'\n";
  }
#endif
