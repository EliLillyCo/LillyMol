...
static uint32_t max_differences_to_report = std::numeric_limits<uint32_t>::max();
static uint32_t records_differing = 0;
...
// clang-format off
  ...
  cerr << " -d <delim>     input token delimiter - things like 'tab' are recognised\n";
  cerr << " -k <number>    only write <number> differences - avoids large outputs\n";
...
static int
setup_default_cross_reference(const IWString& header1, const IWString& header2,
                              int* column_xref) {
  int n = header2.nwords_single_delimiter(column_separator);
...
// We have two headers and need to establish a mapping from columns in the first
// to column numbers in the second.

static int
establish_column_xref(const IWString& file1_header, const int columns_in_file1,
                      const IWString& file2_header, const int columns_in_file2,
                      int* column_xref) {
  ...
  while (file1_header.nextword_single_delimiter(token, i, column_separator)) {
    if (ignore_prefix.length() > 0 && token.starts_with(ignore_prefix) &&
        token.length() > ignore_prefix.length()) {
  ...
  if (verbose && ignore_prefix.length()) {
    cerr << prefix_stripped << " of " << (col - 1)
         << " columns in file 1 stripped prefix '" << ignore_prefix << "'\n";
  }
  ...
  i = 0;
  col = 0;
  for (; file2_header.nextword_single_delimiter(token, i, column_separator); ++col) {
    if (ignore_prefix.length() > 0 && token.starts_with(ignore_prefix) &&
        token.length() > ignore_prefix.length()) {
      token.remove_leading_chars(ignore_prefix.length());
      prefix_stripped++;
    ...
    if (to_col >= 0) {
      ...
      if (verbose > 1) {
        cerr << "Column " << (col + 1) << " '" << token << "' found in column "
             << (to_col + 1);
        if (col != to_col) {
          cerr << " *";
        }
  ...
  if (verbose) {
    ...
    if (ignore_prefix.length()) {
      cerr << prefix_stripped << " of " << (col - 1)
           << " columns in file 2 stripped prefix '" << ignore_prefix << "'\n";
    }
  }
...
static int
tokenise(const const_IWSubstring& buffer, const_IWSubstring* token,
         const int* column_xref, int columns_in_input) {
  int i = 0;
  const_IWSubstring v;
  int col = 0;

  while (buffer.nextword_single_delimiter(v, i, column_separator)) {
...
static void
report_mismatch(const const_IWSubstring& token1, int col, const const_IWSubstring& token2,
                int& differences_this_record, const const_IWSubstring& id) {
  if (0 == differences_this_record) {
    if (records_differing < max_differences_to_report) {
      cout << "ID: " << id << '\n';
    }
  ...
  if (records_differing < max_differences_to_report) {
    cout << " mismatch in column " << (col + 1) << " '" << column_title[col] << "' '"
         << token1 << "' vs '" << token2 << "'\n";
  }
...
static int
jfilecompare(const const_IWSubstring& token1, int col, const const_IWSubstring& token2,
             int& differences_this_record, int& missing_value_mismatches_this_record,
             const const_IWSubstring& id)  // the id for the row
{
  ...
  if (0 == col) {
    if ((ignore_leading_zeros || remove_chars_after_underscore) &&
        tokens_compare_after_exclusions(token1, token2)) {
      return 1;
    }
...
static int
jfilecompare(const const_IWSubstring& buffer1, int columns_in_buffer1,
             const const_IWSubstring& buffer2, int columns_in_buffer2,
             const int* column_xref, const_IWSubstring* token,
  ...
  while (buffer1.nextword_single_delimiter(token1, i, column_separator)) {
    if (0 == col) {
      id = token1;
    ...
    if (!jfilecompare(token1, col, token[col], differences_this_record,
                      missing_value_mismatches_this_record, id)) {
...
static int
fetch_second_record(const const_IWSubstring& buffer1, iwstring_data_source& input2,
                    const_IWSubstring& buffer2, int& eof) {
  eof = 0;
...
static int
jfilecompare(iwstring_data_source& input1, int columns_in_file1,
             iwstring_data_source& input2, int columns_in_file2, const int* column_xref,
  ...
  while (input1.next_record(buffer1)) {
    ...
    if (!jfilecompare(buffer1, columns_in_file1, buffer2, columns_in_file2, column_xref,
                      token, differences_this_record)) {
...
static int
jfilecompare(iwstring_data_source& input1, const IWString& header1, int columns_in_file1,
             iwstring_data_source& input2, const IWString& header2, int columns_in_file2,
             int* column_xref, const_IWSubstring* token) {
  if (assume_columns_in_correct_order) {
    setup_default_cross_reference(header1, header2, column_xref);
  } else if (!establish_column_xref(header1, columns_in_file2, header2, columns_in_file2,
                                    column_xref)) {
  ...
  return jfilecompare(input1, columns_in_file1, input2, columns_in_file2, column_xref,
                      token);
}

static int
jfilecompare(iwstring_data_source& input1, iwstring_data_source& input2) {
  IWString header1;
  ...
  columns_in_file1 = header1.nwords_single_delimiter(column_separator);
  ...
  int columns_in_file2 = header2.nwords_single_delimiter(column_separator);
  ...
  int* column_xref = new int[columns_in_file2];
  std::unique_ptr<int[]> free_column_xref(column_xref);
  ...
  const_IWSubstring* token = new const_IWSubstring[nalloc];
  std::unique_ptr<const_IWSubstring[]> free_token(token);

  return jfilecompare(input1, header1, columns_in_file1, input2, header2,
                      columns_in_file2, column_xref, token);
}
...
establish_identifier_cross_reference(iwstring_data_source& input2) {
  ...
  while (input2.next_record(buffer)) {
    ...
    if (identifier_offset_cross_reference.end() !=
        identifier_offset_cross_reference.find(buffer)) {
  ...
  if (verbose) {
    cerr << "Identifier/offset cross reference contains "
         << identifier_offset_cross_reference.size() << " items\n";
  }
...
final_verbose_report(std::ostream& os) {
  os << "Read " << records_read << " records. " << records_differing
     << " records contained differences\n";
  ...
  if (differences_within_noise) {
    os << differences_within_noise << " differences in values less than noise value "
       << noise << '\n';
  }

  if (missing_value_mismatches) {
    os << missing_value_mismatches << " missing value mismatches on "
       << records_with_missing_value_mismatches << " records ignored\n";
  }

  for (int i = 0; i < columns_in_file1; i++) {
    if (differences_in_column[i]) {
      os << differences_in_column[i] << " differences in column " << (i + 1) << " '"
         << column_title[i] << "'\n";
    }
  }
...
jfilecompare(int argc, char** argv) {
  Command_Line cl(argc, argv, "vt:cM:mxzGul:V:D:n:q:P:eobd:k:");
  ...
  if (cl.option_present('d')) {
    IWString d = cl.option_value('d');
    cerr << "Examining '" << d << "' as separator\n";
    if (!char_name_to_char(d)) {
      cerr << "Unrecognised character name '" << d << "'\n";
      return 1;
    }

    cerr << "Sep is '" << d[0] << '\n';
    column_separator = d[0];
    if (verbose) {
      cerr << "Column separator set\n";
    }
  }
  ...
  if (cl.option_present('k')) {
    if (!cl.value('k', max_differences_to_report) || max_differences_to_report == 0) {
      cerr << "The max differences to report option (-k) must be a whole +ve number\n";
      return 1;
    }

    if (verbose) {
      cerr << "Will only report the first " << max_differences_to_report
           << " differences\n";
    }
  }
  ...
  if (verbose) {
    ...
    cerr << "Warning, " << missing_columns
         << " columns in first file, not in subsequent\n";
  }
