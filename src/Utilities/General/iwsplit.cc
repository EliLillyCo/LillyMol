/*
  splits files. Very similar to Linux split. This started many years
  ago and over time, Linux split has added functionality so today
  there are few gaps. The naming schemes here work better.
*/
...
static int
common_check_items_per_chunk(const int items_in_file, const int number_chunks_reqested,
                             int& items_per_chunk) {
  if (0 == items_in_file) {
  ...
  if (number_chunks_reqested > items_in_file) {
    cerr << "Requested " << number_chunks_reqested << " chunks, but only "
         << items_in_file << " items in input file\n";
    items_per_chunk = 1;
  ...
  if (verbose) {
    cerr << "Input file contains " << items_in_file << " items, will put "
...
static int
determine_number_of_items_per_chunk(iwstring_data_source& input, int& items_in_file,
                                    int number_chunks_reqested, int& items_per_chunk) {
  if (!rx) {
  ...
  return common_check_items_per_chunk(items_in_file, number_chunks_reqested,
                                      items_per_chunk);
}

static int
read_header_records(iwstring_data_source& input, int records_to_read, IWString& header) {
...
static void
determine_where_to_start(const IWString& stem, const IWString& suffix,
...
static int
new_item_found_in_column(const const_IWSubstring& buffer, const int group_column,
                         char group_column_separator, IWString& gid) {
  const_IWSubstring token;
  int i = 0;

  for (auto col = 0; buffer.nextword_single_delimiter(token, i, group_column_separator);
       ++col) {
...
iwsplit(int argc, char** argv) {
  Command_Line_v2 cl(argc, argv,
                     "-v-suffix=s-ssuffix-stem=s-tdt-sdf-n=ipos-nc=ipos-rx=s-stop=ipos-"
                     "append-j=ipos-suffix_len=ipos-gz-size-stdout-w=s-s-g=s-dd=s");
  ...
  if (cl.option_present('g')) {
    ...
    for (auto i = 0; cl.value('g', g, i); ++i) {
      if (g.starts_with("sep=")) {
        ...
        if ("tab" == g) {
          ...
          cerr << "The group by column separator must be a single character '" << g
               << "' invalid\n";
          return 1;
        } else {
    ...
    if (verbose) {
      cerr << "Chunks defined by groups of items in column " << group_column << '\n';
    }
    ...
    while (cl.value("w", w, i++)) {
      if (w.starts_with("char=")) {
        ...
        if (1 != w.length() || ' ' == w[0] || '\t' == w[0]) {
          cerr << "The sequence width fill characters must be a single, non blank "
                  "character, '"
               << w << "' invalid\n";
          usage(1);
        }
        ...
        cerr << "The sequence number width specification must be a whole +ve number, '"
             << w << "' invalid\n";
        usage(2);
      } else if (verbose) {
  ...
  for (int i = 0; i < cl.number_elements(); i++) {
    ...
    if (split_by_size > 0) {
      ...
      if (group_column >= 0) {
        success = determine_number_of_items_per_chunk(
            input, items_in_file, number_chunks_reqested, items_per_chunk, group_column,
            group_column_separator);
      } else {
        success = determine_number_of_items_per_chunk(
            input, items_in_file, number_chunks_reqested, items_per_chunk);
      }
    ...
    while (input.next_record(buffer)) {
      ...
      const int rx_starts_chunks_and_rx_matches_buffer =
          (rx_starts_chunks && RE2::PartialMatch(string_piece, *rx));

      //    cerr << "From '" << buffer << "' match " <<
      //    rx_starts_chunks_and_rx_matches_buffer << " this chunk " <<
      //    items_written_this_chunk << '\n';

      //    if (rx.matches(buffer))
      //      cerr << buffer << " : " << items_written_this_chunk << '\n';
      ...
      if (group_column >= 0) {
        ...
        need_new_file = true;
      } else if (rx_starts_chunks_and_rx_matches_buffer &&
                 items_written_this_chunk >= items_per_chunk) {
        need_new_file = true;
      }
...
// #define DEBUG_IWSPLIT
#ifdef DEBUG_IWSPLIT
      if (need_new_file) {
        cerr << "Just read " << buffer << "', new file needed, items_written_this_chunk "
             << items_written_this_chunk << ", per chunk " << items_per_chunk << ", gid '"
             << gid << "'\n";
      }
#endif
      ...
      if (need_new_file) {
        ...
        if (items_in_file > 0 && lines_written)  // user asked for a specific number of
                                                 // chunks, make sure we deliver that many
        {
          int still_to_write = items_in_file - lines_written;
#ifdef DEBUG_IWSPLIT
          cerr << "Starting " << chunk_file_name << ", started with " << items_in_file
               << " in file, have written " << lines_written << ", remaining "
               << (items_in_file - lines_written) << '\n';
#endif
...
#ifdef DEBUG_IWSPLIT
            cerr << "Remaining " << (items_per_chunk * chunks_remaining) << " compare "
                 << still_to_write << " preliminary items_per_chunk " << items_per_chunk
                 << '\n';
#endif
          ...
          if (verbose > 2) {
            cerr << "still_to_write " << still_to_write << " in " << chunks_remaining
                 << " chunks, items_per_chunk updated to " << items_per_chunk << '\n';
          }
        }
      ...
      if (!rx) {  // every record is an item boundary
        ...
        ;
      } else if (const re2::StringPiece string_piece(buffer.data(), buffer.length());
                 !RE2::PartialMatch(string_piece, *rx)) {  // not an item boundary
        continue;
      }
      ...
      //    cerr << "items_written_this_chunk updated to " << items_written_this_chunk <<
      //    '\n';
