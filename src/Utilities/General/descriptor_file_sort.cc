...
Pair_of_Columns::build(const const_IWSubstring& token) {
  ...
  if (_s1.numeric_value(_c1) && _c1 > 0 && _s2.numeric_value(_c2) && _c2 > 0 &&
      _c1 != _c2) {
    return 1;
  }
...
template <typename T>
int
parse_token_as_numeric(const const_IWSubstring& token, T& v, int col) {
  ...
  cerr << "Illegal numeric '" << token << "' in column " << (col + 1) << '\n';
  return 0;
}

template <typename T>
int
get_next_token(const T& buffer, const_IWSubstring& token, int& i) {
  if (' ' == column_separator) {
...
int
Pair_of_Columns::diff(const const_IWSubstring& buffer, float& d) const {
...
class Descriptor_File_Record : public IWString {
 private:
  // Note that we do not own this pointer, so we do NOT delete it.
  // It is set in initialise_sort_keys.
  float* _sort_key;
 ...
 public:
  ...
  int initialise_sort_keys(const int*, int, const resizable_array_p<Pair_of_Columns>&,
                           int& missing_values);
...
int
Descriptor_File_Record::initialise_sort_keys(const int* sort_column, int columns_in_input,
                                             int n, int& missing_values, float* s) {
  missing_values = 0;
...
Never finished implemention of the difference idea
    ...
    int
    Descriptor_File_Record::initialise_sort_keys(
        const int* sort_column, int n, const resizable_array_p<Pair_of_Columns>& poc,
        int& missing_values) {
  missing_values = 0;
  ...
  for (int i = 0; i < npair; i++) {
    if (!poc[i]->diff(*this, _sort_key[n + i])) {
      cerr << "Descriptor_File_Record::initialise_sort_keys:cannot initialise difference "
              "specification\n";
      return 0;
    }
  }
...
static int
identify_difference_specifications(const const_IWSubstring& header, int columns_in_input,
...
initialise_sort_conditions(const const_IWSubstring& header) {
  ...
  for (int i = 0; i < sort_column.number_elements(); i++) {
    if (sort_column[i] >= columns_in_input) {
      cerr << "Only " << columns_in_input << " columns in input, sorting on column "
           << (sort_column[i] + 1) << " is impossible\n";
      return 0;
    }
  }
  ...
  if (0 == column_differences.number_elements())
    ;
  else if (!identify_difference_specifications(header, columns_in_input, sort_column,
                                               column_differences)) {
  ...
  while (get_next_token(header, d, i)) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      if (verbose) {
        cerr << "Descriptor '" << d << "' found in column " << (col + 1) << '\n';
      }
...
static int
read_data(const char* fname, resizable_array_p<IWString>& header,
          resizable_array_p<Descriptor_File_Record>& zdata) {
...
descriptor_file_sort(int argc, char** argv) {
  ...
  if (cl.option_present('i')) {
    ...
    if (!char_name_to_char(i)) {
      cerr << "The -i option must be followed by a single character, '" << i
           << "' is invalid\n";
      usage(4);
    }
  ...
  if (cl.option_present('m')) {
    ...
    if (verbose) {
      cerr << "Missing values replaced by " << missing_value_replacement_value << '\n';
    }
  }
  ...
  if (verbose) {
    cerr << "Read " << header.number_elements() << " header records and " << n
         << " data records\n";
  }

  int* tmp = new_int(columns_in_input, -1);
  ...
  float* sort_keys = new float[nsort * zdata.number_elements()];
  std::unique_ptr<float[]> free_sort_keys(sort_keys);
  ...
  for (int i = n - 1; i >= 0; --i) {
    ...
    if (!zdata[i]->initialise_sort_keys(tmp, columns_in_input, nsort, missing_values,
                                        sort_keys + nsort * i)) {
      cerr << "Cannot initialise record " << i << '\n';
      cerr << *(zdata[i]) << '\n';
      return 5;
    }
  ...
  if (verbose && records_removed_for_missing_data) {
    cerr << "Discarded " << records_removed_for_missing_data
