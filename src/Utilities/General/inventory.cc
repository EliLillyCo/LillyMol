...
static int
read_restricted_items(iwstring_data_source & input,
                       IW_STL_Hash_Set & restricted)
{
  IWString buffer;
...
static int
read_restricted_items(const char * fname,
                       IW_STL_Hash_Set & restricted)
{
...
// private functions
    ...
    int _filter_record(const const_IWSubstring & buffer,
                                IW_STL_Hash_Set & written,
                                IWString_and_File_Descriptor & output);

  public:
    Inventory_File(const char * fname,
                    float min_amount,
                    int col,
                    const const_IWSubstring & units,
                    const IWString & must_have);

    int lines_read() const { return _lines_read;}
    int lines_written() const { return _lines_written;}
    int duplicates_suppressed() const { return _duplicates_suppressed;}

    int report(std::ostream & os) const;

    int good() const {return _input.is_open();};

    int filter(IW_STL_Hash_Set & written, IWString_and_File_Descriptor &);
};

Inventory_File::Inventory_File(const char * fname,
                    float min_amount,
                    int col,
                    const const_IWSubstring & units,
                    const IWString & must_have) : _fname(fname),
...
int
Inventory_File::filter(IW_STL_Hash_Set & written,
                        IWString_and_File_Descriptor & output)
{
  ...
  while (_input.next_record(buffer))
  {
    ...
    {
      cerr << "Fatal error processing '" << buffer << "', line " << _input.lines_read() << endl;
      return 0;
    }
...
int
Inventory_File::_filter_record(const const_IWSubstring & buffer,
                                IW_STL_Hash_Set & written,
                                IWString_and_File_Descriptor & output)
{
  _lines_read++;
...
{
  os << "Inventory_File::report:file '" << _fname << "' read " << _lines_read << " records, wrote " << _lines_written << ", duplicates_suppressed " << _duplicates_suppressed << endl;
...
inventory(int argc, char ** argv)
{
  ...
  if (nfiles != cl.option_count('c') ||
      nfiles != cl.option_count('u') ||
      nfiles != cl.option_count('C') ||
      nfiles != cl.option_count('M'))
  {
    cerr << "Must specify same number of -u and -c and -C options as -I options " << nfiles << endl;
    usage(5);
  }
  ...
  if (verbose && nfiles > 1)
  {
    cerr << "Read " << lines_read << " lines from " << nfiles << " files, wrote " << lines_written << endl;
    if (check_hash_set)
