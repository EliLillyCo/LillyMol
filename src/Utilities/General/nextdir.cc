// Writes to stdout the next directory one above where you are.
// Designed to work as 
// cd $(nextdir)
// which allows you to cycle through a set of subdirectories.
...
static int
nextdir_reading_file_of_directories(const IWString& pwd, int argc, char** argv) {
...
nextdir(int argc, char** argv) {
  ...
  while (NULL != (d = readdir(parent_directory))) {
    ...
    fprintf(stderr, "Examining directory '%s', dir? %d\n", d->d_name,
            dash_d(tmp->null_terminated_chars()));
#endif
