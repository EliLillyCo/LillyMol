...
/*
  For each column, we keep track of the text representation as well as the corresponding
  numeric value
*/
...
class Column_Data : public const_IWSubstring, public Set_or_Unset<double>
{
 ...
 public:
  ...
  void
  new_value(const const_IWSubstring& rhs);
...
class Column_Condition
{
 ...
 public:
  ...
  void
  set_column_name(const_IWSubstring& s)
  {
    _column_name = s;
  }
  ...
  int
  determine_column_number(const const_IWSubstring&);

  int
  passes(Column_Data* cd);
};
...
Column_Condition::build(const const_IWSubstring& e)
{
  ...
  while (isalnum(expr[i]) || '_' == expr[i] || '-' == expr[i] || ':' == expr[i] ||
         '.' == expr[i]) {
    _column_name += expr[i];
    i++;
...
Column_Condition::determine_column_number(const const_IWSubstring& header)
{
  ...
  cerr << "Column_Condition::determine_column_number: bad news, no '" << _column_name
       << "' in header\n";
  cerr << header << endl;
  return 0;
}
...
Column_Condition::passes(Column_Data* cd)
{
  ...
  if (missing_value == static_cast<const_IWSubstring>(mycd)) {
  ...
  if (CC_OPERATOR_RX == _operator) {
    ...
    return iwre2::RE2PartialMatch(p, *_rx);
  }
...
_passes_all_filters(Column_Data* cd, int ncol)
{
  ...
  for (int i = 0; i < nc; i++) {
    if (!logical_expression.result_needed(
            i)) {  // coupled via an OR with something previously
      continue;
    }
...
passes_all_filters(Column_Data* cd, int ncol)
{
  ...
  for (int i = 1; i < istop; i++) {
    //  cerr << "Check column " << i << "? " << check_column_for_zero[i] << " zero? " <<
    //  cd[i].is_zero() << endl;
...
identify_first_non_reagent_column(const const_IWSubstring& header)
{
  ...
  for (int col = 1; get_next_token(header, token, i); col++) {
    if (token == first_non_reagent_column_header) {
      ...
      if (verbose) {
        cerr << "Column " << col << " contains descriptor '"
             << first_non_reagent_column_header << "'\n";
      }
      return 1;
    }
  }

  cerr << "Did not find '" << first_non_reagent_column_header
       << "' descriptor in header\n";
  return 0;
}

static int
write_record(const_IWSubstring& buffer, IWString_and_File_Descriptor& output)
{
...
static int
dfilefilter(iwstring_data_source& input, int ncol, Column_Data* cd,
...
static int
dfilefilter(iwstring_data_source& input, IWString_and_File_Descriptor& output)
{
...
class Condition
{
 ...
 public:
  ...
  int
  operator_count() const;

  Condition*
  next_condition() const;
...
Operator::build(const IWString& expr, int iptr)
{
  if (iptr + 2 >= expr.length()) {
    cerr << "Operator::build: expression too short '" << expr << "' iptr = " << iptr
         << endl;
    return 0;
  }
  ...
  if (firstchar != expr[iptr + 1]) {
    cerr << "Operator::build: operators must be repeated '" << expr << "' iptr = " << iptr
         << endl;
    return 0;
  }
...
static int
dfilefilter(const char* fname, IWString_and_File_Descriptor& output)
{
...
static int
parse_expression(const const_IWSubstring& e, Condition& c)
{
  if (!c.build(e)) {
...
dfilefilter(int argc, char** argv)
{
  ...
  if (cl.option_present('K')) {
    ...
    if (verbose) {
      cerr << "Special processing for trxn, first non reagent column '"
           << first_non_reagent_column_header << "'\n";
    }
  }
  ...
  if (verbose || !write_passing_records) {
    ...
    if (records_read > 0) {
      cerr << ", fraction "
           << static_cast<float>(records_written) / static_cast<float>(records_read)
           << "\n";
    } else {
      cerr << endl;
    }
