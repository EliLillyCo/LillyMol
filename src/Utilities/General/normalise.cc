...
class NColumn : private Accumulator<double> {
 ...
 public:
  ...
  void
  set_descriptor_name(const const_IWSubstring& d) {
    _descriptor_name = d;
  }

  const IWString&
  descriptor_name() const {
...
int
NColumn::report(int col, std::ostream& output) const {
  output << "Column " << col;
  ...
  if (_is_constant) {
    ...
    output << " between " << Accumulator<double>::minval() << " and "
           << Accumulator<double>::maxval() << " ave " << Accumulator<double>::average()
           << " var " << Accumulator<double>::variance();
  }
...
int
NColumn::write_scaling_information(int col, IWString_and_File_Descriptor& output) const {
  if (_skip) {
    return 1;
  }
  ...
  output << Accumulator<double>::minval() << ' ' << Accumulator<double>::maxval() << ' '
         << Accumulator<double>::average() << ' ';
...
NColumn::establish_ranges() {
  ...
  if (0.0 == _range) {
    cerr << "NColumn::establish_ranges: '" << _descriptor_name
         << "' no variability in column\n";
    _is_constant = 1;
...
NColumn::establish_range_from_pre_existing_data(const IWString& buffer) {
  ...
  if (_minval < _maxval) {
    ...
    cerr << "NColumn::establish_ranges:invalid range specification " << _minval << " to "
         << _maxval << '\n';
    return 0;
  } else {
    _is_constant = 1;
...
int
NColumn::scale(double f, double& rc) const {
  if (_is_constant) {
    rc = f;
...
int
NColumn::unscale(double f, double& rc) const {
  if (NRML_MIN_TO_MAX == scaling_type || NRML_SPREAD_ZERO == scaling_type) {
    if (_is_constant) {  // just echo what came in
      ...
      if (report_out_of_range_values) {
        cerr << "NColumn::unscale:value out of range " << f << " must be " << range_min
             << " to " << range_max;
...
class Data_Item : public Set_or_Unset<double> {
 ...
 public:
  ...
  void
  set_text_rep(const const_IWSubstring& t) {
    _text_rep = t;
  }

  const const_IWSubstring&
  text_representation() const {
    return _text_rep;
  }
};
...
determine_tokens_per_line_and_allocate_arrays(iwstring_data_source& input) {
  ...
  if (initial_columns_to_skip >= columns_in_input) {
    cerr << "Cannot skip " << initial_columns_to_skip << " columns, only "
         << columns_in_input << " in file\n";
    return 0;
  }
...
int
read_record(iwstring_data_source& input, Data_Item* d, int& fatal) {
  const_IWSubstring buffer;
  if (!input.next_record(buffer)) {
    fatal = 0;
    return 0;
...
static int
retrieve_scaled_value(const NColumn& col, const Data_Item& d, double& scaled) {
  double x;
...
static int
parse_scaling_info(const const_IWSubstring& buffer, IW_STL_Hash_Map_String& xref) {
  ...
  if (5 != buffer.nwords()) {
    cerr << "Descriptor scaling info must have exactly 5 tokens, min, max, ave, "
            "variance\n";
    cerr << "'" << buffer << "'\n";
    return 0;
  }
...
static int
read_scaling_file(iwstring_data_source& input, IW_STL_Hash_Map_String& xref) {
  const_IWSubstring buffer;
...
static int
read_scaling_file(const const_IWSubstring& fname, IW_STL_Hash_Map_String& xref) {
...
establish_scaling_from_pre_existing_file(IWString& name_of_existing_scaling_file_to_use) {
  ...
  if (single_column_kludge && 1 == xref.size() &&
      1 == (columns_in_input - initial_columns_to_skip)) {
    ...
    if (!column[col].establish_range_from_pre_existing_data((*f).second)) {
      cerr << "Invalid range specification for descriptor '"
           << column[col].descriptor_name() << "'\n";
      return 0;
    }
...
static void
maybe_do_nearest_integer(double v, IWString_and_File_Descriptor& output,
...
static int
do_normalise(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
  assert(input.ok());
  assert(output.good());
...
static int
normalise(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
  assert(input.ok());
...
static int
normalise(const char* input_fname, IWString_and_File_Descriptor& output) {
  ...
  if ('\t' !=
      input_separator) {  // preserve historical behaviour which always translated tabs
...
normalise(int argc, char** argv) {
  ...
  if (cl.option_present('R')) {
    if (!cl.value('R', nearest_integer) || nearest_integer <= 0.0 ||
        nearest_integer >= 1.0) {
  ...
  if (cl.option_count('X')) {
    if (!cl.value('X', user_specified_range_max_value) ||
        user_specified_range_max_value <= 0.0) {
