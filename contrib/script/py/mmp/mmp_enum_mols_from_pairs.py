...
class MMPEnumerateNewMols(object):
    ...
    def __init__(self, logger_object):
        ...
        if len(logging.Logger.manager.loggerDict) < 1:
            ...
            sys.exit(
                "Invalid or no logger object passed to MMPObjectClass.  Please create \
        ...
        self.rxn_file_rxn_map_sgl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".rxn", encoding="utf-8", mode="wt"
        )
        self.rxn_file_makethese_sgl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".txt", encoding="utf-8", mode="wt"
        )
        ...
        self.rxn_file_rxn_map1_dbl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".rxn", encoding="utf-8", mode="wt"
        )
        self.rxn_file_rxn_map2_dbl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".rxn", encoding="utf-8", mode="wt"
        )
        self.rxn_file_rxn_map3_dbl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".rxn", encoding="utf-8", mode="wt"
        )
        self.rxn_file_rxn_map4_dbl = tempfile.NamedTemporaryFile(
            delete=False, suffix=".rxn", encoding="utf-8", mode="wt"
        )
        self.rxn_file_makethese_dbl1 = tempfile.NamedTemporaryFile(
            delete=False, suffix=".txt", encoding="utf-8", mode="wt"
        )
        self.rxn_file_makethese_dbl2 = tempfile.NamedTemporaryFile(
            delete=False, suffix=".txt", encoding="utf-8", mode="wt"
        )
    ...
    def scan_input_smiles(self, smi_fi, injest=False):
        ...
        with open(smi_fi, "rt") as input_smifi:
            ...
            for line in input_smifi:
                ...
                if "." in smiles:
                    sys.exit(
                        "Failed smiles parser as salt on line %s of file %s (pls clean using fileconv)"
                        % (line_num, smi_fi)
                    )
    ...
    def fragment_reference_smi(self, cut_type, dicer_threshold, exclude_h_subst=True):
        ...
        for num_cuts, mol_id, ctx_orig, frag, fattach, cattach in execute_dicer(
            self.smi_file, cut_type, dicer_threshold, self.logger
        ):
            if num_cuts == 1:
                if parse_single:
    ...
    def pairs_file_to_dict(
        self, pairs_file, frag_l_col="FRAG_L", frag_r_col="FRAG_R", exclude_h_subst=True, num_pairs_limit=999999
    ):
        """
    ...
    def do_reactions(self):
        ...
        if self.do_single:
            self.single_exit_code1 = pymo.make_these_molecules(
                [self.rxn_file_ctx_sgl.name, self.rxn_file_frags_sgl.name],
                self.rxn_file_makethese_sgl.name,
                [self.rxn_file_rxn_map_sgl.name],
                outfile=self.rxn_file_output_sgl.name,
                debug=False,
            )
            ...
            self.logger.info(
                "Ran make_these_molecules with exit code %s and output %s.smi"
                % (self.single_exit_code1, self.rxn_file_output_sgl.name)
            )
        ...
        if self.do_double:
            self.double_exit_code1 = pymo.make_these_molecules(
                ...
                self.rxn_file_makethese_dbl1.name,
                [self.rxn_file_rxn_map1_dbl.name, self.rxn_file_rxn_map2_dbl.name],
                outfile=self.rxn_file_output_dbl1.name,
                debug=False,
            )
            ...
            self.double_exit_code2 = pymo.make_these_molecules(
                ...
                self.rxn_file_makethese_dbl2.name,
                [self.rxn_file_rxn_map3_dbl.name, self.rxn_file_rxn_map4_dbl.name],
                outfile=self.rxn_file_output_dbl2.name,
                debug=False,
            )
    ...
    def yield_products_complex_id(self):
        ...
        if self.do_single:
            if self.single_exit_code1 == 0 and self.file_exists_check(self.rxn_file_output_sgl.name):
                for new_mol, orig_mol_id, frag_removed, frag_added in yield_products_submethod(
                    self.rxn_file_output_sgl.name, 1
                ):
        ...
        if self.do_double:
            if self.double_exit_code1 == 0 and self.file_exists_check(self.rxn_file_output_dbl1.name):
                for new_mol, orig_mol_id, frag_removed, frag_added in yield_products_submethod(
                    self.rxn_file_output_dbl1.name, 2
                ):
            ...
            if self.double_exit_code2 == 0 and self.file_exists_check(self.rxn_file_output_dbl2.name):
                for new_mol, orig_mol_id, frag_removed, frag_added in yield_products_submethod(
                    self.rxn_file_output_dbl2.name, 2
                ):
                    yield new_mol, orig_mol_id, "double", frag_removed, frag_added
            else:
...
class _TestMMPEnumerateNewMols(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.temp_file_input_csv = tempfile.NamedTemporaryFile(delete=False, encoding="utf-8", mode="wt")
        self.temp_file_input_smi = tempfile.NamedTemporaryFile(delete=False, suffix=".smi", encoding="utf-8", mode="wt")
        ...
        self.test_input_single_csv_header = "FRAG_L,FRAG_R,MOL_CLP_DIFF,MOL_CLP_STDEV,NUM_VALS,PERCENT_THAT_INCREASE,PERCENT_THAT_DECREASE,PERCENT_WITH_NOCHANGE,MOL_MWT_DIFF"
        ...
        self.test_output_products_single = {
            ...
            "O=C1N(CC2(CN(CC3=CC=NC=C3)CCN(C2)C(=O)C2=CC=C(C#N)C=C2)C1)CC": [
                3552650,
                "O=[1CH]C(C)C",
                "O=[1CH]c1ccc(C#N)cc1",
            ],
        ...
        self.test_output_products_single_incH.update(
            {
                ...
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C(=O)NC2=NC=CS2)CC(=O)CC)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)(C2=CC=NC=C2)C(=O)CC)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)C2=C(C=NC=C2)C(=O)CC)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                ...
                "FC1=CC=C(N2N=CC3=CC(=C(C=C23)C(=O)CC)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC(=C23)C(=O)CC)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2C3=CC=C(C=C3C(=N2)C(=O)CC)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC(=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1)C(=O)CC": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=C(C=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1)C(=O)CC": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=C2C=CC(=C3C(=O)CC)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)N(C2=NC=CS2)C(=O)CC)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC(=CS2)C(=O)CC)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
                "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=C(S2)C(=O)CC)C2=CC=NC=C2)C=C1": [
                    2426642,
                    "[1H]",
                    "O=[1CH]CC",
                ],
    ...
    def test_scan_input_smiles(self):
        ...
        self.assertEqual(
            self.test_mmp_pairs_object.mol_smi_dict,
            {
                2426642: "CC(C)(C(c1ccncc1)c2ccc3c(cnn3c4ccc(F)cc4)c2)C(=O)Nc5nccs5",
    ...
    def test_fragment_reference_smi(self):
        ...
        self.assertEqual(
            self.test_mmp_pairs_object.unique_frags,
            {
                "s1[1cH][n]cc1",
                "[n]1cc[1cH]cc1",
                "[1CH4]",
    ...
    def test_everything_at_once_single(self):
        ...
        for (
            ...
            frag_removed,
            frag_added,
        ) in self.test_mmp_pairs_object.yield_products_complex_id():
    ...
    def test_everything_at_once_single_simple_dict(self):
        ...
        self.assertEqual(
            {
                (
                    "[1CH3]N1C(=O)CC2(C1)CN(C(=O)C(C)C)CCN(Cc1cc[n]cc1)C2",
                    "[1CH4]",
                    "C(C)N1C(=O)CC2(C1)CN(C(=O)C(C)C)CCN(CC1=CC=NC=C1)C2",
                ): None,
                (
                    "O=C1N(CC)CC2(CN(C(=O)C(C)C)CC[1NH]C2)C1",
                    "[1CH3]c1cc[n]cc1",
                    "O=C1N(CC)CC2(CN(C(=O)C(C)C)CCN(C2)CC2=CC=NC=C2)C1",
                ): None,
                (
                    "[1CH3]N1CC2(CN(C(=O)C2)CC)CN(C(=O)C(C)C)CC1",
                    "[n]1cc[1cH]cc1",
                    "C(N1CC2(CN(C(=O)C2)CC)CN(C(=O)C(C)C)CC1)C1=CC=NC=C1",
                ): None,
                (
                    "O=C1N(CC2(CN(Cc3cc[n]cc3)CC[1NH]C2)C1)CC",
                    "O=[1CH]C(C)C",
                    "O=C1N(CC2(CN(CC3=CC=NC=C3)CCN(C2)C(=O)C(C)C)C1)CC",
                ): None,
                (
                    "CCN1C(=O)CC2(C1)CN(Cc1cc[n]cc1)CCN([1CH]=O)C2",
                    "C[1CH2]C",
                    "CCN1C(=O)CC2(C1)CN(CC1=CC=NC=C1)CCN(C(=O)C(C)C)C2",
                ): None,
                (
                    "O=C([1CH2]C)N1CC2(CN(C(=O)C2)CC)CN(Cc2cc[n]cc2)CC1",
                    "[1CH4]",
                    "O=C(C(C)C)N1CC2(CN(C(=O)C2)CC)CN(CC2=CC=NC=C2)CC1",
                ): None,
                (
                    "O=C(C(C)C)N1CC2(C[1NH]C(=O)C2)CN(Cc2cc[n]cc2)CC1",
                    "[1CH3]C",
                    "O=C(C(C)C)N1CC2(CN(C(=O)C2)CC)CN(CC2=CC=NC=C2)CC1",
                ): None,
                (
                    "[1CH3]CN1C(=O)CC2(C1)CN(C(=O)C(C)C)CCN(Cc1cc[n]cc1)C2",
                    "[1H]",
                    "C([H])CN1C(=O)CC2(C1)CN(C(=O)C(C)C)CCN(CC1=CC=NC=C1)C2",
                ): None,
                (
                    "O=C(C(C)C)N1CC2(CN(C(=O)C2)CC)CN([1CH2]c2cc[n]cc2)CC1",
                    "[1H]",
                    "O=C(C(C)C)N1CC2(CN(C(=O)C2)CC)CN(C([H])C2=CC=NC=C2)CC1",
                ): None,
                (
                    "Fc1ccc([n]2[n]cc3c2ccc(C([1CH](C(=O)Nc2scc[n]2)C)c2cc[n]cc2)c3)cc1",
                    "[1CH4]",
                    "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1",
                ): None,
                (
                    "Fc1ccc([n]2[n]cc3c2ccc([1CH2]C(C(=O)Nc2scc[n]2)(C)C)c3)cc1",
                    "[n]1cc[1cH]cc1",
                    "FC1=CC=C(N2N=CC3=CC(=CC=C23)C(C(C)(C)C(=O)NC2=NC=CS2)C2=CC=NC=C2)C=C1",
                ): None,
                (
                    "O=C(Nc1scc[n]1)C(C(c1cc2c([1nH][n]c2)cc1)c1cc[n]cc1)(C)C",
                    "Fc1cc[1cH]cc1",
                    "O=C(NC1=NC=CS1)C(C)(C)C(C1=CC2=C(N(N=C2)C2=CC=C(F)C=C2)C=C1)C1=CC=NC=C1",
                ): None,
            },
            self.test_dataset_testresults,
        )
