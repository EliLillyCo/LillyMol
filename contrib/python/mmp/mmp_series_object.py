...
from scipy.stats import binomtest, pearsonr, spearmanr, linregress, skew
...
class MMPSeriesObjectClass(MMPDataObjectClass):
    ...
    def _iterator_series_pstats(self, frag_list, series_dataframe, strict_order=False):
        ...
        for series in observed_series:
            ...
            # p-value (corrected)
            # use binomtest(count, nTrials, pEvent) where pEvent is the expected prob (1/n_factorial)
            result = binomtest(observations, total_series_obs, expected_prob)
            # Bonferroni correction, multiplying the original p-value by the degrees of freedom
            # i.e.: p_value * (N! - 1)
            p_value_corrected = result.pvalue * (n_factorial - 1)
            ...
            if strict_order:
                if series == tuple(frag_list):
                    yield series, observations, total_series_obs, observed_prob, expected_prob, enrichment, \
                          result.pvalue, p_value_corrected, significant
                else:
