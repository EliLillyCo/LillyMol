...
# But, this version has a key difference. We just remove
# the items we recognise from ARGV
...
#       $stderr.print "Unrecognised option '#{opt}'\n"
      ...
      $stderr.print "IWCmdlineV2:initialize:invalid qualifier '#{qualifiers}' for option '#{opt}', value discarded\n"
  ...
  def value (opt, v, i)
   ...
   return false unless @_count.has_key(opt)

   tmp = @_option_value[opt]

   return false if i < 0 || i >= tmp.size
  ...
  def value (opt)
    return nil unless @_option_value.has_key?(opt)
