...
test_command=molecular_transformations
case=case_1
case_id="Case 1"
...
in="$test_cmd_top/$case/in/CID7977.smi"
out=test.smi
...
$command -A I -A D -d -z d -m each \
   -R $test_cmd_top/$case/in/heteroHalideTransformationRule.rxn "$in" 1>> "$out" 2>> err.txt
